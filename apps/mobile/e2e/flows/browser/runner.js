...
function checkPort() {
    try {
        (0, child_process_1.execSync)(`lsof -i :${WEB_PORT}`, { stdio: 'pipe' });
        return true;
    }
    catch {
        return false;
...
function checkExpo() {
    try {
        (0, child_process_1.execSync)('npx expo --version', { stdio: 'pipe' });
        return true;
    }
    catch {
        return false;
...
async function startWeb() {
    ...
    webServer = (0, child_process_1.spawn)('npx', ['expo', 'start', '--web', '--no-dev'], {
        cwd: mobileDir,
        stdio: 'pipe',
    });
...
async function stop() {
    console.log('\n🛑 Stopping Web server...');
    try {
        (0, child_process_1.execSync)(`lsof -ti :${WEB_PORT} | xargs kill 2>/dev/null || true`, { stdio: 'pipe' });
        console.log('   ✅ Stopped');
    }
    catch {
