...
function parseArgs() {
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        if (arg === '--layer') {
            ...
            result.layer = (val === 0 || val === 1 || val === 2 ? val : 2);
            continue;
        }
        switch (arg) {
            case '--platform':
                result.platform = args[++i] || 'ios';
                break;
            case '--flow':
                result.flow = args[++i];
                break;
...
function checkMaestroInstalled() {
    try {
        ...
        (0, child_process_1.execSync)(`${maestro} --version`, { stdio: 'pipe' });
        return true;
    }
    catch {
        return false;
...
function checkXcode() {
    try {
        (0, child_process_1.execSync)('xcodebuild -version', { stdio: 'pipe' });
        return true;
    }
    catch {
        return false;
...
function checkSimulator() {
    try {
        const output = (0, child_process_1.execSync)('xcrun simctl list devices available --json', {
            stdio: 'pipe',
            encoding: 'utf-8',
        });
        const devices = JSON.parse(output);
        const hasDevice = Object.values(devices.devices).some((list) => list.length > 0);
        return hasDevice;
    }
    catch {
        return false;
...
function getBootedSimulator() {
    try {
        const output = (0, child_process_1.execSync)('xcrun simctl list devices booted --json', {
            stdio: 'pipe',
            encoding: 'utf-8',
        });
...
function bootSimulator(udid) {
    try {
        ...
        (0, child_process_1.execSync)(`xcrun simctl boot "${udid}"`, { stdio: 'pipe' });
        // 等待模拟器完全启动
        let attempts = 0;
        while (attempts < 30) {
...
function openSimulatorApp() {
    try {
        (0, child_process_1.execSync)('open -a Simulator', { stdio: 'pipe' });
    }
    catch {
        // ignore
    }
...
function isAppInstalled(udid) {
    try {
        const output = (0, child_process_1.execSync)(`xcrun simctl listapps "${udid}"`, {
            stdio: 'pipe',
            encoding: 'utf-8',
        });
...
function killConflictingProcesses() {
    ...
    try {
        (0, child_process_1.execSync)('pkill -f "expo run:ios" 2>/dev/null || true', { stdio: 'pipe' });
        (0, child_process_1.execSync)('pkill -f "react-native start" 2>/dev/null || true', { stdio: 'pipe' });
        (0, child_process_1.execSync)('pkill -f "metro" 2>/dev/null || true', { stdio: 'pipe' });
        (0, child_process_1.execSync)('sleep 2', { stdio: 'pipe' });
...
function shutdownExtraSimulators(keepUdid) {
        ...
        for (const [_runtime, list] of Object.entries(data.devices)) {
            for (const device of list) {
                if (device.udid !== keepUdid && device.state === 'Booted') {
                    ...
                    try {
                        (0, child_process_1.execSync)(`xcrun simctl shutdown "${device.udid}"`, { stdio: 'pipe' });
                    }
                    catch {
                        // ignore
                    }
...
function buildAndInstallApp(platform) {
    ...
    try {
        if (platform === 'ios') {
            ...
            (0, child_process_1.execSync)('npx expo run:ios --scheme BridgeAI --no-install', {
                cwd: PROJECT_ROOT,
                stdio: 'inherit',
                timeout: 300000,
            });
            ...
            (0, child_process_1.execSync)('npx expo run:android --no-install', {
                cwd: PROJECT_ROOT,
                stdio: 'inherit',
                timeout: 300000,
            });
...
function filterFlows(flows, flowName, issueId) {
    if (flowName) {
...
// ─── Flow 执行 ─────────────────────────────────────────────────────
function takeScreenshot(flowName) {
    const booted = getBootedSimulator();
    if (!booted)
        return undefined;
    ...
    try {
        (0, child_process_1.execSync)(`xcrun simctl io booted screenshot "${screenshotPath}"`, { stdio: 'pipe' });
...
function runFlow(flowPath, platform, timeout, screenshotOnFailure) {
...
function checkJava() {
    try {
        const version = (0, child_process_1.execSync)('java -version 2>&1', { stdio: 'pipe', encoding: 'utf-8' });
    ...
    for (const check of checks) {
        if (check.pass) {
            if (check.name === 'Maestro CLI') {
                ...
                try {
                    const maestro = getMaestroCmd();
                    const version = (0, child_process_1.execSync)(`${maestro} --version`, { encoding: 'utf-8', stdio: 'pipe' }).trim();
...
function installMaestro() {
    ...
    try {
        if (process.platform === 'darwin') {
            (0, child_process_1.execSync)('brew install maestro', { stdio: 'inherit' });
        }
        else {
            (0, child_process_1.execSync)('curl -s "https://get.maestro.mobile.dev" | bash', { stdio: 'inherit' });
        }
...
// ─── 环境诊断 ──────────────────────────────────────────────────────
function diagnose(platform) {
    console.log('\n🔍 Environment Check');
    console.log('   ──────────────────');
    let ok = true;
    // Maestro
    if (checkMaestroInstalled()) {
        const version = (0, child_process_1.execSync)(`${getMaestroCmd()} --version`, { encoding: 'utf-8', stdio: 'pipe' }).trim();
    ...
    if (platform === 'ios') {
        // Xcode
        if (checkXcode()) {
            const version = (0, child_process_1.execSync)('xcodebuild -version', { encoding: 'utf-8', stdio: 'pipe' }).split('\n')[0].trim();
...
function checkBackendRunning() {
    try {
        ...
        let output = (0, child_process_1.execSync)('curl -s --connect-timeout 2 http://localhost:3001/health', {
            encoding: 'utf-8',
            stdio: 'pipe',
        });
        ...
        output = (0, child_process_1.execSync)('curl -s --connect-timeout 2 http://localhost:3001/api/health', {
            encoding: 'utf-8',
            stdio: 'pipe',
        });
...
function startBackend() {
    ...
    try {
        ...
        backendProcess = (0, child_process_1.spawn)('npx', ['tsx', 'src/index.ts'], {
            cwd: serverDir,
            detached: false,
            stdio: 'pipe',
        });
...
async function main() {
    ...
    if (args.dev)
        console.log('   Mode: dev (app must be running)');
    if (args.build)
        console.log('   Mode: build (will build & install app)');
    if (args.screenshotOnFailure)
    ...
    if (args.platform === 'ios') {
        ...
        if (args.build || (!args.dev && !isAppInstalled(udid))) {
            ...
            if (!buildAndInstallApp(args.platform))
                process.exit(1);
        }
