...
// Mock Animated API to prevent infinite recursion in the animation loop.
// The real Animated.sequence().start() fires its callback synchronously in
// the test environment, and combined with fake timers' requestAnimationFrame
// mock this causes the animate() function to recurse without limit.
...
jest.spyOn(Animated, 'timing').mockReturnValue(mockAnimated as MockAnimated);
jest.spyOn(Animated, 'parallel').mockReturnValue(mockAnimated as MockAnimated);
jest.spyOn(Animated, 'sequence').mockReturnValue(mockAnimated as MockAnimated);

// Prevent requestAnimationFrame from triggering recursive animate() calls.
jest
  .spyOn(globalThis, 'requestAnimationFrame')
  .mockImplementation((_cb: FrameRequestCallback) => 0);
...
describe('TypingIndicator', () => {
  ...
  it('calls onTimeout after specified timeout', () => {
    ...
    render(<TypingIndicator timeout={5000} onTimeout={mockOnTimeout} testID="typing" />);
  ...
  it('clears timeout on unmount', () => {
    ...
    const { unmount } = render(
      <TypingIndicator timeout={5000} onTimeout={mockOnTimeout} testID="typing" />
    );
