...
describe('LastSeen', () => {
  ...
  describe('LastSeen component', () => {
    ...
    it('updates time at specified interval', async () => {
      const fiveMinutesAgo = Date.now() - 5 * 60 * 1000;
      render(<LastSeen timestamp={fiveMinutesAgo} updateInterval={60000} testID="last-seen" />);
    ...
    it('applies custom textStyle prop', () => {
      ...
      render(<LastSeen timestamp={Date.now()} textStyle={customTextStyle} testID="last-seen" />);
