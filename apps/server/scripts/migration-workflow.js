 ...
 */
function _generateMigrationName(description) {
    const now = new Date();
 ...
 */
function saveApprovalRecords(records) {
...
function createBackup(env) {
    ...
    try {
        ...
        (0, child_process_1.execSync)(`pg_dump "${databaseUrl}" --clean --if-exists --no-owner --no-privileges > "${backupFile}"`, {
            stdio: 'inherit',
 ...
 */
function restoreBackup(backupFile, env) {
    const databaseUrl = getDatabaseUrl(env) || process.env.DATABASE_URL;
    if (!databaseUrl) {
    ...
    try {
        (0, child_process_1.execSync)(`psql "${databaseUrl}" < "${backupFile}"`, {
            stdio: 'inherit',
            timeout: 300000,
        });
 ...
 */
function detectDestructiveChanges(migrationName) {
 ...
 */
function printDestructiveFindings(findings) {
...
function createMigration(name) {
    ...
    try {
        ...
        (0, child_process_1.execSync)(`prisma migrate dev --name "${name}" --create-only`, {
            stdio: 'inherit',
 ...
 */
function reviewMigrations(name) {
    const migrations = listMigrations();
    ...
    for (const migration of targetMigrations) {
        ...
        const statusEmoji = status === 'approved' ? '[APPROVED]' : status === 'rejected' ? '[REJECTED]' : '[PENDING]';
 ...
 * Approve migration
 */
function approveMigration(name) {
    const validation = validateNamingConvention(name);
    if (!validation.valid) {
 ...
 */
function executeMigration(name) {
    const records = getApprovalRecords();
    ...
    (0, child_process_1.execSync)('prisma migrate deploy', { stdio: 'inherit' });
...
function safeDeploy(env) {
    ...
    try {
        ...
        if (envFile) {
            const envPath = path.join(__dirname, '..', envFile);
            if (fs.existsSync(envPath)) {
                (0, child_process_1.execSync)(`dotenv -e ${envPath} -- prisma migrate deploy`, {
                    stdio: 'inherit',
                    cwd: path.join(__dirname, '..'),
                });
            }
 ...
 */
function diffMigrationState(env) {
    ...
    if (env) {
        ...
        try {
            ...
            (0, child_process_1.execSync)(`dotenv -e ${envPath} -- prisma migrate status`, { stdio: 'inherit' });
        }
        catch {
        ...
        try {
            ...
            (0, child_process_1.execSync)('prisma migrate status', { stdio: 'inherit' });
        }
        catch {
...
//# sourceMappingURL=migration-workflow.js.map