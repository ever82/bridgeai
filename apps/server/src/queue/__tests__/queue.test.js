...
function resetAllMocks() {
    ...
    mockQueueAdd.mockImplementation(async (name, data, opts) => ({
        id: `job-${Date.now()}`,
        name,
        data,
        opts,
...
let lastCreatedWorkerMock = null;
jest.mock('bullmq', () => ({
    ...
    Worker: jest.fn().mockImplementation((name, _proc, opts) => {
        const handlers = {};
        const workerMock = {
            name,
...
function resetAllSingletons() {
    ...
    try {
        ...
        const queueEvents = mgr.queueEvents;
    ...
    QueueMonitor.instance = null;
    QueueProducer.instance = null;
    RetryStrategy.instance = null;
    // Reset module-level managerInstance
    try {
        require('../queues').managerInstance = null;
        require('../producer').producerInstance = null;
        require('../worker').workerInstance = null;
        require('../monitoring').monitorInstance = null;
        require('../retry').retryInstance = null;
    }
    catch {
        /* */
    }
...
describe('Queue Config', () => {
    it('should have all required queue names', () => {
        expect(config_1.QueueNames.CREDIT_UPDATE).toBe('credit-update');
        expect(config_1.QueueNames.MATCH_NOTIFICATION).toBe('match-notification');
        expect(config_1.QueueNames.EMAIL_DELIVERY).toBe('email-delivery');
        expect(config_1.QueueNames.PUSH_NOTIFICATION).toBe('push-notification');
        expect(config_1.QueueNames.DATA_EXPORT).toBe('data-export');
        expect(config_1.QueueNames.IMAGE_ANALYSIS).toBe('image-analysis');
        expect(config_1.QueueNames.DEMAND_EXTRACTION).toBe('demand-extraction');
        expect(config_1.QueueNames.DEAD_LETTER).toBe('dead-letter');
        expect(config_1.QueueNames.SCHEDULED_TASK).toBe('scheduled-task');
    });
});
...
describe('Queue Config Structure', () => {
    ...
    it('should have queue-specific rate limits', () => {
        const creditCfg = queueConfigurations[config_1.QueueNames.CREDIT_UPDATE];
        expect(creditCfg.worker?.rateLimitMax).toBe(50);
        const matchCfg = queueConfigurations[config_1.QueueNames.MATCH_NOTIFICATION];
        expect(matchCfg.worker?.rateLimitMax).toBe(200);
        const dataExportCfg = queueConfigurations[config_1.QueueNames.DATA_EXPORT];
        expect(dataExportCfg.worker?.rateLimitMax).toBe(10);
    });
    it('should allow credit update queue to override worker concurrency', () => {
        const creditConfig = queueConfigurations[config_1.QueueNames.CREDIT_UPDATE];
...
describe('QueueManager', () => {
    ...
    it('should return the same queue instance for the same name', () => {
        ...
        const q1 = manager.getQueue(config_1.QueueNames.CREDIT_UPDATE);
        const q2 = manager.getQueue(config_1.QueueNames.CREDIT_UPDATE);
        expect(q1).toBe(q2);
    });
    ...
    it('should gracefully shutdown and clear all resources', async () => {
        ...
        manager.registerWorker(config_1.QueueNames.CREDIT_UPDATE, mockWorker);
...
describe('QueueProducer', () => {
    ...
    it('should add a job to a queue', async () => {
        const job = await producer.add(config_1.QueueNames.CREDIT_UPDATE, { source: 'test' });
        expect(job).toBeDefined();
        expect(job.id).toBeDefined();
        expect(mockQueueAdd).toHaveBeenCalled();
    });
    it('should add a delayed job', async () => {
        await producer.addDelayed(config_1.QueueNames.EMAIL_DELIVERY, { userId: 'u1' }, 5000);
        expect(mockQueueAdd).toHaveBeenCalledWith(config_1.QueueNames.EMAIL_DELIVERY, { userId: 'u1' }, expect.objectContaining({ delay: 5000 }));
    });
    it('should throw when scheduling in the past', async () => {
        const pastDate = new Date(Date.now() - 60000);
        await expect(producer.schedule(config_1.QueueNames.SCHEDULED_TASK, { userId: 'u1' }, pastDate)).rejects.toThrow('Scheduled date must be in the future');
    });
    it('should get job counts by state', async () => {
        ...
        const counts = await producer.getJobCounts(config_1.QueueNames.CREDIT_UPDATE);
    ...
    it('should remove a job from queue', async () => {
        ...
        await producer.removeJob(config_1.QueueNames.CREDIT_UPDATE, 'job-1');
...
describe('QueueWorker', () => {
    ...
    it('should create Worker with limiter option (rate limits applied)', async () => {
        ...
        worker.registerProcessor(config_1.QueueNames.CREDIT_UPDATE, jest.fn());
        await worker.startProcessor(config_1.QueueNames.CREDIT_UPDATE);
        const { Worker: MockWorker } = require('bullmq');
        expect(MockWorker).toHaveBeenCalledWith(config_1.QueueNames.CREDIT_UPDATE, expect.any(Function), expect.objectContaining({
    ...
    it('should apply queue-specific rate limits (50 for CREDIT_UPDATE, 200 for MATCH_NOTIFICATION)', async () => {
        const worker = QueueWorker.getInstance();
        worker.registerProcessor(config_1.QueueNames.MATCH_NOTIFICATION, jest.fn());
        await worker.startProcessor(config_1.QueueNames.MATCH_NOTIFICATION);
        const { Worker: MockWorker } = require('bullmq');
    ...
    it('should call handleFailedJob when BullMQ exhausts all retries', async () => {
        ...
        const failedJob = {
            id: 'job-1',
            queueName: config_1.QueueNames.CREDIT_UPDATE,
            attemptsMade: 3,
            opts: { attempts: 3 },
            data: { source: 'test' },
            name: config_1.QueueNames.CREDIT_UPDATE,
        };
        const handlers = lastCreatedWorkerMock?._handlers?.['failed'] || [];
        expect(handlers.length).toBeGreaterThan(0);
        for (const h of handlers)
            await h(failedJob, new Error('Test failure'));
    ...
    it('should start processors for all registered queues', async () => {
        const worker = QueueWorker.getInstance();
        worker.registerProcessor(config_1.QueueNames.CREDIT_UPDATE, jest.fn());
        worker.registerProcessor(config_1.QueueNames.EMAIL_DELIVERY, jest.fn());
...
describe('QueueMonitor', () => {
    ...
    afterEach(() => {
        if (monitorInstance)
            monitorInstance.stopMetricsCollection();
        resetAllSingletons();
    ...
    it('should get worker status with correct paused boolean value (not Promise)', async () => {
        ...
        const creditWorker = workers.find((w) => w.queueName === config_1.QueueNames.CREDIT_UPDATE);
    ...
    it('should get metrics for a specific queue', async () => {
        ...
        mockQueueIsPaused.mockResolvedValue(false);
        monitorInstance = QueueMonitor.getInstance();
        const metrics = await monitorInstance.getQueueMetrics(config_1.QueueNames.CREDIT_UPDATE);
    ...
    it('should get health status for all queues', async () => {
        ...
        expect(health.length).toBe(Object.values(config_1.QueueNames).length);
...
describe('RetryStrategy', () => {
    ...
    it('should send to DLQ directly without re-adding to original queue', async () => {
        ...
        await retryStrategy.handleFailedJob(failedJob);
        // Should call DLQ add (first call)
        expect(mockQueueAdd).toHaveBeenCalledTimes(1);
        expect(mockQueueAdd).toHaveBeenCalledWith('dead-letter', expect.objectContaining({
            originalQueue: config_1.QueueNames.CREDIT_UPDATE,
            originalJobId: 'job-1',
    ...
    it('should throw when dead letter job not found', async () => {
        ...
        await expect(retryStrategy.reprocessFromDeadLetter(config_1.QueueNames.CREDIT_UPDATE, 'job-1', 'dlq-1')).rejects.toThrow('Dead letter job dlq-1 not found');
    });
    it('should reprocess a job from dead letter queue', async () => {
        ...
        await retryStrategy.reprocessFromDeadLetter(config_1.QueueNames.CREDIT_UPDATE, 'job-1', 'dlq-1');
        expect(mockQueueAdd).toHaveBeenCalledWith(config_1.QueueNames.CREDIT_UPDATE, { source: 'test' }, expect.objectContaining({ attempts: 1 }));
        expect(dlqJob.remove).toHaveBeenCalled();
    });
    it('should get dead letter queue statistics', async () => {
        mockQueueGetJobs.mockResolvedValue([
            { data: { originalQueue: config_1.QueueNames.CREDIT_UPDATE } },
            { data: { originalQueue: config_1.QueueNames.CREDIT_UPDATE } },
            { data: { originalQueue: config_1.QueueNames.EMAIL_DELIVERY } },
        ]);
        const stats = await retryStrategy.getDeadLetterStats();
        expect(stats.total).toBe(3);
        expect(stats.byOriginalQueue[config_1.QueueNames.CREDIT_UPDATE]).toBe(2);
        expect(stats.byOriginalQueue[config_1.QueueNames.EMAIL_DELIVERY]).toBe(1);
    });
