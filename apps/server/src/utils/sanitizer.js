 ...
 */
// 默认脱敏规则
export const defaultSanitizeRules = [
    // 密码
    {
 ...
 * @returns 脱敏后的字符串
 */
export function sanitizeString(str, rules = defaultSanitizeRules) {
 ...
 * @returns 脱敏后的对象
 */
export function sanitizeObject(obj, sensitiveFields = ['password', 'token', 'accessToken', 'refreshToken', 'secret', 'apiKey']) {
    ...
    for (const key of Object.keys(result)) {
        ...
        // 递归处理嵌套对象
            ...
            if (Array.isArray(value)) {
                result[key] = value.map((item) => typeof item === 'object' ? sanitizeObject(item, sensitiveFields) : item);
            }
            else {
