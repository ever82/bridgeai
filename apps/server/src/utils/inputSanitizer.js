 ...
 */
export function sanitizeJavaScript(input) {
    if (!input || typeof input !== 'string') {
        return '';
    }
    return input
 ...
 */
export function containsNoSqlOperators(obj) {
 ...
 */
export function sanitizeNoSqlOperators(obj) {
    if (typeof obj !== 'object' || obj === null) {
        return obj;
    }
    if (Array.isArray(obj)) {
        return obj.map((item) => typeof item === 'object' && item !== null
            ? sanitizeNoSqlOperators(item)
            : item);
    }
    const result = {};
    for (const key of Object.keys(obj)) {
        ...
        const value = obj[key];
        if (typeof value === 'object' && value !== null) {
            result[key] = sanitizeNoSqlOperators(value);
        }
        else {
            result[key] = value;
        }
 ...
 */
export function removeControlChars(input) {
    if (!input || typeof input !== 'string') {
        return '';
    }
 ...
 * Sanitize URL
 */
export function sanitizeUrl(input, allowedProtocols = ['http:', 'https:']) {
    if (!input || typeof input !== 'string') {
        return null;
    }
    try {
 ...
 */
export function sanitizeInputString(input, options = {}) {
    const { escapeHtml: shouldEscapeHtml = false, stripHtml: shouldStripHtml = false, trim: shouldTrim = true, removeControlChars: shouldRemoveControlChars = true, sanitizeJs: shouldSanitizeJs = true, } = options;
    if (!input || typeof input !== 'string') {
        return '';
    }
    let sanitized = input;
 ...
 */
export function sanitizeInputObject(obj, options = {}) {
    const result = {};
    for (const key of Object.keys(obj)) {
        const value = obj[key];
            ...
            if (Array.isArray(value)) {
                result[key] = value.map((item) => typeof item === 'string'
                    ...
                    : typeof item === 'object' && item !== null
                        ? sanitizeInputObject(item, options)
                        : item);
            }
            else {
                result[key] = sanitizeInputObject(value, options);
            }
        }
        else {
            result[key] = value;
