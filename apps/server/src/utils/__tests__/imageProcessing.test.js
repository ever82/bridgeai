...
describe('Image Processing Utilities', () => {
    describe('getImageMetadata', () => {
        it('should extract metadata from a valid image buffer', async () => {
            ...
            const metadata = await (0, imageProcessing_1.getImageMetadata)(buffer);
    ...
    describe('resizeImage', () => {
        it('should resize image to specified width', async () => {
            ...
            const resized = await (0, imageProcessing_1.resizeImage)(buffer, 200);
        ...
        it('should resize to exact dimensions', async () => {
            const buffer = await createTestImage(400, 300);
            const resized = await (0, imageProcessing_1.resizeImage)(buffer, 200, 100);
            const metadata = await (0, sharp_1.default)(resized).metadata();
    ...
    describe('convertImageFormat', () => {
        it('should convert JPEG to PNG', async () => {
            ...
            const converted = await (0, imageProcessing_1.convertImageFormat)(buffer, 'png');
            const metadata = await (0, sharp_1.default)(converted).metadata();
            expect(metadata.format).toBe('png');
        });
        it('should convert to webp', async () => {
            const buffer = await createTestImage();
            const converted = await (0, imageProcessing_1.convertImageFormat)(buffer, 'webp');
            const metadata = await (0, sharp_1.default)(converted).metadata();
    ...
    describe('optimizeImage', () => {
        it('should produce a valid optimized image', async () => {
            ...
            const optimized = await (0, imageProcessing_1.optimizeImage)(buffer, 60);
            expect(optimized).toBeInstanceOf(Buffer);
            expect(optimized.length).toBeGreaterThan(0);
            const metadata = await (0, sharp_1.default)(optimized).metadata();
            expect(metadata.format).toBe('jpeg');
        });
    });
    describe('cropImage', () => {
        it('should crop image to specified region', async () => {
            ...
            const cropped = await (0, imageProcessing_1.cropImage)(buffer, { x: 50, y: 50, width: 100, height: 100 });
            const metadata = await (0, sharp_1.default)(cropped).metadata();
    ...
    describe('applyBlur', () => {
        it('should apply blur to entire image', async () => {
            ...
            const blurred = await (0, imageProcessing_1.applyBlur)(buffer, 5);
            expect(blurred).toBeInstanceOf(Buffer);
            expect(blurred.length).toBeGreaterThan(0);
        });
        it('should apply blur to specific region', async () => {
            const buffer = await createTestImage(200, 200);
            const blurred = await (0, imageProcessing_1.applyBlur)(buffer, 10, { x: 50, y: 50, width: 100, height: 100 });
            expect(blurred).toBeInstanceOf(Buffer);
    ...
    describe('isValidImage', () => {
        it('should detect valid JPEG buffer', async () => {
            const buffer = await createTestImage();
            expect((0, imageProcessing_1.isValidImage)(buffer)).toBe(true);
        });
        ...
        it('should reject invalid buffer', () => {
            expect((0, imageProcessing_1.isValidImage)(Buffer.from('not an image'))).toBe(false);
        });
        it('should reject empty buffer', () => {
            expect((0, imageProcessing_1.isValidImage)(Buffer.alloc(0))).toBe(false);
        });
    });
    describe('getImageFormat', () => {
        it('should identify JPEG format', async () => {
            const buffer = await createTestImage();
            expect((0, imageProcessing_1.getImageFormat)(buffer)).toBe('jpeg');
        });
        it('should identify PNG format', async () => {
            const buffer = await (0, sharp_1.default)({
                ...
                .png()
                .toBuffer();
            expect((0, imageProcessing_1.getImageFormat)(buffer)).toBe('png');
        });
        it('should return null for invalid data', () => {
            expect((0, imageProcessing_1.getImageFormat)(Buffer.from('abc'))).toBeNull();
        });
    });
    describe('stripExif', () => {
        it('should strip EXIF and return valid image', async () => {
            const buffer = await createTestImageWithExif();
            const stripped = await (0, imageProcessing_1.stripExif)(buffer);
            expect(stripped).toBeInstanceOf(Buffer);
            expect(stripped.length).toBeGreaterThan(0);
            // Verify it's still a valid image
            const metadata = await (0, sharp_1.default)(stripped).metadata();
            expect(metadata.width).toBe(100);
            expect(metadata.height).toBe(100);
        });
    });
    describe('BoundingBox utilities', () => {
        describe('scaleBoundingBox', () => {
            it('should scale bounding box proportionally', () => {
                ...
                const result = (0, imageProcessing_1.scaleBoundingBox)(box, { width: 100, height: 100 }, { width: 200, height: 200 });
        ...
        describe('doBoundingBoxesOverlap', () => {
            it('should detect overlapping boxes', () => {
                ...
                expect((0, imageProcessing_1.doBoundingBoxesOverlap)(box1, box2)).toBe(true);
            });
            it('should detect non-overlapping boxes', () => {
                const box1 = { x: 0, y: 0, width: 50, height: 50 };
                const box2 = { x: 100, y: 100, width: 50, height: 50 };
                expect((0, imageProcessing_1.doBoundingBoxesOverlap)(box1, box2)).toBe(false);
            });
        });
        describe('mergeBoundingBoxes', () => {
            it('should merge overlapping boxes', () => {
                ...
                const merged = (0, imageProcessing_1.mergeBoundingBoxes)(boxes);
            ...
            it('should return empty array for empty input', () => {
                expect((0, imageProcessing_1.mergeBoundingBoxes)([])).toEqual([]);
            });
        });
        describe('padBoundingBox', () => {
            it('should add padding to bounding box', () => {
                const box = { x: 20, y: 20, width: 50, height: 50 };
                const result = (0, imageProcessing_1.padBoundingBox)(box, 10, { width: 200, height: 200 });
            ...
            it('should not exceed image boundaries', () => {
                ...
                const result = (0, imageProcessing_1.padBoundingBox)(box, 10, { width: 100, height: 100 });
