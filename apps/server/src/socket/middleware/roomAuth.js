 ...
 */
export function roomAccessMiddleware(options) {
    return (socket, next) => {
 ...
 */
export function checkRoomAccess(socket, roomId, requireRole, allowBanned = false) {
    const userId = socket.user?.id;
    if (!userId) {
 ...
 */
export async function validateRoomJoin(socket, roomId, password) {
    const userId = socket.user?.id;
 ...
 */
export function validateRoomCreate(socket, roomId) {
    const userId = socket.user?.id;
    if (!userId) {
        return { valid: false, error: 'Authentication required' };
    }
 ...
 */
export function requireRoomMember(socket, next) {
 ...
 */
export function requireRoomAdmin(roomId) {
    return (socket, next) => {
        const userId = socket.user?.id;
        if (!userId) {
 ...
 */
export function requireRoomOwner(roomId) {
    return (socket, next) => {
        const userId = socket.user?.id;
        if (!userId) {
            next(new Error('Authentication required'));
 ...
 */
export function canKickUser(roomId, kickerId, targetId) {
 ...
 */
export function canBanUser(roomId, bannerId, targetId) {
 ...
 * Get room blacklist
 */
export function getRoomBlacklist(roomId) {
    const room = roomService.getRoom(roomId);
    if (!room) {
        return [];
    }
 ...
 */
export function isUserBlacklisted(roomId, userId) {
