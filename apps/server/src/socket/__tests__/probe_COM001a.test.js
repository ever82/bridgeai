...
function makeClient(nsp = '', auth) {
    const url = `http://localhost:${TEST_PORT}${nsp}`;
    return (0, socket_io_client_1.io)(url, auth ? { auth } : undefined);
}
function mockUser(userId, email = 'probe@test.com', roles = []) {
    jwtService_1.jwtService.verifyToken.mockResolvedValue({
        userId,
        email,
    });
...
describe('PROBE: ISSUE-COM001a Socket.io Infrastructure', () => {
    ...
    beforeAll(async () => {
        ...
        _io = await (0, index_1.initializeSocketServer)(httpServer, {});
        httpServer.listen(TEST_PORT);
    });
    afterAll(async () => {
        await (0, index_1.closeSocketServer)();
        httpServer.close();
    });
    afterEach(() => {
        jest.clearAllMocks();
    ...
    describe('Authentication Boundary Probes', () => {
        ...
        probe('should reject connection when JWT verifyToken returns null', done => {
            jwtService_1.jwtService.verifyToken.mockResolvedValue(null);
        ...
        probe('should reject connection when JWT verifyToken returns object without userId', done => {
            jwtService_1.jwtService.verifyToken.mockResolvedValue({ email: 'no-id@test.com' });
        ...
        probe('should reject connection when JWT verifyToken throws', done => {
            jwtService_1.jwtService.verifyToken.mockRejectedValue(new Error('JWT expired'));
    ...
    describe('Chat Handler Input Validation Probes', () => {
        ...
        probe('should reject chat:message with empty content', done => {
            client.emit('chat:join', { roomId: 'room-probe' }, () => {
                client.emit('chat:message', { roomId: 'room-probe', content: '' }, (response) => {
        ...
        probe('should reject chat:message with whitespace-only content', done => {
            client.emit('chat:join', { roomId: 'room-probe' }, () => {
                client.emit('chat:message', { roomId: 'room-probe', content: '   \t\n  ' }, (response) => {
                    expect(response.success).toBe(false);
                    done();
                });
            });
        ...
        probe('should reject chat:message without joining room first', done => {
            client.emit('chat:message', { roomId: 'room-not-joined', content: 'hello' }, (response) => {
                expect(response.success).toBe(false);
        ...
        probe('should reject chat:message when roomId is missing', done => {
            client.emit('chat:join', { roomId: 'room-probe' }, () => {
                client.emit('chat:message', { content: 'hello' }, (response) => {
                    expect(response.success).toBe(false);
                    done();
                });
            });
        ...
        probe('should reject chat:message when content is missing entirely', done => {
            client.emit('chat:join', { roomId: 'room-probe' }, () => {
                client.emit('chat:message', { roomId: 'room-probe' }, (response) => {
                    expect(response.success).toBe(false);
                    done();
                });
            });
        ...
        probe('should handle chat:join with empty roomId', done => {
            client.emit('chat:join', { roomId: '' }, (response) => {
        ...
        probe('should handle chat:join with extremely long roomId', done => {
            ...
            client.emit('chat:join', { roomId: longRoomId }, (response) => {
        ...
        probe('should handle chat:message with special characters in content', done => {
            client.emit('chat:join', { roomId: 'room-probe' }, () => {
                client.emit('chat:message', { roomId: 'room-probe', content: '<script>alert("xss")</script>\x00\x01\x02' }, (response) => {
        ...
        probe('should handle chat:read with empty messageIds array', done => {
            client.emit('chat:read', { roomId: 'room-probe', messageIds: [] }, (response) => {
        ...
        probe('should handle chat:read with extremely large messageIds array', done => {
            ...
            client.emit('chat:read', { roomId: 'room-probe', messageIds: largeIds }, (response) => {
    ...
    describe('Namespace & State Bypass Probes', () => {
        ...
        probe('should NOT apply auth middleware to /user namespace (potential bug)', done => {
            ...
            const client = (0, socket_io_client_1.io)(`http://localhost:${TEST_PORT}/user`);
        ...
        probe('should NOT apply auth middleware to /handoff namespace (potential bug)', done => {
            const client = (0, socket_io_client_1.io)(`http://localhost:${TEST_PORT}/handoff`);
            const timeout = setTimeout(() => {
                client.disconnect();
                done.fail('Connection timed out');
            }, 5000);
        ...
        probe('should NOT apply auth middleware to /group namespace (potential bug)', done => {
            const client = (0, socket_io_client_1.io)(`http://localhost:${TEST_PORT}/group`);
            const timeout = setTimeout(() => {
                client.disconnect();
                done.fail('Connection timed out');
            }, 5000);
        ...
        probe('should NOT apply auth middleware to /room namespace (potential bug)', done => {
            const client = (0, socket_io_client_1.io)(`http://localhost:${TEST_PORT}/room`);
            const timeout = setTimeout(() => {
                client.disconnect();
                done.fail('Connection timed out');
            }, 5000);
        ...
        probe('should NOT apply auth middleware to /negotiation namespace (potential bug)', done => {
            const client = (0, socket_io_client_1.io)(`http://localhost:${TEST_PORT}/negotiation`);
            const timeout = setTimeout(() => {
                client.disconnect();
                done.fail('Connection timed out');
            }, 5000);
        ...
        probe('should NOT apply auth middleware to /presence namespace (potential bug)', done => {
            const client = (0, socket_io_client_1.io)(`http://localhost:${TEST_PORT}/presence`);
            const timeout = setTimeout(() => {
                client.disconnect();
                done.fail('Connection timed out');
            }, 5000);
    ...
    describe('Server State Probes', () => {
        probe('should throw when getIO called before initialization', () => {
            ...
            const currentIO = (0, index_1.getSocketServer)();
    ...
    describe('Connection Manager Edge Cases', () => {
        beforeEach(() => {
            ...
            connectionManager_1.connectionManager.initialize({ nsps: new Map() });
        });
        afterEach(() => {
            connectionManager_1.connectionManager.destroy();
        });
        probe('should handle removeConnection for non-existent socket', () => {
            expect(() => connectionManager_1.connectionManager.removeConnection('non-existent-socket-id')).not.toThrow();
        });
        probe('should handle getConnection for non-existent socket', () => {
            const conn = connectionManager_1.connectionManager.getConnection('non-existent-socket-id');
            expect(conn).toBeUndefined();
        });
        probe('should handle getUserConnections for non-existent user', () => {
            const conns = connectionManager_1.connectionManager.getUserConnections('non-existent-user');
            expect(conns).toEqual([]);
        });
        probe('should handle getNamespaceConnections for non-existent namespace', () => {
            const conns = connectionManager_1.connectionManager.getNamespaceConnections('non-existent-ns');
            expect(conns).toEqual([]);
        });
        probe('should handle isUserOnline for non-existent user', () => {
            expect(connectionManager_1.connectionManager.isUserOnline('non-existent-user')).toBe(false);
        });
        probe('should handle getOnlineUserCount with no connections', () => {
            expect(connectionManager_1.connectionManager.getOnlineUserCount()).toBe(0);
        });
        probe('should handle updatePing for non-existent socket', () => {
            expect(() => connectionManager_1.connectionManager.updatePing('non-existent')).not.toThrow();
        });
        probe('should handle cleanupStaleConnections with no connections', () => {
            expect(() => connectionManager_1.connectionManager.cleanupStaleConnections()).not.toThrow();
        });
        ...
        probe('should handle disconnectSocket with no io server', async () => {
            connectionManager_1.connectionManager.destroy(); // destroys io reference
            await expect(connectionManager_1.connectionManager.disconnectSocket('any-socket-id')).resolves.toBeUndefined();
        });
    });
    // ================================================================
    ...
    describe('System Namespace Access Control', () => {
        probe('should reject non-admin from /system namespace (BUG: auth middleware not applied)', done => {
            mockUser('user-non-admin', 'nonadmin@test.com', ['user']);
            const client = (0, socket_io_client_1.io)(`http://localhost:${TEST_PORT}/system`, {
    ...
    describe('User Handler State Manipulation Probes', () => {
        probe('should reject subscribing to another userId room', done => {
            ...
            client.on('connected', () => {
                ...
                client.on('user:subscribed', (data) => {
        ...
        probe('should handle user:presence with empty userIds array', done => {
            ...
            const client = makeClient('/user', { token: VALID_TOKEN });
            client.on('connected', () => {
                client.emit('user:presence', { userIds: [] }, (response) => {
                    expect(response.success).toBe(true);
    ...
    describe('Connection Manager Memory & Race Condition Probes', () => {
        ...
        probe('should handle duplicate addConnection with same socket ID', () => {
            ...
            connectionManager_1.connectionManager.addConnection(mock, 'main');
            // Add same socket ID again - should overwrite
            const mock2 = createMockSocket('dup-socket-1', 'user-dup2');
            connectionManager_1.connectionManager.addConnection(mock2, 'chat');
            const conn = connectionManager_1.connectionManager.getConnection('dup-socket-1');
        ...
        probe('should handle rapid add/remove cycles', () => {
            for (let i = 0; i < 100; i++) {
                ...
                connectionManager_1.connectionManager.addConnection(mock, 'main');
                connectionManager_1.connectionManager.removeConnection(`rapid-${i}`);
            }
            expect(connectionManager_1.connectionManager.getStats().totalConnections).toBe(0);
        });
        probe('should correctly count unique online users with multiple connections', () => {
            ...
            expect(connectionManager_1.connectionManager.getOnlineUserCount()).toBe(1);
            expect(connectionManager_1.connectionManager.getUserConnections('same-user')).toHaveLength(10);
        });
        probe('should not leak connections after cleanupStaleConnections', () => {
            const mock = createMockSocket('stale-1', 'user-stale');
            connectionManager_1.connectionManager.addConnection(mock, 'main');
            const conn = connectionManager_1.connectionManager.getConnection('stale-1');
            expect(conn).toBeDefined();
            ...
            connectionManager_1.connectionManager.cleanupStaleConnections(5 * 60 * 1000);
            ...
            expect(connectionManager_1.connectionManager.getConnection('stale-1')).toBeUndefined();
        });
        probe('should handle getStats with mixed authenticated and anonymous connections', () => {
            ...
            connectionManager_1.connectionManager.addConnection(auth, 'main');
            connectionManager_1.connectionManager.addConnection(anon, 'main');
...
function probe(name, fn, timeout) {
    test(`[PROBE] ${name}`, fn, timeout);
}
//# sourceMappingURL=probe_COM001a.test.js.map