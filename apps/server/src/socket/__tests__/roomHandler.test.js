...
describe('registerRoomHandlers', () => {
    ...
    beforeEach(() => {
        ...
        roomService_1.roomService.createRoom.mockResolvedValue({
            id: 'room-1',
            name: 'Test Room',
        });
        roomService_1.roomService.roomExists.mockReturnValue(true);
        roomService_1.roomService.joinRoom.mockReturnValue({
            userId: 'user-1',
            role: 'member',
        });
        roomService_1.roomService.isUserInRoom.mockReturnValue(false);
        roomService_1.roomService.isUserBanned.mockReturnValue(false);
        roomService_1.roomService.getRoom.mockReturnValue({
            id: 'room-1',
            isPrivate: false,
            maxMembers: 100,
        });
        roomService_1.roomService.getRoomMembers.mockReturnValue([
            { userId: 'user-1', role: 'member', joinedAt: new Date() },
        ]);
        roomService_1.roomService.getUserRooms.mockReturnValue([
            { id: 'room-1', name: 'Test Room' },
        ]);
        roomService_1.roomService.leaveRoom.mockReturnValue(true);
        roomService_1.roomService.getRoomStats.mockReturnValue({
            memberCount: 1,
            onlineCount: 1,
            bannedCount: 0,
        });
    ...
    describe('room:create', () => {
        it('should create room and join as owner', async () => {
            (0, roomHandler_1.registerRoomHandlers)(mockSocket, mockNamespace);
            ...
            expect(roomService_1.roomService.createRoom).toHaveBeenCalledWith('room-1', 'user-1', { name: 'Test Room' });
            expect(roomService_1.roomService.joinRoom).toHaveBeenCalled();
        ...
        it('should handle errors', async () => {
            roomService_1.roomService.createRoom.mockRejectedValue(new Error('Room already exists'));
            (0, roomHandler_1.registerRoomHandlers)(mockSocket, mockNamespace);
            const createHandler = mockSocket.on.mock.calls.find((call) => call[0] === 'room:create')[1];
            const callback = jest.fn();
            await createHandler({ roomId: 'room-1', options: { name: 'Test Room' } }, callback);
    ...
    describe('room:join', () => {
        it('should allow user to join room', () => {
            ...
            expect(roomService_1.roomService.joinRoom).toHaveBeenCalledWith('room-1', expect.any(Object));
            expect(mockSocket.join).toHaveBeenCalledWith('room-1');
        ...
        it('should reject if room does not exist', () => {
            roomService_1.roomService.roomExists.mockReturnValue(false);
            (0, roomHandler_1.registerRoomHandlers)(mockSocket, mockNamespace);
            const joinHandler = mockSocket.on.mock.calls.find((call) => call[0] === 'room:join')[1];
            const callback = jest.fn();
        ...
        it('should reject if user already in room', () => {
            roomService_1.roomService.isUserInRoom.mockReturnValue(true);
            (0, roomHandler_1.registerRoomHandlers)(mockSocket, mockNamespace);
            const joinHandler = mockSocket.on.mock.calls.find((call) => call[0] === 'room:join')[1];
            const callback = jest.fn();
            joinHandler({ roomId: 'room-1' }, callback);
        ...
        it('should reject if user is banned', () => {
            roomService_1.roomService.isUserBanned.mockReturnValue(true);
            (0, roomHandler_1.registerRoomHandlers)(mockSocket, mockNamespace);
            const joinHandler = mockSocket.on.mock.calls.find((call) => call[0] === 'room:join')[1];
            const callback = jest.fn();
            joinHandler({ roomId: 'room-1' }, callback);
    ...
    describe('room:leave', () => {
        it('should allow user to leave room', () => {
            ...
            expect(roomService_1.roomService.leaveRoom).toHaveBeenCalledWith('room-1', 'user-1');
    ...
    describe('room:broadcast', () => {
        beforeEach(() => {
            roomService_1.roomService.isUserInRoom.mockReturnValue(true);
            roomService_1.roomService.isUserMuted.mockReturnValue(false);
        });
        it('should broadcast to room', () => {
            ...
            expect(connectionService_1.connectionService.updateActivity).toHaveBeenCalledWith('socket-1');
            expect(presenceService_1.presenceService.updateActivity).toHaveBeenCalledWith('user-1');
            expect(callback).toHaveBeenCalledWith({ success: true });
        });
        it('should reject if not in room', () => {
            roomService_1.roomService.isUserInRoom.mockReturnValue(false);
        ...
        it('should reject if user is muted', () => {
            roomService_1.roomService.isUserMuted.mockReturnValue(true);
            (0, roomHandler_1.registerRoomHandlers)(mockSocket, mockNamespace);
            const broadcastHandler = mockSocket.on.mock.calls.find((call) => call[0] === 'room:broadcast')[1];
            const callback = jest.fn();
            broadcastHandler({
    ...
    describe('room:info', () => {
        ...
        it('should return error for non-existent room', () => {
            roomService_1.roomService.getRoom.mockReturnValue(undefined);
            (0, roomHandler_1.registerRoomHandlers)(mockSocket, mockNamespace);
            const infoHandler = mockSocket.on.mock.calls.find((call) => call[0] === 'room:info')[1];
            const callback = jest.fn();
    ...
    describe('room:my_rooms', () => {
        it('should return user rooms', () => {
            ...
            expect(roomService_1.roomService.getUserRooms).toHaveBeenCalledWith('user-1');
            expect(callback).toHaveBeenCalledWith({
                success: true,
    ...
    describe('room:kick', () => {
        beforeEach(() => {
            roomService_1.roomService.kickUser.mockReturnValue(true);
        });
        it('should kick user from room', () => {
            ...
            expect(roomService_1.roomService.kickUser).toHaveBeenCalledWith('room-1', 'user-2', 'user-1');
            expect(callback).toHaveBeenCalledWith({ success: true });
        });
        it('should handle kick errors', () => {
            roomService_1.roomService.kickUser.mockImplementation(() => {
    ...
    describe('room:ban', () => {
        beforeEach(() => {
            roomService_1.roomService.banUser.mockReturnValue(true);
        });
        it('should ban user from room', () => {
            ...
            expect(roomService_1.roomService.banUser).toHaveBeenCalledWith('room-1', 'user-2', 'user-1');
            expect(callback).toHaveBeenCalledWith({ success: true });
        });
        it('should handle ban errors', () => {
            roomService_1.roomService.banUser.mockImplementation(() => {
    ...
    describe('room:set_role', () => {
        beforeEach(() => {
            roomService_1.roomService.setUserRole.mockReturnValue(true);
        });
        it('should set user role', () => {
            ...
            expect(roomService_1.roomService.setUserRole).toHaveBeenCalledWith('room-1', 'user-2', 'admin', 'user-1');
            expect(callback).toHaveBeenCalledWith({ success: true });
        });
        it('should handle role setting errors', () => {
            roomService_1.roomService.setUserRole.mockImplementation(() => {
    ...
    describe('disconnect', () => {
        it('should clean up rooms on disconnect', () => {
            ...
            disconnectHandler();
            expect(roomService_1.roomService.leaveRoom).toHaveBeenCalledWith('room-1', 'user-1');
            expect(roomService_1.roomService.leaveRoom).toHaveBeenCalledWith('room-2', 'user-1');
        });
    });
});
//# sourceMappingURL=roomHandler.test.js.map