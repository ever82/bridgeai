import { HandoffStatus, HandoffRequestStatus, SenderType, HandoffSocketEvents, HandoffErrorCode, DEFAULT_HANDOFF_CONFIG, } from '@bridgeai/shared';
// In-memory store for handoff states (in production, use Redis)
const handoffStates = new Map();
const handoffRequests = new Map();
const handoffAuditLogs = [];
const userHandoffTimestamps = new Map();
/**
 * Register handoff event handlers
 */
export function registerHandoffHandlers(socket, nsp) {
    ...
    socket.on(HandoffSocketEvents.REQUEST_TAKEOVER, async (data, callback) => {
        try {
    ...
    socket.on(HandoffSocketEvents.REQUEST_HANDOFF, async (data, callback) => {
        try {
    ...
    socket.on(HandoffSocketEvents.CONFIRM_HANDOFF, async (data, callback) => {
        try {
    ...
    socket.on(HandoffSocketEvents.REJECT_HANDOFF, async (data, callback) => {
        try {
    ...
    socket.on(HandoffSocketEvents.CANCEL_HANDOFF, async (data, callback) => {
        try {
            const { requestId } = data;
            const request = handoffRequests.get(requestId);
            if (!request) {
    ...
    // Get handoff status
    socket.on('handoff:get_status', (data, callback) => {
 ...
 */
async function completeTakeover(conversationId, requestId, userId, nsp) {
    const state = handoffStates.get(conversationId);
    if (!state)
        return;
 ...
 */
async function completeHandoff(conversationId, requestId, nsp) {
    const state = handoffStates.get(conversationId);
    if (!state)
        return;
    const previousStatus = state.currentStatus;
 ...
 */
function handleTimeout(requestId, conversationId, nsp) {
    const request = handoffRequests.get(requestId);
    const state = handoffStates.get(conversationId);
    if (!request || !state)
        return;
 ...
 */
export function getHandoffState(conversationId) {
    return handoffStates.get(conversationId);
}
/**
 * Get handoff request by ID
 */
export function getHandoffRequest(requestId) {
    return handoffRequests.get(requestId);
}
/**
 * Get audit logs for a conversation
 */
export function getHandoffAuditLogs(conversationId) {
...
//# sourceMappingURL=handoffHandler.js.map