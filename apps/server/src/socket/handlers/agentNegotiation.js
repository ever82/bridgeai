 ...
 */
export function registerAgentNegotiationHandlers(socket, nsp) {
    /**
     * 创建协商房间
     * Event: negotiation:create_room
     */
    socket.on('negotiation:create_room', async (data, callback) => {
        try {
            if (!socket.user?.id) {
     ...
     */
    socket.on('negotiation:generate_introduction', async (data, callback) => {
        try {
            if (!socket.user?.id) {
                callback?.({ success: false, error: 'Authentication required' });
                return;
     ...
     */
    socket.on('negotiation:submit_offer', async (data, callback) => {
        try {
            if (!socket.user?.id) {
                callback?.({ success: false, error: 'Authentication required' });
                return;
     ...
     */
    socket.on('negotiation:get_questions', async (data, callback) => {
        try {
            if (!socket.user?.id) {
                callback?.({ success: false, error: 'Authentication required' });
                return;
     ...
     */
    socket.on('negotiation:ask_question', async (data, callback) => {
        try {
            if (!socket.user?.id) {
                callback?.({ success: false, error: 'Authentication required' });
                return;
     ...
     */
    socket.on('negotiation:compare_offers', async (data, callback) => {
        try {
            if (!socket.user?.id) {
                callback?.({ success: false, error: 'Authentication required' });
                return;
     ...
     */
    socket.on('negotiation:get_recommendation', async (data, callback) => {
        try {
            if (!socket.user?.id) {
                callback?.({ success: false, error: 'Authentication required' });
                return;
     ...
     */
    socket.on('negotiation:confirm_selection', async (data, callback) => {
        try {
            if (!socket.user?.id) {
                callback?.({ success: false, error: 'Authentication required' });
                return;
     ...
     */
    socket.on('negotiation:join_room', async (data, callback) => {
        try {
            if (!socket.user?.id) {
                callback?.({ success: false, error: 'Authentication required' });
                return;
     ...
     */
    socket.on('negotiation:leave_room', (data, callback) => {
        try {
            const { roomId } = data;
     ...
     */
    socket.on('negotiation:get_status', async (data, callback) => {
        try {
            if (!socket.user?.id) {
                callback?.({ success: false, error: 'Authentication required' });
                return;
