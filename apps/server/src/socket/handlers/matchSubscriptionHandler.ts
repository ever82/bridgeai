...
import {
  ...
  SubscriptionEvent,
} from '../../services/matching/querySubscriptionManager';
...
export function registerMatchSubscriptionHandlers(
  ...
  const socketSubscriptionIds = new Set<string>();
   ...
   */
  const eventListener = (event: SubscriptionEvent): void => {
  ...
  socket.on(
    'subscribe',
    async (
      payload: { query: FilterDSL; filters?: Record<string, unknown> },
      ack?: (response: { ok: boolean; subscriptionId?: string; error?: string }) => void
    ) => {
      try {
        if (!payload?.query) {
        ...
        const message =
          err instanceof MatchQueryValidationError
            ? err.message
