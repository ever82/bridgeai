 ...
 */
export function registerRoomHandlers(socket, nsp) {
    ...
    socket.on('room:create', async (data, callback) => {
        try {
            ...
            roomService.joinRoom(roomId, {
                ...
                socketId: socket.id,
                role: 'owner',
                deviceInfo: socket.handshake.query,
            });
            // Join socket.io room
            socket.join(roomId);
    ...
    // Broadcast to room
    socket.on('room:broadcast', (data, callback) => {
        try {
            if (!userId) {
                callback?.({ success: false, error: 'Authentication required' });
                return;
    ...
    socket.on('room:broadcast_multi', (data, callback) => {
        try {
            if (!userId) {
                callback?.({ success: false, error: 'Authentication required' });
                return;
    ...
    // Get room members
    socket.on('room:members', (data, callback) => {
        try {
            const { roomId } = data;
            if (!roomService.roomExists(roomId)) {
                callback?.({ success: false, error: 'Room not found' });
    ...
    socket.on('room:unban', (data, callback) => {
        try {
            if (!userId) {
                callback?.({ success: false, error: 'Authentication required' });
                return;
    ...
    socket.on('room:set_role', (data, callback) => {
        try {
            if (!userId) {
                callback?.({ success: false, error: 'Authentication required' });
                return;
