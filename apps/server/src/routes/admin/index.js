...
import { getBlockedIPs, unblockIP, blockIP, getSecurityStats, getSecurityEvents, resolveSecurityEvent, bulkBlockIPs, bulkUnblockIPs, } from '../../services/firewall';
import { getWhitelistedIPs, addToWhitelist, removeFromWhitelist, addWhitelistRange, removeWhitelistRange, getConfig as getIPFilterConfig, updateConfig as updateIPFilterConfig, } from '../../middleware/ipFilter';
...
router.get('/security/realtime', (_req, res) => {
...
router.get('/security/ddos', (_req, res) => {
...
// Block an IP
router.post('/security/blocked-ips', validate({ body: blockIPSchema }), (req, res) => {
...
// Bulk block IPs
router.post('/security/blocked-ips/bulk', validate({ body: bulkBlockSchema }), (req, res) => {
...
// Unblock an IP
router.delete('/security/blocked-ips/:ip', (req, res) => {
    const { ip } = req.params;
    const adminId = req.user?.id;
...
// Bulk unblock IPs
router.post('/security/blocked-ips/bulk-unblock', validate({ body: z.object({ ips: z.array(z.string().ip()).min(1).max(100) }) }), (req, res) => {
    const { ips } = req.body;
    const adminId = req.user?.id;
...
// Get security events
router.get('/security/events', validate({ query: dateRangeSchema }), (req, res) => {
...
router.patch('/security/events/:id/resolve', (req, res) => {
    const { id } = req.params;
    const adminId = req.user?.id;
...
// Get whitelisted IPs
router.get('/security/whitelist', (_req, res) => {
...
// Add IP to whitelist
router.post('/security/whitelist', validate({ body: whitelistSchema }), (req, res) => {
...
router.post('/security/whitelist/ranges', validate({ body: cidrSchema }), (req, res) => {
...
router.delete('/security/whitelist/ranges/:range', (req, res) => {
...
router.get('/security/export', (req, res) => {
    const { format = 'json', since } = req.query;
    const data = exportSecurityData(format, since ? new Date(since) : undefined);
...
router.patch('/security/config/mode', validate({
    body: z.object({
