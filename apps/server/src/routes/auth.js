...
import { registerSchema, loginSchema, refreshTokenSchema, forgotPasswordSchema, resetPasswordSchema, changePasswordSchema, oauthBindSchema, } from '../schemas/authSchemas';
const router = Router();
// 登录限流
 ...
 * 申请密码重置
 */
router.post('/forgot-password', passwordResetLimiter, validate({ body: forgotPasswordSchema }), async (req, res) => {
    try {
...
router.get('/oauth/:provider', (req, res) => {
    try {
        ...
        const authUrl = oauthService.getOAuthAuthorizationUrl(provider, state);
        res.json({
            success: true,
            data: { authUrl },
        });
        ...
        logger.error('Get OAuth URL failed', error);
        res.status(500).json({
            success: false,
            error: error.message,
        });
 ...
 * 绑定 OAuth 账户
 */
router.post('/oauth/:provider/bind', authenticate, validate({ body: oauthBindSchema }), async (req, res) => {
    try {
        const { provider } = req.params;
        const { code } = req.body;
        const userId = req.user?.id || req.user?.userId;
        ...
        await oauthService.bindOAuthAccount(userId, provider, code);
        res.json({
            success: true,
        ...
        logger.error('OAuth bind failed', error);
        res.status(400).json({
            success: false,
            error: error.message,
        });
 ...
 * 解绑 OAuth 账户
 */
router.delete('/oauth/:provider', authenticate, async (req, res) => {
    try {
        const { provider } = req.params;
        const userId = req.user?.id || req.user?.userId;
        if (!userId) {
        ...
        logger.error('OAuth unbind failed', error);
        res.status(500).json({
            success: false,
            error: error.message,
        });
 ...
 * 获取 OAuth 绑定列表
 */
router.get('/oauth/connections', authenticate, async (req, res) => {
    try {
        const userId = req.user?.id || req.user?.userId;
        if (!userId) {
            return res.status(401).json({ error: '未登录' });
        ...
        logger.error('Get OAuth connections failed', error);
        res.status(500).json({
            success: false,
            error: error.message,
        });
