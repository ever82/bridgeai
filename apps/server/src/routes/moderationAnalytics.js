 ...
 * @access Admin only
 */
router.get('/content-types', asyncHandler(async (req, res) => {
    if (req.user?.role !== 'admin') {
        throw new AppError('Admin access required', 'FORBIDDEN', 403);
    }
    const { startDate, endDate } = parseDateRange(req.query);
 ...
 * @access Admin only
 */
router.get('/moderator/:id', asyncHandler(async (req, res) => {
    if (req.user?.role !== 'admin') {
        throw new AppError('Admin access required', 'FORBIDDEN', 403);
    }
    const { id } = req.params;
    const { startDate, endDate } = parseDateRange(req.query);
    const performance = await moderationAnalytics.getModeratorPerformance(id, startDate, endDate);
