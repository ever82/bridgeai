 ...
 * @access Private
 */
router.post('/offers/extract', authenticate, asyncHandler(async (req, res) => {
    if (!req.user) {
    ...
    if (!result.success || !result.data) {
        throw new AppError(`Failed to extract offer: ${result.errors?.join(', ')}`, 'EXTRACTION_FAILED', 400);
    }
 ...
 * @access Private
 */
router.post('/offers/preview', authenticate, asyncHandler(async (req, res) => {
    if (!req.user) {
        throw new AppError('Unauthorized', 'UNAUTHORIZED', 401);
    }
    const { text } = req.body;
...
//# sourceMappingURL=offerExtraction.js.map