 ...
 * @access  Private
 */
router.post('/extract-supply', authenticateToken, validate({ body: extractSupplySchema }), async (req, res) => {
    try {
        await ensureService();
        ...
        const statusCode = error instanceof Error && error.message.includes('Circuit breaker') ? 503 : 500;
 ...
 * @access  Private
 */
router.post('/extract-supply/bulk', authenticateToken, validate({ body: bulkExtractSupplySchema }), async (req, res) => {
    try {
        await ensureService();
 ...
 * @access  Private
 */
router.get('/extract-supply/quality/:agentId', authenticateToken, async (req, res) => {
    try {
 ...
 */
router.post('/extract-supply/sync', authenticateToken, validate({ body: updateSupplySchema }), async (req, res) => {
    try {
 ...
 */
async function storeExtractionResult(agentId, scene, result) {
