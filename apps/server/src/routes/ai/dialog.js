 ...
 * @access  Private
 */
router.post('/sessions', authenticateToken, validate(createSessionSchema), async (req, res) => {
    try {
        const { type, participants, scene, initialContext } = req.body;
        const session = await agentDialogService.createSession(type, participants, scene, initialContext);
        res.status(201).json({
            success: true,
            data: {
                session,
 ...
 * @access  Private
 */
router.post('/sessions/:sessionId/messages', authenticateToken, validate(generateMessageSchema), async (req, res) => {
    try {
        const { sessionId } = req.params;
 ...
 * @access  Private
 */
router.post('/sessions/:sessionId/messages/stream', authenticateToken, validate(generateMessageSchema), async (req, res) => {
    try {
        const { sessionId } = req.params;
        const { senderId, senderType, content, options } = req.body;
        const session = await agentDialogService.getSessionAsync(sessionId);
 ...
 * @access  Private
 */
router.post('/agent-to-agent', authenticateToken, validate(agentDialogSchema), async (req, res) => {
    try {
 ...
 * @access  Private
 */
router.post('/user-to-agent', authenticateToken, validate(userDialogSchema), async (req, res) => {
    try {
 ...
 * @access  Private
 */
router.get('/participants/:participantId/sessions', authenticateToken, async (req, res) => {
    try {
 ...
 * @access  Private
 */
router.post('/sessions/:sessionId/archive', authenticateToken, async (req, res) => {
    try {
        const { sessionId } = req.params;
