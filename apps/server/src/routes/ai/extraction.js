 ...
 * 需求提取主端点
 */
router.post('/extract-demand', async (req, res) => {
    ...
    try {
        ...
        // Step 3: Validate
        const validationReport = extractionValidator.validate(mappingResult.data, schema, demand, {
        ...
        const clarificationNeeded = demand.clarificationNeeded ||
 ...
 * 批量需求提取
 */
router.post('/extract-demand/batch', async (req, res) => {
    const startTime = Date.now();
 ...
 * 确认提取结果
 */
router.post('/extract-demand/:id/confirm', async (req, res) => {
    const { id } = req.params;
 ...
 * 获取提取状态
 */
router.get('/extract-demand/:id/status', async (req, res) => {
    const { id } = req.params;
    try {
 ...
 * 提交提取反馈
 */
router.post('/extract-demand/feedback', async (req, res) => {
 ...
 * WebSocket实时提取反馈
 */
export function handleWebSocketConnection(ws, requestId) {
 ...
 */
async function handleRealtimeExtraction(ws, payload) {
 ...
 * 获取场景配置
 */
router.get('/scenes/:scene/config', async (req, res) => {
 ...
 * 消费者需求 AI 提取端点
 */
router.post('/extract-consumer-demand', async (req, res) => {
    const startTime = Date.now();
