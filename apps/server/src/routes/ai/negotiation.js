...
const validateRequest = _validateRequest;
 ...
 * @access Private
 */
router.post('/rooms', requireAuth, validateRequest({ body: createRoomBodySchema }), async (req, res, next) => {
    try {
        ...
        const merchantAgents = merchantAgentIds.map((id, index) => ({
            id,
            type: 'merchant',
            name: `商家Agent ${index + 1}`,
        }));
        const room = await agentNegotiationService.createNegotiationRoom(consumerAgent, merchantAgents, consumerDemand, config);
        res.status(201).json({
            success: true,
            data: {
                roomId: room.id,
 ...
 * @access Private
 */
router.get('/rooms/:roomId', requireAuth, validateRequest({ params: roomIdParamSchema }), async (req, res, next) => {
    try {
 ...
 * @access Private
 */
router.post('/rooms/:roomId/introduction', requireAuth, validateRequest({ params: roomIdParamSchema }), async (req, res, next) => {
    try {
        const { roomId } = req.params;
 ...
 * @access Private
 */
router.post('/rooms/:roomId/offers', requireAuth, validateRequest({ params: roomIdParamSchema, body: submitOfferBodySchema }), async (req, res, next) => {
    try {
        const { roomId } = req.params;
        const { merchantId, offer } = req.body;
        logger.info('Submitting merchant offer', { roomId, merchantId, offerId: offer.id });
        const message = await agentNegotiationService.generateMerchantOfferPresentation(roomId, merchantId, offer);
        res.json({
            success: true,
            data: {
                messageId: message.id,
 ...
 * @access Private
 */
router.get('/rooms/:roomId/questions', requireAuth, validateRequest({ params: roomIdParamSchema, query: questionsQuerySchema }), async (req, res, next) => {
    try {
        const { roomId } = req.params;
        ...
        const questions = await agentNegotiationService.generateFollowUpQuestions(roomId, targetMerchantId);
        res.json({
            success: true,
            data: {
 ...
 * @access Private
 */
router.post('/rooms/:roomId/questions', requireAuth, validateRequest({ params: roomIdParamSchema, body: followUpQuestionBodySchema }), async (req, res, next) => {
    try {
        const { roomId } = req.params;
 ...
 * @access Private
 */
router.post('/rooms/:roomId/compare', requireAuth, validateRequest({ params: roomIdParamSchema }), async (req, res, next) => {
    try {
        const { roomId } = req.params;
        logger.info('Comparing offers', { roomId });
        const comparisonResult = await agentNegotiationService.compareOffers(roomId);
        const reportMessage = await agentNegotiationService.generateComparisonReport(roomId, comparisonResult);
        res.json({
            success: true,
            data: {
                comparison: {
 ...
 * @access Private
 */
router.post('/rooms/:roomId/recommend', requireAuth, validateRequest({ params: roomIdParamSchema }), async (req, res, next) => {
    try {
        const { roomId } = req.params;
        ...
        const message = await agentNegotiationService.generateRecommendationMessage(roomId, recommendation);
        res.json({
            success: true,
            data: {
                recommendation: {
 ...
 * @access Private
 */
router.post('/rooms/:roomId/confirm', requireAuth, validateRequest({ params: roomIdParamSchema, body: confirmSelectionBodySchema }), async (req, res, next) => {
    try {
        const { roomId } = req.params;
 ...
 * @access Private
 */
router.get('/rooms/:roomId/messages', requireAuth, validateRequest({ params: roomIdParamSchema }), async (req, res, next) => {
    try {
        const { roomId } = req.params;
