...
describe('Supply AI Routes', () => {
    ...
    beforeEach(() => {
        ...
        app.use('/api/v1/ai', supply_1.default);
    ...
    describe('POST /api/v1/ai/extract-supply', () => {
        ...
        it('should store result when agent_id is provided', async () => {
            mockExtract.mockResolvedValue(createMockExtractionResult());
            mockedPrisma.scene.upsert.mockResolvedValue({ id: 'scene-1' });
            mockedPrisma.agentProfile.upsert.mockResolvedValue({});
            const response = await (0, supertest_1.default)(app)
                .post('/api/v1/ai/extract-supply')
                .send({
                text: '我是一名专业摄影师，提供人像摄影和婚礼摄影服务',
        ...
        it('should handle store failure gracefully', async () => {
            mockExtract.mockResolvedValue(createMockExtractionResult());
            mockedPrisma.scene.upsert.mockRejectedValue(new Error('DB error'));
            const response = await (0, supertest_1.default)(app)
                .post('/api/v1/ai/extract-supply')
                .send({
                text: '我是一名专业摄影师，提供人像摄影和婚礼摄影服务',
    ...
    describe('GET /api/v1/ai/extract-supply/quality/:agentId', () => {
        it('should return quality report for agent', async () => {
            mockedPrisma.agentProfile.findMany.mockResolvedValue([
                {
                    sceneId: 'scene-1',
                    l2Data: { title: '摄影服务' },
        ...
        it('should handle database errors', async () => {
            mockedPrisma.agentProfile.findMany.mockRejectedValue(new Error('Database connection failed'));
            const response = await (0, supertest_1.default)(app)
                .get('/api/v1/ai/extract-supply/quality/test-agent-id')
                .expect(500);
            expect(response.body.success).toBe(false);
...
//# sourceMappingURL=supply.test.js.map