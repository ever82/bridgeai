...
const extraction_1 = __importDefault(require("../extraction"));
// Mock dependencies
...
const demandExtractionService_1 = require("../../../services/ai/demandExtractionService");
const demandToL2Mapper_1 = require("../../../services/ai/mappers/demandToL2Mapper");
const extractionValidator_1 = require("../../../services/ai/validators/extractionValidator");
const mockedDemandExtractionService = demandExtractionService_1.demandExtractionService;
const mockedDemandToL2Mapper = demandToL2Mapper_1.demandToL2Mapper;
const mockedExtractionValidator = extractionValidator_1.extractionValidator;
const mockedGetL2Schema = shared_1.getL2Schema;
describe('AI Extraction Routes', () => {
    ...
    beforeEach(() => {
        ...
        app.use('/api/v1/ai', extraction_1.default);
        jest.clearAllMocks();
    });
    describe('POST /api/v1/ai/extract-demand', () => {
        ...
        it('should extract demand successfully', async () => {
            mockedGetL2Schema.mockReturnValue(mockSchema);
            mockedDemandExtractionService.extract.mockResolvedValue(mockDemand);
            mockedDemandToL2Mapper.map.mockReturnValue(mockMappingResult);
            mockedExtractionValidator.validate.mockReturnValue(mockValidationReport);
        ...
        it('should include clarification needed flag', async () => {
            ...
            mockedGetL2Schema.mockReturnValue(mockSchema);
            mockedDemandExtractionService.extract.mockResolvedValue(demandWithClarification);
            mockedDemandToL2Mapper.map.mockReturnValue(mockMappingResult);
            mockedExtractionValidator.validate.mockReturnValue(mockValidationReport);
            const response = await (0, supertest_1.default)(app)
                .post('/api/v1/ai/extract-demand')
        ...
        it('should handle validation errors', async () => {
            ...
            mockedDemandExtractionService.extract.mockResolvedValue(mockDemand);
            mockedDemandToL2Mapper.map.mockReturnValue(mockMappingResult);
            mockedExtractionValidator.validate.mockReturnValue(validationWithErrors);
            const response = await (0, supertest_1.default)(app)
                .post('/api/v1/ai/extract-demand')
                .send({
                text: '我想在北京拍摄',
    ...
    describe('GET /api/v1/ai/scenes/:scene/config', () => {
        it('should return scene configuration', async () => {
            ...
            mockedGetL2Schema.mockReturnValue(mockSchema);
            const response = await (0, supertest_1.default)(app).get('/api/v1/ai/scenes/visionShare/config').expect(200);
            expect(response.body.success).toBe(true);
        ...
        it('should return 404 for unknown scene', async () => {
            mockedGetL2Schema.mockReturnValue(null);
            const response = await (0, supertest_1.default)(app).get('/api/v1/ai/scenes/unknown/config').expect(404);
            expect(response.body.success).toBe(false);
