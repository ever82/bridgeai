...
const aiRoute_1 = __importDefault(require("../../aiRoute"));
...
describe('AI Routes', () => {
    ...
    beforeEach(() => {
        ...
        app.use('/api/v1/ai', aiRoute_1.default);
        jest.clearAllMocks();
    ...
    describe('POST /api/v1/ai/embeddings', () => {
        it('should return embedding response', async () => {
            ...
            expect(mockEmbeddings).toHaveBeenCalledWith(expect.objectContaining({
                model: 'text-embedding-3-small',
                input: 'Hello world',
            }));
        });
        it('should accept custom model', async () => {
            ...
            expect(res.status).toBe(200);
            expect(mockEmbeddings).toHaveBeenCalledWith(expect.objectContaining({ model: 'custom-emb' }));
        });
    ...
    describe('GET /api/v1/ai/health', () => {
        it('should return health status', async () => {
            const res = await (0, supertest_1.default)(app).get('/api/v1/ai/health');
            expect(res.status).toBe(200);
            expect(res.body.success).toBe(true);
    ...
    describe('GET /api/v1/ai/metrics', () => {
        it('should return prometheus metrics', async () => {
            const res = await (0, supertest_1.default)(app).get('/api/v1/ai/metrics');
            expect(res.status).toBe(200);
    ...
    describe('GET /api/v1/ai/circuit-breakers', () => {
        it('should return circuit breaker stats', async () => {
            const res = await (0, supertest_1.default)(app).get('/api/v1/ai/circuit-breakers');
            expect(res.status).toBe(200);
            expect(res.body.success).toBe(true);
    ...
    describe('GET /api/v1/ai/models', () => {
        it('should return model list', async () => {
            ...
            const res = await (0, supertest_1.default)(app).get('/api/v1/ai/models');
            expect(res.status).toBe(200);
            expect(res.body.success).toBe(true);
...
//# sourceMappingURL=aiRoute.test.js.map