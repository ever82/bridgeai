...
jest.mock('../../../middleware/validation', () => ({
    validate: (_schema) => (_req, _res, next) => next(),
}));
const dialog_1 = __importDefault(require("../dialog"));
const agentDialogService_1 = require("../../../services/ai/agentDialogService");
const router = (0, express_2.Router)();
router.use('/dialog', dialog_1.default);
...
describe('Dialog Routes', () => {
    ...
    describe('POST /api/v1/ai/dialog/sessions', () => {
        it('should create a new dialog session', async () => {
            agentDialogService_1.agentDialogService.createSession.mockResolvedValue(mockSession);
        ...
        it('should return 500 on error', async () => {
            agentDialogService_1.agentDialogService.createSession.mockRejectedValue(new Error('DB error'));
            const response = await (0, supertest_1.default)(app)
                .post('/api/v1/ai/dialog/sessions')
                .send({
                type: 'agent_to_user',
    ...
    describe('GET /api/v1/ai/dialog/sessions/:sessionId', () => {
        it('should get session by ID', async () => {
            agentDialogService_1.agentDialogService.getSessionAsync.mockResolvedValue(mockSession);
            const response = await (0, supertest_1.default)(app).get('/api/v1/ai/dialog/sessions/dialog-123');
            expect(response.status).toBe(200);
            expect(response.body.success).toBe(true);
            expect(response.body.data.session.id).toBe('dialog-123');
        });
        it('should return 404 when session not found', async () => {
            agentDialogService_1.agentDialogService.getSessionAsync.mockResolvedValue(null);
            const response = await (0, supertest_1.default)(app).get('/api/v1/ai/dialog/sessions/not-found');
    ...
    describe('GET /api/v1/ai/dialog/sessions/:sessionId/messages', () => {
        it('should get session messages', async () => {
            agentDialogService_1.agentDialogService.getSessionAsync.mockResolvedValue(mockSession);
            agentDialogService_1.agentDialogService.getSessionMessages.mockResolvedValue([mockMessage]);
            const response = await (0, supertest_1.default)(app).get('/api/v1/ai/dialog/sessions/dialog-123/messages');
            expect(response.status).toBe(200);
            expect(response.body.success).toBe(true);
    ...
    describe('POST /api/v1/ai/dialog/sessions/:sessionId/messages', () => {
        it('should generate a message in session', async () => {
            agentDialogService_1.agentDialogService.generateMessage.mockResolvedValue(mockMessage);
            const response = await (0, supertest_1.default)(app)
    ...
    describe('POST /api/v1/ai/dialog/agent-to-agent', () => {
        it('should handle agent-to-agent dialog', async () => {
            agentDialogService_1.agentDialogService.agentToAgentDialog.mockResolvedValue(mockMessage);
            const response = await (0, supertest_1.default)(app).post('/api/v1/ai/dialog/agent-to-agent').send({
    ...
    describe('POST /api/v1/ai/dialog/user-to-agent', () => {
        it('should handle user-to-agent dialog', async () => {
            agentDialogService_1.agentDialogService.userToAgentDialog.mockResolvedValue(mockMessage);
            const response = await (0, supertest_1.default)(app).post('/api/v1/ai/dialog/user-to-agent').send({
                userId: 'user-1',
                agentId: 'agent-1',
                content: 'Hello',
                scene: 'test',
    ...
    describe('GET /api/v1/ai/dialog/participants/:participantId/sessions', () => {
        it('should get participant sessions', async () => {
            agentDialogService_1.agentDialogService.getParticipantHistory.mockResolvedValue([mockSession]);
            const response = await (0, supertest_1.default)(app).get('/api/v1/ai/dialog/participants/user-1/sessions');
            expect(response.status).toBe(200);
            expect(response.body.success).toBe(true);
    ...
    describe('PUT /api/v1/ai/dialog/sessions/:sessionId/context', () => {
        it('should update session context', async () => {
            agentDialogService_1.agentDialogService.updateSessionContext.mockResolvedValue(mockSession);
            const response = await (0, supertest_1.default)(app)
    ...
    describe('POST /api/v1/ai/dialog/sessions/:sessionId/archive', () => {
        it('should archive session', async () => {
            agentDialogService_1.agentDialogService.archiveSession.mockResolvedValue(undefined);
            const response = await (0, supertest_1.default)(app).post('/api/v1/ai/dialog/sessions/dialog-123/archive');
            expect(response.status).toBe(200);
            expect(response.body.success).toBe(true);
        });
    });
    describe('GET /api/v1/ai/dialog/stats', () => {
        it('should return dialog service stats', async () => {
            const response = await (0, supertest_1.default)(app).get('/api/v1/ai/dialog/stats');
            expect(response.status).toBe(200);
            expect(response.body.success).toBe(true);
