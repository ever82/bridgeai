 ...
 * @access Private
 */
router.get('/agents/:agentId/profile', authenticate, asyncHandler(async (req, res) => {
    if (!req.user) {
    ...
    res.json(ApiResponse.success({
        ...
        completion: {
            ...
            filledSections: quality.metrics.completenessScore > 0
 ...
 * @access Private
 */
router.post('/agents/:agentId/profile', authenticate, asyncHandler(async (req, res) => {
    if (!req.user) {
        throw new AppError('Unauthorized', 'UNAUTHORIZED', 401);
    }
    const { agentId } = req.params;
    const { basicConditions, personality, interests, lifestyle, expectations, description, privacySettings, } = req.body;
    const data = {
        agentId,
        basicConditions,
        personality,
 ...
 * @access Private
 */
router.put('/agents/:agentId/profile', authenticate, asyncHandler(async (req, res) => {
    if (!req.user) {
        throw new AppError('Unauthorized', 'UNAUTHORIZED', 401);
    }
    const { agentId } = req.params;
 ...
 * @access Private
 */
router.delete('/agents/:agentId/profile', authenticate, asyncHandler(async (req, res) => {
    if (!req.user) {
        throw new AppError('Unauthorized', 'UNAUTHORIZED', 401);
    }
    const { agentId } = req.params;
 ...
 * @access Private
 */
router.get('/agents/:agentId/profile/completeness', authenticate, asyncHandler(async (req, res) => {
    if (!req.user) {
        throw new AppError('Unauthorized', 'UNAUTHORIZED', 401);
    }
    const { agentId } = req.params;
 ...
 * @access Private
 */
router.get('/agents/:agentId/profile/quality', authenticate, asyncHandler(async (req, res) => {
    if (!req.user) {
        throw new AppError('Unauthorized', 'UNAUTHORIZED', 401);
    }
    const { agentId } = req.params;
