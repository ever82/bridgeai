...
// ---------------------------------------------------------------------------
router.post('/', authenticate, reportLimiter, falseReportPenaltyCheck, asyncHandler(async (req, res) => {
    if (!req.user) {
        throw new AppError('Unauthorized', 'UNAUTHORIZED', 401);
    }
    ...
    if (!targetType || !targetId || !reason) {
        throw new AppError('targetType, targetId, and reason are required', 'INVALID_REQUEST', 400);
    }
    // Validate targetType
    const validTargetTypes = ['MESSAGE', 'USER', 'CONTENT'];
    if (!validTargetTypes.includes(targetType.toUpperCase())) {
        throw new AppError(`Invalid targetType. Must be one of: ${validTargetTypes.join(', ')}`, 'INVALID_REQUEST', 400);
    }
    // Validate reason
    if (!reportReasonEnum.includes(reason.toUpperCase())) {
        throw new AppError(`Invalid reason. Must be one of: ${reportReasonEnum.join(', ')}`, 'INVALID_REQUEST', 400);
    }
    ...
    if (existing) {
        throw new AppError('You already have a pending report for this target', 'DUPLICATE_REPORT', 400);
    }
...
// ---------------------------------------------------------------------------
router.post('/:id/handle', authenticate, asyncHandler(async (req, res) => {
    if (!req.user) {
        throw new AppError('Unauthorized', 'UNAUTHORIZED', 401);
    }
    const { id } = req.params;
    ...
    if (!action || !validActions.includes(action.toUpperCase())) {
        throw new AppError(`Invalid action. Must be one of: ${validActions.join(', ')}`, 'INVALID_REQUEST', 400);
    }
...
// ---------------------------------------------------------------------------
router.post('/:id/evidence', authenticate, uploadImages('files', 5, 10 * 1024 * 1024), handleUploadError, asyncHandler(async (req, res) => {
    if (!req.user) {
        throw new AppError('Unauthorized', 'UNAUTHORIZED', 401);
    }
    const { id } = req.params;
    ...
    if (report.reporterId !== req.user.id) {
        throw new AppError('Only the reporter can add evidence', 'FORBIDDEN', 403);
    }
    // Cannot add evidence to resolved/dismissed reports
    if (report.status !== 'PENDING') {
        throw new AppError('Cannot add evidence to a closed report', 'INVALID_REQUEST', 400);
    }
    ...
    res.status(201).json(ApiResponse.success({ filePaths, evidence: updated.evidence }, 'Evidence uploaded'));
}));
