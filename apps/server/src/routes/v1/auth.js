...
router.post('/refresh', async (req, res, next) => {
    try {
        ...
        await rotateRefreshToken(refreshToken, tokens.refreshToken, decoded.userId, req.body.deviceInfo, req.ip);
        res.json(ApiResponse.success({
            tokens: {
...
router.post('/logout', authenticate, async (req, res, next) => {
    try {
        ...
        const { refreshToken } = req.body;
        if (refreshToken) {
            await revokeRefreshToken(refreshToken, req.user?.userId || req.user?.id);
        }
