...
import { getProvinces, getCitiesByProvince, getDistrictsByCity, getLocationHierarchy, getLocationNameByCode, searchLocations, findAgentsWithinRadius, } from '../services/locationSearchService';
...
router.get('/hierarchy', async (req, res, next) => {
    try {
        ...
        const hierarchy = await getLocationHierarchy(provinceCode, cityCode);
        res.json({
            success: true,
            data: hierarchy,
        });
...
router.get('/agents', authenticate, validate({ query: locationFilterSchema }), async (req, res, next) => {
    try {
        ...
        const results = await searchAgentsByLocation(filter, parseInt(page) || 1, parseInt(limit) || 20);
        res.json({
            success: true,
            data: results,
        });
 ...
 */
router.get('/agents/nearby', authenticate, validate({ query: radiusSearchSchema }), async (req, res, next) => {
    try {
 ...
 */
router.get('/distance', authenticate, validate({ query: distanceSchema }), async (req, res, next) => {
    try {
        const { agentId1, agentId2 } = req.query;
        const distance = await getDistanceBetweenAgents(agentId1, agentId2);
        if (distance === null) {
            return res.status(404).json({
                success: false,
                error: {
