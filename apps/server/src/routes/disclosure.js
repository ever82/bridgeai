...
// ============================================
async function requireAgentOwner(agentId, userId) {
 ...
 */
router.get('/:agentId/settings', authenticate, async (req, res) => {
    try {
 ...
 */
router.put('/:agentId/settings', authenticate, async (req, res) => {
    try {
        const { agentId } = agentIdSchema.parse(req.params);
 ...
 */
router.post('/:agentId/bulk', authenticate, async (req, res) => {
    try {
        const { agentId } = agentIdSchema.parse(req.params);
        ...
        const settings = await disclosureService.bulkUpdateDisclosure(agentId, fieldUpdates, userId, notifyAffectedUsers);
        res.json({ success: true, data: settings });
    }
    catch (error) {
 ...
 */
router.post('/:agentId/reset', authenticate, async (req, res) => {
    try {
        const { agentId } = agentIdSchema.parse(req.params);
        const userId = req.user?.id || '';
        if (!(await requireAgentOwner(agentId, userId))) {
 ...
 */
router.post('/:agentId/check', authenticate, async (req, res) => {
    try {
        const { agentId } = agentIdSchema.parse(req.params);
 ...
 */
router.post('/:agentId/filter', authenticate, async (req, res) => {
    try {
        const { agentId } = agentIdSchema.parse(req.params);
 ...
 */
router.get('/:agentId/preview', authenticate, async (req, res) => {
    try {
        const { agentId } = agentIdSchema.parse(req.params);
        const settings = await disclosureService.getDisclosureSettings(agentId);
        const stages = [
 ...
 */
router.get('/:agentId/access-log', authenticate, async (req, res) => {
    try {
        const { agentId } = agentIdSchema.parse(req.params);
 ...
 */
router.get('/:agentId/stats', authenticate, async (req, res) => {
    try {
        const { agentId } = agentIdSchema.parse(req.params);
 ...
 */
router.get('/levels/info', (_req, res) => {
...
function maskFieldForPreview(fieldName, stage) {
