...
router.post('/', authenticate, asyncHandler(async (req, res) => {
    ...
    const { matchId, revieweeId, sceneType, rating, honestyRating, politenessRating, responsivenessRating, satisfactionRating, title, content, tags, } = req.body;
    if (!matchId || !revieweeId || !rating || !content) {
        throw new AppError('缺少必要参数', 'INVALID_REQUEST', 400);
    }
    // 反作弊检查
    const antiCheatResult = await antiCheatService.performAntiCheatCheck(matchId, req.user.id, revieweeId, content);
...
router.get('/received', authenticate, asyncHandler(async (req, res) => {
    ...
    const sceneType = req.query.sceneType;
...
router.put('/:id', authenticate, asyncHandler(async (req, res) => {
    ...
    const { id } = req.params;
    const { rating, honestyRating, politenessRating, responsivenessRating, satisfactionRating, title, content, tags, } = req.body;
    // 如果修改了内容，进行审核
    if (content) {
        const moderationResult = await moderationService.moderateContent(content, title);
        if (moderationResult.blocked) {
 ...
 * @access Private
 */
router.put('/:id/reply', authenticate, asyncHandler(async (req, res) => {
    if (!req.user) {
        throw new AppError('Unauthorized', 'UNAUTHORIZED', 401);
    }
 ...
 * @access Private
 */
router.delete('/reply/:replyId', authenticate, asyncHandler(async (req, res) => {
    if (!req.user) {
        throw new AppError('Unauthorized', 'UNAUTHORIZED', 401);
    }
 ...
 * @access Private
 */
router.post('/:id/report', authenticate, reportLimiter, falseReportPenaltyCheck, asyncHandler(async (req, res) => {
    if (!req.user) {
        throw new AppError('Unauthorized', 'UNAUTHORIZED', 401);
    }
    const { id: reviewId } = req.params;
