    ...
    try {
        ...
        res.json({
            ...
            data: {
                ...
                lastUpdatedAt: creditScore.lastUpdatedAt ?? creditScore.lastUpdated,
                nextUpdateAt: creditScore.nextUpdateAt,
                updateCount: creditScore.updateCount ?? creditScore.metadata?.updateCount ?? 0,
            },
        });
    }
    catch (error) {
        ...
        res.json({
            ...
            data: {
                histories: history.histories.map(h => ({
                    id: h.id,
                    oldScore: h.oldScore ?? h.score - h.delta,
                    newScore: h.newScore ?? h.score,
                    delta: h.delta,
                    reason: h.reason,
 ...
 * 触发重新计算 (管理员)
 */
router.post('/recalculate', authenticate, requireRole('admin'), validate({ body: recalculateSchema }), async (req, res, next) => {
    try {
        const { userIds } = req.body;
        if (userIds && userIds.length > 0) {
            // 重新计算指定用户
            const results = await Promise.all(userIds.map(async (id) => {
                const result = await creditScoreService.updateCreditScore(id, 'system');
