 ...
 * @access Private
 */
router.post('/avatar', authenticate, upload.single('avatar'), handleUploadError, validateImageContent, asyncHandler(async (req, res) => {
    if (!req.user) {
    ...
    if (uploadAuditService.isUserRestricted(req.user.id)) {
        ...
        throw new AppError('Upload denied: your account has restricted upload access', 'UPLOAD_RESTRICTED', 403);
    }
    if (!req.file) {
    ...
    const securityResult = await securityService.secureImage(req.file.buffer, req.file.mimetype, req.file.originalname, { scanForMalware: true, stripMetadata: true });
    if (!securityResult.passed) {
        ...
        throw new AppError(`Upload rejected: ${securityResult.violations.join(', ')}`, 'UPLOAD_SECURITY_VIOLATION', 400);
    }
 ...
 * @access Private
 */
router.post('/image', authenticate, upload.single('image'), handleUploadError, validateImageContent, asyncHandler(async (req, res) => {
    if (!req.user) {
        throw new AppError('Unauthorized', 'UNAUTHORIZED', 401);
    }
    // Check if user is restricted
