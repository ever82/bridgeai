 ...
 * @access Admin only
 */
router.post('/queue/:id/assign', asyncHandler(async (req, res) => {
    const { id } = req.params;
 ...
 * @access Admin only
 */
router.post('/queue/:id/resolve', asyncHandler(async (req, res) => {
    const { id } = req.params;
    const { action, note } = req.body;
    if (!action || !['APPROVE', 'HIDE', 'WARN', 'BAN'].includes(action)) {
        throw new AppError('Valid action is required (APPROVE, HIDE, WARN, BAN)', 'INVALID_REQUEST', 400);
    }
 ...
 * @access Admin only
 */
router.post('/queue/:id/escalate', asyncHandler(async (req, res) => {
    const { id } = req.params;
    const { note } = req.body;
 ...
 * @access Admin only
 */
router.post('/queue/claim', asyncHandler(async (req, res) => {
 ...
 * @access Admin only
 */
router.post('/queue/:id/reopen', asyncHandler(async (req, res) => {
    const { id } = req.params;
    const { note } = req.body;
 ...
 * @access Admin only
 */
router.post('/queue/batch-resolve', asyncHandler(async (req, res) => {
