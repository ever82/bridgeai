...
router.put('/', authenticate, async (req, res) => {
    try {
        ...
        const { pushEnabled, emailEnabled, smsEnabled, inAppEnabled, matchNotifications, messageNotifications, ratingNotifications, systemNotifications, promotionNotifications, dailyLimit, quietHoursStart, quietHoursEnd, quietHoursEnabled, } = req.body;
        // 确保偏好记录存在
 ...
 * 批量更新渠道开关
 */
router.patch('/channels', authenticate, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: 'Unauthorized' });
 ...
 * 批量更新通知类型偏好
 */
router.patch('/types', authenticate, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: 'Unauthorized' });
        }
        const { matchNotifications, messageNotifications, ratingNotifications, systemNotifications, promotionNotifications, } = req.body;
        const preferences = await prisma.notificationPreference.upsert({
            where: { userId },
            update: {
                ...(matchNotifications !== undefined && { matchNotifications }),
 ...
 * 更新免打扰设置
 */
router.patch('/quiet-hours', authenticate, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: 'Unauthorized' });
...
router.get('/push-tokens', authenticate, async (req, res) => {
    try {
        ...
        logger.error('Failed to get push tokens', error);
        res.status(500).json({
            success: false,
            error: error.message,
        });
 ...
 * 删除推送令牌
 */
router.delete('/push-tokens/:id', authenticate, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: 'Unauthorized' });
 ...
 * 停用推送令牌
 */
router.post('/push-tokens/:id/deactivate', authenticate, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: 'Unauthorized' });
