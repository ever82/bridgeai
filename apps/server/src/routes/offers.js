...
router.post('/', authenticate, asyncHandler(async (req, res) => {
    ...
    const { merchantId, title, description, type, discountValue, minPurchaseAmount, maxDiscountAmount, applicableScope, validFrom, validUntil, totalStock, dailyLimit, stockAlertThreshold, publishType, scheduledPublishAt, } = req.body;
    if (!merchantId || !title || !type || !validFrom || !validUntil || !totalStock) {
        throw new AppError('Merchant ID, title, type, valid dates, and stock are required', 'MISSING_FIELDS', 400);
    }
...
router.put('/:id', authenticate, asyncHandler(async (req, res) => {
    ...
    const { id } = req.params;
    const { title, description, type, discountValue, minPurchaseAmount, maxDiscountAmount, applicableScope, validFrom, validUntil, totalStock, dailyLimit, stockAlertThreshold, status, publishType, scheduledPublishAt, } = req.body;
 ...
 * @access Private
 */
router.get('/low-stock/list', authenticate, asyncHandler(async (req, res) => {
    if (!req.user) {
        throw new AppError('Unauthorized', 'UNAUTHORIZED', 401);
    }
    const { merchantId } = req.query;
    const alerts = await offerService.getLowStockOffers(merchantId);
 ...
 * @access Private
 */
router.post('/:id/deduct-stock', authenticate, asyncHandler(async (req, res) => {
    if (!req.user) {
        throw new AppError('Unauthorized', 'UNAUTHORIZED', 401);
    }
    const { id } = req.params;
 ...
 * @access Private
 */
router.post('/extract-from-ai', authenticate, asyncHandler(async (req, res) => {
    if (!req.user) {
        throw new AppError('Unauthorized', 'UNAUTHORIZED', 401);
    }
