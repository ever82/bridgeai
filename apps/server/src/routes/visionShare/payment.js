 ...
 */
router.post('/pay', authenticate, validateRequest({
    body: {
    ...
    try {
        ...
        res
            .status(500)
            .json({ success: false, error: { code: 'PAYMENT_ERROR', message: '支付失败' } });
    }
});
/**
 ...
 */
router.get('/balance', authenticate, async (_req, res) => {
    try {
 ...
 */
router.get('/confirmation/:id', authenticate, async (req, res) => {
    try {
        const transactionId = req.params.id;
 ...
 */
router.get('/photos/:photoId/unlock-status', authenticate, async (req, res) => {
    try {
        const userId = req.user.id;
 ...
 */
router.post('/photos/:photoId/download-token', authenticate, async (req, res) => {
    try {
        const userId = req.user.id;
        const { photoId } = req.params;
 ...
 */
router.post('/photos/download/:token', authenticate, async (req, res) => {
    try {
        const userId = req.user.id;
