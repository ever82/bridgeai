 ...
 * 创建草稿任务
 */
router.post('/tasks', authenticate, validateRequest({
    body: {
 ...
 * AI提炼需求
 */
router.post('/tasks/:id/refine', authenticate, async (req, res) => {
    try {
        const userId = req.user.id;
 ...
 * 发布任务
 */
router.post('/tasks/:id/publish', authenticate, async (req, res) => {
    try {
        const userId = req.user.id;
        const taskId = req.params.id;
...
router.put('/tasks/:id', authenticate, async (req, res) => {
    try {
        ...
        if (error.message?.includes('只有草稿状态')) {
            return res.status(400).json({
                success: false,
                error: error.message,
            });
 ...
 * 检查用户接单资格
 */
router.post('/tasks/eligibility', authenticate, async (req, res) => {
    try {
        const userId = req.user.id;
        ...
        const eligibility = await checkTaskEligibility(taskId, userId, { latitude, longitude }, creditScore, points);
        res.json({
            success: true,
            data: eligibility,
        });
 ...
 * 验证发布资格
 */
router.post('/tasks/validate', authenticate, async (req, res) => {
    try {
        const userId = req.user.id;
 ...
 * 获取发布限制信息
 */
router.get('/publish-limits', authenticate, async (req, res) => {
    try {
        const userId = req.user.id;
 ...
 * 分享任务
 */
router.post('/tasks/:id/share', authenticate, async (req, res) => {
    try {
        const userId = req.user.id;
        const taskId = req.params.id;
 ...
 * 分析描述文本（AI提炼预览）
 */
router.post('/analyze-description', authenticate, async (req, res) => {
    try {
        const userId = req.user.id;
        const { description } = req.body;
