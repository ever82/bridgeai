...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const { page, limit, type, status, startDate, endDate } = req.query as Record<string, string>;
 ...
 * 获取退款详情（含申诉记录）
 */
router.get('/refunds/:refundId', authenticate, async (req: Request, res: Response) => {
  try {
    const userId = req.user!.id;
 ...
 * 取消退款申请
 */
router.post('/refunds/:refundId/cancel', authenticate, async (req: Request, res: Response) => {
  try {
    const userId = req.user!.id;
    const { refundId } = req.params;
