 ...
 * 更新用户隐私设置
 */
router.put('/me/privacy', authenticate, validate({ body: privacySettingsSchema }), async (req, res, next) => {
    try {
        const userId = req.user.id;
 ...
 * 修改密码
 */
router.post('/me/password', authenticate, validate({ body: changePasswordSchema }), async (req, res, next) => {
    try {
        const userId = req.user.id;
 ...
 * 发送手机验证码
 */
router.post('/me/phone/send-code', authenticate, validate({ body: sendCodeSchema }), async (req, res, next) => {
    try {
 ...
 * 绑定手机号
 */
router.post('/me/phone/bind', authenticate, validate({ body: bindPhoneSchema }), async (req, res, next) => {
    try {
        const userId = req.user.id;
 ...
 * 发送邮箱验证码
 */
router.post('/me/email/send-code', authenticate, validate({ body: sendCodeSchema }), async (req, res, next) => {
    try {
        const { target } = req.body;
        // TODO: 集成邮件服务发送验证码
        // 这里模拟发送成功
 ...
 * 绑定邮箱
 */
router.post('/me/email/bind', authenticate, validate({ body: bindEmailSchema }), async (req, res, next) => {
    try {
        const userId = req.user.id;
...
//# sourceMappingURL=userPrivacy.js.map