...
// Get coupon by ID
router.get('/:couponId', authenticate, async (req: Request, res: Response) => {
...
// Cancel coupon
router.post('/:couponId/cancel', authenticate, async (req: Request, res: Response) => {
  const userId = (req as any).user?.id;
  if (!userId) {
    throw new AppError('Unauthorized', 'AUTH_REQUIRED', 401);
  }
...
router.get('/:couponId/ratings', authenticate, async (req: Request, res: Response) => {
  const { couponId } = req.params;
...
router.get('/ratings/my', authenticate, async (req: Request, res: Response) => {
  const userId = (req as any).user?.id;
  if (!userId) {
    throw new AppError('Unauthorized', 'AUTH_REQUIRED', 401);
  }
...
router.get('/:couponId/online-url', authenticate, async (req: Request, res: Response) => {
  const userId = (req as any).user?.id;
  if (!userId) {
    throw new AppError('Unauthorized', 'AUTH_REQUIRED', 401);
  }
