...
import { z } from 'zod';
import { getSceneConfig, getActiveSceneConfigs, getAllSceneInfos, hasScene, } from '@bridgeai/shared';
import { getEnabledCapabilities, getCapabilityStatus, getSceneCapabilitiesSummary, } from '../services/sceneCapabilityService';
import { getTemplatesByScene, getPresetTemplates, applyTemplate, setDefaultTemplate, getDefaultTemplate, } from '../services/templateService';
import { generateMigrationPlan, previewMigration, executeMigration, validateMigration, estimateDataLoss, } from '../services/sceneMigrationService';
 ...
 */
router.get('/:sceneId', validateParams(sceneIdSchema), (req, res) => {
    try {
 ...
 * Get scene fields
 */
router.get('/:sceneId/fields', validateParams(sceneIdSchema), (req, res) => {
    try {
        const { sceneId } = req.params;
        const config = getSceneConfig(sceneId);
        if (!config) {
 ...
 */
router.get('/:sceneId/ui', validateParams(sceneIdSchema), (req, res) => {
    try {
        const { sceneId } = req.params;
        const config = getSceneConfig(sceneId);
        if (!config) {
 ...
 */
router.get('/:sceneId/capabilities', validateParams(sceneIdSchema), (req, res) => {
    try {
        const { sceneId } = req.params;
        if (!hasScene(sceneId)) {
            return res.status(404).json({
        ...
        const capabilities = getEnabledCapabilities(sceneId);
        res.json({
            success: true,
            data: capabilities,
        });
 ...
 */
router.get('/:sceneId/capabilities/all', validateParams(sceneIdSchema), (req, res) => {
    try {
        const { sceneId } = req.params;
        const config = getSceneConfig(sceneId);
        if (!config) {
        ...
        const capabilities = config.capabilities.map(cap => ({
            ...cap,
            status: getCapabilityStatus(sceneId, cap.id),
        }));
        res.json({
            success: true,
            data: capabilities,
 ...
 */
router.get('/:sceneId/capabilities/summary', validateParams(sceneIdSchema), (req, res) => {
    try {
        const { sceneId } = req.params;
        if (!hasScene(sceneId)) {
            return res.status(404).json({
        ...
        const summary = getSceneCapabilitiesSummary(sceneId);
        res.json({
            success: true,
            data: summary,
        });
 ...
 */
router.get('/:sceneId/capabilities/:capabilityId', validateParams(sceneIdSchema), (req, res) => {
    try {
        ...
        const status = getCapabilityStatus(sceneId, capabilityId);
        res.json({
            success: true,
            data: status,
        });
 ...
 * Get scene templates
 */
router.get('/:sceneId/templates', authenticate, validateParams(sceneIdSchema), async (req, res) => {
    try {
        const { sceneId } = req.params;
        ...
        const presetTemplates = getPresetTemplates(sceneId);
        // Get user templates if authenticated
        let userTemplates = [];
        if (userId) {
            const result = await getTemplatesByScene(sceneId, {
                userId,
 ...
 */
router.post('/:sceneId/templates/:templateId/apply', authenticate, validateParams(sceneIdSchema), async (req, res) => {
    try {
 ...
 */
router.get('/:sceneId/templates/default', authenticate, validateParams(sceneIdSchema), async (req, res) => {
    try {
        const { sceneId } = req.params;
        const userId = req.user?.id;
        if (!userId) {
        ...
        const template = await getDefaultTemplate(sceneId, userId);
        res.json({
            success: true,
            data: template,
        });
 ...
 */
router.post('/:sceneId/templates/default', authenticate, validateParams(sceneIdSchema), validateBody(setDefaultTemplateSchema), async (req, res) => {
    try {
        const { sceneId } = req.params;
        const userId = req.user?.id;
        ...
        const result = await setDefaultTemplate(templateId, userId, sceneId);
        res.json({
            success: true,
            data: { success: result },
        });
 ...
 */
router.post('/migration/validate', validateBody(migrationPreviewSchema), (req, res) => {
    try {
 ...
 * Preview migration
 */
router.post('/migration/preview', authenticate, validateBody(migrationPreviewSchema), async (req, res) => {
    try {
        const { fromScene, toScene } = req.body;
 ...
 * Execute migration
 */
router.post('/migration/execute', authenticate, validateBody(migrationExecuteSchema), async (req, res) => {
    try {
 ...
 * Estimate data loss
 */
router.post('/migration/estimate', validateBody(migrationPreviewSchema), (req, res) => {
    try {
        const { fromScene, toScene } = req.body;
 ...
 */
router.post('/migration/plan', validateBody(migrationPreviewSchema), (req, res) => {
    try {
        const { fromScene, toScene } = req.body;
