...
import { notifyNewMatch, notifyHighMatchJob, notifyResumeViewed, notifyMatchStatusChange, getNotificationPreferences, updateNotificationPreferences, } from '../../services/job/jobMatchNotifications';
import { AppError } from '../../errors';
const router = Router();
// ---------------------------------------------------------------------------
// Trigger notifications (for internal / agent use)
// ---------------------------------------------------------------------------
router.post('/notify/new-match', async (req, res, next) => {
    try {
        if (!req.user) {
            throw new AppError('Authentication required', 'UNAUTHORIZED', 401);
        }
        const { matchId, matchScore, jobTitle, candidateName, metadata, targetUserId } = req.body;
        const effectiveTargetUserId = targetUserId && req.user.role === 'admin' ? targetUserId : req.user.id;
...
router.post('/notify/high-match', async (req, res, next) => {
    try {
        if (!req.user) {
            throw new AppError('Authentication required', 'UNAUTHORIZED', 401);
        }
...
router.post('/notify/resume-viewed', async (req, res, next) => {
    try {
        if (!req.user) {
            throw new AppError('Authentication required', 'UNAUTHORIZED', 401);
        }
...
router.post('/notify/match-status', async (req, res, next) => {
    try {
        if (!req.user) {
            throw new AppError('Authentication required', 'UNAUTHORIZED', 401);
        }
        ...
        if (!['accepted', 'rejected', 'completed'].includes(status)) {
            throw new AppError('Invalid status. Must be: accepted, rejected, completed', 'INVALID_STATUS', 400);
        }
        const effectiveTargetUserId = targetUserId && req.user.role === 'admin' ? targetUserId : req.user.id;
...
//# sourceMappingURL=jobMatchNotificationsRoutes.js.map