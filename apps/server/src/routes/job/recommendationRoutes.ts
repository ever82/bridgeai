...
import {
  ...
  CandidateSummary,
  Recommendation,
} from '../../services/job/jobRecommendation';
...
router.post('/recommendations/jobs', async (req: Request, res: Response) => {
  try {
    const { seekerProfile, page, pageSize } = req.body as {
    ...
    const { jobs } = req.body as { jobs?: JobSummary[] };
    ...
    const result = await jobRecommendationService.recommendJobsForSeeker(
      seekerProfile,
      jobs,
      page,
      pageSize
...
router.post('/recommendations/candidates', async (req: Request, res: Response) => {
  try {
    const { jobCriteria, candidates, page, pageSize } = req.body as {
    ...
    const result = await jobRecommendationService.recommendCandidatesForJob(
      jobCriteria,
      candidates,
      page,
      pageSize
...
router.get('/recommendations/:id/explain', async (req: Request, res: Response) => {
  try {
    ...
    const { score, reasons, matchedSkills, gaps } = req.query as {
      score?: string;
      reasons?: string;
...
router.post('/recommendations/feedback', async (req: Request, res: Response) => {
  try {
    const { userId, recommendationId, itemId, action } = req.body as {
      userId: string;
...
router.get('/recommendations/history/:userId', async (req: Request, res: Response) => {
  try {
...
router.post('/recommendations/refresh', async (req: Request, res: Response) => {
  try {
    const { userId, seenItemIds } = req.body as {
      userId: string;
      seenItemIds: string[];
    };
