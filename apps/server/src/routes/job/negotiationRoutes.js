...
import { NegotiationTopic, MessageSender, isValidNegotiationStatus, isValidNegotiationTopic } from '../../models/NegotiationRoom';
const router = Router();
/**
...
router.post('/', async (req, res) => {
    try {
        const { jobApplicationId, jobSeekerId, jobSeekerAgentId, employerId, employerAgentId, topics, maxRounds, initialOffer, targetRange, currency } = req.body;
        ...
        // Validate topics
        const validTopics = topics?.filter((t) => isValidNegotiationTopic(t)) || [NegotiationTopic.SALARY];
...
router.get('/', async (req, res) => {
    try {
        const { jobApplicationId, jobSeekerId, employerId, status } = req.query;
        const filter = {};
        if (jobApplicationId)
            filter.jobApplicationId = jobApplicationId;
        if (jobSeekerId)
            filter.jobSeekerId = jobSeekerId;
        if (employerId)
            filter.employerId = employerId;
        if (status && isValidNegotiationStatus(status)) {
...
router.get('/:id/messages', async (req, res) => {
    try {
        ...
        const messages = await negotiationRoomService.getMessages(id, round ? parseInt(round, 10) : undefined);
        return res.status(200).json({
            success: true,
            data: messages
        });
...
router.post('/:id/confirm', async (req, res) => {
    try {
        ...
        const confirmation = await negotiationResultService.confirmAgreement(id, party, conditions, notes);
...
router.post('/:id/finalize', async (req, res) => {
    try {
        ...
        const result = await negotiationResultService.finalizeAgreement(id, agreedAmount, agreedBenefits || []);
        return res.status(200).json({
            success: true,
            data: result
        });
...
router.post('/:id/reject', async (req, res) => {
    try {
        ...
        const rejection = await negotiationResultService.recordRejection(id, rejectedBy, reason, alternativeProposed, alternativeAmount);
        return res.status(200).json({
            success: true,
            data: rejection
        });
...
//# sourceMappingURL=negotiationRoutes.js.map