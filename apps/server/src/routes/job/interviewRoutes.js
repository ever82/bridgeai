...
import { InterviewRound, isValidInterviewStatus, isValidInterviewType, isValidInterviewRound, } from '../../models/Interview';
const router = Router();
/**
...
router.post('/', async (req, res) => {
    try {
        const { jobApplicationId, jobSeekerId, employerId, positionId, round, roundNumber, type, interviewers, notes, } = req.body;
        ...
        // Validate round
        let validRound = InterviewRound.SCREENING;
        if (round) {
...
router.get('/', async (req, res) => {
    try {
        ...
        if (status && isValidInterviewStatus(status)) {
            filter.status = status;
        }
        if (type && isValidInterviewType(type)) {
            filter.type = type;
        }
        if (round && isValidInterviewRound(round)) {
            filter.round = round;
        }
        if (upcoming === 'true')
 ...
 */
router.get('/reminders/pending', async (_req, res) => {
    try {
...
router.post('/:id/feedback', async (req, res) => {
    try {
        const { id } = req.params;
        const { interviewerId, rating, technicalSkills, communicationSkills, cultureFit, strengths, weaknesses, overallImpression, recommendation, notes, } = req.body;
        if (!interviewerId ||
            !rating ||
            !strengths ||
            !weaknesses ||
 ...
 */
router.get('/:id/aggregate-feedback', async (req, res) => {
    try {
        const { id } = req.params;
 ...
 */
router.get('/:id/next-round', async (req, res) => {
    try {
        const { id } = req.params;
 ...
 */
router.get('/application/:applicationId/series', async (req, res) => {
    try {
