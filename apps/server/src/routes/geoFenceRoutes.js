...
import { createGeoFence, getGeoFence, listGeoFences, updateGeoFence, deleteGeoFence, checkPointInFence, checkMultipleFences, findContainingFences, createCircularFence, } from '../services/geoFenceService';
const router = Router();
// Validation schemas
...
router.post('/check-multiple', async (req, res, next) => {
    try {
        ...
        const results = await checkMultipleFences({ latitude: parsed.data.lat, longitude: parsed.data.lng }, parsed.data.fenceIds);
