 ...
 * 建议运行频率：每日凌晨
 */
export async function runCreditUpdateJob(config = {}) {
    ...
    try {
        ...
        for (let i = 0; i < users.length; i += finalConfig.batchSize) {
            ...
            const batchResults = await Promise.allSettled(batch.map(async (user) => {
                try {
                    const result = await creditScoreService.updateCreditScore(user.id, CreditSourceType.SCHEDULED);
 ...
 * 找出信用分异常变化的用户
 */
export async function detectCreditFluctuations(threshold = 50) {
    const fluctuations = [];
    // 获取最近24小时的更新记录
 ...
 * 保留最近一年的记录
 */
export async function cleanupOldCreditHistory() {
...
export async function generateCreditReport() {
    ...
    const averageScore = totalUsers > 0 ? scores.reduce((sum, s) => sum + s.score, 0) / totalUsers : 0;
...
//# sourceMappingURL=creditUpdateJob.js.map