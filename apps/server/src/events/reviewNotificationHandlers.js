...
import { sendNewReviewNotification, sendReviewReplyNotification, sendPendingReviewReminder, sendBadReviewWarning, sendCreditScoreChangeNotification, } from '../services/notificationService';
...
(function (ReviewNotificationType) {
    ReviewNotificationType["REVIEW_CREATED"] = "REVIEW_CREATED";
    ReviewNotificationType["REVIEW_REPLIED"] = "REVIEW_REPLIED";
    ReviewNotificationType["REVIEW_REMINDER"] = "REVIEW_REMINDER";
    ReviewNotificationType["REVIEW_REPORTED"] = "REVIEW_REPORTED";
    ReviewNotificationType["CREDIT_SCORE_UPDATED"] = "CREDIT_SCORE_UPDATED";
 ...
 */
export async function handleReviewCreatedNotification(ratingId) {
    try {
 ...
 */
export async function handleReviewReplyNotification(ratingId, replyContent) {
    try {
        const review = await prisma.review.findUnique({
            where: { id: ratingId },
            include: {
 ...
 */
export async function handlePendingReviewReminder(matchId, userId, partnerName) {
    try {
        ...
        console.log(`[NOTIFICATION] Pending review reminder sent to user ${userId} for match ${matchId}`);
    }
    catch (error) {
 ...
 */
export async function handleBadReviewWarningNotification(ratingId, creditDelta) {
    try {
        const review = await prisma.review.findUnique({
            where: { id: ratingId },
            include: {
 ...
 */
export async function handleCreditScoreChangeNotification(userId, previousScore, newScore, reason) {
    try {
 ...
 */
export async function handleReviewReportedNotification(reportId) {
    try {
        ...
        console.log(`[NOTIFICATION] Review reported notification sent to ${adminUsers.length} admins for report ${reportId}`);
        // Emit event
...
export function setupNotificationListeners() {
    ...
    notificationEvents.on('notificationSent', (data) => {
        console.log(`[NOTIFICATION_EVENT] Notification sent to ${data.userId} via ${data.channels.join(', ')}`);
    });
 ...
 */
export function setupReviewEventListeners() {
    // Listen for rating submitted events
    reviewEvents.on(ReviewEventType.RATING_SUBMITTED, async (payload) => {
        if (payload.processed) {
            await handleReviewCreatedNotification(payload.ratingId);
        }
    });
    // Listen for match completed events
    reviewEvents.on(ReviewEventType.MATCH_COMPLETED, async (payload) => {
 ...
 */
export function initializeReviewNotificationHandlers() {
    ...
    notificationEvents.on(ReviewNotificationType.CREDIT_SCORE_UPDATED, data => {
        console.log(`[NOTIFICATION_EVENT] Credit score updated: ${data.userId} (${data.previousScore} -> ${data.newScore})`);
    });
 ...
 */
export async function getNotificationHistory(userId, limit = 20, offset = 0) {
...
//# sourceMappingURL=reviewNotificationHandlers.js.map