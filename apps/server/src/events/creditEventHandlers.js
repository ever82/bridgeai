 ...
 * 用户资料更新事件
 */
export async function handleProfileUpdate(userId) {
    try {
        const result = await creditScoreService.updateCreditScore(userId, CreditSourceType.PROFILE_UPDATE);
 ...
 * 交易完成事件
 */
export async function handleTransactionComplete(userId, transactionId) {
    try {
        const result = await creditScoreService.updateCreditScore(userId, CreditSourceType.TRANSACTION, transactionId);
 ...
 * 评价提交事件
 */
export async function handleRatingSubmitted(userId, ratingId) {
    try {
        const result = await creditScoreService.updateCreditScore(userId, CreditSourceType.RATING, ratingId);
 ...
 * 投诉事件 (负面事件)
 */
export async function handleComplaintReceived(userId, complaintId) {
    try {
        const result = await creditScoreService.updateCreditScore(userId, CreditSourceType.COMPLAINT, complaintId);
 ...
 * 匹配完成事件
 */
export async function handleMatchCompleted(userId, matchId) {
    try {
        const result = await creditScoreService.updateCreditScore(userId, CreditSourceType.TRANSACTION, matchId);
 ...
 * 消息响应事件
 */
export async function handleMessageResponse(userId, messageId) {
    try {
        // 消息响应不立即更新，避免频繁更新
        // 可以记录到缓存，批量处理
...
//# sourceMappingURL=creditEventHandlers.js.map