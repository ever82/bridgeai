 ...
 * 构建约会对话系统提示
 */
export function buildDatingSystemPrompt(agentName, persona, profileSummary, otherProfileSummary) {
 ...
 * 构建话题引导提示
 */
export function buildTopicGuidancePrompt(topic, category, sharedInterests, round, previousMessages) {
    return `当前话题：${topic}
话题类别：${category}
 ...
 * 构建对话质量评估提示
 */
export function buildQualityAssessmentPrompt(messages, topics, round) {
    return `请评估以下对话片段的质量。
 ...
 * 构建对话摘要生成提示
 */
export function buildSummaryPrompt(messages, profileA, profileB) {
 ...
 * 构建安全检查提示
 */
export function buildSafetyCheckPrompt(content) {
