...
function createToken(userId, email) {
    return jsonwebtoken_1.default.sign({ userId, email }, TEST_JWT_SECRET, { expiresIn: '1h' });
}
...
function sendMessage(client, roomId, content, type = 'TEXT') {
    return new Promise((resolve, reject) => {
        client.emit('chat:message', { roomId, content, type }, (response) => {
            resolve(response);
        });
    ...
    describe('VisionShare Message Flow', () => {
        ...
        it('should route demand update messages through chat', async () => {
            ...
            try {
                ...
                const result = await sendMessage(client, 'room-vs-demand', 'Demand status updated to accepted');
                expect(result.success).toBe(true);
            }
            finally {
                client.disconnect();
        ...
        it('should deliver photo upload events to room participants', async () => {
            ...
            try {
                ...
                const receivePromise = new Promise(resolve => {
                    receiver.on('chat:new_message', (data) => {
                        resolve(data);
                    });
                });
    ...
    describe('AgentDate Message Flow', () => {
        it('should handle matching notification messages', async () => {
            ...
            try {
                ...
                const result = await sendMessage(client, 'room-agentdate', 'New match found! Compatibility: 85%');
                expect(result.success).toBe(true);
                expect(result.data).toHaveProperty('messageId');
            }
            finally {
        ...
        it('should support consent flow messages between matched users', async () => {
            ...
            try {
                ...
                const receivePromise = new Promise(resolve => {
                    user2.on('chat:new_message', (data) => {
                        resolve(data);
                    });
                });
    ...
    describe('AgentJob Message Flow', () => {
        it('should handle interview scheduling messages', async () => {
            ...
            try {
                ...
                const result = await sendMessage(client, 'room-agentjob', 'Interview scheduled: 2026-05-01 10:00');
                expect(result.success).toBe(true);
                expect(result.data).toHaveProperty('messageId');
            }
            finally {
        ...
        it('should deliver application status updates to job seekers', async () => {
            ...
            try {
                ...
                const receivePromise = new Promise(resolve => {
                    seeker.on('chat:new_message', (data) => {
                        resolve(data);
                    });
                });
        ...
        it('should handle salary negotiation messages', async () => {
            ...
            try {
                ...
                const result = await sendMessage(client, 'room-job-negotiate', 'Counter-offer: 35k with stock options');
                expect(result.success).toBe(true);
            }
            finally {
                client.disconnect();
    ...
    describe('AgentAd Message Flow', () => {
        it('should handle promotional message delivery', async () => {
            ...
            try {
                ...
                const result = await sendMessage(client, 'room-agentad', 'Flash sale: 50% off this weekend!');
                expect(result.success).toBe(true);
                expect(result.data).toHaveProperty('messageId');
            }
            finally {
        ...
        it('should route deal notifications to interested consumers', async () => {
            ...
            try {
                ...
                const receivePromise = new Promise(resolve => {
                    consumer.on('chat:new_message', (data) => {
                        resolve(data);
                    });
                });
        ...
        it('should handle coupon code delivery messages', async () => {
            ...
            try {
                ...
                const result = await sendMessage(client, 'room-ad-coupon', 'Your coupon code: SAVE20 (20% off)');
                expect(result.success).toBe(true);
            }
            finally {
                client.disconnect();
    ...
    describe('emitToUser for Scenario Notifications', () => {
        it('should emit notification to connected user', async () => {
            ...
            try {
                ...
                const _notificationPromise = new Promise(resolve => {
                    client.on('notification', (data) => {
                        resolve(data);
                    });
                });
                // Server-side emit to user
                (0, socket_1.emitToUser)('notify-user', 'notification', {
                    type: 'match_found',
                    scenario: 'agentdate',
...
//# sourceMappingURL=messageFlow.integration.test.js.map