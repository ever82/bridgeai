...
describe('PROBE-1: creditScore Boundary Values', () => {
    ...
    it('should accept creditScore of 0 (minimum)', async () => {
        const agent = await prisma.agent.create({
            data: {
                ...
                type: client_1.AgentType.DEMAND,
    ...
    it('should accept creditScore of 100 (maximum)', async () => {
        const agent = await prisma.agent.create({
            data: {
                userId: cleanupIds.users[0],
                type: client_1.AgentType.SUPPLY,
    ...
    it('should reject negative creditScore', async () => {
        await expect(prisma.agent.create({
            data: {
                userId: cleanupIds.users[0],
                type: client_1.AgentType.DEMAND,
...
describe('PROBE-3: seed.ts Enum vs String Mismatches', () => {
    it('should seed.ts use SenderType enum instead of string literal', () => {
        ...
        const usesStringLiteral = /senderType:\s*['"]AGENT['"]/.test(content) || /senderType:\s*['"]USER['"]/.test(content);
        if (usesStringLiteral && !hasSenderTypeImport) {
            console.log('WARNING: seed.ts uses string literals for senderType without importing SenderType enum');
        }
    });
    it('should MessageStatus enum map correctly', () => {
        ...
        expect(client_1.MessageStatus.SENT).toBe('SENT');
        expect(client_1.MessageStatus.DELIVERED).toBe('DELIVERED');
        expect(client_1.MessageStatus.READ).toBe('READ');
    });
});
// ==========================================
...
describe('PROBE-4: ChatRoom participantIds vs RoomParticipant', () => {
    ...
    it('should create ChatRoom with participantIds array', async () => {
        const room = await prisma.chatRoom.create({
            data: {
                type: client_1.ChatRoomType.QUAD,
    ...
    it('should store budget with 2 decimal precision', async () => {
        const demand = await prisma.demand.create({
            data: {
                ...
                budgetMin: 10.99,
                budgetMax: 99.99,
                status: client_1.DemandStatus.OPEN,
            },
        });
    ...
    it('should store hourlyRate with 2 decimal precision', async () => {
        const supply = await prisma.supply.create({
            data: {
                ...
                title: 'Test Supply',
                hourlyRate: 150.75,
                status: client_1.SupplyStatus.AVAILABLE,
            },
        });
...
describe('PROBE-6: Match Score Range', () => {
    beforeAll(async () => {
        ...
        const demandAgent = await prisma.agent.create({
            data: { userId: user.id, type: client_1.AgentType.DEMAND, name: 'Demand' },
        });
        cleanupIds.agents.push(demandAgent.id);
        const supplyAgent = await prisma.agent.create({
            data: { userId: user.id, type: client_1.AgentType.SUPPLY, name: 'Supply' },
        });
        cleanupIds.agents.push(supplyAgent.id);
        const demand = await prisma.demand.create({
            data: { agentId: demandAgent.id, title: 'D', status: client_1.DemandStatus.OPEN },
        });
        const supply = await prisma.supply.create({
            data: { agentId: supplyAgent.id, title: 'S', status: client_1.SupplyStatus.AVAILABLE },
        });
    ...
    it('should accept score of 100.00 (max)', async () => {
        const demand2 = await prisma.demand.create({
            data: { agentId: cleanupIds.agents[0], title: 'D2', status: client_1.DemandStatus.OPEN },
        });
        const supply2 = await prisma.supply.create({
            data: { agentId: cleanupIds.agents[1], title: 'S2', status: client_1.SupplyStatus.AVAILABLE },
        });
        const match = await prisma.match.create({
            data: {
                demandId: demand2.id,
    ...
    it('should reject negative score', async () => {
        const demand3 = await prisma.demand.create({
            data: { agentId: cleanupIds.agents[0], title: 'D3', status: client_1.DemandStatus.OPEN },
        });
        const supply3 = await prisma.supply.create({
            data: { agentId: cleanupIds.agents[1], title: 'S3', status: client_1.SupplyStatus.AVAILABLE },
        });
        await expect(prisma.match.create({
            data: {
                demandId: demand3.id,
                supplyId: supply3.id,
                score: -1,
    ...
    it('should use transactions for bulk operations', async () => {
        ...
        const result = await prisma.$transaction(async (tx) => {
            const agent1 = await tx.agent.create({
                data: { userId: user.id, type: client_1.AgentType.DEMAND, name: 'T1' },
            });
            const agent2 = await tx.agent.create({
                data: { userId: user.id, type: client_1.AgentType.SUPPLY, name: 'T2' },
            });
...
describe('PROBE-8: Geolocation Index Coverage', () => {
    ...
    `);
        const hasGeoIndex = indexes.some(idx => idx.indexdef.includes('latitude') && idx.indexdef.includes('longitude'));
...
describe('PROBE-10: SceneCode Enum Validation', () => {
    ...
    it('should accept all four scene codes', async () => {
        const scenes = await Promise.all([
            prisma.scene.create({ data: { code: client_1.SceneCode.VISION_SHARE, name: 'VisionShare' } }),
            prisma.scene.create({ data: { code: client_1.SceneCode.AGENT_DATE, name: 'AgentDate' } }),
            prisma.scene.create({ data: { code: client_1.SceneCode.AGENT_JOB, name: 'AgentJob' } }),
            prisma.scene.create({ data: { code: client_1.SceneCode.AGENT_AD, name: 'AgentAd' } }),
        ]);
        cleanupIds.scenes = scenes.map(s => s.id);
        expect(scenes).toHaveLength(4);
    });
    it('should reject invalid scene code', async () => {
        await expect(prisma.scene.create({
            data: { code: 'INVALID_SCENE', name: 'Invalid' },
        })).rejects.toThrow();
    });
});
// ==========================================
...
describe('PROBE-12: Transaction Status Transitions', () => {
    ...
    it('should allow PENDING -> SUCCESS transition', async () => {
        ...
        expect(updated.status).toBe(client_1.TransactionStatus.SUCCESS);
    });
    it('should allow PENDING -> FAILED transition', async () => {
        ...
        expect(updated.status).toBe(client_1.TransactionStatus.FAILED);
    });
    it('should allow PENDING -> CANCELLED transition', async () => {
        ...
        expect(updated.status).toBe(client_1.TransactionStatus.CANCELLED);
    });
});
//# sourceMappingURL=probe-f002.test.js.map