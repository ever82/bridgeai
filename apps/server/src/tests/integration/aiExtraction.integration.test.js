...
let mockAdapterChatCompletionResponses = [];
    ...
    return {
        OpenAIAdapter: jest.fn().mockImplementation(() => ({
            ...
            chatCompletion: jest.fn().mockImplementation(() => {
                ...
                if (globalThis.__llmServiceShouldError) {
                    return Promise.reject(new Error('LLM Service Error'));
                }
                const response = mockAdapterChatCompletionResponses[mockAdapterCallIndex % Math.max(1, mockAdapterChatCompletionResponses.length)];
...
const llmService_1 = require("../../services/ai/llmService");
const L2FieldType = {
    TEXT: 'text',
    NUMBER: 'number',
    ENUM: 'enum',
...
describe('AI Extraction Integration', () => {
    ...
    let validator;
    beforeEach(async () => {
        extractionService = new demandExtractionService_1.DemandExtractionService();
        mapper = new demandToL2Mapper_1.DemandToL2Mapper();
        validator = new extractionValidator_1.ExtractionValidator();
    ...
    describe('Full Pipeline: Extract → Map → Validate', () => {
        it('should process a complete demand through the full pipeline', async () => {
            ...
            // Step 3: Validate
            const validationReport = validator.validate(mappingResult.data, mockVisionShareSchema, demand);
        ...
        it('should handle budget range validation in pipeline', async () => {
            ...
            const demand = await extractionService.extract(request);
            const mappingResult = mapper.map(demand, mockVisionShareSchema);
            const validationReport = validator.validate(mappingResult.data, mockVisionShareSchema, demand, { checkBusinessRules: true });
    ...
    describe('Error Handling Integration', () => {
        it('should propagate LLM errors through the pipeline', async () => {
            ...
            globalThis.__llmServiceShouldError = true;
            const request = {
                text: '测试文本',
                scene: 'visionShare',
            };
            ...
            // Clean up
            delete globalThis.__llmServiceShouldError;
        });
...
//# sourceMappingURL=aiExtraction.integration.test.js.map