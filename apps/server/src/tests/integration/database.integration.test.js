...
describe('AC2: Core Entity Models', () => {
    ...
    it('should create User with all required fields', async () => {
        ...
        expect(user.status).toBe(client_1.UserStatus.ACTIVE);
    ...
    it('should create Agent with all required fields including creditScore', async () => {
        ...
        expect(agent.type).toBe(client_1.AgentType.DEMAND);
    ...
    it('should create Scene with all four scene codes', async () => {
        const scenes = await Promise.all([
            prisma.scene.create({
                data: { code: client_1.SceneCode.VISION_SHARE, name: 'VisionShare', description: '视野分享' },
            }),
            prisma.scene.create({
                data: { code: client_1.SceneCode.AGENT_DATE, name: 'AgentDate', description: '交友约会' },
            }),
            prisma.scene.create({
                data: { code: client_1.SceneCode.AGENT_JOB, name: 'AgentJob', description: '求职招聘' },
            }),
            prisma.scene.create({
                data: { code: client_1.SceneCode.AGENT_AD, name: 'AgentAd', description: '优惠广告' },
            }),
        ]);
    ...
    it('should create AgentProfile with L1/L2/L3 data', async () => {
        ...
        const scene = await prisma.scene.upsert({
            where: { code: client_1.SceneCode.AGENT_JOB },
            update: {},
            create: { code: client_1.SceneCode.AGENT_JOB, name: 'TestJob' },
        });
...
describe('AC3: Communication Models', () => {
    ...
    it('should create ChatRoom with type, participants, and metadata', async () => {
        ...
        expect(chatRoom.type).toBe(client_1.ChatRoomType.QUAD);
        expect(chatRoom.status).toBe(client_1.ChatRoomStatus.ACTIVE);
    ...
    it('should create ChatMessage with status field for read tracking', async () => {
        ...
        expect(message.type).toBe(client_1.MessageType.TEXT);
        expect(message.status).toBe(client_1.MessageStatus.SENT);
    });
    it('should support MessageStatus transitions (SENT -> DELIVERED -> READ)', async () => {
        ...
        expect(delivered.status).toBe(client_1.MessageStatus.DELIVERED);
        const read = await prisma.chatMessage.update({
            where: { id: msg.id },
            data: { status: client_1.MessageStatus.READ },
        });
        expect(read.status).toBe(client_1.MessageStatus.READ);
    });
    it('should create Connection with socket and status tracking', async () => {
        const connection = await prisma.connection.create({
            data: {
                ...
                deviceInfo: { type: 'web' },
                status: client_1.ConnectionStatus.ONLINE,
            },
        });
        expect(connection.id).toBeDefined();
        expect(connection.socketId).toContain('socket-');
        expect(connection.status).toBe(client_1.ConnectionStatus.ONLINE);
    });
    it('should enforce Connection socketId uniqueness', async () => {
        ...
        await expect(prisma.connection.create({
            data: {
                userId: testUserId,
                socketId,
                status: client_1.ConnectionStatus.ONLINE,
...
describe('AC4: Match and Credit Models', () => {
    ...
    it('should create Match with score and status', async () => {
        ...
        expect(match.status).toBe(client_1.MatchStatus.PENDING);
    ...
    it('should create Transaction with amount, type, and status', async () => {
        ...
        expect(txn.type).toBe(client_1.TransactionType.RECHARGE);
        expect(txn.status).toBe(client_1.TransactionStatus.SUCCESS);
    });
...
describe('AC7: Database Performance Optimization', () => {
    ...
    it('should have composite index on Agent for geolocation', async () => {
      ...
      WHERE tablename = 'agents'
    `);
        const hasGeoIndex = indexes.some(i => i.indexdef.includes('latitude') && i.indexdef.includes('longitude'));
