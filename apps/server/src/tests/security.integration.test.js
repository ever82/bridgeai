...
describe('Security Middleware Integration', () => {
    ...
    beforeEach(() => {
        ...
        (0, ddosProtection_1.resetState)();
        app = (0, express_1.default)();
        // Apply security middleware stack
        app.use((0, helmet_1.default)(cors_2.securityHeaders));
        app.use((0, cors_1.default)(cors_2.corsConfig));
        app.use(express_1.default.json({ limit: '10mb' }));
        app.use(ipFilter_1.ipFilter);
        ...
        app.use((0, security_1.securityProtection)());
        // Test routes
    ...
    describe('Rate Limiting', () => {
        it('should return 429 when rate limit exceeded', async () => {
            ...
            const requests = [];
            for (let i = 0; i < 110; i++) {
                requests.push((0, supertest_1.default)(app).get('/api/test'));
            }
    ...
    describe('DDoS Protection', () => {
        ...
        it('should detect and handle burst traffic from non-whitelisted IPs', async () => {
            const burst = [];
            for (let i = 0; i < 60; i++) {
                burst.push((0, supertest_1.default)(app).get('/api/test').set('X-Forwarded-For', '198.51.100.1'));
            }
    ...
    describe('IP Filtering', () => {
        it('should respect X-Forwarded-For header', async () => {
            const response = await (0, supertest_1.default)(app).get('/api/test').set('X-Forwarded-For', '203.0.113.1');
    ...
    describe('XSS Protection', () => {
        ...
        it('should block JavaScript protocol URLs', async () => {
            const response = await (0, supertest_1.default)(app).post('/api/test').send({ url: 'javascript:alert(1)' });
            expect(response.status).toBe(400);
        });
    });
    ...
    describe('Combined Security Stack', () => {
        it('should handle normal requests without interference', async () => {
            const response = await (0, supertest_1.default)(app).post('/api/test').send({ name: 'John Doe', age: 30 });
