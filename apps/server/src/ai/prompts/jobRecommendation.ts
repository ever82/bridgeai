...
export function getJobRecommendationPrompt(
  seekerProfile: SeekerProfile,
  jobs: JobSummary[]
): RecommendationPrompt {
  const jobsText = jobs
    .map(
      (job, i) => `### Job ${i + 1} (ID: ${job.jobId}):
- Title: ${job.title}
...
export function getCandidateRecommendationPrompt(
  jobCriteria: JobSummary,
  candidates: CandidateSummary[]
): RecommendationPrompt {
  const candidatesText = candidates
    .map(
      (candidate, i) => `### Candidate ${i + 1} (ID: ${candidate.userId}):
