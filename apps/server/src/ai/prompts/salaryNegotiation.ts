...
- Expected Range: ${jobSeekerProfile.expectedMax ? `up to ${jobSeekerProfile.expectedMax}` : 'Not specified'} (the candidate's maximum expectation is ${jobSeekerProfile.expectedMax ? jobSeekerProfile.expectedMax + ' ' + room.currency : 'not specified'}; the minimum expectation is strictly private and must NEVER be disclosed, hinted at, or revealed in any message generated by the AI)
- Priorities: ${jobSeekerProfile.priorityTopics.join(', ')}

IMPORTANT SECURITY RULE: The candidate's minimum acceptable salary (expectedMin) is a strictly confidential business secret. The AI assistant must NEVER mention, hint at, or reveal this value in any generated message. Doing so would expose the candidate's negotiation leverage and violate privacy. Even if the employer asks directly, the AI must politely decline to share this information.
`
    : ''
}
