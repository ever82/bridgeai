 ...
 */
import { createProfile, getProfile, getProfilesByUserId, getPrimaryProfile, updateProfile, deleteProfile, updateVisibility, getQualityReport, getWorkTimeline, listProfiles, exportToMarkdown, maskProfileData, calculateQualityScore, } from '../../services/jobSeeker/profileService';
 ...
 */
export async function getMyProfiles(req, res, next) {
    try {
        if (!req.user) {
            throw new AppError('Authentication required', 'UNAUTHORIZED', 401);
        }
        const isPrimary = req.query.isPrimary === 'true' ? true : req.query.isPrimary === 'false' ? false : undefined;
 ...
 * Get primary profile
 */
export async function getMyPrimaryProfile(req, res, next) {
    try {
        if (!req.user) {
            throw new AppError('Authentication required', 'UNAUTHORIZED', 401);
        }
 ...
 * Get quality report
 */
export async function getQuality(req, res, next) {
    try {
        if (!req.user) {
            throw new AppError('Authentication required', 'UNAUTHORIZED', 401);
        }
 ...
 */
export async function exportResume(req, res, next) {
    try {
        if (!req.user) {
            throw new AppError('Authentication required', 'UNAUTHORIZED', 401);
        }
...
export async function parseResume(req, res, next) {
    try {
        ...
        const { extractFromNaturalLanguage } = await import('../../services/jobSeeker/resumeExtractionService');
