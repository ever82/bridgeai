...
const roomAuth_1 = require("../socket/middleware/roomAuth");
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
...
describe('PROBE: RoomService boundary inputs', () => {
    let service;
    beforeEach(() => {
        service = new roomService_1.RoomService();
    });
    afterEach(() => {
        service.clearAllRooms();
    });
    it('should reject empty string room ID', async () => {
        await expect(service.createRoom('', 'user-1', { name: 'Room' })).rejects.toThrow('roomId must be at least 3 characters');
        expect(service.roomExists('')).toBe(false);
    });
    it('should handle extremely long room ID', async () => {
        const longId = 'a'.repeat(100000);
        await expect(service.createRoom(longId, 'user-1', { name: 'Room' })).rejects.toThrow('roomId must not exceed 200 characters');
    ...
    it('should handle room ID with special/injection characters', async () => {
        ...
        await expect(service.createRoom('<>\'"&', 'user-1', { name: 'Test' })).rejects.toThrow('roomId must be at least 3 characters');
    });
    ...
    it('should handle maxMembers edge cases', async () => {
        ...
        await expect(service.createRoom('room-1', 'user-1', { name: 'R', maxMembers: 0 })).rejects.toThrow('maxMembers must be at least 1');
        // Negative maxMembers - should be rejected
        await expect(service.createRoom('room-2', 'user-1', { name: 'R', maxMembers: -1 })).rejects.toThrow('maxMembers must be at least 1');
        // Valid maxMembers
...
describe('PROBE: RoomService privilege escalation', () => {
    ...
    it('should NOT allow admin to set another admin (only owner can)', () => {
        service.joinRoom('room-1', { userId: 'admin-1', socketId: 's-admin', role: 'admin' });
        service.joinRoom('room-1', { userId: 'member-1', socketId: 's-member' });
        expect(() => service.setUserRole('room-1', 'member-1', 'admin', 'admin-1')).toThrow('Only room owner can set admin role');
    });
    ...
    it('should NOT allow changing creator role to member', () => {
        expect(() => service.setUserRole('room-1', 'creator', 'member', 'creator')).toThrow("Cannot change room creator's role");
    });
    ...
    it('should NOT allow self-promotion', () => {
        service.joinRoom('room-1', { userId: 'member-1', socketId: 's-member' });
        expect(() => service.setUserRole('room-1', 'member-1', 'admin', 'member-1')).toThrow('Only room owner can set admin role');
    });
});
// ---------------------------------------------------------------------------
...
describe('PROBE: roomAuth middleware boundary cases', () => {
    let roomSvc;
    beforeEach(() => {
        roomSvc = new roomService_1.RoomService();
    });
    afterEach(() => {
    ...
    function patchRoomService(svc) {
        const mod = require('../services/roomService');
        mod.roomService = svc;
    }
    it('validateRoomCreate should reject empty roomId', () => {
        const socket = makeSocket();
        const result = (0, roomAuth_1.validateRoomCreate)(socket, '');
        expect(result.valid).toBe(false);
    });
    it('validateRoomCreate should reject short roomId (< 3 chars)', () => {
        const socket = makeSocket();
        const result = (0, roomAuth_1.validateRoomCreate)(socket, 'ab');
        expect(result.valid).toBe(false);
    ...
    it('validateRoomCreate should reject unauthenticated user', () => {
        ...
        const result = (0, roomAuth_1.validateRoomCreate)(socket, 'room-1');
        expect(result.valid).toBe(false);
    ...
    it('validateRoomJoin should handle non-existent room', async () => {
        ...
        const socket = makeSocket();
        const result = await (0, roomAuth_1.validateRoomJoin)(socket, 'non-existent');
        expect(result.valid).toBe(false);
    ...
    it('checkRoomAccess should return "none" level for non-existent room (unauthenticated)', () => {
        patchRoomService(roomSvc);
        const socket = makeSocket({ id: null });
        const result = (0, roomAuth_1.checkRoomAccess)(socket, 'non-existent');
    ...
    it('canKickUser should check that both users are members', async () => {
        ...
        const result = (0, roomAuth_1.canKickUser)('room-1', 'non-member', 'creator');
    ...
    it('isUserBlacklisted should delegate to roomService.isUserBanned', async () => {
        ...
        expect((0, roomAuth_1.isUserBlacklisted)('room-1', 'user-2')).toBe(true);
        expect((0, roomAuth_1.isUserBlacklisted)('room-1', 'creator')).toBe(false);
    });
});
// ---------------------------------------------------------------------------
...
describe('PROBE: ConnectionService boundary cases', () => {
    let service;
    beforeEach(() => {
        service = new connectionService_1.ConnectionService();
    });
    afterEach(() => {
...
describe('PROBE: PresenceService boundary cases', () => {
    let service;
    beforeEach(() => {
        service = new presenceService_1.PresenceService();
    });
    afterEach(() => {
        service.destroy();
    });
    ...
    function mockConnectionService(connected, count = 1) {
        const mod = require('../services/connectionService');
...
describe('PROBE: Private room password validation', () => {
    ...
    function _patchRoomService(svc) {
        const mod = require('../services/roomService');
        mod.roomService = svc;
    }
...
//# sourceMappingURL=probe_com001.test.js.map