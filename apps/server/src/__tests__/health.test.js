...
describe('Health Check Endpoints', () => {
    ...
    describe('GET /ready', () => {
        it('should return ready status', async () => {
            const res = await (0, supertest_1.default)(app_1.default).get('/ready');
            expect(res.status).toBe(200);
            expect(res.body).toHaveProperty('success', true);
    ...
    describe('GET /api/v1/health/detailed', () => {
        it('should return detailed health with checks when database is healthy', async () => {
            ...
            client_1.prisma.$queryRaw.mockResolvedValueOnce([1]);
            const res = await (0, supertest_1.default)(app_1.default).get('/api/v1/health/detailed');
            expect(res.status).toBe(200);
            expect(res.body.success).toBe(true);
            expect(res.body.data).toHaveProperty('status');
            expect(res.body.data).toHaveProperty('checks');
        ...
        it('should return unhealthy status when database is down', async () => {
            ...
            client_1.prisma.$queryRaw.mockRejectedValueOnce(new Error('DB connection failed'));
            const res = await (0, supertest_1.default)(app_1.default).get('/api/v1/health/detailed');
