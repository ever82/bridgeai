...
const mockBcryptHash = jest.fn().mockResolvedValue('$2b$12$hashednewpassword');
...
describe('ISSUE-A003a Probe Tests', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  describe('PT-001: 密码修改 - 相同密码检测', () => {
    test('BUG-1: userService.changePassword 缺少 SAME_PASSWORD 检查', async () => {
    ...
    test('BUG-2: users.ts POST /password 路由缺少 SAME_PASSWORD 检查', async () => {
    ...
    test('BUG-3: Bcrypt salt rounds 不一致', async () => {
    ...
    test('BUG-4: userService.verifyPassword 返回统一错误', async () => {
  ...
  describe('PT-002: 手机/邮箱绑定 - 重复绑定检测', () => {
    test('BUG-5: userService.updatePhone 检查手机号是否已被使用', async () => {
    ...
    test('BUG-6: userService.updateEmail 检查邮箱是否已被使用', async () => {
    ...
    test('BUG-7: updatePhone/updateEmail 不验证验证码', async () => {
  ...
  describe('PT-003: 账号删除 - 边界情况', () => {
    test('BUG-9: deleteUser 删除后 avatar cleanup 存在竞态条件', async () => {
    ...
    test('BUG-10: deleteUser 的 $transaction 返回值未处理', async () => {
    ...
    test('BUG-11: deleteUser 连接删除可能失败', async () => {
      ...
      mockPrisma.$transaction.mockRejectedValue(
        new Error('Foreign key constraint violation on connections')
      );
  ...
  describe('PT-004: 隐私设置 - 无效枚举值', () => {
    test('BUG-12: Zod schema 应拒绝无效的枚举值', async () => {
    ...
    test('BUG-13: 空 body 应该被接受但不更新任何字段', async () => {
      // 探针: 空 body {} 通过 Zod 验证
    ...
    test('BUG-14: updatePrivacySettings 部分更新正确合并', async () => {
      ...
      expect(newSettings.profileVisibility).toBe('private');
      expect(newSettings.onlineStatusVisibility).toBe('everyone');
      expect(newSettings.phoneVisibility).toBe('hidden');

      console.log('✓ 部分更新正确合并');
    });

    test('BUG-15: getPrivacySettings 返回完整默认设置', async () => {
  ...
  describe('PT-005: 设备管理 - 移除当前设备', () => {
    test('BUG-16: removeDevice 不检查设备是否存在', async () => {
    ...
    test('BUG-17: 移除当前设备后其他设备不会变成 current', async () => {
    ...
    test('BUG-18: registerDevice 的并发更新', async () => {
      // 探针: 并发注册同一设备可能产生竞争
  ...
  describe('PT-006: 拉黑/取消拉黑 - 重复操作', () => {
    test('BUG-19: blockUser 重复拉黑触发数据库错误', async () => {
      ...
      // 第二次拉黑同一用户 - 应该被拒绝
      mockPrisma.blockedUser.create.mockRejectedValueOnce(
    ...
    test('BUG-20: unblockUser 对未拉黑用户静默成功', async () => {
    ...
    test('BUG-21: blockUser 应拒绝拉黑自己', async () => {
    ...
    test('BUG-22: getBlockedUsers 正确返回拉黑列表', async () => {
  ...
  describe('PT-007: 并发安全性', () => {
    test('BUG-23: 并发修改密码可能导致数据不一致', async () => {
    ...
    test('BUG-24: isUserBlocked 使用 findUnique', async () => {
  ...
  describe('PT-008: API 响应一致性', () => {
    test('BUG-25: users.ts 和 userPrivacy.ts 响应格式不一致', async () => {
