...
const querySubscriptionManager_1 = require("../services/matching/querySubscriptionManager");
const queryEngine_1 = require("../services/matching/queryEngine");
describe('QuerySubscriptionManager', () => {
    let manager;
    beforeEach(() => {
        querySubscriptionManager_1.QuerySubscriptionManager.resetInstance();
        manager = querySubscriptionManager_1.QuerySubscriptionManager.getInstance();
        manager.destroy();
        jest.clearAllMocks();
    });
    afterEach(() => {
    ...
    describe('createSubscription', () => {
        it('should create a subscription for a user', async () => {
            queryEngine_1.queryEngine.execute.mockResolvedValue({
                data: [{ id: '1', status: 'active' }],
                cached: false,
            });
            const subscription = await manager.createSubscription('user1', { where: { field: 'status', operator: 'eq', value: 'active' } });
        ...
        it('should emit data event with initial results', async () => {
            ...
            queryEngine_1.queryEngine.execute.mockResolvedValue({ data: mockResults, cached: false });
        ...
        it('should emit error event when query fails', async () => {
            queryEngine_1.queryEngine.execute.mockRejectedValue(new Error('Query failed'));
            const eventHandler = jest.fn();
            manager.on('event', eventHandler);
            await manager.createSubscription('user1', { where: { field: 'status', operator: 'eq', value: 'active' } });
            const errorEvent = eventHandler.mock.calls.find((call) => call[0].type === 'error');
        ...
        it('should throw when user reaches subscription limit', async () => {
            ...
            queryEngine_1.queryEngine.execute.mockResolvedValue({ data: [], cached: false });
            for (let i = 0; i < 20; i++) {
                await manager.createSubscription(`user-limit-test`, dsl);
            }
            await expect(manager.createSubscription(`user-limit-test`, dsl)).rejects.toThrow(/Maximum subscriptions/);
        });
    });
    describe('removeSubscription', () => {
        ...
        it('should emit subscription_removed event', async () => {
            ...
            const removedEvent = eventHandler.mock.calls.find((call) => call[0].type === 'subscription_removed');
    ...
    describe('getUserSubscriptions', () => {
        it('should return all subscriptions for a user', async () => {
            queryEngine_1.queryEngine.execute.mockResolvedValue({ data: [], cached: false });
            await manager.createSubscription('user1', { where: { field: 'status', operator: 'eq', value: 'active' } });
            await manager.createSubscription('user1', { where: { field: 'score', operator: 'gte', value: 50 } });
    ...
    describe('refreshSubscription', () => {
        it('should execute the query and emit data event', async () => {
            const mockExecute = queryEngine_1.queryEngine.execute;
...
//# sourceMappingURL=querySubscriptionManager.test.js.map