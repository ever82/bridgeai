...
// Rate limiting
const acceptRateLimit = new Map();
const MAX_ACCEPTS_PER_HOUR = 10;
// Task storage (in production, this would be from Prisma)
const taskStore = new Map();
const userTaskMap = new Map(); // userId -> taskIds
/**
 * Check if user is eligible to accept a task
 * 检查用户是否有资格接单
 */
export async function checkTaskEligibility(taskId, userId, userLocation, userCreditScore, userPoints) {
    try {
 ...
 * 接单
 */
export async function acceptTask(request, userCreditScore, userPoints) {
    try {
        const { taskId, userId, userLocation, message } = request;
        // Check eligibility
        const eligibility = await checkTaskEligibility(taskId, userId, userLocation, userCreditScore, userPoints);
 ...
 * 将资格原因映射到错误码
 */
function mapEligibilityToErrorCode(reasons) {
    if (reasons.some((r) => r.includes('信用分')))
        return 'INSUFFICIENT_CREDIT';
    if (reasons.some((r) => r.includes('积分')))
        return 'INSUFFICIENT_POINTS';
    if (reasons.some((r) => r.includes('距离')))
        return 'TOO_FAR';
    if (reasons.some((r) => r.includes('已被接单')))
        return 'ALREADY_ACCEPTED_BY_OTHER';
    if (reasons.some((r) => r.includes('已取消')))
 ...
 * 更新任务状态
 */
export async function updateTaskStatus(update) {
    try {
 ...
 * 获取用户已接任务
 */
export async function getUserAcceptedTasks(userId) {
 ...
 * 取消任务
 */
export async function cancelTask(taskId, userId, reason) {
 ...
 * 申诉任务
 */
export async function disputeTask(taskId, userId, reason) {
    const task = taskStore.get(taskId);
    if (!task) {
        return null;
    }
