...
export class VisionShareTaskService {
    logger = logger.child({ module: 'VisionShareTaskService' });
    /**
     * 创建草稿任务
     */
     ...
     * 使用AI提炼需求
     */
    async refineDemand(taskId, description, userId) {
        try {
     ...
     * 发布任务
     */
    async publishTask(userId, taskId) {
        try {
            ...
            const estimatedMatchTime = this.calculateEstimatedMatchTime(updatedTask);
     ...
     * 取消任务
     */
    async cancelTask(userId, taskId, reason) {
        try {
     ...
     * 创建历史记录
     */
    async createHistoryRecord(taskId, status, metadata) {
        try {
     ...
     * 推送状态更新
     */
    pushStatusUpdate(taskId, status, userId) {
        try {
     ...
     * 计算预计匹配时间
     */
    calculateEstimatedMatchTime(task) {
        // 基于任务特征计算预计匹配时间
