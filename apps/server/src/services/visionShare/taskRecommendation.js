...
export async function getTaskRecommendations(request) {
    ...
    try {
        ...
        let tasks = nearbyResults.filter(result => !preferences.excludeTaskIds?.includes(result.task.id));
        ...
        const recommendations = tasks.map(result => ({
            task: result.task,
 ...
 * 计算任务的匹配度评分
 */
function calculateMatchScore(task, userLocation, userId, preferences) {
    let score = 0;
 ...
 * 生成匹配原因
 */
function generateMatchReasons(task, distanceKm, preferences) {
    const reasons = [];
    // Distance reason
    if (distanceKm < 1) {
 ...
 * 获取实时任务推荐
 */
export async function getRealtimeRecommendations(userId, userLocation) {
    try {
 ...
 * 记录用户任务交互
 */
export async function recordTaskInteraction(userId, taskId, accepted) {
 ...
 * 获取用户已接任务
 */
export async function getUserAcceptedTasks(_userId) {
 ...
 * 推送推荐任务通知
 */
export async function pushRecommendationNotification(userId, recommendation) {
 ...
 * 订阅附近任务更新
 */
export function subscribeToNearbyTasks(userId, userLocation, radiusKm, _callback) {
...
//# sourceMappingURL=taskRecommendation.js.map