 ...
 * 任务筛选服务
 */
import { calculateDistance, DEFAULT_TASK_FILTER, } from '@bridgeai/shared';
 ...
 */
export function initializeMockTasks() {
    const now = new Date();
...
export async function searchTasks(request) {
    try {
        ...
        if (keyword) {
            ...
            tasks = tasks.filter(task => task.title.toLowerCase().includes(lowerKeyword) ||
 ...
 * 获取附近的任务
 */
export async function getNearbyTasks(query) {
    try {
 ...
 * 计算预计到达时间（分钟）
 */
function calculateEstimatedArrival(from, to) {
