...
export class PublishValidationService {
    logger = logger.child({ module: 'PublishValidation' });
    // 配置常量
    MIN_CREDIT_SCORE = 500; // 最低信用分要求
    DAILY_PUBLISH_LIMIT = 10; // 每日发布限制
    MAX_LOCATION_OFFSET = 180; // 最大经纬度偏移
    /**
     * 执行完整的发布验证
     */
    async validate(options) {
        ...
        try {
            ...
            // 并行执行所有验证
            const [creditResult, balanceResult, limitResult, contentResult, locationResult,] = await Promise.all([
     ...
     * 检查用户积分/余额
     */
    async checkBalance(userId, budgetAmount, budgetType) {
        try {
...
//# sourceMappingURL=publishValidation.js.map