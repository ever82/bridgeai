...
export class AppealService {
    ...
    async submitAppeal(data) {
        ...
        if (existing) {
            throw new AppError('An appeal has already been submitted for this item', 'APPEAL_EXISTS', 409);
        }
        ...
        if (targetCreatedAt) {
            ...
            if (Date.now() - targetCreatedAt.getTime() > windowMs) {
                throw new AppError(`Appeal window of ${APPEAL_WINDOW_DAYS} days has expired`, 'APPEAL_EXPIRED', 400);
            }
        }
     ...
     */
    async reviewAppeal(appealId, data) {
        ...
        if (appeal.status !== 'PENDING') {
            throw new AppError('Appeal has already been reviewed', 'APPEAL_ALREADY_REVIEWED', 400);
        }
     ...
     */
    async restoreCreditScore(appeal) {
        if (appeal.targetType === 'violation') {
     ...
     */
    async withdrawAppeal(appealId, userId) {
        const appeal = await prisma.appeal.findUnique({ where: { id: appealId } });
        if (!appeal) {
            throw new AppError('Appeal not found', 'APPEAL_NOT_FOUND', 404);
        }
        if (appeal.userId !== userId) {
            throw new AppError('You can only withdraw your own appeals', 'FORBIDDEN', 403);
        }
        if (appeal.status !== 'PENDING') {
            throw new AppError('Only pending appeals can be withdrawn', 'APPEAL_NOT_PENDING', 400);
        }
        const updated = await prisma.appeal.update({
            where: { id: appealId },
            data: {
     ...
     */
    async getAppealsByUser(userId, page = 1, limit = 20) {
