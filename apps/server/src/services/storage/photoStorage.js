...
export class PhotoStorageService {
    ...
    async getS3Client() {
        ...
        if (!accessKeyId || !secretAccessKey) {
            console.warn('[PhotoStorageService] S3 credentials not configured (S3_ACCESS_KEY / S3_SECRET_KEY). ' +
     ...
     */
    async getPhotosByUser(userId, options) {
        const orderBy = options?.orderBy || 'uploadedAt';
        const order = options?.order || 'desc';
        try {
            // Map application-level sort keys to Prisma field names
            const orderByField = orderBy === 'uploadedAt' ? 'createdAt' : orderBy === 'accessCount' ? 'accessCount' : 'size';
     ...
     */
    getStorageUrl(photoId, type = 'original') {
     ...
     */
    async generatePresignedUploadUrl(userId, options) {
     ...
     * credentials are available. Falls back to a mock URL otherwise.
     */
    async generatePresignedDownloadUrl(photoId, options) {
    ...
    async applyLifecycle() {
        ...
        for (const metadata of this.metadataCache.values()) {
            ...
            if (!rule)
                continue;
            let newTier = metadata.lifecycle.currentTier;
     ...
     */
    async createThumbnail(photoId, imageBuffer) {
        if (imageBuffer) {
    ...
    rowToMetadata(row) {
        return {
            ...
            lifecycle: {
                currentTier: row.storageTier || 'hot',
     ...
     */
    async transitionStorageTier(photoId, tier) {
