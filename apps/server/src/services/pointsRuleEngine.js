 ...
 * 处理积分规则的校验、计算和执行
 */
import { PointsTransactionType, } from '@bridgeai/shared';
import { prisma } from '../db/client';
import { AppError } from '../errors/AppError';
import { getRuleByCode, getPointsLimitConfig, getTransferConfig, getFreezeConfig, calculateRechargePoints, calculateTransferFee, EARN_RULES, SPEND_RULES, } from '../config/pointsRules';
// 规则引擎类
export class PointsRuleEngine {
    ...
    async checkLimits(context) {
        ...
        if (rule.cooldownMinutes && rule.cooldownMinutes > 0) {
            ...
            if (recentTransaction) {
                const remainingCooldown = Math.ceil((recentTransaction.createdAt.getTime() + rule.cooldownMinutes * 60 * 1000 - now.getTime()) / 60000);
                return {
                    allowed: false,
                    currentDailyCount: dailyCount,
                    currentWeeklyCount: weeklyCount,
    ...
    async calculatePoints(context) {
        ...
        switch (ruleCode) {
            ...
            case 'CHECKIN_CONTINUOUS':
                // 连续签到额外奖励
                const continuousDays = metadata?.continuousDays || 0;
     ...
     * 检查全局每日/每周获取限制
     */
    async checkGlobalEarnLimits(userId, amount) {
        ...
        if (dailyTotal > limits.dailyEarnLimit) {
            throw new AppError(`Daily earn limit exceeded. Limit: ${limits.dailyEarnLimit}`, 'DAILY_EARN_LIMIT_EXCEEDED', 400);
        }
        if (weeklyTotal > limits.weeklyEarnLimit) {
            throw new AppError(`Weekly earn limit exceeded. Limit: ${limits.weeklyEarnLimit}`, 'WEEKLY_EARN_LIMIT_EXCEEDED', 400);
        }
        return true;
    }
    /**
     * 检查全局每日/每周消耗限制
     */
    async checkGlobalSpendLimits(userId, amount) {
        const limits = getPointsLimitConfig();
        const now = new Date();
        const today = new Date(now.getFullYear(), now.getMonth(), now.getDate());
        const weekStart = new Date(today);
        ...
        if (dailyTotal > limits.dailySpendLimit) {
            throw new AppError(`Daily spend limit exceeded. Limit: ${limits.dailySpendLimit}`, 'DAILY_SPEND_LIMIT_EXCEEDED', 400);
        }
        if (weeklyTotal > limits.weeklySpendLimit) {
            throw new AppError(`Weekly spend limit exceeded. Limit: ${limits.weeklySpendLimit}`, 'WEEKLY_SPEND_LIMIT_EXCEEDED', 400);
        }
        return true;
    }
    /**
     ...
     * 验证冻结参数
     */
    validateFreeze(accountBalance, currentFrozen, freezeAmount) {
    ...
    getRulesByScene(scene) {
        return Object.values(SPEND_RULES).filter(rule => rule.enabled && rule.scene === scene);
    }
    /**
     * 获取规则详情
     */
...
//# sourceMappingURL=pointsRuleEngine.js.map