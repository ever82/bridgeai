 ...
 * 匹配算法 A/B 测试框架
 */
import { MatchScoringModel } from '../matchAlgorithm/matchAlgorithm';
export class MatchABTestService {
     ...
     * 获取实验对应的匹配模型
     */
    getModelForExperiment(experimentId, userId) {
     ...
     * 计算平均匹配时间
     */
    calculateAverageTimeToMatch(matchedEvents) {
        ...
        for (const event of matchedEvents) {
            // 查找对应的计算事件
            const calculatedEvent = this.events.find((e) => e.matchId === event.matchId && e.eventType === 'calculated');
            if (calculatedEvent) {
     ...
     * 获取优胜 variant
     */
    getWinningVariant(experimentId, metric = 'successRate') {
        const experiment = this.experiments.get(experimentId);
        if (!experiment)
            return null;
...
//# sourceMappingURL=matchABTest.js.map