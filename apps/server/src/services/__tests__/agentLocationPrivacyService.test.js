...
const agentLocationPrivacyService_1 = require("../agentLocationPrivacyService");
// Mock Prisma
jest.mock('../../db/client', () => ({
    prisma: {
...
describe('agentLocationPrivacyService', () => {
    ...
    describe('getAgentLocationPrivacy', () => {
        it('should return privacy settings for agent with settings', async () => {
            mockPrisma.agentLocationPrivacy.findUnique.mockResolvedValue({
                agentId: 'agent-123',
                privacyLevel: 'CITY',
                showExactCoords: false,
                hideFromPublic: false,
            });
            const result = await (0, agentLocationPrivacyService_1.getAgentLocationPrivacy)('agent-123');
        ...
        it('should return null for agent without privacy settings', async () => {
            mockPrisma.agentLocationPrivacy.findUnique.mockResolvedValue(null);
            const result = await (0, agentLocationPrivacyService_1.getAgentLocationPrivacy)('agent-unknown');
            expect(result).toBeNull();
        });
        it('should return null on database error', async () => {
            mockPrisma.agentLocationPrivacy.findUnique.mockRejectedValue(new Error('DB error'));
            const result = await (0, agentLocationPrivacyService_1.getAgentLocationPrivacy)('agent-123');
            expect(result).toBeNull();
        });
    });
    describe('setAgentLocationPrivacy', () => {
        it('should create privacy settings for new agent', async () => {
            mockPrisma.agentLocationPrivacy.upsert.mockResolvedValue({
                agentId: 'agent-new',
                privacyLevel: 'DISTRICT',
                showExactCoords: true,
                hideFromPublic: false,
            });
            const result = await (0, agentLocationPrivacyService_1.setAgentLocationPrivacy)('agent-new', {
                privacyLevel: 'DISTRICT',
                showExactCoords: true,
            });
        ...
        it('should update existing privacy settings', async () => {
            ...
            const result = await (0, agentLocationPrivacyService_1.setAgentLocationPrivacy)('agent-123', {
                privacyLevel: 'HIDDEN',
                hideFromPublic: true,
            });
            expect(result).toBe(true);
        ...
        it('should return false on database error', async () => {
            mockPrisma.agentLocationPrivacy.upsert.mockRejectedValue(new Error('DB error'));
            const result = await (0, agentLocationPrivacyService_1.setAgentLocationPrivacy)('agent-123', {
                privacyLevel: 'CITY',
            });
    ...
    describe('applyPrivacyFilter', () => {
        it('should hide all data when hideFromPublic is true', async () => {
            ...
            const result = await (0, agentLocationPrivacyService_1.applyPrivacyFilter)('agent-123', {
        ...
        it('should hide all data when no privacy settings exist', async () => {
            mockPrisma.agentLocationPrivacy.findUnique.mockResolvedValue(null);
            const result = await (0, agentLocationPrivacyService_1.applyPrivacyFilter)('agent-no-privacy', {
                location: { province: '110000', city: '110100' },
                latitude: 39.9,
                longitude: 116.4,
            });
        ...
        it('should apply EXACT privacy level - return all data when showExactCoords is true', async () => {
            ...
            const result = await (0, agentLocationPrivacyService_1.applyPrivacyFilter)('agent-123', locationData);
    ...
    describe('deleteAgentLocationPrivacy', () => {
        it('should delete privacy settings successfully', async () => {
            mockPrisma.agentLocationPrivacy.delete.mockResolvedValue({
                agentId: 'agent-123',
                privacyLevel: 'CITY',
                showExactCoords: false,
                hideFromPublic: false,
            });
            const result = await (0, agentLocationPrivacyService_1.deleteAgentLocationPrivacy)('agent-123');
            expect(result).toBe(true);
        ...
        it('should return false on database error', async () => {
            mockPrisma.agentLocationPrivacy.delete.mockRejectedValue(new Error('DB error'));
            const result = await (0, agentLocationPrivacyService_1.deleteAgentLocationPrivacy)('agent-123');
            expect(result).toBe(false);
        });
    });
});
//# sourceMappingURL=agentLocationPrivacyService.test.js.map