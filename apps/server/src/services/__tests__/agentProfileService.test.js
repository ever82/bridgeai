...
const agentProfileService = __importStar(require("../agentProfileService"));
...
const { __mockAgentFindUnique: mockAgentFindUnique, __mockAgentProfileFindFirst: mockAgentProfileFindFirst, __mockAgentProfileUpdate: mockAgentProfileUpdate, __mockAgentProfileCreate: mockAgentProfileCreate, __mockProfileHistoryCreate: mockProfileHistoryCreate, } = jest.requireMock('../../db/client');
// Mock logger
...
describe('AgentProfileService - L1 Operations', () => {
    ...
    describe('getL1Profile', () => {
        it('should return L1 data for owned agent', async () => {
            ...
            mockAgentProfileFindFirst.mockResolvedValue({
                id: 'profile-1',
                agentId: mockAgentId,
                l1Data: { age: shared_1.AgeRange.AGE_26_30, gender: shared_1.Gender.MALE },
                l2Data: null,
                l3Description: null,
            });
            const result = await agentProfileService.getL1Profile(mockAgentId, mockUserId);
            expect(result).toEqual({ age: shared_1.AgeRange.AGE_26_30, gender: shared_1.Gender.MALE });
        ...
        it('should throw AGENT_NOT_FOUND for non-existent agent', async () => {
            ...
            await expect(agentProfileService.getL1Profile('nonexistent', mockUserId)).rejects.toThrow(AppError_1.AppError);
            await expect(agentProfileService.getL1Profile('nonexistent', mockUserId)).rejects.toMatchObject({ code: 'AGENT_NOT_FOUND', statusCode: 404 });
        });
        it('should throw UNAUTHORIZED for wrong user', async () => {
            ...
            await expect(agentProfileService.getL1Profile(mockAgentId, mockUserId)).rejects.toMatchObject({ code: 'UNAUTHORIZED', statusCode: 403 });
        });
    ...
    describe('updateL1Profile', () => {
        ...
        it('should update L1 profile with valid data', async () => {
            ...
            const result = await agentProfileService.updateL1Profile(mockAgentId, mockUserId, validL1Data);
        ...
        it('should throw VALIDATION_ERROR for invalid age enum', async () => {
            ...
            await expect(agentProfileService.updateL1Profile(mockAgentId, mockUserId, {
                age: 'INVALID_AGE',
        ...
        it('should throw AGENT_NOT_FOUND for non-existent agent', async () => {
            mockAgentFindUnique.mockResolvedValue(null);
            await expect(agentProfileService.updateL1Profile('nonexistent', mockUserId, validL1Data)).rejects.toMatchObject({ code: 'AGENT_NOT_FOUND', statusCode: 404 });
        });
        it('should throw UNAUTHORIZED for wrong user', async () => {
            mockAgentFindUnique.mockResolvedValue({ id: mockAgentId, userId: 'different-user' });
            await expect(agentProfileService.updateL1Profile(mockAgentId, mockUserId, validL1Data)).rejects.toMatchObject({ code: 'UNAUTHORIZED', statusCode: 403 });
        });
    });
});
describe('AgentProfileService - L2 Operations', () => {
    ...
    describe('getL2Profile', () => {
        ...
        it('should throw AGENT_NOT_FOUND for non-existent agent', async () => {
            mockAgentFindUnique.mockResolvedValue(null);
            await expect(agentProfileService.getL2Profile('nonexistent', mockUserId)).rejects.toMatchObject({ code: 'AGENT_NOT_FOUND', statusCode: 404 });
        });
    });
    describe('updateL2Profile', () => {
        ...
        it('should update L2 profile with valid data', async () => {
            ...
            mockProfileHistoryCreate.mockResolvedValue({ id: 'history-1' });
            const result = await agentProfileService.updateL2Profile(mockAgentId, mockUserId, validL2Data);
            expect(result).toEqual(validL2Data);
            expect(mockProfileHistoryCreate).toHaveBeenCalledWith(expect.objectContaining({
                data: expect.objectContaining({ layer: 'L2', action: 'update' }),
            }));
        });
        it('should throw VALIDATION_ERROR for description > 2000 chars', async () => {
            mockAgentFindUnique.mockResolvedValue({ id: mockAgentId, userId: mockUserId });
            await expect(agentProfileService.updateL2Profile(mockAgentId, mockUserId, {
        ...
        it('should throw AGENT_NOT_FOUND for non-existent agent', async () => {
            mockAgentFindUnique.mockResolvedValue(null);
            await expect(agentProfileService.updateL2Profile('nonexistent', mockUserId, validL2Data)).rejects.toMatchObject({ code: 'AGENT_NOT_FOUND', statusCode: 404 });
        });
        it('should throw UNAUTHORIZED for wrong user', async () => {
            mockAgentFindUnique.mockResolvedValue({ id: mockAgentId, userId: 'different-user' });
            await expect(agentProfileService.updateL2Profile(mockAgentId, mockUserId, validL2Data)).rejects.toMatchObject({ code: 'UNAUTHORIZED', statusCode: 403 });
        });
...
describe('AgentProfileService - L3 Operations', () => {
    ...
    describe('getL3Profile', () => {
        ...
        it('should throw AGENT_NOT_FOUND for non-existent agent', async () => {
            mockAgentFindUnique.mockResolvedValue(null);
            await expect(agentProfileService.getL3Profile('nonexistent', mockUserId)).rejects.toMatchObject({ code: 'AGENT_NOT_FOUND', statusCode: 404 });
        });
    });
    describe('updateL3Profile', () => {
        ...
        it('should throw VALIDATION_ERROR for description > 10000 chars', async () => {
            mockAgentFindUnique.mockResolvedValue({ id: mockAgentId, userId: mockUserId });
            await expect(agentProfileService.updateL3Profile(mockAgentId, mockUserId, {
                description: 'x'.repeat(10001),
            })).rejects.toMatchObject({ code: 'VALIDATION_ERROR', statusCode: 400 });
        });
        it('should throw AGENT_NOT_FOUND for non-existent agent', async () => {
            mockAgentFindUnique.mockResolvedValue(null);
            await expect(agentProfileService.updateL3Profile('nonexistent', mockUserId, {
                description: 'desc',
...
describe('AgentProfileService - L1 Basic Info Model', () => {
    describe('calculateL1Completion', () => {
        ...
        it('should count age field correctly', () => {
            const result = agentProfileService.calculateL1Completion({ age: shared_1.AgeRange.AGE_26_30 });
        ...
        it('should count gender field correctly', () => {
            const result = agentProfileService.calculateL1Completion({ gender: shared_1.Gender.MALE });
            expect(result.l1Percentage).toBe(20);
            expect(result.l1FilledFields).toBe(1);
        });
        ...
        it('should calculate weighted score correctly', () => {
            const result = agentProfileService.calculateL1Completion({
                age: shared_1.AgeRange.AGE_26_30, // 20
                gender: shared_1.Gender.FEMALE, // 15
                location: { province: 'Beijing', city: 'Beijing' }, // 25
                occupation: 'Engineer', // 20
                education: shared_1.EducationLevel.MASTER, // 20
            });
        ...
        it('should handle partial completion', () => {
            const result = agentProfileService.calculateL1Completion({
                age: shared_1.AgeRange.AGE_31_35,
                occupation: 'Designer',
            });
...
//# sourceMappingURL=agentProfileService.test.js.map