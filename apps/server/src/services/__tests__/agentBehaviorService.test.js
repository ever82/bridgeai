...
const agentBehaviorService_1 = require("../agentBehaviorService");
describe('AgentBehaviorService', () => {
    let service;
    let mockMessage;
    const agentId = 'agent_test_123';
    beforeEach(() => {
        service = new agentBehaviorService_1.AgentBehaviorService();
        mockMessage = {
            id: 'msg_test_123',
            type: shared_1.AgentMessageType.DIRECT,
            sender: {
                agentId,
                displayName: 'Test Agent',
                type: shared_1.AgentType.PERSONAL,
                ownerId: 'user_456',
                ownerName: 'Test User',
                trustScore: 95,
                isVerified: true,
            ...
            metadata: {
                ...
                priority: shared_1.MessagePriority.NORMAL,
                requireAck: false,
                ttl: 0,
                traceId: 'trace_123',
            },
    ...
    describe('checkBehavior', () => {
        ...
        it('should reject message from suspended agent', async () => {
            ...
            expect(result.error?.code).toBe(shared_1.AgentProtocolErrorCode.UNAUTHORIZED);
        });
        it('should reject restricted agent sending group messages', async () => {
            ...
            const groupMessage = {
                ...mockMessage,
                type: shared_1.AgentMessageType.GROUP,
            };
        ...
        it('should reject message exceeding rate limit', async () => {
            ...
            expect(result.allowed).toBe(false);
            expect(result.error?.code).toBe(shared_1.AgentProtocolErrorCode.RATE_LIMITED);
        });
        it('should reject message with prohibited content', async () => {
            ...
            expect(result.allowed).toBe(false);
            expect(result.error?.code).toBe(shared_1.AgentProtocolErrorCode.CONTENT_VIOLATION);
        });
...
//# sourceMappingURL=agentBehaviorService.test.js.map