...
const privacyService = __importStar(require("../dating/privacyService"));
describe('DatingPrivacyService', () => {
    ...
    describe('getVisibleFieldsAtStage', () => {
        it('should show all fields to owner', () => {
            const result = privacyService.getVisibleFieldsAtStage(baseProfile, 'initial', true, false);
            expect(result).toContain('basicInfo');
            expect(result).toContain('income');
            expect(result).toContain('location');
        });
        it('should hide matched-only fields at initial stage', () => {
            const result = privacyService.getVisibleFieldsAtStage(baseProfile, 'initial', false, false);
            expect(result).toContain('basicInfo');
            expect(result).not.toContain('income');
            expect(result).not.toContain('location');
        });
        it('should show matched-only fields after matching', () => {
            const result = privacyService.getVisibleFieldsAtStage(baseProfile, 'matched', false, true);
            expect(result).toContain('income');
            expect(result).toContain('location');
        });
        it('should hide all fields for private profile', () => {
            ...
            const result = privacyService.getVisibleFieldsAtStage(privateProfile, 'initial', false, false);
    ...
    describe('filterProfileForViewer', () => {
        it('should return full profile for owner', () => {
            const result = privacyService.filterProfileForViewer(baseProfile, 'user-123', 'initial', false);
        ...
        it('should filter sensitive fields for non-owner', () => {
            const result = privacyService.filterProfileForViewer(baseProfile, 'other-user', 'initial', false);
        ...
        it('should show more fields after matching', () => {
            const result = privacyService.filterProfileForViewer(baseProfile, 'other-user', 'matched', true);
    ...
    describe('canAccessField', () => {
        it('should allow owner to access any field', () => {
            const result = privacyService.canAccessField(baseProfile, 'income', 'user-123', 'initial', false);
            expect(result).toBe(true);
        });
        it('should deny access to private fields', () => {
            const result = privacyService.canAccessField(baseProfile, 'contactInfo', 'other-user', 'initial', false);
            expect(result).toBe(false);
        });
        it('should deny access to matched-only fields before matching', () => {
            const result = privacyService.canAccessField(baseProfile, 'income', 'other-user', 'initial', false);
            expect(result).toBe(false);
        });
        it('should allow access to matched-only fields after matching', () => {
            const result = privacyService.canAccessField(baseProfile, 'income', 'other-user', 'matched', true);
            expect(result).toBe(true);
        });
        it('should deny access to public fields for private profile', () => {
            ...
            const result = privacyService.canAccessField(privateProfile, 'basicInfo', 'other-user', 'initial', false);
            expect(result).toBe(false);
        });
    });
...
//# sourceMappingURL=datingPrivacyService.test.js.map