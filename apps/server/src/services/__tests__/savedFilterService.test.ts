...
import * as savedFilterService from '../savedFilterService';
...
describe('createSavedFilter', () => {
  ...
  it('defaults isPublic to false when not provided', async () => {
    ...
    const result = await savedFilterService.createSavedFilter(
      mockUserId,
      makeInput({ isPublic: undefined })
    );
...
describe('updateSavedFilter', () => {
  ...
  it('re-throws on database error', async () => {
    ...
    await expect(
      savedFilterService.updateSavedFilter(mockFilterId, mockUserId, { name: 'X' })
    ).rejects.toThrow('DB error');
  });
});
