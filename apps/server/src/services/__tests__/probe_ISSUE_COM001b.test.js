...
describe('PROBE: ConnectionService - Adversarial Tests', () => {
    ...
    it('PROBE-C014: parseDeviceInfo with crafted user agent containing multiple OS patterns', () => {
        ...
        const info = service.parseDeviceInfo('Mozilla/5.0 (Linux; Android 10; iPhone) AppleWebKit/537.36 Chrome/91.0', {});
...
describe('PROBE: RoomService - Adversarial Tests', () => {
    ...
    it('PROBE-R003: createRoom with maxMembers=0 should be rejected (throws)', async () => {
        await expect(service.createRoom('room-1', 'user-1', { name: 'Room', maxMembers: 0 })).rejects.toThrow('maxMembers must be at least 1');
    });
    ...
    it('PROBE-R012: unbanUser requires owner/admin permission', async () => {
        ...
        expect(() => service.unbanUser('room-1', 'user-3', 'member-1')).toThrow('Insufficient permissions');
    });
    it('PROBE-R013: muteUser requires owner/admin permission', async () => {
        ...
        expect(() => service.muteUser('room-1', 'user-1', 'member-1')).toThrow('Insufficient permissions');
    });
    it('PROBE-R014: setUserRole cannot create multiple owners (single-owner model)', async () => {
        ...
        expect(() => service.setUserRole('room-1', 'user-2', 'owner', 'user-1')).toThrow('Cannot set owner role; room owner role cannot be transferred');
    ...
    it('PROBE-R015: destroyRoom requires owner/admin permission', async () => {
        ...
        expect(() => service.destroyRoom('room-2', 'member-1')).toThrow('Insufficient permissions');
    });
    ...
    it('PROBE-R017: updateRoom allows overwriting createdBy via Object.assign', async () => {
        ...
        service.updateRoom('room-1', { name: 'Hacked' });
        const room = service.getRoom('room-1');
...
describe('PROBE: PresenceService - Adversarial Tests', () => {
    ...
    it('PROBE-P001: setPresence with invalid status should throw', () => {
        expect(() => {
            service.setPresence('user-1', 'invalid-status');
...
describe('PROBE: Cross-Service Integration - Adversarial Tests', () => {
    let roomServiceInstance;
    beforeEach(() => {
        roomServiceInstance = new roomService_1.RoomService();
        // Clear singletons to ensure clean state for each test
        connectionService_1.connectionService.clearAllConnections();
        presenceService_1.presenceService.clearAll();
    });
    afterEach(() => {
    ...
    it('PROBE-X001: user disconnects from connection service but stays in room', async () => {
        // Register connection
        connectionService_1.connectionService.registerConnection('s1', 'u1', { deviceType: 'mobile' }, '1.1.1.1');
        ...
        connectionService_1.connectionService.unregisterConnection('s1');
        ...
        expect(connectionService_1.connectionService.isUserConnected('u1')).toBe(false);
        // State inconsistency!
    });
    ...
    it('PROBE-X003: connectionService tracks rooms that roomService does not know about', () => {
        connectionService_1.connectionService.registerConnection('s1', 'u1', { deviceType: 'mobile' }, '1.1.1.1');
        connectionService_1.connectionService.addConnectionRoom('s1', 'phantom-room');
        const connsInRoom = connectionService_1.connectionService.getConnectionsInRoom('phantom-room');
    ...
    it('PROBE-X004: presence deviceCount desync from actual connections', () => {
        ...
        presenceService_1.presenceService.setPresence('u1', 'online');
        expect(presenceService_1.presenceService.getPresence('u1').deviceCount).toBe(0);
        // Add connections via connectionService
        connectionService_1.connectionService.registerConnection('s1', 'u1', { deviceType: 'mobile' }, '1.1.1.1');
        // getPresence refreshes deviceCount via connectionService
        const presence = presenceService_1.presenceService.getPresence('u1');
        expect(presence.deviceCount).toBe(1);
        // But the stored presence object is updated in-place, which could cause issues
    });
});
//# sourceMappingURL=probe_ISSUE_COM001b.test.js.map