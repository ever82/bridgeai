 ...
 * - 需求预览和发布
 */
import { AgentStatus, AgentAdRole, ConsumerDemandStatus, } from '@bridgeai/shared';
import { prisma } from '../db/client';
import { AppError } from '../errors/AppError';
import { logger } from '../utils/logger';
// Re-export types from shared
export { AgentAdRole, ConsumerDemandStatus, } from '@bridgeai/shared';
/**
 * Create a consumer Agent for AgentAd scenario
 * 为消费者创建 Agent
 */
export async function createConsumerAgent(userId, request) {
    logger.info('Creating consumer agent', { userId, name: request.name });
    // Validate role
    if (request.role !== AgentAdRole.CONSUMER) {
        throw new AppError('Invalid role for consumer agent. Must be CONSUMER', 'INVALID_ROLE', 400);
    }
    // Validate name
 ...
 * 配置消费类别（最多5个）
 */
export async function configureCategories(agentId, userId, categoryIds) {
    logger.info('Configuring categories', { agentId, categoryCount: categoryIds.length });
    // Validate category count
    if (categoryIds.length > 5) {
        throw new AppError('Maximum 5 categories allowed', 'TOO_MANY_CATEGORIES', 400);
    }
    if (categoryIds.length === 0) {
        throw new AppError('At least one category is required', 'NO_CATEGORIES', 400);
    }
 ...
 * 配置预算范围
 */
export async function configureBudget(agentId, userId, budget) {
 ...
 * 配置品牌和商家偏好
 */
export async function configurePreferences(agentId, userId, preferences) {
 ...
 * 更新时间要求配置
 */
export async function configureTimeline(agentId, userId, timeline) {
 ...
 * 应用 AI 提取的需求数据
 */
export async function applyAIExtractedData(agentId, userId, extractedData) {
    ...
    const profile = config.consumerProfile;
    ...
    if (extractedData.budget) {
        ...
        if (extractedData.budget.type) {
            profile.budget.type = extractedData.budget.type;
        }
    }
 ...
 * 生成需求画像预览
 */
export async function previewDemandProfile(agentId, userId) {
    ...
    if (locationSummary)
        summaryParts.push(locationSummary);
 ...
 */
function buildBrandSummary(brandPref) {
 ...
 */
function getMerchantTypeLabel(type) {
    const labels = {
        CHAIN: '连锁品牌',
        LOCAL: '本地商家',
        INDIVIDUAL: '个人商家',
 ...
 */
function getDisclosureLabel(disclosure) {
    const labels = {
        PUBLIC: '公开',
        PRIVATE: '私密',
        RANGE_ONLY: '仅范围',
 ...
 * 发布消费者需求
 */
export async function publishDemand(agentId, userId) {
 ...
 * 获取消费者 Agent 及完整画像
 */
export async function getConsumerAgent(agentId, userId) {
    const agent = await prisma.agent.findUnique({
        where: { id: agentId },
    });
    if (!agent) {
    ...
    return {
        ...agent,
        config,
        profile: config.consumerProfile,
    };
}
/**
 ...
 * 更新位置配置
 */
export async function configureLocation(agentId, userId, location) {
...
//# sourceMappingURL=consumerDemandService.js.map