...
const desensitization_1 = require("../desensitization");
...
function createDetection(type, bbox = { x: 50, y: 50, width: 80, height: 80 }) {
    return {
        type: type,
        boundingBox: bbox,
        confidence: 0.9,
...
describe('Image Desensitization Service', () => {
    describe('desensitizeImage', () => {
        it('should apply blur desensitization to detected regions', async () => {
            ...
            const result = await (0, desensitization_1.desensitizeImage)(buffer, detections, {
                method: 'blur',
                intensity: 70,
            });
        ...
        it('should return valid image even with zero detections', async () => {
            const buffer = await createTestImage();
            const result = await (0, desensitization_1.desensitizeImage)(buffer, [], { method: 'blur', intensity: 70 });
            expect(result.processedImageBuffer).toBeInstanceOf(Buffer);
        ...
        it('should handle bounding boxes at image edges', async () => {
            ...
            const result = await (0, desensitization_1.desensitizeImage)(buffer, detections, { method: 'blur', intensity: 50 });
            expect(result.processedImageBuffer).toBeInstanceOf(Buffer);
        });
    });
    describe('applyMultiStageDesensitization', () => {
        it('should apply different methods to different regions', async () => {
            ...
            const result = await (0, desensitization_1.applyMultiStageDesensitization)(buffer, regions);
            expect(result.processedImageBuffer).toBeInstanceOf(Buffer);
            expect(result.appliedRegions).toHaveLength(2);
    ...
    describe('previewDesensitization', () => {
        it('should generate a preview with reduced intensity', async () => {
            const buffer = await createTestImage();
            const preview = await (0, desensitization_1.previewDesensitization)(buffer, { x: 50, y: 50, width: 80, height: 80 }, 'blur', 70);
    ...
    describe('getRecommendedMethod', () => {
        it('should recommend blur for faces', () => {
            expect((0, desensitization_1.getRecommendedMethod)('face')).toBe('blur');
        });
        it('should recommend mosaic for license plates', () => {
            expect((0, desensitization_1.getRecommendedMethod)('license_plate')).toBe('mosaic');
        });
        it('should recommend pixelate for text', () => {
            expect((0, desensitization_1.getRecommendedMethod)('text')).toBe('pixelate');
        });
        it('should recommend pixelate for addresses', () => {
            expect((0, desensitization_1.getRecommendedMethod)('address')).toBe('pixelate');
        });
        it('should recommend replace_background for sensitive objects', () => {
            expect((0, desensitization_1.getRecommendedMethod)('sensitive_object')).toBe('replace_background');
        });
        it('should default to blur for unknown types', () => {
            expect((0, desensitization_1.getRecommendedMethod)('unknown')).toBe('blur');
        });
    });
    describe('calculateDefaultIntensity', () => {
        it('should return appropriate intensity for faces', () => {
            // base=80, adjustment=(0.9-0.5)*20=8 => 88
            expect((0, desensitization_1.calculateDefaultIntensity)('face', 0.9)).toBe(88);
        });
        it('should return higher intensity for sensitive objects', () => {
            // base=95, adjustment=8 => 103, clamped to 100
            expect((0, desensitization_1.calculateDefaultIntensity)('sensitive_object', 0.9)).toBe(100);
        });
        it('should adjust intensity based on confidence', () => {
            const lowConf = (0, desensitization_1.calculateDefaultIntensity)('face', 0.5);
            const highConf = (0, desensitization_1.calculateDefaultIntensity)('face', 0.95);
            expect(highConf).toBeGreaterThan(lowConf);
        });
        it('should clamp to 0-100 range', () => {
            const result = (0, desensitization_1.calculateDefaultIntensity)('face', 0.1);
            expect(result).toBeGreaterThanOrEqual(0);
            expect(result).toBeLessThanOrEqual(100);
        });
    });
});
//# sourceMappingURL=desensitization.test.js.map