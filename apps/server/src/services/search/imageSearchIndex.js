...
export class ImageSearchIndexService {
    ...
    async search(query) {
        ...
        // Calculate scores
        let results = candidates.map(index => ({
            index,
    ...
    applyFilters(indexes, filters) {
        return indexes.filter(index => {
            ...
            if (filters.location && index.metadata.location) {
                const distance = this.calculateDistance(index.metadata.location, { lat: filters.location.lat, lng: filters.location.lng });
            ...
            // Quality filter
            if (filters.quality) {
                if (index.metadata.quality < filters.quality.min ||
     ...
     */
    calculateTagScore(indexTags, queryTags) {
        let matches = 0;
     ...
     */
    calculateCategoryScore(indexCategories, queryCategories) {
        const matches = queryCategories.filter(qc => indexCategories.some(ic => ic.toLowerCase() === qc.toLowerCase()));
    ...
    sortResults(results, sortBy) {
        switch (sortBy) {
            ...
            case 'date':
                return results.sort((a, b) => b.index.metadata.createdAt.getTime() - a.index.metadata.createdAt.getTime());
            case 'quality':
...
export function getImageSearchIndexService() {
    if (!serviceInstance) {
        serviceInstance = new ImageSearchIndexService();
    }
    return serviceInstance;
}
//# sourceMappingURL=imageSearchIndex.js.map