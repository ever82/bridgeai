...
class TransactionService {
  ...
  async getTransactionStats(userId: string): Promise<TransactionStats> {
    ...
    const [totalCount, incomeAgg, expenseAgg, refundAgg, monthIncomeAgg, monthExpenseAgg] =
      await Promise.all([
  ...
  async exportTransactions(
    ...
    const where: Record<string, unknown> = { userId, status: 'SUCCESS' };
    if (options.type && options.type !== 'all') {
      where.type = options.type.toUpperCase();
    }
    if (options.startDate || options.endDate) {
   ...
   */
  async getUserRefunds(userId: string, options: RefundListOptions = {}): Promise<RefundListResult> {
    const page = options.page || 1;
    const limit = options.limit || 20;
    const skip = (page - 1) * limit;
   ...
   */
  async getRefundDetail(userId: string, refundId: string) {
