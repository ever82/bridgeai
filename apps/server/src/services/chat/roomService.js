...
export async function createRoom(input) {
    ...
    const room = await prisma.chatRoom.create({
        data: {
            ...
            matchId,
            participantIds,
            metadata: { ...metadata, settings: settings || getDefaultSettings(type) },
 ...
 * 创建参与者记录
 */
async function createParticipants(roomId, participantIds, createdBy) {
 ...
 * 查找私聊房间（检查是否已存在）
 */
async function findPrivateRoom(participantIds) {
    ...
    return (rooms.find(room => {
        ...
        return (roomIds.length === sortedIds.length && roomIds.every((id, index) => id === sortedIds[index]));
    }) || null);
}
/**
 * 获取默认房间设置
 ...
 * 获取用户的房间列表
 */
export async function getUserRooms(userId, options = {}) {
    const { type, status = ChatRoomStatus.ACTIVE, search, page = 1, limit = 20, sortBy = 'lastMessageAt', sortOrder = 'desc', } = options;
    const where = {
        participants: {
            some: {
                userId,
    ...
    return { rooms: roomsWithUnread, total };
}
/**
 * 搜索房间
 */
export async function searchRooms(userId, query, options = {}) {
    return getUserRooms(userId, { ...options, search: query });
}
/**
 * 检查用户是否在房间中
 */
export async function isUserInRoom(roomId, userId) {
 ...
 * 重置用户未读数
 */
export async function resetUnreadCount(roomId, userId) {
