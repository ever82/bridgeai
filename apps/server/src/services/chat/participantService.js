 ...
 * 聊天室参与者管理服务
 */
import { ParticipantRole, ChatRoomType } from '@prisma/client';
 ...
 * 移除参与者
 */
export async function removeParticipant(roomId, userId, removedBy) {
    // 检查移除者是否有权限
 ...
 * 更新参与者信息
 */
export async function updateParticipant(roomId, userId, input, updatedBy) {
    // 检查更新者权限
 ...
 * 获取参与者的房间
 */
export async function getParticipantRooms(userId) {
 ...
 * 获取单个参与者信息
 */
export async function getParticipant(roomId, userId) {
 ...
 * 转移房主权限
 */
export async function transferOwnership(roomId, newOwnerId, currentOwnerId) {
 ...
 * 检查是否有权限移除参与者
 */
function canRemoveParticipant(removerRole, targetRole) {
 ...
 * 检查用户是否有特定权限
 */
export async function hasPermission(roomId, userId, permission) {
    const participant = await prisma.roomParticipant.findUnique({
        where: {
            roomId_userId: {
                roomId,
