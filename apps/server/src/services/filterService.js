...
import { buildCreditFilterCondition, CREDIT_LEVEL_THRESHOLDS, getCreditLevel, } from './creditFilterService';
/**
 * 构建完整的 Agent 过滤查询条件
 * Build complete agent filter query conditions
 */
export function buildAgentFilterWhere(options) {
    const conditions = [];
    // 基础过滤条件
    if (options.userId) {
 ...
 */
export async function filterAgents(options) {
 ...
 */
export async function checkAgentCredit(agentId, sceneId) {
    try {
 ...
 */
export function extractCreditFilterFromDSL(dsl) {
    const options = {};
    let hasCreditFilter = false;
    function traverseCondition(condition) {
        ...
        if (condition.field === 'creditScore' || condition.field === 'user.creditScore.score') {
            ...
            switch (condition.operator) {
                case 'gte':
                case 'gt':
                    options.minCreditScore = condition.value;
                    break;
                case 'lte':
                case 'lt':
                    options.maxCreditScore = condition.value;
                    break;
                case 'eq':
                    options.minCreditScore = condition.value;
                    options.maxCreditScore = condition.value;
        ...
        if (condition.field === 'creditLevel') {
            hasCreditFilter = true;
            options.creditLevel = condition.value;
        }
    }
    // 遍历 DSL 的所有条件
    if (dsl.filter) {
 ...
 */
export function addCreditFilterToDSL(dsl, creditOptions) {
    ...
    const newDSL = {
        ...dsl,
        filter: {
            AND: [dsl.filter || {}, convertPrismaConditionToDSL(creditCondition)],
        },
    };
    return newDSL;
}
 ...
 */
function convertPrismaConditionToDSL(condition) {
    // 简化的转换实现
    // 实际项目中可能需要更复杂的转换逻辑
 ...
 */
export async function getCreditFilterStatistics() {
    try {
        ...
        for (const level of ['excellent', 'good', 'general', 'poor']) {
 ...
 */
export function validateCreditFilterParams(options) {
    const errors = [];
    ...
    if (options.creditLevel) {
        const validLevels = ['excellent', 'good', 'general', 'poor'];
