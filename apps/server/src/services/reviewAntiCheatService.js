 ...
 * @returns 是否已经评价
 */
export async function hasAlreadyReviewedMatch(matchId, reviewerId) {
 ...
 * @returns 评价次数
 */
export async function countRecentReviews(reviewerId, timeWindowHours = 24) {
 ...
 * @returns 评价次数
 */
export async function countReviewsBetweenUsers(reviewerId, revieweeId) {
    return prisma.review.count({
        where: {
            reviewerId,
            revieweeId,
 ...
 * @returns 相似评价数量
 */
export async function findSimilarReviews(content, userId, threshold = 0.8) {
 ...
 */
function calculateContentSimilarity(a, b) {
    if (!a || !b)
        return 0;
    if (a === b)
        return 1;
 ...
 * @returns 是否是新账户
 */
export async function isNewAccount(userId, minAgeDays = 1) {
 ...
 * @returns 反作弊结果
 */
export async function performAntiCheatCheck(matchId, reviewerId, revieweeId, content) {
    const reasons = [];
    let riskScore = 0;
    // 1. 检查是否已经评价过同一匹配
 ...
 * - 评价者账户年龄极低
 */
export async function autoFlagSuspiciousReviews() {
 ...
 * 批量检查内容相似度
 */
function checkContentSimilarityBatch(contents) {
    ...
    for (let i = 0; i < contents.length; i++) {
        for (let j = i + 1; j < contents.length; j++) {
            if (calculateContentSimilarity(contents[i], contents[j]) >
...
//# sourceMappingURL=reviewAntiCheatService.js.map