 ...
 */
import { ViolationAction, NotificationType, PriorityLevel, UserStatus, } from '@prisma/client';
 ...
 */
function determineAction(severity, activeCount) {
 ...
 */
function calculateCreditDeduction(severity) {
...
export class ViolationHandler {
     ...
     */
    async recordViolation(userId, data, action) {
     ...
     */
    async applyAction(userId, action, moderatorId) {
     ...
     */
    async sendWarning(userId, violation, reason) {
     ...
     */
    async muteUser(userId, durationHours) {
     ...
     */
    async banUser(userId, isPermanent = false, until) {
     ...
     */
    async checkActiveViolationCount(userId) {
     ...
     */
    async checkActiveViolations(userId) {
        const thirtyDaysAgo = new Date();
        thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
     ...
     */
    async linkToCreditScore(userId, severity, violationId) {
        ...
        try {
            ...
            await sendCreditScoreChangeNotification(userId, previousScore, newScore, `违规处罚：严重程度 ${severity}/10`);
        }
        catch {
     ...
     */
    async handleViolation(userId, data, options = {}) {
        try {
     ...
     */
    async liftAction(userId, moderatorId) {
        try {
...
//# sourceMappingURL=violationHandler.js.map