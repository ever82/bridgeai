 ...
 */
async function loadPreferencesFromDb(userId) {
 ...
 */
async function learnUserPreferencesFromRatings(userId) {
    ...
    const ratings_scores = [];
    let totalSpent = 0;
    let hireCount = 0;
    for (const rating of ratings) {
        // Determine which agent was rated in this match
        const matchedAgent = rating.rateeId === rating.match?.demand?.agent?.userId
            ? rating.match?.supply?.agent
            : rating.match?.demand?.agent;
        if (!matchedAgent)
            continue;
        const agentConfig = matchedAgent.config;
        const skills = agentConfig?.skills || [];
        const category = agentConfig?.category;
        const hourlyRate = agentConfig?.hourlyRate || 50;
        // Count skills
    ...
    const avgRating = ratings_scores.length > 0
...
async function findSimilarUsers(userId) {
    ...
    for (const { raterId } of allRatings) {
        ...
        const skillIntersection = targetPrefs.preferredSkills.filter(s => prefs.preferredSkills.includes(s)).length;
        ...
        const categoryIntersection = targetPrefs.preferredCategories.filter(c => prefs.preferredCategories.includes(c)).length;
 ...
 */
async function computeUserPreferencesWithoutCache(userId) {
    const ratings = await prisma.rating.findMany({
        where: { raterId: userId },
        include: {
            match: {
...
async function getCollaborativeRecommendations(userId) {
    ...
    const recommendedAgentIds = new Set();
 ...
 */
async function hasUserInteractedWithAgent(userId, agentId) {
 ...
 */
async function calculateRecommendationScore(agent, preferences, userId) {
    let score = calculateCompositeScore(agent);
    const reasons = [];
    // Skill match bonus
    const skillMatches = agent.skills?.filter(s => preferences.preferredSkills.includes(s)).length || 0;
    if (skillMatches > 0) {
 ...
 */
async function getCachedOrLoadPreferences(userId) {
 ...
 */
export async function getRecommendationsForUser(userId, agents) {
 ...
 */
export async function recordInteraction(userId, agentId, type, rating) {
 ...
 */
export async function getRecommendationExplanation(userId, agent) {
    const preferences = await getCachedOrLoadPreferences(userId);
 ...
 */
export function recordRecommendationFeedback(userId, agentId, helpful) {
    const feedback = {
        userId,
        agentId,
        helpful,
