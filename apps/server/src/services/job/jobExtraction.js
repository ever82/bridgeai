 ...
 */
export async function extractJobFromDescription(description, options = {}) {
 ...
 */
export async function evaluateJobQuality(jobData) {
    const strengths = [];
    const improvements = [];
    const missingFields = [];
    let score = 50;
 ...
 */
export async function analyzeCompetitiveness(jobData, similarJobs) {
 ...
 */
export async function generateJobDescription(structuredData, tone = 'professional') {
...
function extractExperienceInfo(description) {
    const patterns = [
...
function extractPreferredQualifications(description) {
    const patterns = [
        /优先[：:]([^。！\n]+)/g,
        /加分[：:]([^。！\n]+)/g,
        /有([^。！\n]+)经验优先/g,
...
function generateSuggestions(description, skills) {
    const suggestions = [];
...
//# sourceMappingURL=jobExtraction.js.map