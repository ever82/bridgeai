...
export class ResumeScreeningService {
   ...
   */
  async screen(request: ResumeScreeningRequest): Promise<ResumeScreeningResult> {
   ...
   */
  async screenBatch(request: BatchScreeningRequest): Promise<BatchScreeningResult> {
    const startTime = Date.now();
  ...
  private parseScreeningResult(
    text: string
  ): Omit<ResumeScreeningResult, 'provider' | 'model' | 'latencyMs'> {
    try {
