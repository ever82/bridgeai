 ...
 * 协商房间服务
 */
import { NegotiationStatus, MessageSender, createNegotiationRoom, createNegotiationMessage, calculateNegotiationProgress, shouldHandoffToHuman, } from '../../models/NegotiationRoom';
...
export class NegotiationRoomService {
    ...
    async createRoom(request) {
        ...
        const systemMessage = createNegotiationMessage(room.id, MessageSender.SYSTEM, 'system', 'Negotiation room created. Waiting for both parties to join.', 0, generateMessageId);
    ...
    async startNegotiation(roomId) {
        ...
        if (updatedRoom) {
            ...
            // Add system message
            const systemMessage = createNegotiationMessage(roomId, MessageSender.SYSTEM, 'system', `Round 1 has started. Negotiation topics: ${updatedRoom.topics.join(', ')}`, 1, generateMessageId);
    ...
    async sendMessage(request) {
        ...
        const message = createNegotiationMessage(request.roomId, request.sender, request.senderId, request.content, room.currentRound, generateMessageId, {
            topic: request.topic,
    ...
    async advanceRound(roomId) {
        ...
        // Add system message
        const systemMessage = createNegotiationMessage(roomId, MessageSender.SYSTEM, 'system', `Round ${newRoundNumber} has started.`, newRoundNumber, generateMessageId);
        await this.addMessage(roomId, systemMessage);
        return newRound;
    }
    /**
     ...
     */
    async handoffToHuman(roomId, reason) {
        ...
        if (room) {
            // Add system message
            const systemMessage = createNegotiationMessage(roomId, MessageSender.SYSTEM, 'system', `Negotiation handed off to human. Reason: ${reason}`, room.currentRound, generateMessageId);
            await this.addMessage(roomId, systemMessage);
        }
        return room;
    }
     ...
     */
    async markAsReached(roomId, agreedAmount, agreedBenefits) {
        const room = await this.updateRoom(roomId, {
        ...
        if (room) {
            // Add system message
            const systemMessage = createNegotiationMessage(roomId, MessageSender.SYSTEM, 'system', `Negotiation reached agreement! Agreed amount: ${agreedAmount} ${room.currency}`, room.currentRound, generateMessageId);
            await this.addMessage(roomId, systemMessage);
        }
        return room;
    }
    ...
    async markAsFailed(roomId) {
        ...
        if (room) {
            // Add system message
            const systemMessage = createNegotiationMessage(roomId, MessageSender.SYSTEM, 'system', 'Negotiation failed. No agreement reached.', room.currentRound, generateMessageId);
            await this.addMessage(roomId, systemMessage);
        }
        return room;
    }
     ...
     */
    async cancelRoom(roomId, reason) {
        const room = await this.updateRoom(roomId, {
            status: NegotiationStatus.CANCELLED,
            completedAt: new Date(),
        });
        if (room) {
            const systemMessage = createNegotiationMessage(roomId, MessageSender.SYSTEM, 'system', `Negotiation cancelled. ${reason || ''}`, room.currentRound, generateMessageId);
            await this.addMessage(roomId, systemMessage);
        }
        return room;
    }
