...
const mockScreeningResponse = {
  ...
  dimensions: {
    ...
    implicitSkillsInferred: { score: 75, skills: ['Node.js', 'System Design'], details: 'Inferred from experience' },
...
describe('ResumeScreeningService', () => {
  ...
  describe('screenBatch', () => {
    it('should screen multiple resumes in batch', async () => {
      const batchResponse = [
        { resumeId: 'r1', screeningScore: 85, recommendation: 'GO', matchedSkills: ['TypeScript'], missingSkills: [], concerns: [], screeningNotes: 'Good' },
        { resumeId: 'r2', screeningScore: 60, recommendation: 'HOLD', matchedSkills: [], missingSkills: ['React'], concerns: ['Limited experience'], screeningNotes: 'Average' },
      ];
    ...
    it('should rank resumes by score (individual screening for small batches)', async () => {
      (llmService.generateText as jest.Mock)
        .mockResolvedValueOnce({
          text: JSON.stringify({ ...mockScreeningResponse, screeningScore: 90, recommendation: 'STRONG_GO' }),
          provider: 'openai',
          model: 'gpt-4',
          latencyMs: 500,
        })
        .mockResolvedValueOnce({
          text: JSON.stringify({ ...mockScreeningResponse, screeningScore: 65, recommendation: 'HOLD' }),
          provider: 'openai',
          model: 'gpt-4',
          latencyMs: 500,
        });
  ...
  describe('explainRecommendation', () => {
    it('should generate recommendation explanation', async () => {
      const explanationResponse = {
        ...
        recommendedNextSteps: ['Schedule technical interview', 'Ask about system design experience'],
      };
      ...
      const result = await service.explainRecommendation(
        { name: 'John Doe', skills: ['TypeScript', 'React'], experienceYears: 5, title: 'Frontend Developer' },
        { title: 'Senior Frontend Engineer', requiredSkills: ['TypeScript', 'React'], companyName: 'TechCorp' },
        82,
      );
