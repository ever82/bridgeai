...
const interviewScheduling_1 = require("../interviewScheduling");
describe('InterviewSchedulingService', () => {
    let service;
    beforeEach(() => {
        service = new interviewScheduling_1.InterviewSchedulingService();
        service.clearAll();
    });
    describe('createInterview', () => {
        it('should create a new interview', async () => {
            const request = {
                ...
                positionId: 'pos_123',
                round: Interview_1.InterviewRound.TECHNICAL,
                roundNumber: 1,
                type: Interview_1.InterviewType.VIDEO,
            ...
            expect(interview.status).toBe(Interview_1.InterviewStatus.PENDING);
        ...
        it('should create interview with default values', async () => {
            const request = {
                ...
                employerId: 'employer_123',
                positionId: 'pos_123',
                round: Interview_1.InterviewRound.SCREENING,
                type: Interview_1.InterviewType.PHONE
            };
            const interview = await service.createInterview(request);
    ...
    describe('getInterviews', () => {
        ...
        it('should filter interviews by status', async () => {
            ...
            const interviews = await service.getInterviews({ status: Interview_1.InterviewStatus.PENDING });
            expect(interviews.length).toBeGreaterThan(0);
            expect(interviews.every(i => i.status === Interview_1.InterviewStatus.PENDING)).toBe(true);
        });
    });
    describe('proposeTimeSlots', () => {
        it('should propose time slots for an interview', async () => {
            ...
            expect(updatedInterview?.status).toBe(Interview_1.InterviewStatus.PROPOSED);
        });
    ...
    describe('scheduleInterview', () => {
        it('should schedule an interview with a time slot', async () => {
            ...
            expect(scheduled.status).toBe(Interview_1.InterviewStatus.CONFIRMED);
    ...
    describe('rescheduleInterview', () => {
        it('should reschedule an interview', async () => {
            ...
            const rescheduled = await service.rescheduleInterview(interview.id, slot2.id, 'Conflict with another meeting');
            expect(rescheduled.status).toBe(Interview_1.InterviewStatus.RESCHEDULED);
    ...
    describe('cancelInterview', () => {
        it('should cancel an interview', async () => {
            ...
            expect(cancelled?.status).toBe(Interview_1.InterviewStatus.CANCELLED);
...
describe('Interview Model Helpers', () => {
    describe('createInterview', () => {
        it('should create an interview with correct initial state', () => {
            ...
            const interview = (0, Interview_1.createInterview)(input, () => 'test_id');
    ...
    describe('createTimeSlot', () => {
        it('should create a time slot with correct duration', () => {
            ...
            const slot = (0, Interview_1.createTimeSlot)(startTime, endTime, 'UTC', () => 'slot_id');
    ...
    describe('isInterviewUpcoming', () => {
        it('should return true for confirmed future interview', () => {
            ...
            tomorrow.setDate(tomorrow.getDate() + 1);
            const interview = {
                status: Interview_1.InterviewStatus.CONFIRMED,
                schedule: {
                    scheduledAt: tomorrow
                }
            };
            expect((0, Interview_1.isInterviewUpcoming)(interview)).toBe(true);
        });
        it('should return false for past interview', () => {
            ...
            expect((0, Interview_1.isInterviewUpcoming)(interview)).toBe(false);
        });
    });
    describe('canReschedule', () => {
        it('should allow rescheduling pending interviews', () => {
            const interview = { status: Interview_1.InterviewStatus.PENDING };
            expect((0, Interview_1.canReschedule)(interview)).toBe(true);
        });
        it('should allow rescheduling confirmed interviews', () => {
            const interview = { status: Interview_1.InterviewStatus.CONFIRMED };
            expect((0, Interview_1.canReschedule)(interview)).toBe(true);
        });
        it('should not allow rescheduling cancelled interviews', () => {
            const interview = { status: Interview_1.InterviewStatus.CANCELLED };
            expect((0, Interview_1.canReschedule)(interview)).toBe(false);
        });
    });
    describe('getNextRound', () => {
        it('should return next round after screening', () => {
            const nextRound = (0, Interview_1.getNextRound)(Interview_1.InterviewRound.SCREENING);
            expect(nextRound).toBe(Interview_1.InterviewRound.TECHNICAL);
        });
        it('should return null for final round', () => {
            const nextRound = (0, Interview_1.getNextRound)(Interview_1.InterviewRound.OFFER_DISCUSSION);
            expect(nextRound).toBeNull();
        });
    });
    describe('aggregateFeedback', () => {
        it('should return null for interview with no feedback', () => {
            const interview = { id: 'test', feedbacks: [] };
            expect((0, Interview_1.aggregateFeedback)(interview)).toBeNull();
        });
        it('should aggregate multiple feedbacks', () => {
            const interview = {
                id: 'test',
                feedbacks: [
                    { rating: 4, recommendation: 'hire', strengths: ['Good'], weaknesses: ['None'] },
                    { rating: 5, recommendation: 'strong_hire', strengths: ['Excellent'], weaknesses: ['Minor'] }
                ]
            };
            const result = (0, Interview_1.aggregateFeedback)(interview);
...
//# sourceMappingURL=interviewScheduling.test.js.map