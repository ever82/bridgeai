...
const negotiationResult_1 = require("../negotiationResult");
describe('NegotiationResultService', () => {
    let roomService;
    let resultService;
    beforeEach(() => {
        roomService = new negotiationRoom_1.NegotiationRoomService();
        resultService = new negotiationResult_1.NegotiationResultService();
    ...
    describe('confirmAgreement', () => {
        it('should record agreement confirmation from a party', async () => {
            ...
            const confirmation = await resultService.confirmAgreement(room.id, 'jobseeker', ['Remote work option'], 'Happy with the offer');
    ...
    describe('finalizeAgreement', () => {
        it('should finalize negotiation with agreed terms', async () => {
            const room = await createAndStartRoom();
            const result = await resultService.finalizeAgreement(room.id, 55000, ['Health insurance', '401k matching', 'Remote work']);
    ...
    describe('recordRejection', () => {
        it('should record rejection with reason', async () => {
            const room = await createAndStartRoom();
            const rejection = await resultService.recordRejection(room.id, 'jobseeker', 'Salary too low for my experience', true, 65000);
        ...
        it('should mark room as failed when rejection is recorded', async () => {
            ...
            expect(updatedRoom?.status).toBe(NegotiationRoom_1.NegotiationStatus.FAILED);
        });
    });
    describe('getResult', () => {
        ...
        it('should return result for cancelled negotiation', async () => {
            ...
            const result = await resultService.getResult(room.id);
            expect(result).toBeDefined();
            expect(result?.status).toBe(NegotiationRoom_1.NegotiationStatus.CANCELLED);
        });
    });
    ...
    describe('cancelNegotiation', () => {
        it('should cancel the negotiation', async () => {
            ...
            expect(cancelled?.status).toBe(NegotiationRoom_1.NegotiationStatus.CANCELLED);
        });
    });
});
//# sourceMappingURL=negotiationResult.test.js.map