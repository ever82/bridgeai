...
const NegotiationRoom_1 = require("../../../models/NegotiationRoom");
const negotiationRoom_1 = require("../negotiationRoom");
describe('NegotiationRoomService', () => {
    let service;
    beforeEach(() => {
        service = new negotiationRoom_1.NegotiationRoomService();
        service.clearAll();
    });
    describe('createRoom', () => {
        it('should create a new negotiation room', async () => {
            const request = {
                ...
                topics: [NegotiationRoom_1.NegotiationTopic.SALARY, NegotiationRoom_1.NegotiationTopic.BENEFITS],
                maxRounds: 5,
                initialOffer: 50000,
            ...
            expect(room.status).toBe(NegotiationRoom_1.NegotiationStatus.PENDING);
        ...
        it('should create room with default values', async () => {
            ...
            const room = await service.createRoom(request);
            expect(room.topics).toEqual([NegotiationRoom_1.NegotiationTopic.SALARY]);
    ...
    describe('getRooms', () => {
        ...
        it('should filter rooms by status', async () => {
            ...
            const rooms = await service.getRooms({ status: NegotiationRoom_1.NegotiationStatus.PENDING });
            expect(rooms.length).toBeGreaterThan(0);
            expect(rooms.every(r => r.status === NegotiationRoom_1.NegotiationStatus.PENDING)).toBe(true);
        });
    });
    describe('startNegotiation', () => {
        it('should start negotiation and change status to active', async () => {
            ...
            expect(started?.status).toBe(NegotiationRoom_1.NegotiationStatus.ACTIVE);
    ...
    describe('sendMessage', () => {
        it('should send a message in the room', async () => {
            ...
            const message = await service.sendMessage({
                roomId: room.id,
                sender: NegotiationRoom_1.MessageSender.JOBSEEKER_AGENT,
                senderId: 'agent_seeker_123',
                content: 'Hello, I would like to discuss the salary.',
                topic: NegotiationRoom_1.NegotiationTopic.SALARY
            });
            expect(message).toBeDefined();
            expect(message.content).toBe('Hello, I would like to discuss the salary.');
            expect(message.sender).toBe(NegotiationRoom_1.MessageSender.JOBSEEKER_AGENT);
        ...
        it('should update room status to negotiating on first message', async () => {
            ...
            expect(updatedRoom?.status).toBe(NegotiationRoom_1.NegotiationStatus.NEGOTIATING);
        });
    });
    describe('advanceRound', () => {
        ...
        it('should return null when max rounds reached', async () => {
            ...
            const updatedRoom = await service.getRoom(room.id);
            expect(updatedRoom?.status).toBe(NegotiationRoom_1.NegotiationStatus.HANDOFF_TO_HUMAN);
        });
    });
    describe('markAsReached', () => {
        it('should mark negotiation as reached with agreement', async () => {
            ...
            expect(result?.status).toBe(NegotiationRoom_1.NegotiationStatus.REACHED);
...
describe('NegotiationRoom Model Helpers', () => {
    describe('createNegotiationRoom', () => {
        it('should create a room with correct initial state', () => {
            ...
            const room = (0, NegotiationRoom_1.createNegotiationRoom)(input, () => 'test_id');
    ...
    describe('createNegotiationMessage', () => {
        it('should create a message with correct properties', () => {
            const message = (0, NegotiationRoom_1.createNegotiationMessage)('room_123', NegotiationRoom_1.MessageSender.JOBSEEKER_AGENT, 'agent_123', 'Test message', 1, () => 'msg_id');
    ...
    describe('calculateNegotiationProgress', () => {
        it('should calculate correct progress', () => {
            const room = {
                ...
                maxRounds: 5,
                updatedAt: new Date(),
                status: NegotiationRoom_1.NegotiationStatus.NEGOTIATING
            };
            const progress = (0, NegotiationRoom_1.calculateNegotiationProgress)(room);
    ...
    describe('shouldHandoffToHuman', () => {
        it('should trigger handoff when max rounds reached', () => {
            ...
            const result = (0, NegotiationRoom_1.shouldHandoffToHuman)(room);
...
//# sourceMappingURL=negotiationRoom.test.js.map