 ...
 * 面试安排服务
 */
import { InterviewStatus, createInterview, createTimeSlot, createInterviewSchedule, isInterviewUpcoming, getInterviewTimeRemaining, canReschedule, shouldSendReminder, getNextRound, aggregateFeedback, } from '../../models/Interview';
...
export class InterviewSchedulingService {
     ...
     * Update interview
     */
    async updateInterview(interviewId, update) {
     ...
     */
    async proposeTimeSlots(request) {
        ...
        for (const slotData of request.slots) {
            const slot = createTimeSlot(slotData.startTime, slotData.endTime, slotData.timezone, generateId, slotData.proposedBy);
            slots.push(slot);
        }
    ...
    async scheduleInterview(request) {
        ...
        // Create schedule
        const schedule = createInterviewSchedule(interview.id, slot, interview.type, interview.round, generateId, {
     ...
     */
    async rescheduleInterview(interviewId, newSlotId, reason) {
        const interview = interviews.get(interviewId);
        if (!interview) {
            throw new Error('Interview not found');
        }
     ...
     */
    async aggregateFeedback(interviewId) {
        const interview = interviews.get(interviewId);
        if (!interview)
            return null;
    ...
    async sendReminder(interviewId) {
        const interview = interviews.get(interviewId);
        if (!interview || !interview.schedule)
            return false;
     ...
     */
    async getUpcomingInterviewsNeedingReminders() {
     ...
     */
    async getNextRound(interviewId) {
        const interview = interviews.get(interviewId);
        if (!interview)
            return null;
     ...
     */
    async getInterviewSeries(jobApplicationId) {
        const applicationInterviews = await this.getInterviews({ jobApplicationId });
        const totalRounds = applicationInterviews.length;
        const completedRounds = applicationInterviews.filter(i => i.status === InterviewStatus.COMPLETED || i.status === InterviewStatus.CLOSED).length;
        const cancelledCount = applicationInterviews.filter(i => i.status === InterviewStatus.CANCELLED).length;
     ...
     */
    async getAvailableSlots(interviewId) {
...
//# sourceMappingURL=interviewScheduling.js.map