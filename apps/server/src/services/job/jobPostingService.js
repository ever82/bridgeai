...
import { JobStatus, ApplicationStatus, createJobPostingSchema, updateJobPostingSchema, } from '@bridgeai/shared';
 ...
 */
export async function createJobPosting(options) {
 ...
 */
export async function getJobPosting(jobId, options = {}) {
 ...
 */
export async function updateJobPosting(jobId, employerId, data) {
    const job = jobPostings.get(jobId);
    if (!job) {
        throw new AppError('Job not found', 'JOB_NOT_FOUND', 404);
    }
    ...
    if (validated.department)
        job.department = validated.department;
    if (validated.type)
        job.type = validated.type;
    if (validated.positions)
        job.positions = validated.positions;
    ...
    if (validated.benefits)
        job.benefits = { ...job.benefits, ...validated.benefits };
    if (validated.location)
        job.location = validated.location;
    if (validated.validUntil)
        job.validUntil = validated.validUntil;
    if (validated.isUrgent !== undefined)
        job.isUrgent = validated.isUrgent;
    job.updatedAt = now;
    // TODO: Update in database
 ...
 * Update job status
 */
export async function updateJobStatus(jobId, employerId, data) {
    const job = jobPostings.get(jobId);
    if (!job) {
        throw new AppError('Job not found', 'JOB_NOT_FOUND', 404);
    }
    ...
    if (!validTransitions[job.status].includes(data.status)) {
        throw new AppError(`Invalid status transition from ${job.status} to ${data.status}`, 'INVALID_STATUS_TRANSITION', 400);
    }
    const now = new Date().toISOString();
 ...
 */
export async function refreshJobPosting(jobId, employerId, data = {}) {
    const job = jobPostings.get(jobId);
    if (!job) {
        throw new AppError('Job not found', 'JOB_NOT_FOUND', 404);
    }
 ...
 */
export async function deleteJobPosting(jobId, employerId) {
    await updateJobStatus(jobId, employerId, { status: JobStatus.CLOSED });
}
/**
 * List job postings with filtering
 */
export async function listJobPostings(filter = {}) {
    ...
    if (filter.skills && filter.skills.length > 0) {
        jobs = jobs.filter(j => filter.skills.some(skill => j.requirements.skills.includes(skill) ||
 ...
 */
export async function getEmployerJobs(employerId, filter = {}) {
 ...
 * Get job statistics
 */
export async function getJobStats(jobId, employerId) {
 ...
 */
export async function createJobApplication(jobId, applicantId, applicantAgentId, coverLetter, resumeUrl, answers) {
    const job = await getJobPosting(jobId);
    if (job.status !== JobStatus.PUBLISHED) {
 ...
 */
export async function getJobApplications(jobId, employerId, filter = {}) {
    const job = await getJobPosting(jobId);
    if (job.employerId !== employerId) {
        throw new AppError('Unauthorized', 'UNAUTHORIZED', 403);
    }
 ...
 */
export async function updateApplicationStatus(applicationId, employerId, status, notes) {
 ...
 */
export async function getEmployerJobStats(employerId) {
 ...
 */
export async function evaluateJob(jobId, employerId) {
    const job = await getJobPosting(jobId);
    if (job.employerId !== employerId) {
        throw new AppError('Unauthorized', 'UNAUTHORIZED', 403);
    }
...
//# sourceMappingURL=jobPostingService.js.map