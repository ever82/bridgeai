...
export async function createMerchant(input) {
    ...
    if (agent.type !== AgentType.AGENTAD) {
        throw new AppError('Merchant can only be created for AGENTAD type agents', 'INVALID_AGENT_TYPE', 400);
    }
    ...
    if (existingMerchant) {
        throw new AppError('Merchant already exists for this agent', 'MERCHANT_EXISTS', 409);
    }
 ...
 */
export async function getMerchantByAgentId(agentId) {
    const merchant = await prisma.merchant.findUnique({
        where: { agentId },
    });
    if (!merchant) {
 ...
 * Update merchant
 */
export async function updateMerchant(id, input) {
    const existingMerchant = await prisma.merchant.findUnique({
        where: { id },
    });
    if (!existingMerchant) {
...
export async function deleteMerchant(id) {
    ...
    if (activeOffersCount > 0) {
        throw new AppError('Cannot delete merchant with active offers', 'ACTIVE_OFFERS_EXIST', 400);
    }
...
export async function listMerchants(options) {
    const { status, category, page = 1, limit = 20, sortBy = 'createdAt', sortOrder = 'desc', } = options;
    const where = {};
    if (status) {
        where.status = status;
    }
 ...
 */
function mapPrismaMerchantToMerchant(prismaMerchant) {
    return {
...
export async function getMerchantStats(merchantId) {
    ...
    return {
        ...
        totalUsageCount: offers.reduce((sum, o) => sum + (o.usageCount || 0), 0),
        lowStockOffers: offers.filter((o) => o.remainingStock <= o.stockAlertThreshold && o.remainingStock > 0).length,
    };
}
