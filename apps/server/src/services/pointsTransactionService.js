...
export class PointsTransactionService {
    prisma;
    ruleEngine;
    constructor(prismaClient = prisma, ruleEngine = pointsRuleEngine) {
     ...
     * 使用乐观锁防止并发问题
     */
    async earnPoints(userId, amount, options = {}) {
            ...
            try {
                const result = await this.prisma.$transaction(async (tx) => {
                    ...
                    if (availableBalance < amount) {
                        throw new AppError(`Insufficient points. Available: ${availableBalance}, Required: ${amount}`, 'INSUFFICIENT_POINTS', 400);
                    }
                    // 计算新余额
                    const newBalance = account.balance - amount;
        ...
        for (let attempt = 0; attempt < maxRetries; attempt++) {
            try {
                const result = await this.prisma.$transaction(async (tx) => {
                    ...
                    // 验证冻结参数
                    const validation = this.ruleEngine.validateFreeze(account.balance, account.frozenAmount, amount);
                    if (!validation.valid) {
     ...
     * 解冻积分（释放冻结）
     */
    async unfreezePoints(userId, freezeId, options = {}) {
        const maxRetries = 3;
        let lastError = null;
        for (let attempt = 0; attempt < maxRetries; attempt++) {
            try {
                const result = await this.prisma.$transaction(async (tx) => {
                    ...
                    if (freezeRecord.status !== 'FROZEN') {
                        throw new AppError(`Freeze record is already ${freezeRecord.status}`, 'FREEZE_ALREADY_PROCESSED', 400);
                    }
     ...
     * 确认使用冻结积分（完成交易）
     */
    async confirmFrozenPoints(userId, freezeId, options = {}) {
        const maxRetries = 3;
        let lastError = null;
        for (let attempt = 0; attempt < maxRetries; attempt++) {
            try {
     ...
     * 积分转账（用户间）
     */
    async transferPoints(fromUserId, toUserId, amount, options = {}) {
            ...
            try {
                const result = await this.prisma.$transaction(async (tx) => {
                    ...
                    if (availableBalance < totalDeduction) {
                        throw new AppError(`Insufficient points. Available: ${availableBalance}, Required: ${totalDeduction} (including ${fee} fee)`, 'INSUFFICIENT_POINTS', 400);
                    }
                    // 获取或创建转入方账户
     ...
     * 批量处理积分操作（用于批量奖励）
     */
    async batchEarnPoints(operations) {
        const transactions = [];
        let totalEarned = 0;
        const totalSpent = 0;
        // 串行处理避免并发冲突
...
//# sourceMappingURL=pointsTransactionService.js.map