...
const IMAGE_SIGNATURES: { signature: Buffer; format: string; mime: string }[] = [
...
export class ImageUploadSecurityService {
  private static instance: ImageUploadSecurityService;
  ...
  async secureImage(
    ...
    declaredMime: string,
    filename: string,
    config: Partial<UploadSecurityConfig> = {}
    ...
    if (buffer.length > cfg.maxFileSize) {
      violations.push(
        `File size ${(buffer.length / 1024 / 1024).toFixed(1)}MB exceeds limit ${(cfg.maxFileSize / 1024 / 1024).toFixed(1)}MB`
      );
      return this.failResult(violations, buffer.length);
    }
    ...
    if (declaredMime && detectedMime && declaredMime !== detectedMime) {
      violations.push(
        `MIME type mismatch: declared "${declaredMime}" but content is "${detectedMime}"`
      );
    }
    ...
    if (isSvg) {
      sharpMeta = { format: 'svg', width: undefined, height: undefined } as sharp.Metadata;
    } else {
      try {
    ...
    if (!isSvg) {
      if (width < cfg.minDimensions.width || height < cfg.minDimensions.height) {
        violations.push(
          `Image too small: ${width}x${height} (min ${cfg.minDimensions.width}x${cfg.minDimensions.height})`
        );
      }
      if (width > cfg.maxDimensions.width || height > cfg.maxDimensions.height) {
        violations.push(
          `Image too large: ${width}x${height} (max ${cfg.maxDimensions.width}x${cfg.maxDimensions.height})`
        );
      }
    }
    ...
    if (isSvg || declaredMime === 'image/svg+xml') {
      ...
      if (svgViolation) violations.push(svgViolation);
    }
  ...
  private getExpectedMime(format: string): string | null {
    const sig = IMAGE_SIGNATURES.find(s => s.format === format);
    return sig?.mime ?? null;
  }

  private checkFilename(filename: string): string | null {
    // Null bytes
  ...
  private checkSvgSafety(buffer: Buffer): string | null {
  ...
  private toSharpFormat(format: string): 'jpeg' | 'png' | 'webp' | 'gif' {
    switch (format.toLowerCase()) {
      case 'jpg':
        return 'jpeg';
      case 'gif':
        return 'gif' as 'jpeg' | 'png' | 'webp' | 'gif';
      default:
        return (format.toLowerCase() || 'jpeg') as 'jpeg' | 'png' | 'webp' | 'gif';
    }
  }

  private failResult(violations: string[], originalSize: number): UploadSecurityResult {
    return {
      passed: false,
      violations,
      warnings: [],
