...
export type VirusThreatCategory =
  | 'eicar_test'
...
export class VirusScanService {
   ...
   */
  scanFile(buffer: Buffer, options: { filenameHint?: string } = {}): VirusScanResult {
   ...
   */
  private detectEicar(buffer: Buffer): VirusThreat | null {
   ...
   */
  private detectExecutableSignatures(buffer: Buffer): VirusThreat[] {
    const threats: VirusThreat[] = [];
   ...
   */
  private detectScriptPatterns(buffer: Buffer): VirusThreat[] {
    const threats: VirusThreat[] = [];
  ...
  private detectPolyglot(
    ...
    existingThreats: VirusThreat[]
  ): string | null {
    // Check if any existing threats are executable signatures
    const hasExecutableSig = existingThreats.some(
      t => t.category === 'executable_signature' || t.category === 'polyglot_file'
    );
    ...
    const imageMagicNumbers: Array<{ ext: string; magic: Buffer }> = [
