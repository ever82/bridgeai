...
export class QuerySubscriptionManager extends EventEmitter {
    ...
    userSubscriptions = new Map(); // userId -> Set<subscriptionId>
    subscriptionTimers = new Map();
    updateProcessorTimer = null;
    updateQueue = [];
    isProcessingUpdates = false;
    static instance;
    // Configuration
    HEARTBEAT_INTERVAL = 30000; // 30 seconds
    SUBSCRIPTION_TIMEOUT = 300000; // 5 minutes
    ...
    static resetInstance() {
        if (QuerySubscriptionManager.instance) {
            ...
            QuerySubscriptionManager.instance = undefined;
        }
    }
    /**
     * Create a new query subscription
     */
    async createSubscription(userId, query, filters) {
    ...
    getUserSubscriptions(userId) {
        ...
        return Array.from(subIds)
            ...
            .filter((sub) => sub !== undefined && sub.active);
    }
    /**
     ...
     */
    notifyDataChange(affectedSubscriptions, updateType, changedData) {
        const affectedIds = changedData?.map((d) => d.id) || [];
     ...
     */
    handleHeartbeat(subscriptionId) {
        const subscription = this.subscriptions.get(subscriptionId);
        if (!subscription) {
            return false;
        }
...
//# sourceMappingURL=querySubscriptionManager.js.map