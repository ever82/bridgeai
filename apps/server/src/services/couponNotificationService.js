...
import { getExpiringCoupons, markReminderSent, getCouponById, } from './promoCodeService';
/**
 ...
 */
export async function sendExpiringCouponReminders() {
    const expiringCoupons = await getExpiringCoupons(24); // 24 hours before expiry
    let sentCount = 0;
    for (const coupon of expiringCoupons) {
        try {
            const hoursUntilExpiry = Math.ceil((coupon.validUntil.getTime() - Date.now()) / (1000 * 60 * 60));
 ...
 */
export async function sendCouponUsedNotification(couponId) {
 ...
 */
export async function sendCouponExpiredNotification(couponId) {
    const coupon = await getCouponById(couponId);
    if (!coupon) {
        throw new Error('Coupon not found');
    }
 ...
 */
export async function sendCouponCreatedNotification(couponId) {
    const coupon = await getCouponById(couponId);
    if (!coupon) {
        throw new Error('Coupon not found');
    }
 ...
 */
export async function sendRatingReminderNotification(couponId) {
    const coupon = await getCouponById(couponId);
    if (!coupon) {
        throw new Error('Coupon not found');
    }
...
//# sourceMappingURL=couponNotificationService.js.map