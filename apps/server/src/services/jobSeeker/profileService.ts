...
export function calculateQualityScore(profile: Partial<JobSeekerProfile>): ResumeQualityReport {
  ...
  if (profile.workExperiences) {
    const withAchievements = profile.workExperiences.filter(
 ...
 * Get profile by ID
 */
export async function getProfile(profileId: string, userId?: string): Promise<JobSeekerProfile> {
 ...
 * Get work timeline
 */
export async function getWorkTimeline(profileId: string, userId: string): Promise<WorkTimeline> {
  const profile = await getProfile(profileId, userId);
