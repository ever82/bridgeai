...
import { getProfile, maskProfileData } from './profileService';
...
function recordDisclosures(deliveryId: string, profile: any): void {
  ...
  if (profile.contactInfo?.wechat) fields.push({ field: 'wechat', value: profile.contactInfo.wechat });
...
export async function listDeliveriesForSeeker(
  ...
  let results = Array.from(deliveries.values()).filter(
    d => d.seekerId === seekerId
  );
...
function calculateStats(deliveries: ResumeDelivery[]): DeliveryStats {
  ...
  for (const d of deliveries) {
    switch (d.status) {
      case DeliveryStatus.PENDING: counts.pending++; break;
      case DeliveryStatus.VIEWED: counts.viewed++; break;
      case DeliveryStatus.SHORTLISTED: counts.shortlisted++; break;
      case DeliveryStatus.REJECTED: counts.rejected++; break;
      case DeliveryStatus.INTERVIEWING: counts.interviewing++; break;
      case DeliveryStatus.OFFERED: counts.offered++; break;
      case DeliveryStatus.HIRED: counts.hired++; break;
      case DeliveryStatus.WITHDRAWN: counts.withdrawn++; break;
    }
  }
