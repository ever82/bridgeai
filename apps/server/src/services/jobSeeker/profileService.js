...
import { ResumeVisibility as Visibility, MaskingType as MT, DEFAULT_MASKING_RULES, } from '@bridgeai/shared';
import { createJobSeekerProfileSchema, updateJobSeekerProfileSchema } from '@bridgeai/shared';
 ...
 */
export function maskProfileData(profile, visibility) {
    ...
    for (const rule of DEFAULT_MASKING_RULES) {
        if (!rule.visibleTo.includes(visibility)) {
            ...
            if (fieldPath.length === 2 && fieldPath[0] === 'contactInfo') {
                const contactField = fieldPath[1];
                if (masked.contactInfo && masked.contactInfo[contactField]) {
                    masked.contactInfo = { ...masked.contactInfo };
                    masked.contactInfo[contactField] = applyMask(masked.contactInfo[contactField], rule.type);
                }
            }
        }
    }
 ...
 */
export function calculateQualityScore(profile) {
    const scores = {
        completeness: 0,
        clarity: 0,
        keyword: 0,
    ...
    if (profile.currentTitle)
        ...
        missingFields.push('currentTitle');
    if (profile.summary)
        completenessFields++;
    else
    ...
    if (profile.workExperiences) {
        ...
        achievementScore =
            (withAchievements.length / Math.max(profile.workExperiences.length, 1)) * 100;
    }
    scores.achievement = achievementScore;
    const overallScore = Math.round(scores.completeness * 0.4 +
        scores.clarity * 0.2 +
        scores.keyword * 0.2 +
    ...
    const suggestions = [];
    if (!profile.summary)
        suggestions.push('Add a professional summary to highlight your career goals');
    if (!profile.skills || profile.skills.length < 5)
        suggestions.push('List at least 5 relevant skills');
    if (!profile.resumeUrl)
        suggestions.push('Upload a resume file for better visibility');
    if (profile.workExperiences) {
 ...
 */
export function generateWorkTimeline(profile) {
    const items = [];
 ...
 */
export async function createProfile(userId, agentId, data) {
    ...
    const profile = {
        ...
        aiExtracted: false,
        qualityScore: calculateQualityScore(validated).overallScore,
        createdAt: now,
        updatedAt: now,
    };
 ...
 */
export async function getProfilesByUserId(userId, options = {}) {
...
export async function getPrimaryProfile(userId) {
    const userProfiles = Array.from(profiles.values()).filter(p => p.userId === userId && p.isPrimary);
    return userProfiles[0] || null;
}
/**
 * Update a profile
 */
export async function updateProfile(profileId, userId, data) {
    const profile = profiles.get(profileId);
    if (!profile) {
        throw new AppError('Profile not found', 'PROFILE_NOT_FOUND', 404);
    }
    ...
    const updated = {
        ...profile,
        ...Object.fromEntries(Object.entries(validated).filter(([_, v]) => v !== undefined)),
        id: profile.id,
        userId: profile.userId,
        agentId: profile.agentId,
        resumeVersion: (validated.workExperiences ||
            validated.skills ||
 ...
 */
export async function updateVisibility(profileId, userId, visibility, maskedFields) {
    const profile = profiles.get(profileId);
    if (!profile) {
        throw new AppError('Profile not found', 'PROFILE_NOT_FOUND', 404);
    }
 ...
 * Get quality report
 */
export async function getQualityReport(profileId, userId) {
 ...
 * Get work timeline
 */
export async function getWorkTimeline(profileId, userId) {
    const profile = await getProfile(profileId, userId);
 ...
 */
export function exportToMarkdown(profile) {
    const lines = [];
    lines.push(`# ${profile.name}`);
    if (profile.currentTitle)
        lines.push(`**${profile.currentTitle}**`);
    if (profile.location)
    ...
    if (profile.contactInfo && Object.keys(profile.contactInfo).length > 0) {
        ...
        if (profile.contactInfo.email)
            lines.push(`- Email: ${profile.contactInfo.email}`);
        if (profile.contactInfo.phone)
            lines.push(`- 电话: ${profile.contactInfo.phone}`);
        if (profile.contactInfo.linkedin)
            lines.push(`- LinkedIn: ${profile.contactInfo.linkedin}`);
        if (profile.contactInfo.github)
            lines.push(`- GitHub: ${profile.contactInfo.github}`);
    }
    return lines.join('\n');
}
