 ...
 */
export async function extractFromNaturalLanguage(text: string): Promise<ResumeExtractionResult> {
...
function extractCertifications(text: string): string[] {
  const certs: string[] = [];
  const certPattern =
    /(?:证书|Certification|Certified|认证)[：:]\s*\n?([\s\S]{10,1000}?)(?:\n\n|\n#|$)/i;
 ...
 */
export function matchSkills(profileSkills: SkillTag[], requiredSkills: string[]): SkillMatchResult {
