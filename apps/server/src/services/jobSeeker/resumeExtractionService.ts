 ...
 */
export async function extractFromNaturalLanguage(
  text: string
 ...
 */
function extractSkills(text: string): SkillTag[] {
  const skillKeywords: Record<string, string[]> = {
    frontend: ['React', 'Vue', 'Angular', 'TypeScript', 'JavaScript', 'HTML', 'CSS', 'Next.js', 'Nuxt.js', 'Tailwind'],
    backend: ['Node.js', 'Python', 'Java', 'Go', 'Rust', 'Express', 'Django', 'Flask', 'Spring', 'FastAPI'],
 ...
 */
function inferSkillLevel(text: string, skill: string): SkillLevel {
  const lower = text.toLowerCase();
  const skillLower = skill.toLowerCase();

  // Look for level indicators near the skill mention
  const patterns = [
    { pattern: new RegExp(`${skillLower}[^.]{0,30}(expert|master|精通)`, 'i'), level: SkillLevel.EXPERT },
    { pattern: new RegExp(`(expert|master|精通)${skillLower}`, 'i'), level: SkillLevel.EXPERT },
    { pattern: new RegExp(`${skillLower}[^.]{0,30}(advanced|熟练|高级)`, 'i'), level: SkillLevel.ADVANCED },
    { pattern: new RegExp(`(advanced|熟练|高级)${skillLower}`, 'i'), level: SkillLevel.ADVANCED },
    { pattern: new RegExp(`${skillLower}[^.]{0,30}(beginner|入门|初级|了解)`, 'i'), level: SkillLevel.BEGINNER },
    { pattern: new RegExp(`(beginner|入门|初级|了解)${skillLower}`, 'i'), level: SkillLevel.BEGINNER },
  ];
 ...
 */
function extractWorkExperiences(text: string): WorkExperience[] {
...
export function matchSkills(
  ...
  for (const required of requiredSkills) {
    const found = profileSkills.some(
      s => s.name.toLowerCase() === required.toLowerCase() ||
    ...
    if (found) {
      ...
      const related = profileSkills.some(
        s => levenshteinDistance(s.name.toLowerCase(), required.toLowerCase()) <= 3
      );
      if (related) {
...
export function scoreForJob(
  ...
  jobRequirements: { skills: string[]; experienceLevel?: string; educationLevel?: string }
): ResumeQualityReport & { skillMatch: SkillMatchResult } {
...
function calculateCompetitiveness(profile: JobSeekerProfile, skillMatchScore: number): number {
