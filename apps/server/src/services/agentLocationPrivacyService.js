 ...
 */
export async function getAgentLocationPrivacy(agentId) {
    try {
        ...
        return {
            privacyLevel: (privacy.privacyLevel || 'CITY'),
 ...
 */
export async function setAgentLocationPrivacy(agentId, settings) {
    try {
        const data = {};
        if (settings.privacyLevel !== undefined)
            data.privacyLevel = settings.privacyLevel;
        if (settings.showExactCoords !== undefined)
            data.showExactCoords = settings.showExactCoords;
        if (settings.hideFromPublic !== undefined)
            data.hideFromPublic = settings.hideFromPublic;
 ...
 */
export async function applyPrivacyFilter(agentId, locationData) {
 ...
 */
export async function deleteAgentLocationPrivacy(agentId) {
    try {
...
//# sourceMappingURL=agentLocationPrivacyService.js.map