...
export class CreditPaymentService {
  ...
  async processPayment(userId: string, request: PaymentRequest): Promise<PaymentResponse> {
    ...
    if (calculatedTotal !== totalAmount) {
      return {
        ...
        remainingBalance: 0,
        unlockedPhotos: [],
        error: { code: 'AMOUNT_MISMATCH', message: `Expected ${calculatedTotal}, got ${totalAmount}` },
      };
    }
    ...
    if (balance.available < totalAmount) {
      return {
        ...
        unlockedPhotos: [],
        error: { code: 'INSUFFICIENT_BALANCE', message: `Insufficient credits. Available: ${balance.available}, Required: ${totalAmount}` },
      };
    }
   ...
   */
  async validateAndUseDownloadToken(token: string, userId: string): Promise<{
    valid: boolean;
    photoId?: string;
    error?: string;
  }> {
