...
function recordHit(strategy) {
    strategyStats[strategy].hits++;
}
function recordMiss(strategy) {
    strategyStats[strategy].misses++;
}
function recordFill(strategy) {
...
const slowQueryLog = [];
function recordSlowQuery(operation, key, durationMs) {
    slowQueryLog.push({
        operation,
        key,
...
async function measureCacheOp(operation, key, fn) {
...
async function checkCapacity() {
    if (!isRedisConnected())
        return;
    const now = Date.now();
    if (now - lastCapacityCheck < CAPACITY_CHECK_INTERVAL_MS)
        return;
 ...
 */
export async function cacheGetOrSet(key, factory, ttlSeconds) {
 ...
 */
export async function cacheAside(key, factory, ttlSeconds, _writeFn) {
    // Try cache first
    const cached = await cacheGet(key);
    if (cached !== null) {
 ...
 */
export async function writeThrough(key, value, ttlSeconds, writeFn) {
 ...
 */
export async function readThrough(key, factory, ttlSeconds) {
    // Try cache first
    const cached = await cacheGet(key);
    if (cached !== null) {
 ...
 */
export async function cacheCreditScore(userId, score, level) {
 ...
 */
export async function getCachedCreditScore(userId) {
    const key = `${CacheKeys.CREDIT_SCORE}${userId}`;
    return cacheGet(key);
}
/**
 * Cache agent profile
 */
export async function cacheAgentProfile(agentId, profile) {
 ...
 */
export async function getCachedAgentProfile(agentId) {
    const key = `${CacheKeys.AGENT_PROFILE}${agentId}`;
    return cacheGet(key);
}
/**
 ...
 */
export async function invalidateAgentProfile(agentId) {
    const key = `${CacheKeys.AGENT_PROFILE}${agentId}`;
    await cacheDel(key);
}
/**
 * Cache match results
 */
export async function cacheMatchResults(userId, results) {
 ...
 */
export async function getCachedMatchResults(userId) {
    const key = `${CacheKeys.MATCH_RESULTS}${userId}`;
    return cacheGet(key);
}
/**
 ...
 */
export async function invalidateMatchResults(userId) {
    const key = `${CacheKeys.MATCH_RESULTS}${userId}`;
    await cacheDel(key);
}
/**
 ...
 */
export async function acquireLock(resource, ttlSeconds = 10) {
 ...
 */
export function getCacheStrategyStats() {
    const result = {};
    for (const [name, s] of Object.entries(strategyStats)) {
 ...
 */
export function resetCacheStrategyStats() {
 ...
 */
export async function invalidateByVersion(namespace, version) {
    if (!isRedisConnected()) {
        return 0;
    }
    try {
 ...
 */
export async function getCacheVersion(namespace) {
    if (!isRedisConnected()) {
        return null;
    }
    try {
 ...
 * Set cache version
 */
export async function setCacheVersion(namespace, version) {
    if (!isRedisConnected()) {
        return;
    }
    try {
 ...
 */
export function versionedKey(namespace, version, key) {
 ...
 */
export async function batchCacheGet(keys) {
 ...
 */
export async function batchCacheSet(entries) {
 ...
 *   }
 */
export function bustNamespaceCache(namespace, version = 'v1') {
    return function (target, propertyKey, descriptor) {
        const originalMethod = descriptor.value;
        descriptor.value = async function (...args) {
            const result = await originalMethod.apply(this, args);
