...
const supplyToL2Mapper_1 = require("../supplyToL2Mapper");
...
describe('SupplyToL2Mapper', () => {
    ...
    const createMockSchema = (fields) => ({
        ...
        title: 'Test Schema',
        fields: (fields || [
            { id: 'title', type: shared_1.L2FieldType.TEXT, label: 'Title', required: true },
            { id: 'description', type: shared_1.L2FieldType.LONG_TEXT, label: 'Description' },
            { id: 'priceMin', type: shared_1.L2FieldType.NUMBER, label: 'Min Price', min: 0 },
            { id: 'priceMax', type: shared_1.L2FieldType.NUMBER, label: 'Max Price', min: 0 },
            { id: 'priceCurrency', type: shared_1.L2FieldType.TEXT, label: 'Currency' },
            { id: 'priceRange', type: shared_1.L2FieldType.RANGE, label: 'Price Range', min: 0 },
            { id: 'locationCity', type: shared_1.L2FieldType.TEXT, label: 'City' },
            { id: 'locationRemote', type: shared_1.L2FieldType.BOOLEAN, label: 'Remote' },
            { id: 'experienceYears', type: shared_1.L2FieldType.NUMBER, label: 'Years', min: 0 },
            { id: 'skills', type: shared_1.L2FieldType.TEXT, label: 'Skills' },
            {
                id: 'serviceType',
                type: shared_1.L2FieldType.ENUM,
                label: 'Service Type',
                options: [
    ...
    const createMockSupply = (overrides) => ({
        id: 'supply-1',
        agentId: 'agent-1',
    ...
    beforeEach(() => {
        mapper = new supplyToL2Mapper_1.SupplyToL2Mapper();
    });
    describe('map', () => {
        ...
        it('should map capabilities with level mapping', () => {
            const supply = createMockSupply();
            const schema = createMockSchema([
                { id: 'capabilities', type: shared_1.L2FieldType.TEXT, label: 'Capabilities' },
            ]);
            const result = mapper.map(supply, schema);
    ...
    describe('transformValue - number', () => {
        it('should clamp numbers to min/max', () => {
            ...
            const schema = createMockSchema([
                { id: 'experienceYears', type: shared_1.L2FieldType.NUMBER, label: 'Years', min: 0, max: 50 },
            ]);
            const result = mapper.map(supply, schema);
    ...
    describe('transformValue - range', () => {
        it('should parse range from string with separator', () => {
            ...
            const schema = createMockSchema([
                { id: 'priceRange', type: shared_1.L2FieldType.RANGE, label: 'Price', min: 0 },
            ]);
            const result = mapper.map(supply, schema);
    ...
    describe('inferFieldValue', () => {
        it('should infer service category from description', () => {
            ...
            const schema = createMockSchema([
                { id: 'serviceCategory', type: shared_1.L2FieldType.TEXT, label: 'Category' },
            ]);
            const result = mapper.map(supply, schema);
...
//# sourceMappingURL=supplyToL2Mapper.test.js.map