...
export class AgentNegotiationService {
     ...
     * AD003-CR01: 匹配商家群聊创建
     */
    async createNegotiationRoom(consumerAgent, merchantAgents, consumerDemand, config) {
     ...
     */
    async generateConsumerIntroduction(roomId, consumerProfile) {
        ...
        try {
            ...
            const message = {
                ...
                senderType: 'consumer',
                creditScore: room.consumerAgent.profile?.creditScore,
     ...
     */
    async generateMerchantOfferPresentation(roomId, merchantId, offer) {
        const room = this.getRoom(roomId);
        const startTime = Date.now();
        try {
            ...
            const message = {
                ...
                senderId: merchantId,
                senderType: 'merchant',
                creditScore: merchant.profile?.creditScore,
                content: response.text.trim(),
                messageType: 'offer',
                timestamp: new Date(),
                metadata: {
     ...
     */
    async generateFollowUpQuestions(roomId, targetMerchantId) {
        const room = this.getRoom(roomId);
        const startTime = Date.now();
        try {
     ...
     * 执行协商追问
     */
    async executeFollowUpQuestion(roomId, question) {
        const room = this.getRoom(roomId);
        try {
     ...
     * 生成对比分析报告消息
     */
    async generateComparisonReport(roomId, comparisonResult) {
        const room = this.getRoom(roomId);
        try {
     ...
     */
    async generateRecommendation(roomId) {
        const room = this.getRoom(roomId);
        const startTime = Date.now();
        try {
     ...
     * 生成推荐消息
     */
    async generateRecommendationMessage(roomId, recommendation) {
        const room = this.getRoom(roomId);
        try {
...
请问您希望怎么做？`;
     ...
     * 确认选择方案
     */
    async confirmSelection(roomId, offerId, confirmed) {
        const room = this.getRoom(roomId);
        try {
     ...
     * 构建开场白提示词
     */
    buildIntroductionPrompt(room, _consumerProfile) {
...
5. 用中文回复，控制在150字以内`;
     ...
     * 构建优惠展示提示词
     */
    buildOfferPresentationPrompt(room, merchant, offer) {
...
5. 用中文回复，控制在200字以内`;
     ...
     * 构建追问问题提示词
     */
    buildFollowUpQuestionsPrompt(room, merchant, offers) {
...
请以JSON格式返回：
{
  "questions": [
     ...
     * 构建商家回复提示词
     */
    buildMerchantResponsePrompt(room, merchant, question) {
        const offers = room.context.merchantOffers.get(merchant.id) || [];
...
4. 用中文回复，控制在150字以内`;
     ...
     * 构建对比提示词
     */
    buildComparisonPrompt(room, allOffers) {
        const consumerDemand = room.context.consumerDemand;
...
请以JSON格式返回分析结果：
{
  ...
  "bestMatchOffer": "最匹配需求优惠ID"
     ...
     * 构建推荐提示词
     */
    buildRecommendationPrompt(room, comparisonResult) {
...
请以JSON格式返回：
{
  ...
  "savingsEstimate": {
     ...
     * 解析追问问题
     */
    parseFollowUpQuestions(text, targetMerchantId) {
        try {
     ...
     * 解析对比结果
     */
    parseComparisonResult(text, allOffers, room) {
        try {
            const jsonMatch = text.match(/\{[\s\S]*\}/);
            if (!jsonMatch) {
            ...
            return {
                offers: allOffers.map(({ merchantId, offer }) => {
                    ...
                    let matchScore = 60;
                    if (demand.category && offer.applicableProducts?.some(p => p.toLowerCase().includes(demand.category.toLowerCase()) ||
                        demand.category.toLowerCase().includes(p.toLowerCase()))) {
                        matchScore = 85;
                    }
                    if (demand.brandPreferences?.length && offer.applicableProducts?.some(p => demand.brandPreferences.some(bp => p.toLowerCase().includes(bp.toLowerCase())))) {
                    ...
                    if (offer.terms?.length)
                        convenienceScore -= offer.terms.length * 5;
                    if (offer.exclusions?.length)
                        convenienceScore -= 5;
     ...
     * 解析推荐结果
     */
    parseRecommendationResult(text, comparisonResult) {
        try {
            const jsonMatch = text.match(/\{[\s\S]*\}/);
            if (!jsonMatch) {
                throw new Error('No JSON found in response');
            ...
            const bestOffer = comparisonResult.offers.reduce((best, current) => current.scores.overall > best.scores.overall ? current : best);
            return {
     ...
     * 构建对比报告内容
     */
    buildComparisonReportContent(comparisonResult) {
        const lines = ['📊 **方案对比分析**\n'];
     ...
     * 记录指标
     */
    async recordMetrics(operation, response, startTime) {
...
//# sourceMappingURL=agentNegotiationService.js.map