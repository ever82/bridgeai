...
export class ImageModerationService {
     ...
     * 批量审核图像
     */
    async moderateBatch(images, context) {
        const results = [];
        // 限制并发数
        const concurrency = 3;
        for (let i = 0; i < images.length; i += concurrency) {
            const batch = images.slice(i, i + concurrency);
            const batchPromises = batch.map(image => this.moderate(image, context).catch(error => {
     ...
     * 获取特定违规类型的风险评分
     */
    async getViolationScore(image, violationType) {
        const result = await this.moderate(image);
        return result.categoryScores[violationType] || 0;
    }
    /**
     * 更新阈值配置
     */
...
Be thorough but fair. Consider context and intent. When in doubt, err on the side of caution.`;
     ...
     * 获取默认违规描述
     */
    getDefaultViolationDescription(violationType) {
        const descriptions = {
            none: '',
    ...
    applyStrictMode(result) {
        ...
        for (const [type, score] of Object.entries(result.categoryScores)) {
            const threshold = this.thresholds[type] * 0.8; // 降低阈值20%
            if (score > threshold) {
                return false;
            }
        }
...
//# sourceMappingURL=imageModerationService.js.map