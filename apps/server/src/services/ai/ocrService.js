...
export class OCRService {
     ...
     * 批量提取文字
     */
    async extractTextBatch(images, options) {
        const results = [];
        // 限制并发数
        const concurrency = 3;
        for (let i = 0; i < images.length; i += concurrency) {
            const batch = images.slice(i, i + concurrency);
            const batchPromises = batch.map(image => this.extractText(image, options).catch(error => {
...
If no text is present, return an empty array [].`;
     ...
     * 提取特定区域的文字
     */
    async extractTextInRegion(image, boundingBox) {
...
Return results in JSON format:
{
  ...
  "confidence": 0.0-1.0
     ...
     * 结构化数据提取（如发票、名片等）
     */
    async extractStructuredData(image, schema) {
...
If a field is not found, use null or empty string.`;
     ...
     * 构建OCR提示
     */
    buildOCRPrompt(language, detectHandwriting, preserveLayout) {
...
- Estimate bounding boxes for text regions (relative coordinates 0-1)`;
     ...
     * 解析语言响应
     */
    parseLanguageResponse(response) {
        try {
            const jsonMatch = response.match(/\[[\s\S]*\]/);
            if (jsonMatch) {
                const data = JSON.parse(jsonMatch[0]);
