...
import { buildExtractionPrompt, buildIntroductionExtractionPrompt, buildImplicitPreferencePrompt, buildSmartCompletionPrompt, } from '../../prompts/dating';
...
export class DatingExtractionService {
     ...
     * 从自然语言文本中提取约会资料数据
     */
    async extractFromText(text, currentProfile) {
        ...
        try {
            ...
            const prompt = buildExtractionPrompt(text, currentProfile);
     ...
     * 从自我介绍中提取约会资料数据
     */
    async extractFromIntroduction(introduction) {
        const startTime = Date.now();
        try {
     ...
     * 检测文本中的隐含偏好（否定、强调、推断）
     */
    async detectImplicitPreferences(text) {
        const startTime = Date.now();
        try {
     ...
     * 为部分填写的约会资料生成智能补全建议
     */
    async suggestCompletions(partialProfile, description) {
        const startTime = Date.now();
        try {
            logger.info('Starting smart completion suggestions', {
                hasDescription: !!description,
            });
            const prompt = buildSmartCompletionPrompt(partialProfile, description);
            const response = await llmService.generateText(prompt, {
                temperature: 0.4,
                maxTokens: 1500,
            });
     ...
     */
    async extractFromDescription(request) {
    ...
    parseImplicitResult(text) {
        try {
            ...
            return {
                negations: (result.negations || []).map((n) => ({
                ...
                emphases: (result.emphases || []).map((e) => ({
                ...
                inferred: (result.inferred || []).map((i) => ({
    ...
    parseCompletionResult(text) {
        try {
            ...
            return {
                ...
                missingSections: (result.missingSections || []).map((m) => ({
                    section: m.section || '',
     ...
     */
    buildProfileFromExtraction(parsed) {
        const profile = {};
     ...
     */
    buildExtractedPreferences(parsed) {
        const extracted = (parsed.extracted || []).map((e) => ({
            category: e.category || '',
            value: e.value,
            confidence: e.confidence || 50,
     ...
     */
    getFieldsExtracted(profile) {
        const fields = [];
        if (profile.basicConditions)
            fields.push('basicConditions');
        if (profile.personality)
            fields.push('personality');
        if (profile.interests)
            fields.push('interests');
        if (profile.lifestyle)
            fields.push('lifestyle');
        if (profile.expectations)
            fields.push('expectations');
        return fields;
    }
    /**
     * Get list of fields that failed to extract
     */
    getFieldsFailed(parsed) {
        const failed = [];
     ...
     */
    normalizeBasicConditions(data) {
        const result = {};
        if (data.ageRange)
            result.ageRange = data.ageRange;
        if (data.heightRange)
            result.heightRange = data.heightRange;
        if (data.education)
        ...
        if (data.hasPhoto !== undefined)
            result.hasPhoto = data.hasPhoto;
     ...
     */
    normalizePersonality(data) {
        const result = {};
        if (data.traits)
            result.traits = data.traits;
        if (data.preferredTraits)
            result.preferredTraits = data.preferredTraits;
        if (data.dislikedTraits)
            result.dislikedTraits = data.dislikedTraits;
        if (data.mbti)
            result.mbti = data.mbti;
        return result;
    }
    /**
    ...
    normalizeInterests(data) {
        ...
        if (Array.isArray(data.interests)) {
            result.interests = data.interests.map((i) => {
                const interest = {
                    category: i.category,
                    name: i.name || i.category,
                };
                if (i.level)
                    interest.level = i.level;
                return interest;
            });
        }
        if (data.customInterests)
            result.customInterests = data.customInterests;
        if (data.preferredInPartner)
            result.preferredInPartner = data.preferredInPartner;
        if (data.sharedInterests)
            result.sharedInterests = data.sharedInterests;
        return result;
    }
    /**
     * Normalize lifestyle from LLM output
     */
    normalizeLifestyle(data) {
        const result = {};
        if (data.sleepSchedule)
            result.sleepSchedule = data.sleepSchedule;
        if (data.smoking)
            result.smoking = data.smoking;
        if (data.drinking)
            result.drinking = data.drinking;
        if (data.pets)
            result.pets = data.pets;
        if (data.exercise)
            result.exercise = data.exercise;
        if (data.diet)
            result.diet = data.diet;
        if (data.workLifeBalance)
            result.workLifeBalance = data.workLifeBalance;
        if (data.socialFrequency)
            result.socialFrequency = data.socialFrequency;
        return result;
    }
    /**
     * Normalize relationship expectations from LLM output
     */
    normalizeExpectations(data) {
        const result = {
        ...
        if (data.living)
            result.living = data.living;
        if (data.familyPlan)
            result.familyPlan = data.familyPlan;
        if (data.marriageTimeline)
            result.marriageTimeline = data.marriageTimeline;
        if (data.longDistance)
            result.longDistance = data.longDistance;
        return result;
    }
    /**
     * Record metrics for the LLM call
     */
    async recordMetrics(operation, response, latencyMs, success) {
        try {
...
//# sourceMappingURL=datingExtractionService.js.map