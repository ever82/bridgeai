...
export class ImageSearchService {
     ...
     * 为图像生成嵌入向量
     */
    async generateImageEmbedding(image) {
...
Keep the description concise but comprehensive.`;
     ...
     * 索引图像到搜索库
     */
    async indexImage(imageId, image, metadata) {
        // 生成图像嵌入
        const embedding = await this.generateImageEmbedding(image);
        // 提取语义标签
    ...
    async indexImages(images) {
        ...
        for (let i = 0; i < images.length; i += concurrency) {
            ...
            await Promise.all(batch.map(({ imageId, image, metadata }) => this.indexImage(imageId, image, metadata).catch(error => {
     ...
     * 提取图像的语义标签
     */
    async extractSemanticTags(image) {
    ...
    parseSemanticTags(response) {
        ...
        try {
            ...
            return {
                ...
                attributes: typeof data.attributes === 'object' && data.attributes !== null ? data.attributes : {},
            };
        }
        catch {
            return defaultTags;
