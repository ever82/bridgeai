...
const vision_1 = require("../adapters/vision");
const imageAnalysisService_1 = require("../imageAnalysisService");
const imageModerationService_1 = require("../imageModerationService");
const ocrService_1 = require("../ocrService");
const imageSearchService_1 = require("../imageSearchService");
...
class MockVisionAdapter {
    ...
    async generateEmbedding(image) {
        ...
        const embedding = new Array(dimension).fill(0).map((_, i) => Math.sin(i * 0.1) * 0.5);
        // Normalize
...
describe('VisionModelAdapters', () => {
    describe('GPT4VisionAdapter', () => {
        let adapter;
        beforeEach(() => {
            adapter = new vision_1.GPT4VisionAdapter({
                apiKey: 'test-api-key'
            });
        });
        ...
        it('should validate image input', () => {
            // Valid URL
            expect(() => {
                adapter.validateImageInput(mockImageUrl);
            }).not.toThrow();
            // Valid base64
            expect(() => {
                adapter.validateImageInput(mockImageBase64);
            }).not.toThrow();
            // Invalid - no data
            expect(() => {
                adapter.validateImageInput({ type: 'url', data: '' });
            }).toThrow('Image data is required');
            // Invalid URL
            expect(() => {
                adapter.validateImageInput({ type: 'url', data: 'not-a-url' });
            }).toThrow('Invalid image URL');
        });
        it('should format image for API', () => {
            const urlResult = adapter.formatImageForAPI(mockImageUrl);
            expect(urlResult.type).toBe('image_url');
            expect(urlResult.image_url.url).toBe(mockImageUrl.data);
            const base64Result = adapter.formatImageForAPI(mockImageBase64);
    ...
    describe('ClaudeVisionAdapter', () => {
        let adapter;
        beforeEach(() => {
            adapter = new vision_1.ClaudeVisionAdapter({
                apiKey: 'test-api-key'
            });
        });
        it('should have correct id and provider', () => {
        ...
        it('should throw when initialized without API key', async () => {
            const adapterWithoutKey = new vision_1.ClaudeVisionAdapter({
                apiKey: ''
            });
...
describe('ImageAnalysisService', () => {
    ...
    beforeEach(() => {
        ...
        service = new imageAnalysisService_1.ImageAnalysisService({ adapter: mockAdapter });
    });
    describe('analyze', () => {
...
describe('ImageModerationService', () => {
    ...
    beforeEach(() => {
        mockAdapter = new MockVisionAdapter();
        service = new imageModerationService_1.ImageModerationService({
            adapter: mockAdapter,
            safetyThreshold: 0.7,
            strictMode: false
        });
...
describe('OCRService', () => {
    ...
    beforeEach(() => {
        mockAdapter = new MockVisionAdapter();
        service = new ocrService_1.OCRService({
            adapter: mockAdapter,
...
describe('ImageSearchService', () => {
    ...
    beforeEach(() => {
        mockAdapter = new MockVisionAdapter();
        service = new imageSearchService_1.ImageSearchService({
            adapter: mockAdapter,
            defaultTopK: 10,
