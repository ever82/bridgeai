...
const datingConversationService_1 = require("../datingConversationService");
describe('DatingConversationService', () => {
    ...
    beforeEach(() => {
        service = new datingConversationService_1.DatingConversationService();
    });
    describe('DatingConversationService class instantiation', () => {
        it('should create service with default config', () => {
            const defaultService = new datingConversationService_1.DatingConversationService();
            expect(defaultService).toBeDefined();
        });
        it('should create service with custom config', () => {
            const customService = new datingConversationService_1.DatingConversationService({
                maxRounds: 30,
    ...
    describe('startConversation', () => {
        it('should create session with profiles', async () => {
            const session = await service.startConversation('agent-a', 'agent-b', 'user-a', 'user-b', mockProfileA, mockProfileB);
        ...
        it('should build topic queue', async () => {
            ...
            for (let i = 1; i < session.topics.length; i++) {
                expect(session.topics[i - 1].relevanceScore).toBeGreaterThanOrEqual(session.topics[i].relevanceScore);
            }
    ...
    describe('getSession', () => {
        it('should return session by id', async () => {
            const created = await service.startConversation('agent-a', 'agent-b', 'user-a', 'user-b', mockProfileA, mockProfileB);
    ...
    describe('Topic queue building', () => {
        ...
        it('should handle profiles with no shared interests', async () => {
            const uniqueProfileA = {
                ...mockProfileA,
                interests: ['登山', '潜水'],
            };
            const uniqueProfileB = {
                ...mockProfileB,
                interests: ['烹饪', '绘画'],
            };
            const session = await service.startConversation('agent-a', 'agent-b', 'user-a', 'user-b', uniqueProfileA, uniqueProfileB);
    ...
    describe('generateFallbackResponse', () => {
        it('should generate response based on profile', async () => {
            ...
            const response = await service.generateFallbackResponse(session, 'agent-a', {
                id: 'topic-1',
                name: 'Test Topic',
                category: 'icebreaker',
                relevanceScore: 0.8,
    ...
    describe('updateSessionStatus', () => {
        ...
        it('should throw error for non-existent session', async () => {
            await expect(service.updateSessionStatus('non-existent', 'completed')).rejects.toThrow('Session not found');
        });
    });
    describe('getConversationSummary', () => {
        ...
        it('should throw error for non-existent session', async () => {
            await expect(service.getConversationSummary('non-existent')).rejects.toThrow('Session not found');
        });
    });
});
//# sourceMappingURL=datingConversationService.test.js.map