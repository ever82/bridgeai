...
const agentNegotiationService_1 = require("../agentNegotiationService");
...
describe('AgentNegotiationService', () => {
    ...
    beforeEach(() => {
        service = new agentNegotiationService_1.AgentNegotiationService();
        jest.clearAllMocks();
        // Mock LLM service
    ...
    describe('createNegotiationRoom', () => {
        it('should create a negotiation room successfully', async () => {
            const room = await service.createNegotiationRoom(mockConsumerAgent, mockMerchantAgents, mockConsumerDemand);
        ...
        it('should throw error if less than 2 merchant agents', async () => {
            await expect(service.createNegotiationRoom(mockConsumerAgent, [mockMerchantAgents[0]], mockConsumerDemand)).rejects.toThrow('At least 2 merchant agents are required');
        });
        it('should apply custom config', async () => {
            const room = await service.createNegotiationRoom(mockConsumerAgent, mockMerchantAgents, mockConsumerDemand, { maxRounds: 10, autoNegotiate: false });
    ...
    describe('generateConsumerIntroduction', () => {
        let room;
        beforeEach(async () => {
            room = await service.createNegotiationRoom(mockConsumerAgent, mockMerchantAgents, mockConsumerDemand);
        });
        ...
        it('should throw error for non-existent room', async () => {
            await expect(service.generateConsumerIntroduction('non-existent')).rejects.toThrow('Room non-existent not found');
        });
    });
    describe('generateMerchantOfferPresentation', () => {
        ...
        it('should generate merchant offer presentation', async () => {
            ...
            const message = await service.generateMerchantOfferPresentation(room.id, 'merchant-1', mockMerchantOffer);
            expect(message).toBeDefined();
        ...
        it('should throw error for non-existent merchant', async () => {
            await expect(service.generateMerchantOfferPresentation(room.id, 'non-existent', mockMerchantOffer)).rejects.toThrow('Merchant non-existent not found in room');
        });
    });
    ...
    describe('compareOffers', () => {
        ...
        it('should throw error if less than 2 offers', async () => {
            const newRoom = await service.createNegotiationRoom(mockConsumerAgent, mockMerchantAgents, mockConsumerDemand);
            await expect(service.compareOffers(newRoom.id)).rejects.toThrow('At least 2 offers are required for comparison');
        });
    ...
    describe('generateRecommendation', () => {
        ...
        it('should generate recommendation', async () => {
            llmService_1.llmService.generateText.mockResolvedValue({
                text: JSON.stringify({
                    ...
                    recommendationReason: '天猫官方店优惠力度最大(800元)，且品牌可靠、服务有保障，最符合您的需求。',
        ...
        it('should throw error if comparison not performed', async () => {
            const newRoom = await service.createNegotiationRoom(mockConsumerAgent, mockMerchantAgents, mockConsumerDemand);
            await expect(service.generateRecommendation(newRoom.id)).rejects.toThrow('Comparison must be performed before generating recommendation');
        });
    });
...
//# sourceMappingURL=agentNegotiationService.test.js.map