...
const clarificationService_1 = require("../clarificationService");
describe('ClarificationService', () => {
    let service;
    beforeEach(() => {
        service = new clarificationService_1.ClarificationService();
    });
    afterEach(() => {
    ...
    describe('processClarification', () => {
        ...
        it('should throw error for invalid session', async () => {
            await expect(service.processClarification({
    ...
    describe('generateClarificationQuestions', () => {
        it('should generate questions for missing fields', async () => {
            const questions = await service.generateClarificationQuestions(['photographyType', 'budget'], 'visionshare');
            expect(questions).toHaveLength(2);
            expect(questions[0]).toContain('摄影');
        });
    });
});
//# sourceMappingURL=clarificationService.test.js.map