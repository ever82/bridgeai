...
import { ILLMAdapter } from '../adapters/base';

// ---------------------------------------------------------------------------
// Helpers & test data
// ---------------------------------------------------------------------------

const PROVIDERS: LLMProvider[] = ['openai', 'claude', 'wenxin'];

function makeModel(overrides: Partial<ModelInfo> & Pick<ModelInfo, 'id' | 'provider'>): ModelInfo {
  return {
    name: overrides.id,
    capabilities: {
      chatCompletion: true,
...
function createStubAdapter(
  ...
  errorMsg: string = 'Provider error'
): ILLMAdapter {
  return {
    id: `stub-${provider}`,
    provider,
    ...
    getModelInfo: async (modelId: string) => MODELS.find(m => m.id === modelId) ?? null,
...
class InMemoryCache implements ResponseCache {
  private store = new Map<string, ChatCompletionResponse>();
  async get(key: string) {
    return this.store.get(key) ?? null;
  }
  async set(key: string, value: ChatCompletionResponse) {
    this.store.set(key, value);
  }
  generateKey(request: ChatCompletionRequest) {
...
function setupRouter(): LLMRouter {
  const router = new LLMRouter({ strategy: 'round-robin' });
...
describe('AI Service Integration', () => {
  ...
  describe('Circuit breaker + adapter integration', () => {
    ...
    it('should reject requests when breaker is OPEN', async () => {
      ...
      await expect(
        breaker.execute(() => healthyAdapter.chatCompletion(request, ctx))
      ).rejects.toThrow('Circuit breaker is OPEN for provider openai');
    });
    ...
    it('should reopen breaker if HALF_OPEN probe fails', async () => {
      ...
      await expect(
        breaker.execute(() => failingAdapter.chatCompletion(request, ctx))
  ...
  describe('Router + CircuitBreaker coordination', () => {
    ...
    it('should integrate full request flow: route -> check breaker -> call adapter', async () => {
      const adapters = new Map<LLMProvider, ILLMAdapter>([
  ...
  describe('Fallback chain with circuit breaker and router', () => {
    let breakerManager: CircuitBreakerManager;
    let models: Map<string, ModelInfo>;
    ...
    it('should chain through multiple strategies until one succeeds', async () => {
      ...
      const limitedModels = new Map<string, ModelInfo>();
      ...
      const chain = new FallbackChain();
      chain.addStrategy(new CacheFallbackStrategy()); // no cache -> fail
      chain.addStrategy(new ModelDowngradeStrategy()); // no cheaper openai model -> fail
      chain.addStrategy(new ProviderSwitchStrategy()); // should find claude or wenxin
  ...
  describe('End-to-end: failure -> circuit open -> fallback -> recovery', () => {
    it('should handle complete failure and recovery lifecycle', async () => {
      ...
      await expect(
        openaiBreaker.execute(() => adapters.get('openai')!.chatCompletion(request, ctx1))
      ).rejects.toThrow();
      await expect(
        openaiBreaker.execute(() => adapters.get('openai')!.chatCompletion(request, ctx2))
      ).rejects.toThrow();
      expect(openaiBreaker.getState()).toBe('OPEN');
      ...
      const response = await fallbackBreaker.execute(() =>
        adapters.get(decision.provider)!.chatCompletion(request, fallbackCtx)
      );
    ...
    it('should handle multiple providers failing sequentially with fallback chain', async () => {
      ...
      for (const provider of ['openai', 'claude'] as LLMProvider[]) {
      ...
      const response = await wenxinBreaker.execute(() =>
        adapters.get('wenxin')!.chatCompletion(request, ctx)
      );
      expect(response).toBeDefined();
      expect(response.choices).toHaveLength(1);
  ...
  describe('CircuitBreakerManager with multiple providers', () => {
    ...
    it('should emit state change events across integrated components', () => {
      const manager = new CircuitBreakerManager();
      const stateChanges: Array<{ provider: LLMProvider; state: CircuitBreakerState }> = [];
