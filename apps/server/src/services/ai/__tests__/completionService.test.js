...
describe('SupplyCompletionService', () => {
    let service;
    const createFullSupply = (overrides) => ({
        id: 'supply-1',
    ...
    const createSparseSupply = (overrides) => ({
        id: 'supply-sparse',
        title: '',
        description: '',
        serviceType: '',
    ...
    const createPartialSupply = (overrides) => ({
        id: 'supply-partial',
        title: 'Dev',
    ...
    beforeEach(() => {
        service = new completionService_1.SupplyCompletionService();
    });
    ...
    describe('detectIncomplete', () => {
        ...
        it('should detect capability without level', () => {
            const supply = createFullSupply({
                capabilities: [{
                        name: 'Test', description: 'Test', level: '', category: 'test', keywords: [],
                    }],
            });
            const { incomplete } = service.detectIncomplete(supply);
        ...
        it('should detect missing pricing type', () => {
            const supply = createFullSupply({ pricing: { type: '', currency: 'CNY' } });
            const { incomplete } = service.detectIncomplete(supply);
        ...
        it('should detect missing location mode', () => {
            const supply = createFullSupply({
                location: { city: '上海', country: 'China', remote: undefined, onsite: undefined, hybrid: false },
            });
            const { incomplete } = service.detectIncomplete(supply);
    ...
    describe('generateSuggestions', () => {
        ...
        it('should suggest location mode from service type defaults', () => {
            const supply = createFullSupply({
                serviceType: 'development',
                location: { city: '北京', country: 'China', remote: undefined, onsite: undefined, hybrid: false },
            });
            const suggestions = service.generateSuggestions(supply);
...
//# sourceMappingURL=completionService.test.js.map