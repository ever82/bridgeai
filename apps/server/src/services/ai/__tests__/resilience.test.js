...
const degradationStrategy_1 = require("../resilience/degradationStrategy");
const degradationController_1 = require("../resilience/degradationController");
const responseCacheService_1 = require("../resilience/responseCacheService");
const templateResponseService_1 = require("../resilience/templateResponseService");
const asyncQueueService_1 = require("../resilience/asyncQueueService");
const metricsService_1 = require("../resilience/metricsService");
...
function createDefaultContext() {
    return {
        ...
        cache: new responseCacheService_1.AIResponseCacheService(),
        templateService: new templateResponseService_1.TemplateResponseService(),
        queueService: new asyncQueueService_1.AIAsyncQueueService(),
    };
}
...
describe('DegradationStrategy', () => {
    let strategy;
    beforeEach(() => {
        strategy = new degradationStrategy_1.DegradationStrategy();
    });
    test('should start at NORMAL level', () => {
        expect(strategy.getCurrentLevel()).toBe(degradationStrategy_1.DegradationLevel.NORMAL);
    });
    test('should have 5 registered levels', () => {
        const levels = strategy.getRegisteredLevels();
        expect(levels).toHaveLength(5);
        expect(levels).toContain(degradationStrategy_1.DegradationLevel.L1_BACKUP_MODEL);
        expect(levels).toContain(degradationStrategy_1.DegradationLevel.L2_LOW_COST_MODEL);
        expect(levels).toContain(degradationStrategy_1.DegradationLevel.L3_TEMPLATE);
        expect(levels).toContain(degradationStrategy_1.DegradationLevel.L4_ASYNC_QUEUE);
        expect(levels).toContain(degradationStrategy_1.DegradationLevel.L5_UNAVAILABLE);
    });
    test('L1: should switch to backup model', async () => {
        const context = createDefaultContext();
        const result = await strategy.execute(createRequest(), context);
        expect(result.success).toBe(true);
        expect(result.level).toBe(degradationStrategy_1.DegradationLevel.L1_BACKUP_MODEL);
    ...
    test('L1: should fail when no backup model available', async () => {
        ...
        expect(result.level).toBeGreaterThanOrEqual(degradationStrategy_1.DegradationLevel.L2_LOW_COST_MODEL);
    });
    test('L2: should find cheaper model across providers', async () => {
        ...
        const result = await strategy.execute(createRequest(), context, degradationStrategy_1.DegradationLevel.L2_LOW_COST_MODEL);
        expect(result.success).toBe(true);
        expect(result.level).toBe(degradationStrategy_1.DegradationLevel.L2_LOW_COST_MODEL);
        expect(result.model).toBeDefined();
    });
    test('L3: should use template response when no models available', async () => {
        ...
        const result = await strategy.execute(createRequest(), context);
        expect(result.success).toBe(true);
        expect(result.level).toBe(degradationStrategy_1.DegradationLevel.L3_TEMPLATE);
    ...
    test('L4: should queue request when templates fail', async () => {
        ...
        const result = await strategy.execute(createRequest(), context);
        expect(result.success).toBe(true);
        expect(result.level).toBe(degradationStrategy_1.DegradationLevel.L4_ASYNC_QUEUE);
        expect(result.jobId).toBeDefined();
    });
    test('L5: should return unavailable when all levels fail', async () => {
        const queueService = new asyncQueueService_1.AIAsyncQueueService();
        const context = {
            ...createDefaultContext(),
            models: new Map(),
            availableProviders: [],
        ...
        const result = await strategy.execute(createRequest(), context, degradationStrategy_1.DegradationLevel.L5_UNAVAILABLE);
        expect(result.success).toBe(false);
        expect(result.level).toBe(degradationStrategy_1.DegradationLevel.L5_UNAVAILABLE);
    });
    ...
    test('reset should return to NORMAL level', async () => {
        await strategy.execute(createRequest(), createDefaultContext());
        expect(strategy.getCurrentLevel()).not.toBe(degradationStrategy_1.DegradationLevel.NORMAL);
        strategy.reset();
        expect(strategy.getCurrentLevel()).toBe(degradationStrategy_1.DegradationLevel.NORMAL);
    });
    test('setLevel should set level directly', () => {
        strategy.setLevel(degradationStrategy_1.DegradationLevel.L3_TEMPLATE);
        expect(strategy.getCurrentLevel()).toBe(degradationStrategy_1.DegradationLevel.L3_TEMPLATE);
    });
});
// ============================================
...
describe('AIResponseCacheService', () => {
    let cache;
    beforeEach(() => {
        cache = new responseCacheService_1.AIResponseCacheService({ defaultTtlMs: 1000 });
    });
    const mockResponse = {
        id: 'resp-1',
        model: 'gpt-4',
    ...
    test('should expire entries based on TTL', async () => {
        const shortCache = new responseCacheService_1.AIResponseCacheService({
            defaultTtlMs: 50,
    ...
    test('should warm cache with multiple entries', async () => {
        await cache.warmCache([
            ...
            {
                request: { ...createRequest(), messages: [{ role: 'user', content: 'Test' }] },
    ...
    test('should enforce max entries with eviction', async () => {
        const smallCache = new responseCacheService_1.AIResponseCacheService({ maxEntries: 2 });
...
describe('TemplateResponseService', () => {
    let service;
    beforeEach(() => {
        service = new templateResponseService_1.TemplateResponseService();
    });
...
describe('AIAsyncQueueService', () => {
    let queue;
    beforeEach(() => {
        queue = new asyncQueueService_1.AIAsyncQueueService();
    });
    afterEach(() => {
    ...
    test('should retry on failure', async () => {
        ...
        expect(queue.getStats().retried + queue.getStats().completed + queue.getStats().failed).toBeGreaterThan(0);
    });
...
describe('ResilienceMetricsService', () => {
    let metrics;
    beforeEach(() => {
        metrics = new metricsService_1.ResilienceMetricsService();
    });
    test('should record degradation events', () => {
        metrics.recordDegradationEvent({
            fromLevel: degradationStrategy_1.DegradationLevel.NORMAL,
            toLevel: degradationStrategy_1.DegradationLevel.L1_BACKUP_MODEL,
            reason: 'Test degradation',
            affectedProviders: ['openai'],
        });
        const events = metrics.getDegradationEvents();
        expect(events).toHaveLength(1);
        expect(events[0].fromLevel).toBe(degradationStrategy_1.DegradationLevel.NORMAL);
        expect(events[0].toLevel).toBe(degradationStrategy_1.DegradationLevel.L1_BACKUP_MODEL);
    });
    test('should record provider request metrics', () => {
        metrics.recordRequest('openai', {
            success: true,
            latencyMs: 100,
            degradationLevel: degradationStrategy_1.DegradationLevel.NORMAL,
        });
        metrics.recordRequest('openai', {
            success: false,
            latencyMs: 5000,
            errorType: 'timeout',
            degradationLevel: degradationStrategy_1.DegradationLevel.L1_BACKUP_MODEL,
        });
    ...
    test('should provide summary with current level', () => {
        metrics.recordDegradationEvent({
            fromLevel: degradationStrategy_1.DegradationLevel.NORMAL,
            toLevel: degradationStrategy_1.DegradationLevel.L2_LOW_COST_MODEL,
            reason: 'High error rate',
            affectedProviders: ['openai'],
        });
        const summary = metrics.getSummary();
        expect(summary.currentLevel).toBe(degradationStrategy_1.DegradationLevel.L2_LOW_COST_MODEL);
        expect(summary.totalDegradationEvents).toBe(1);
    });
    test('should track degradation duration', () => {
        metrics.recordDegradationEvent({
            fromLevel: degradationStrategy_1.DegradationLevel.NORMAL,
            toLevel: degradationStrategy_1.DegradationLevel.L3_TEMPLATE,
            reason: 'Test',
            affectedProviders: [],
        });
    ...
    test('should respect alert cooldown', () => {
        ...
        metrics.recordDegradationEvent({
            fromLevel: degradationStrategy_1.DegradationLevel.L1_BACKUP_MODEL,
            toLevel: degradationStrategy_1.DegradationLevel.L2_LOW_COST_MODEL,
            reason: 'Test 2',
            affectedProviders: ['openai'],
        });
    ...
    test('should reset all metrics', () => {
        ...
        expect(metrics.getSummary().currentLevel).toBe(degradationStrategy_1.DegradationLevel.NORMAL);
...
describe('DegradationController', () => {
    ...
    beforeEach(() => {
        strategy = new degradationStrategy_1.DegradationStrategy();
        metrics = new metricsService_1.ResilienceMetricsService();
        controller = new degradationController_1.DegradationController(strategy, metrics, {
            checkIntervalMs: 50,
    ...
    test('should start at NORMAL level', () => {
        expect(controller.getCurrentLevel()).toBe(degradationStrategy_1.DegradationLevel.NORMAL);
    });
    ...
    test('should escalate degradation on consecutive failures', () => {
        ...
        controller.recordFailure('openai', 5000, 'timeout');
        controller.recordFailure('openai', 5000, 'timeout');
        expect(controller.getCurrentLevel()).toBeGreaterThan(degradationStrategy_1.DegradationLevel.NORMAL);
    });
    test('should recover when providers succeed', () => {
        controller.startMonitoring(['openai']);
        // Degrade first
        controller.manualDegradation(degradationStrategy_1.DegradationLevel.L1_BACKUP_MODEL, 'Test');
        expect(controller.getCurrentLevel()).toBe(degradationStrategy_1.DegradationLevel.L1_BACKUP_MODEL);
    ...
    test('should provide user impact assessment', () => {
        ...
        expect(impact.level).toBe(degradationStrategy_1.DegradationLevel.NORMAL);
        expect(impact.affectedFeatures).toHaveLength(0);
        controller.manualDegradation(degradationStrategy_1.DegradationLevel.L3_TEMPLATE, 'Test');
        const degradedImpact = controller.getUserImpact();
        expect(degradedImpact.level).toBe(degradationStrategy_1.DegradationLevel.L3_TEMPLATE);
    ...
    test('should track decision history', () => {
        controller.manualDegradation(degradationStrategy_1.DegradationLevel.L1_BACKUP_MODEL, 'Test reason');
        controller.manualDegradation(degradationStrategy_1.DegradationLevel.L2_LOW_COST_MODEL, 'Another reason');
        const history = controller.getDecisionHistory();
        expect(history).toHaveLength(2);
        expect(history[0].reason).toBe('Test reason');
        expect(history[1].level).toBe(degradationStrategy_1.DegradationLevel.L2_LOW_COST_MODEL);
    });
    test('should limit decision history', () => {
        for (let i = 0; i < 110; i++) {
            controller.manualDegradation(degradationStrategy_1.DegradationLevel.L1_BACKUP_MODEL, `Reason ${i}`);
        }
        const history = controller.getDecisionHistory();
    ...
    test('should support manual recovery', () => {
        controller.manualDegradation(degradationStrategy_1.DegradationLevel.L3_TEMPLATE, 'Test');
        expect(controller.getCurrentLevel()).toBe(degradationStrategy_1.DegradationLevel.L3_TEMPLATE);
    ...
    test('should emit events', () => {
        ...
        controller.manualDegradation(degradationStrategy_1.DegradationLevel.L2_LOW_COST_MODEL, 'Test');
        expect(degradedSpy).toHaveBeenCalled();
    });
