...
const DEFAULT_TEMPLATES = [
    ...
    },
    {
        ...
        content: '我理解你的需求是：{{demand_summary}}。由于AI服务暂时受限，我将为你记录原始需求，待服务恢复后进行详细分析。',
    ...
    },
    {
        ...
        scene: 'agentad',
        intent: 'negotiation',
        content: '根据你的需求「{{demand}}」，我找到了以下优惠信息：{{offer_summary}}。当前AI深度分析暂时不可用，以上是基础匹配结果。',
    ...
    },
    {
        ...
        scene: 'agentdate',
        intent: 'match',
        content: '根据你的偏好「{{preferences}}」，我为你找到了一些潜在匹配。由于AI深度分析暂时受限，匹配基于基础条件筛选。',
    ...
    },
    {
        ...
        scene: 'agentjob',
        intent: 'job_search',
        content: '根据你的简历「{{resume_summary}}」，我搜索到以下职位。由于AI深度分析暂时受限，匹配基于关键词筛选。',
...
export class TemplateResponseService extends EventEmitter {
     ...
     */
    match(scene, intent, tags, variables) {
        const candidates = [];
        ...
        if (!sceneTemplates || !intentTemplates)
            return null;
        for (const id of sceneTemplates) {
            if (!intentTemplates.has(id))
                continue;
            const template = this.templates.get(id);
     ...
     */
    renderMatch(scene, intent, variables = {}, tags) {
...
//# sourceMappingURL=templateResponseService.js.map