 ...
 */
function computeSimilarityHash(request) {
    const normalized = request.messages.map(m => `${m.role}:${normalizeText(m.content)}`).join('|');
...
export class AIResponseCacheService extends EventEmitter {
    ...
    async set(key, value, ttlMs) {
        ...
        const entry = {
            ...
            provider: value.model.includes('claude')
                ? 'claude'
                : value.model.includes('ernie')
                    ? 'wenxin'
                    : 'openai',
            model: value.model,
        };
     ...
     */
    async setByRequest(request, response, ttlMs) {
        const key = this.generateKey(request);
