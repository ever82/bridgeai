 ...
 */
import { getRecommendedTemplateForScene } from '../privacy/desensitizationPolicy';
/**
 * Generate comprehensive AI recommendations based on image analysis
 */
export function generateRecommendations(detections, scene, userPreferences) {
 ...
 */
function generateSceneBasedRecommendation(scene, detections) {
 ...
 */
function generateContentBasedRecommendations(detections) {
    const recommendations = [];
 ...
 */
function generatePrivacyLevelRecommendation(detections, currentLevel) {
 ...
 */
function generateManualReviewRecommendation(detections) {
    const criticalDetections = detections.filter((d) => (d.type === 'face' && d.confidence > 0.95) ||
 ...
 */
export function generateOneClickRecommendation(detections, scene) {
 ...
 */
export function getHighlightedRegions(detections) {
 ...
 */
export function generateBatchSuggestion(imageCount, allDetections) {
    ...
    const canAutoProcess = flatDetections.length > 0 &&
 ...
 */
function scoreToPrivacyLevel(score) {
    if (score < 30)
        return 'relaxed';
    if (score < 70)
        return 'standard';
...
//# sourceMappingURL=desensitizationAdvisor.js.map