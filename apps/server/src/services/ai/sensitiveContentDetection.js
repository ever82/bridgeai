 ...
 */
function parseDetectionResponse(response, types, imageWidth, imageHeight) {
    const detections = [];
    try {
        ...
        for (const item of parsed) {
            ...
            detections.push({
                type,
                boundingBox: {
                    x: Math.round(typeof bbox.x === 'number'
                        ? bbox.x
                        : bbox.x_percent
                            ? (bbox.x_percent / 100) * imageWidth
                            : 0),
                    y: Math.round(typeof bbox.y === 'number'
                        ? bbox.y
                        : bbox.y_percent
                            ? (bbox.y_percent / 100) * imageHeight
                            : 0),
                    width: Math.round(typeof bbox.width === 'number'
                        ? bbox.width
                        : bbox.w_percent
                            ? (bbox.w_percent / 100) * imageWidth
                            : 100),
                    height: Math.round(typeof bbox.height === 'number'
                        ? bbox.height
                        : bbox.h_percent
 ...
 */
function buildDetectionPrompt(types) {
 ...
 */
async function runDetectionAttempt(imageBuffer, metadata, imageWidth, imageHeight, timeoutMs, types, minConfidence) {
    ...
    const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('Detection timeout')), timeoutMs));
 ...
 */
export async function detectSensitiveContent(imageBuffer, options = {
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
        try {
            detections = await runDetectionAttempt(cleanedBuffer, metadata, imageWidth, imageHeight, timeoutMs, options.types, options.minConfidence);
            ...
            if (attempt < MAX_RETRIES) {
                console.warn(`[SensitiveContentDetection] Attempt ${attempt + 1} failed: ${lastError.message}. Retrying...`);
            }
        }
    }
    // Graceful degradation: if all attempts failed, return empty detections with warning
    if (detections.length === 0 && lastError !== null) {
        console.warn(`[SensitiveContentDetection] All AI detection attempts failed (${MAX_RETRIES + 1} tries), returning empty results: ${lastError.message}`);
    }
 ...
 */
export async function stripExifFromImage(imageBuffer) {
 ...
 */
export function calculatePrivacyRisk(detections) {
...
//# sourceMappingURL=sensitiveContentDetection.js.map