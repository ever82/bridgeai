...
export class ExtractionValidator {
     ...
     * 验证提取结果
     */
    validate(data, schema, demand, options = {}) {
     ...
     * 字段完整性检查
     */
    validateCompleteness(context, report) {
     ...
     * 业务规则验证
     */
    validateBusinessRules(context, report) {
     ...
     * 格式验证
     */
    validateFormats(context, report) {
        const { data } = context;
        // Email format
     ...
     * Apply custom rules
     */
    applyCustomRules(context, report) {
        // Global custom rules
     ...
     */
    calculateCompletenessScore(data, schema) {
     ...
     */
    determineConfirmationNeeded(report, options) {
     ...
     */
    registerSceneRules(scene, rules) {
     ...
     */
    validateField(fieldId, value, schema) {
...
//# sourceMappingURL=extractionValidator.js.map