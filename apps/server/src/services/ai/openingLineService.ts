...
export interface OpeningLineResult {
  line: string;
  tone: string;
  basedOn: string[]; // 基于哪些匹配亮点
  alternatives: string[]; // 备选开场白
}
...
const INTEREST_BASED_TEMPLATES: Record<string, string[]> = {
  SPORTS: [
...
const PERSONALITY_BASED_TEMPLATES: Record<string, string[]> = {
...
const LIFESTYLE_BASED_TEMPLATES: Record<string, string[]> = {
 ...
 * 根据匹配亮点生成个性化开场白
 */
export function generateOpeningLine(request: OpeningLineRequest): OpeningLineResult {
 ...
 * 基于兴趣生成开场白
 */
function generateInterestBasedLine(matchScore: MatchScore): string | null {
  const interestHighlights = matchScore.highlights.filter(
    h => h.includes('兴趣') || h.includes('共同')
  );

  if (interestHighlights.length === 0) return null;
 ...
 * 基于性格生成开场白
 */
function generatePersonalityBasedLine(matchScore: MatchScore): string | null {
  const personalityHighlights = matchScore.highlights.filter(
    h => h.includes('性格') || h.includes('MBTI') || h.includes('互补')
  );

  if (personalityHighlights.length === 0) return null;
 ...
 * 基于生活方式生成开场白
 */
function generateLifestyleBasedLine(matchScore: MatchScore): string | null {
  const lifestyleHighlights = matchScore.highlights.filter(
    h => h.includes('同城') || h.includes('作息') || h.includes('生活') || h.includes('同省')
  );

  if (lifestyleHighlights.length === 0) return null;
 ...
 * 批量生成多个开场白选项
 */
export function generateOpeningLineOptions(
  request: OpeningLineRequest,
  count: number = 3
): OpeningLineResult[] {
  const results: OpeningLineResult[] = [];
  const tones: Array<'casual' | 'friendly' | 'humorous' | 'sincere'> = [
    'casual',
    'friendly',
    'humorous',
    'sincere',
