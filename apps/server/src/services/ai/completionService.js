...
export class SupplyCompletionService {
    ...
    detectIncomplete(supply) {
        ...
        if (!supply.pricing) {
            ...
            if (!supply.pricing.currency)
                incomplete.push('pricing.currency');
        }
        // Check capabilities
        if (!supply.capabilities || supply.capabilities.length === 0) {
            ...
            for (let i = 0; i < supply.capabilities.length; i++) {
                ...
                if (!cap.level)
                    incomplete.push(`capabilities[${i}].level`);
                if (!cap.category)
                    incomplete.push(`capabilities[${i}].category`);
            }
        }
        // Check skills
        ...
        if (!supply.experience) {
            ...
            if (supply.experience.years === undefined)
                incomplete.push('experience.years');
        }
        // Check availability
     ...
     */
    applySuggestions(supply, suggestions) {
    ...
    inferDescription(supply) {
        const parts = [];
        if (supply.serviceType)
            parts.push(`提供${supply.serviceType}服务`);
    ...
    inferSkillsFromCapabilities(capabilities) {
        const skills = [];
