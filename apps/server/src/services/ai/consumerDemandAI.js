 ...
 */
import { DemandUrgency, } from '@bridgeai/shared';
import { logger } from '../../utils/logger';
import { sceneDetector, AgentAdExtractor, extractionValidator, } from './index';
/**
 ...
 * @returns 提取的结构化需求数据
 */
export async function extractConsumerDemand(text, context) {
 ...
 */
function mapToConsumerDemand(extractionResult, originalText) {
 ...
 * 将产品类别映射为消费者类别 ID
 */
function mapCategoryToConsumerCategory(productCategory) {
    const categoryMap = {
 ...
 * 将购买时间线映射为紧急程度
 */
function mapTimelineToUrgency(timeline) {
 ...
 * @returns 是否需要澄清
 */
export function needsClarification(extractedData) {
 ...
 * @returns 问题列表
 */
export function generateClarificationQuestions(extractedData) {
    const questions = [];
    if (!extractedData.categories || extractedData.categories.length === 0) {
 ...
 * 这是 AI 集成的主要入口
 */
export async function processNaturalLanguageDemand(text, options) {
 ...
 * 生成提取需求的人类可读摘要
 */
function generateDemandSummary(extractedData, originalText) {
    const parts = [];
    if (extractedData.categories && extractedData.categories.length > 0) {
...
//# sourceMappingURL=consumerDemandAI.js.map