...
export class ConversationReportService {
  ...
  private reports: Map<string, ConversationReport> = new Map();
  ...
  async generateReport(
    ...
    try {
      ...
      // 7. 评估兼容性分数
      const compatibilityScore = await this.evaluateCompatibility(
        { id: sessionData.agentAId, userId: sessionData.userIdA, interests: [] },
        { id: sessionData.agentBId, userId: sessionData.userIdB, interests: [] },
        {
...
摘要格式：概括对话主题、参与者的互动感受和氛围。`;
        ...
        logger.warn('LLM摘要生成失败，使用默认摘要', { error: (error as Error)?.message });
      }
    }
  ...
  private async evaluateCompatibility(
    profileA: { id: string; userId: string; interests: string[] },
    profileB: { id: string; userId: string; interests: string[] },
    conversationData: {
      messages: ConversationMessage[];
      topics: TopicSummary[];
      highlights: ConversationHighlight[];
    ...
    // 2. 话题深度加分 (最高15分)
    const avgDepth =
      conversationData.topics.reduce((sum, t) => sum + t.depth, 0) /
   ...
   * 提取话题信息
   */
  private extractTopics(messages: ConversationMessage[]): TopicSummary[] {
    const topicMap = new Map<string, { rounds: number; depth: number; engagement: number }>();
   ...
   * 估算话题深度
   */
  private estimateTopicDepth(content: string): number {
    // 根据消息长度和问题类型估算深度
    const lengthScore = Math.min(1, content.length / 200);

    // 是否包含深度问题的标志
    const hasDeepQuestions =
      /[吗?？]/.test(content) &&
  ...
  private extractHighlights(
    ...
    for (const topic of topics) {
      ...
      const relevantMessages = messages.filter(m =>
        this.detectTopics(m.content).includes(topic.name)
      );
   ...
   * 提取共同兴趣
   */
  private extractSharedInterests(messages: ConversationMessage[]): string[] {
   ...
   * 计算质量指标
   */
  private calculateQualityMetrics(assessments: QualityAssessment[]): {
    fluency: number;
    engagement: number;
    depth: number;
  } {
   ...
   * 生成对话建议
   */
  private generateSuggestions(report: Partial<ConversationReport>): string[] {
   ...
   * 获取报告
   */
  async getReport(reportId: string): Promise<ConversationReport | null> {
    // 先从内存获取
   ...
   * 根据用户ID获取报告列表
   */
  async getReportsByUser(userId: string): Promise<ConversationReport[]> {
    const userReports: ConversationReport[] = [];
   ...
   * 持久化报告
   */
  private async persistReport(report: ConversationReport): Promise<void> {
    // 在生产环境中应保存到数据库
    // 此处暂时保存在内存中
    this.reports.set(report.id, report);
   ...
   * 从数据库加载报告
   */
  private async loadReport(_reportId: string): Promise<ConversationReport | null> {
    // 在生产环境中应从数据库加载
    // 此处暂时返回null
    return null;
  }
