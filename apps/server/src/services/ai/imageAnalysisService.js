...
export class ImageAnalysisService {
     ...
     * 批量分析多张图像
     */
    async analyzeBatch(images, context) {
        const results = [];
        // 限制并发数
        const concurrency = 3;
        for (let i = 0; i < images.length; i += concurrency) {
            const batch = images.slice(i, i + concurrency);
            const batchPromises = batch.map(image => this.analyze(image, context).catch(error => {
     ...
     * 提取视觉特征
     */
    async extractVisualFeatures(image) {
...
[
  ...
  {"label": "car", "confidence": 0.87}
     ...
     * 生成场景描述
     */
    async generateDescription(image, options) {
...
Focus on: scene setting, main subjects, activities, mood/atmosphere.`;
     ...
     * 提取活动标签
     */
    async extractActivityTags(image) {
...
- Be specific and accurate in your analysis`;
     ...
     * 规范化视觉特征
     */
    normalizeVisualFeatures(features) {
        const clamp = (val, min = 0, max = 100) => Math.max(min, Math.min(max, Number(val) || 50));
        return {
...
//# sourceMappingURL=imageAnalysisService.js.map