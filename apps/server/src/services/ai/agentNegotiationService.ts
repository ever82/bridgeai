...
参与的商家：
${room.merchantAgents.map(m => `- ${m.name} (${(m.profile as any)?.businessType || '商家'})`).join('\n')}
...
商家信息：
- 名称：${merchant.name}
${(merchant.profile as any)?.businessType ? `- 类型：${(merchant.profile as any).businessType}` : ''}
${(merchant.profile as any)?.rating ? `- 评分：${(merchant.profile as any).rating}` : ''}
