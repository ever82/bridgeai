...
export class VisionShareDemandRefinementService {
    logger = logger.child({ module: 'VisionShareDemandRefinement' });
    /**
     * 提炼需求描述
     * 解析自然语言描述，提取关键信息，生成结构化数据
     */
    async refineDemand(rawDescription, userId, options) {
        ...
        try {
            ...
            // 解析LLM响应
            const refinementResult = this.parseRefinementResponse(llmResponse.content, rawDescription);
            // 记录指标
     ...
     * 生成提炼提示词
     */
    buildRefinementPrompt(description, language = 'zh') {
...
4. 改进建议应该具体且可操作`;
     ...
     * 解析LLM响应
     */
    parseRefinementResponse(response, rawDescription) {
        try {
            // 提取JSON部分
     ...
     * 生成降级结果
     */
    createFallbackResult(rawDescription) {
        return {
     ...
     * 生成智能标签
     */
    async generateTags(description, category, options) {
        const provider = options?.provider || 'claude';
        try {
...
//# sourceMappingURL=visionShareDemandRefinement.js.map