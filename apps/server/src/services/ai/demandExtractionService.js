...
export class DemandExtractionService {
    ...
    async extract(request, options = {}) {
        ...
        try {
            ...
            const { clarificationNeeded, questions } = this.checkClarificationNeeded(structured, intent, opts.requireClarification);
            ...
            const demand = {
                ...
                metadata: {
                    ...
                    provider: 'openai', // Will be set from actual LLM response
                    model: 'gpt-4',
     ...
     */
    buildIntentClassificationPrompt(text, context) {
...
Respond with ONLY the JSON object.`;
     ...
     */
    parseEntities(responseText, originalText) {
        try {
     ...
     */
    buildStructuredDemand(entities, rawText) {
        const structured = {
            title: undefined,
            description: rawText,
            location: {},
     ...
     */
    processLocationEntity(entity, location) {
     ...
     */
    processPersonEntity(entity, people) {
        const value = entity.normalizedValue?.toString() || entity.value;
     ...
     */
    processBudgetEntity(entity, budget) {
     ...
     */
    checkClarificationNeeded(structured, intent, requireClarification) {
        const questions = [];
     ...
     */
    calculateConfidence(intent, entities, structured) {
        const scores = [];
        // Intent confidence
     ...
     */
    async recordMetrics(demand, startTime) {
        await metricsService.recordRequest({
...
//# sourceMappingURL=demandExtractionService.js.map