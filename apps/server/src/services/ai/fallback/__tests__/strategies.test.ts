...
function makeModel(overrides: Record<string, any> & { id: string; provider: string }): ModelInfo {
  return {
    name: overrides.id,
...
class MockCache implements ResponseCache {
  ...
  async set(key: string, value: ChatCompletionResponse, _ttlMs?: number): Promise<void> {
  ...
  // Test helper
  preload(request: ChatCompletionRequest, response: ChatCompletionResponse): void {
...
describe('ModelDowngradeStrategy', () => {
  let strategy: ModelDowngradeStrategy;

  beforeEach(() => {
    strategy = new ModelDowngradeStrategy();
  });
  ...
  it('should fail when current model is not in registry', async () => {
    const result = await strategy.execute(
      makeRequest({ model: 'unknown-model' }),
      new Error('fail'),
      makeContext()
    );
...
describe('ProviderSwitchStrategy', () => {
  let strategy: ProviderSwitchStrategy;

  beforeEach(() => {
    strategy = new ProviderSwitchStrategy();
  });
  ...
  it('should fail when no alternative providers are available', async () => {
    const result = await strategy.execute(
      makeRequest(),
      new Error('fail'),
      makeContext({ availableProviders: ['openai'] })
    );
    expect(result.success).toBe(false);
  ...
  it('should fail when alternative providers have no suitable models', async () => {
    ...
    models.set('claude-nochat', makeModel({ id: 'claude-nochat', provider: 'claude', capabilities: { chatCompletion: false, embeddings: false, streaming: false, maxTokens: 0, supportedLanguages: [] } } as any));
    const result = await strategy.execute(
      makeRequest(),
      new Error('fail'),
      makeContext({ models })
...
describe('SimplifiedOutputStrategy', () => {
  let strategy: SimplifiedOutputStrategy;

  beforeEach(() => {
    strategy = new SimplifiedOutputStrategy();
  });
  ...
  it('should fail when model not found for token error', async () => {
    const result = await strategy.execute(
      makeRequest({ model: 'unknown-model' }),
      new Error('token error occurred'),
      makeContext()
    );
    expect(result.success).toBe(false);
    expect(result.message).toContain('not found');
...
describe('AsyncQueueFallbackStrategy', () => {
  let strategy: AsyncQueueFallbackStrategy;

  beforeEach(() => {
    strategy = new AsyncQueueFallbackStrategy();
  });
