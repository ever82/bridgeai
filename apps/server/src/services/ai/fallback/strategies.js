 ...
 * 切换到更便宜的模型
 */
export class ModelDowngradeStrategy {
    name = 'model-downgrade';
    async execute(originalRequest, error, context) {
        ...
        const cheaperModels = Array.from(context.models.values())
            ...
            .sort((a, b) => (a.costPer1KTokens.input + a.costPer1KTokens.output) -
 ...
 * 切换到备用提供商
 */
export class ProviderSwitchStrategy {
    ...
    async execute(originalRequest, error, context) {
        ...
        // 获取其他可用提供商
        const otherProviders = context.availableProviders.filter(p => p !== currentModel.provider);
 ...
 * 使用更简单的提示或限制token
 */
export class SimplifiedOutputStrategy {
 ...
 * 将请求加入异步队列稍后处理
 */
export class AsyncQueueFallbackStrategy {
    name = 'async-queue';
    queue = [];
    maxQueueSize;
    ...
    async execute(originalRequest, _error, _context) {
 ...
 * 创建包含常用降级策略的链
 */
export function createDefaultFallbackChain(cache) {
