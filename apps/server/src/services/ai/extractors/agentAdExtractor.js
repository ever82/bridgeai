...
export class AgentAdExtractor extends BaseSceneExtractor {
     ...
     */
    extractProductInfo(text, entities) {
        const product = {};
        // Extract product name
        const productPatterns = [
            { pattern: /iPhone\s*(\d+(?:\s+(?:Pro(?:\s+Max)?|Plus|Mini))?)/i, name: (m) => `iPhone ${m[1]}` },
            { pattern: /小米\s*(\d+)/, name: (m) => `小米${m[1]}` },
            { pattern: /华为\s*([\u4e00-\u9fa5\w]+)/, name: (m) => `华为${m[1]}` },
            { pattern: /MacBook\s*(Pro|Air)?/i, name: (m) => `MacBook${m[1] ? ' ' + m[1] : ''}` },
        ];
     ...
     */
    extractBrandPreferences(text, entities) {
        const brands = [];
        const brandPatterns = [
     ...
     */
    extractPlatform(text, _entities) {
        const platforms = [];
...
//# sourceMappingURL=agentAdExtractor.js.map