...
4. Return ONLY the JSON object"`;
    ...
    buildAgentDateDemand(text, data) {
        const entities = [];
        if (data.location) {
            entities.push({
                type: 'location',
        ...
        if (data.meetingTime) {
            entities.push({
                ...
                endIndex: text.indexOf(data.meetingTime) >= 0
                    ? text.indexOf(data.meetingTime) + data.meetingTime.length
                    : 0,
                confidence: 0.8,
            });
        }
        ...
        demand.structured = {
            ...
            constraints: [
                ...(data.ageRange?.min
                    ? [`年龄: ${data.ageRange.min}-${data.ageRange.max || '不限'}`]
                    : []),
                ...(data.heightRange?.min
                    ? [`身高: ${data.heightRange.min}-${data.heightRange.max || '不限'}cm`]
                    : []),
            ].filter(Boolean),
        };
