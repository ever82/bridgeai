...
5. Return ONLY the JSON object"`;
    ...
    buildVisionShareDemand(text, data) {
        const entities = [];
        ...
        if (data.photoType) {
            entities.push({
                ...
                endIndex: text.indexOf(data.photoType) >= 0
                    ? text.indexOf(data.photoType) + data.photoType.length
                    : 0,
                confidence: 0.9,
            });
        }
        ...
        demand.structured = {
            ...
            people: {},
            budget: data.budget || {},
            requirements: [...(data.modelRequirements || []), ...(data.photographerRequirements || [])],
            preferences: [
                data.style || '',
                ...(data.equipmentNeeds || []),
    ...
    calculateConfidence(data) {
        const fields = [data.photoType, data.shootDate, data.budget?.max, data.location];
...
//# sourceMappingURL=visionShare.js.map