...
import { BaseSceneExtractor } from './base';
...
4. Return ONLY the JSON object"`;
    ...
    buildAgentAdDemand(text, data) {
        ...
        if (data.productName) {
            entities.push({
                ...
                endIndex: text.indexOf(data.productName) >= 0
                    ? text.indexOf(data.productName) + data.productName.length
                    : 0,
                confidence: 0.95,
            });
        }
        ...
        if (data.targetLocation) {
            entities.push({
                ...
                endIndex: text.indexOf(data.targetLocation) >= 0
                    ? text.indexOf(data.targetLocation) + data.targetLocation.length
                    : 0,
                confidence: 0.85,
            });
        }
        ...
        demand.structured = {
            ...
            constraints: [...(data.productCategory ? [`产品类别: ${data.productCategory}`] : [])].filter(Boolean),
        };
    ...
    generateQuestions(missingFields) {
        ...
        return missingFields.map(field => fieldToQuestion[field]).filter(Boolean);
    }
}
// Export singleton instance
export const agentAdExtractor = new AgentAdExtractor();
//# sourceMappingURL=agentAd.js.map