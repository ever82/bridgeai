...
4. Return ONLY the JSON object"`;
  ...
  private buildAgentJobDemand(text: string, data: Partial<AgentJobFields>): Partial<Demand> {
    ...
    if (data.position) {
      entities.push({
        ...
        endIndex:
          text.indexOf(data.position) >= 0 ? text.indexOf(data.position) + data.position.length : 0,
        confidence: 0.95,
      });
    }
    ...
    if (data.location) {
      entities.push({
        ...
        endIndex:
          text.indexOf(data.location) >= 0 ? text.indexOf(data.location) + data.location.length : 0,
        confidence: 0.9,
      });
    }
    ...
    demand.structured = {
      ...
      constraints: [
        ...(data.experienceYears?.min
          ? [`经验: ${data.experienceYears.min}-${data.experienceYears.max || '不限'}年`]
          : []),
      ].filter(Boolean),
    };
  ...
  private generateQuestions(missingFields: string[], jobType?: string): string[] {
    const fieldToQuestion: Record<string, string> = {
      ...
      'structured.budget.max':
        jobType === 'hire' ? '请问提供的薪资范围是多少？' : '请问您的期望薪资是多少？',
    };

    return missingFields.map(field => fieldToQuestion[field]).filter(Boolean) as string[];
  }
}
