...
export class VisionShareExtractor extends BaseSceneExtractor {
    ...
    requiredFields = ['photographyType'];
    optionalFields = [
        'photographyTime',
        'budget',
        'location',
     ...
     */
    extractVisionShareEntities(text) {
        const patterns = {
            photographyType: [
     ...
     */
    extractPhotographyType(text, entities) {
        const types = [];
        const typePatterns = [
     ...
     */
    extractPhotographyTime(text, _entities) {
     ...
     */
    extractRequirements(text, _entities) {
        const requirements = [];
     ...
     */
    extractPhotographerPreferences(text) {
        const preferences = {};
        ...
        const equipment = [];
        if (/佳能|Canon/.test(text))
            equipment.push('佳能');
        if (/尼康|Nikon/.test(text))
            equipment.push('尼康');
        if (/索尼|Sony/.test(text))
            equipment.push('索尼');
        if (/富士|Fujifilm/.test(text))
            equipment.push('富士');
...
//# sourceMappingURL=visionShareExtractor.js.map