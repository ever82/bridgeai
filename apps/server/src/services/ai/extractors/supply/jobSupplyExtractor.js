 ...
 */
export class JobSupplyExtractor extends BaseSupplyExtractor {
    sceneTypeValue = 'agentjob';
    detectionKeywords = [
        '求职',
        '找工作',
        '应聘',
    ...
    optionalFields = ['expectations', 'education', 'qualification'];
    getSceneType() {
    ...
    async extract(text, _context) {
        ...
        if (skills.technical.length > 0 || skills.soft.length > 0)
            extractedFields++;
        if (experience.totalYears > 0 || experience.roles.length > 0)
            extractedFields++;
        if (expectations.jobTypes.length > 0)
            extractedFields++;
        if (education)
            extractedFields++;
        const qualityMetrics = this.calculateQualityMetrics(extractedFields, totalFields, expectations.salaryRange !== undefined, experience.companies.length > 0);
    ...
    extractExperience(text) {
        ...
        const rolePatterns = [
            ...
            { pattern: /(产品经理|产品总监)/, role: (m) => m[1] },
            { pattern: /(UI|UX)\s*(?:设计师|设计)/, role: (m) => m[1] + '设计师' },
            { pattern: /(项目经理|技术经理|技术总监|架构师|CTO)/, role: (m) => m[1] },
            {
    ...
    extractEducation(text) {
        ...
        let school;
        // Degree
        if (/博士|PhD|Ph\.D/i.test(text))
            degree = '博士';
        else if (/硕士|研究生|Master/i.test(text))
            degree = '硕士';
        else if (/本科|学士|Bachelor/i.test(text))
            degree = '本科';
        else if (/大专|专科|Associate/i.test(text))
            degree = '大专';
        else
            return undefined;
        // Major
     ...
     */
    extractQualification(text, skills, experience) {
     ...
     */
    calculateConfidence(skills, experience, expectations) {
        let score = 0.3;
...
//# sourceMappingURL=jobSupplyExtractor.js.map