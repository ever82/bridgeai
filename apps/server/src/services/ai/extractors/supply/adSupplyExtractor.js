 ...
 */
export class AdSupplyExtractor extends BaseSupplyExtractor {
    sceneTypeValue = 'agentad';
    detectionKeywords = [
        '出售',
        '售卖',
        '供货',
    ...
    optionalFields = ['offers', 'business', 'qualification'];
    getSceneType() {
    ...
    async extract(text, _context) {
        ...
        if (offers.length > 0)
            extractedFields++;
        if (business.name || business.platforms.length > 0)
            extractedFields++;
        const qualityMetrics = this.calculateQualityMetrics(extractedFields, totalFields, products.some(p => p.pricing !== undefined), products.some(p => p.features && p.features.length > 0));
    ...
    extractCondition(text) {
        if (/全新|new|未拆封|全新未使用|新品/.test(text))
            return 'new';
        if (/二手|used|旧|pre-owned/.test(text))
            return 'used';
        if (/官翻|翻新|refurbished/.test(text))
            return 'refurbished';
        return 'new';
    }
    /**
     ...
     */
    extractQualification(text, products) {
        const certifications = [];
        const specializations = [];
        // Business certifications
        if (/品牌授权|授权代理/.test(text))
            certifications.push('品牌授权');
        if (/官方认证|平台认证/.test(text))
            certifications.push('官方认证');
        if (/营业执照/.test(text))
            certifications.push('营业执照');
        if (/食品经营许可证/.test(text))
            certifications.push('食品经营许可');
        if (/3C认证/.test(text))
            certifications.push('3C认证');
     ...
     */
    calculateConfidence(products, offers, business) {
        let score = 0.3;
        if (products.length > 0)
            score += 0.3;
        if (products.some(p => p.pricing))
            score += 0.1;
        if (offers.length > 0)
            score += 0.1;
        if (business.name)
            score += 0.1;
        if (business.platforms.length > 0)
            score += 0.1;
...
//# sourceMappingURL=adSupplyExtractor.js.map