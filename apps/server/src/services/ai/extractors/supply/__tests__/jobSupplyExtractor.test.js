...
describe('JobSupplyExtractor', () => {
    let extractor;
    beforeEach(() => {
        extractor = new jobSupplyExtractor_1.JobSupplyExtractor();
    });
    ...
    describe('extract', () => {
        it('should extract technical skills', async () => {
            const result = await extractor.extract('精通Java、Python、React，熟悉Docker和Kubernetes');
        ...
        it('should extract soft skills', async () => {
            const result = await extractor.extract('具有良好的沟通能力和团队协作精神，有项目管理经验');
        ...
        it('should extract salary expectations', async () => {
            const result = await extractor.extract('期望薪资15K-25K/月');
        ...
        it('should extract job type preferences', async () => {
            const result = await extractor.extract('求职全职或远程工作');
        ...
        it('should extract education', async () => {
            const result = await extractor.extract('本科毕业，计算机科学专业，985大学');
            expect(result.education).toBeDefined();
            expect(result.education?.degree).toBe('本科');
        });
        it('should build qualification', async () => {
            const result = await extractor.extract('8年互联网经验，PMP认证，擅长项目管理和团队协作');
        ...
        it('should calculate confidence', async () => {
            const result = await extractor.extract('5年Java开发经验，精通Spring Boot，期望薪资20K-30K，求职后端开发');
            expect(result.confidence).toBeGreaterThan(0.5);
        });
    });
    describe('validate', () => {
        it('should validate data with skills and experience', async () => {
            const data = await extractor.extract('3年Python开发经验，擅长数据分析和机器学习');
...
//# sourceMappingURL=jobSupplyExtractor.test.js.map