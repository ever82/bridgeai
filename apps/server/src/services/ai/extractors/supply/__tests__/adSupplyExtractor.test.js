...
describe('AdSupplyExtractor', () => {
    let extractor;
    beforeEach(() => {
        extractor = new adSupplyExtractor_1.AdSupplyExtractor();
    });
    ...
    describe('extract', () => {
        it('should extract named products', async () => {
            const result = await extractor.extract('出售iPhone 15 Pro 256G，全新正品，价格7999元');
        ...
        it('should extract product categories', async () => {
            const result = await extractor.extract('专营各类运动鞋和服装，大量现货');
            expect(result.products.length).toBeGreaterThan(0);
        ...
        it('should extract pricing', async () => {
            const result = await extractor.extract('出售全新笔记本，价格5000-8000元');
            expect(result.products.some(p => p.pricing !== undefined)).toBe(true);
        });
        it('should extract product features', async () => {
            const result = await extractor.extract('出售正品手表，包邮，7天无理由退换，提供发票');
        ...
        it('should extract offers/discounts', async () => {
            const result = await extractor.extract('全场8折优惠，满200减30，包邮，买就送手机壳');
        ...
        it('should extract business information', async () => {
            const result = await extractor.extract('华为官方旗舰店，品牌授权，天猫/京东均有店铺');
        ...
        it('should extract business location', async () => {
            const result = await extractor.extract('深圳华强北实体店铺，主营手机和数码产品');
            expect(result.business.location).toBeDefined();
        });
        it('should build qualification', async () => {
            const result = await extractor.extract('品牌授权经销商，3C认证，营业执照齐全');
        ...
        it('should calculate confidence', async () => {
            const result = await extractor.extract('出售全新iPhone 15，8折优惠，全国包邮，苹果旗舰店');
            expect(result.confidence).toBeGreaterThan(0.5);
        });
        it('should include quality metrics', async () => {
            const result = await extractor.extract('供应各类手机配件，价格优惠');
    ...
    describe('validate', () => {
        it('should validate data with products', async () => {
            const data = await extractor.extract('出售各类数码产品，大量现货');
...
//# sourceMappingURL=adSupplyExtractor.test.js.map