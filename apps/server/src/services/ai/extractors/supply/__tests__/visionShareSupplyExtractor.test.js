...
describe('VisionShareSupplyExtractor', () => {
    let extractor;
    beforeEach(() => {
        extractor = new visionShareSupplyExtractor_1.VisionShareSupplyExtractor();
    });
    ...
    describe('extract', () => {
        it('should extract equipment information', async () => {
            const result = await extractor.extract('我是专业摄影师，使用佳能5D4和索尼A7M4，有85mm定焦和24-70变焦镜头');
        ...
        it('should extract experience information', async () => {
            const result = await extractor.extract('5年+婚礼摄影经验，擅长人像写真和婚纱摄影');
        ...
        it('should extract style information', async () => {
            const result = await extractor.extract('我的拍摄风格是自然纪实风格，也擅长唯美浪漫风格');
        ...
        it('should extract pricing information', async () => {
            const result = await extractor.extract('人像写真收费500-2000元，婚礼摄影3000-8000元起');
            expect(result.pricing).toBeDefined();
        });
        it('should extract availability', async () => {
            const result = await extractor.extract('周末可约拍，也可工作日晚上拍摄，支持旅拍');
        ...
        it('should build qualification', async () => {
            const result = await extractor.extract('资深摄影师8年经验，专业摄影师认证');
        ...
        it('should calculate confidence', async () => {
            const result = await extractor.extract('8年人像摄影经验，使用佳能相机，擅长自然纪实风格，人像写真500元起');
            expect(result.confidence).toBeGreaterThan(0.5);
        });
        it('should include quality metrics', async () => {
            const result = await extractor.extract('专业摄影师5年经验，商业摄影作品丰富');
    ...
    describe('validate', () => {
        it('should validate complete data', async () => {
            const data = await extractor.extract('8年婚礼摄影经验，擅长唯美浪漫风格，使用佳能5D4');
...
//# sourceMappingURL=visionShareSupplyExtractor.test.js.map