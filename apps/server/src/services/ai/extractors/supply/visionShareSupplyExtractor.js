 ...
 */
export class VisionShareSupplyExtractor extends BaseSupplyExtractor {
    sceneTypeValue = 'visionshare';
    detectionKeywords = [
        '摄影师',
        '摄影服务',
        '拍照服务',
    ...
    requiredFields = ['experience', 'style'];
    optionalFields = ['equipment', 'pricing', 'availability', 'qualification'];
    getSceneType() {
    ...
    async extract(text, _context) {
        ...
        const totalFields = 5;
        if (equipment.cameras.length > 0 || equipment.lenses.length > 0)
            extractedFields++;
        if (experience.years > 0 || experience.photographyTypes.length > 0)
            extractedFields++;
        if (style.primary)
            extractedFields++;
        if (pricing)
        ...
        if (availability)
            extractedFields++;
        const qualityMetrics = this.calculateQualityMetrics(extractedFields, totalFields, pricing !== undefined, experience.portfolio.length > 0);
    ...
    extractEquipment(text) {
        ...
        const cameraPatterns = [
            { pattern: /佳能|Canon\s*(\w+)?/gi, name: (m) => m[0] },
            { pattern: /尼康|Nikon\s*(\w+)?/gi, name: (m) => m[0] },
            { pattern: /索尼|Sony\s*(\w+)?/gi, name: (m) => m[0] },
            { pattern: /富士|Fujifilm\s*(\w+)?/gi, name: (m) => m[0] },
            { pattern: /徕卡|Leica\s*(\w+)?/gi, name: (m) => m[0] },
            { pattern: /哈苏|Hasselblad/gi, name: (m) => m[0] },
        ...
        if (/反光板/.test(text))
            lighting.push('反光板');
        if (/补光灯|LED/.test(text))
            lighting.push('补光灯');
        if (/影棚灯|摄影灯/.test(text))
            lighting.push('影棚灯');
        // Other equipment
        if (/无人机|航拍|大疆|DJI/.test(text))
            other.push('无人机');
        if (/稳定器|云台/.test(text))
            other.push('稳定器');
        if (/三脚架|脚架/.test(text))
    ...
    extractExperience(text) {
        ...
        if (portfolioMatch) {
            portfolio.push(...portfolioMatch[1]
                .split(/[,，、]/)
                .map(s => s.trim())
                .filter(Boolean));
        }
        ...
        if (projectMatch) {
            notableProjects.push(...projectMatch[1]
                .split(/[,，、]/)
                .map(s => s.trim())
                .filter(Boolean));
        }
    ...
    extractStyle(text) {
        ...
        if (/HDR/.test(text))
            techniques.push('HDR');
        if (/延时/.test(text))
            techniques.push('延时摄影');
        if (/慢门|长曝光/.test(text))
            techniques.push('长曝光');
        if (/双重曝光/.test(text))
            techniques.push('双重曝光');
        if (/航拍/.test(text))
            techniques.push('航拍');
        if (/全景/.test(text))
            techniques.push('全景');
     ...
     */
    extractQualification(text, experience, style) {
        const certifications = [];
        const specializations = [];
        // Certifications
        if (/专业摄影师|高级摄影师/.test(text))
            certifications.push('专业摄影师');
        if (/PS|Photoshop/.test(text))
            certifications.push('Photoshop');
        if (/Lightroom|LR/.test(text))
            certifications.push('Lightroom');
        if (/摄影协会|PSA/.test(text))
            certifications.push('摄影协会会员');
        // Specializations from photography types and style
        specializations.push(...experience.photographyTypes);
        if (style.primary)
            specializations.push(style.primary);
        return this.buildQualification(text, certifications, specializations);
    }
    /**
     * Calculate overall confidence
     */
    calculateConfidence(experience, style, equipment) {
        let score = 0.3;
        if (experience.years > 0)
            score += 0.15;
        if (experience.photographyTypes.length > 0)
            score += 0.15;
        if (style.primary)
            score += 0.15;
        if (equipment.cameras.length > 0)
            score += 0.1;
        if (equipment.lenses.length > 0)
            score += 0.05;
        if (experience.portfolio.length > 0)
            score += 0.1;
...
//# sourceMappingURL=visionShareSupplyExtractor.js.map