 ...
 */
import { PersonalityTrait, DatingPurpose, } from '@bridgeai/shared';
...
export class AgentDateExtractor extends BaseSceneExtractor {
     ...
     */
    buildStructuredData(text, entities) {
        const structured = {};
     ...
     */
    extractPartnerPreferences(text, _entities) {
        const preferences = {};
        // Extract age range
     ...
     */
    extractInterests(text, _entities) {
        const interests = [];
     ...
     */
    calculateConfidence(entities, structured) {
     ...
     */
    generateIceBreakingMessage(ownerName, partnerProfile) {
        const greetings = [
            '你好呀，很高兴认识你！',
            '你好！今天过得怎么样？',
            '嗨，你好！终于有机会认识你了～',
     ...
     */
    evaluatePersonalityMatch(ownerTraits, partnerTraits, ownerInterests, partnerInterests, datingPurpose) {
        ...
        const totalScore = Math.round((traitScore * 0.4 + interestScore * 0.35 + mbtiScore * 0.25) * purposeWeight);
        ...
        const matchLevel = totalScore >= 75 ? 'high' :
     ...
     */
    scoreTraitCompatibility(ownerTraits, partnerTraits) {
        if (ownerTraits.length === 0 || partnerTraits.length === 0)
            return 50;
     ...
     */
    scoreInterestOverlap(ownerInterests, partnerInterests) {
        if (ownerInterests.length === 0 && partnerInterests.length === 0)
            return 50;
        if (ownerInterests.length === 0 || partnerInterests.length === 0)
            return 30;
     ...
     */
    scoreMbtiComplement(ownerTraits, partnerTraits) {
        const isIntrovert = (traits) => traits.includes(PersonalityTrait.INTROVERTED);
        const isExtrovert = (traits) => traits.includes(PersonalityTrait.EXTROVERTED);
        // Same energy type gets a decent baseline
        if ((isIntrovert(ownerTraits) && isIntrovert(partnerTraits)) ||
            (isExtrovert(ownerTraits) && isExtrovert(partnerTraits))) {
            return 70;
        }
        // Complementary personalities can be great for relationships
        if ((isIntrovert(ownerTraits) && isExtrovert(partnerTraits)) ||
     ...
     */
    getPurposeWeight(datingPurpose) {
     ...
     */
    generateMatchAdvice(score, sharedTraits, sharedInterests, matchLevel) {
        if (matchLevel === 'high') {
...
//# sourceMappingURL=agentDateExtractor.js.map