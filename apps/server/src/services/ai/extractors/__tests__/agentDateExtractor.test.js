...
describe('AgentDateExtractor', () => {
    let extractor;
    beforeEach(() => {
        extractor = new agentDateExtractor_1.AgentDateExtractor();
    });
    describe('canHandle', () => {
    ...
    describe('extract', () => {
        ...
        it('should calculate confidence', async () => {
            const result = await extractor.extract('想找25-30岁的对象，本科以上，喜欢旅游和电影');
            expect(result.confidence).toBeGreaterThan(0.5);
        });
    });
    ...
    describe('evaluatePersonalityMatch', () => {
        ...
        it('should score high when shared traits exist', () => {
            const result = extractor.evaluatePersonalityMatch([shared_1.PersonalityTrait.OPTIMISTIC, shared_1.PersonalityTrait.HUMOROUS], [shared_1.PersonalityTrait.OPTIMISTIC, shared_1.PersonalityTrait.HUMOROUS], [], []);
            expect(result.traitScore).toBe(100);
            expect(result.sharedTraitsCount).toBe(2);
        });
        it('should score lower when no shared traits', () => {
            const result = extractor.evaluatePersonalityMatch([shared_1.PersonalityTrait.INTROVERTED, shared_1.PersonalityTrait.RATIONAL], [shared_1.PersonalityTrait.EXTROVERTED, shared_1.PersonalityTrait.EMOTIONAL], [], []);
            expect(result.traitScore).toBe(0);
            expect(result.sharedTraitsCount).toBe(0);
        });
        it('should score interest overlap correctly', () => {
            const result = extractor.evaluatePersonalityMatch([], [], [shared_1.InterestCategory.MOVIES, shared_1.InterestCategory.MUSIC, shared_1.InterestCategory.TRAVEL], [shared_1.InterestCategory.MOVIES, shared_1.InterestCategory.MUSIC]);
        ...
        it('should give MBTI complement bonus for introvert-extrovert pairs', () => {
            const introvert = [shared_1.PersonalityTrait.INTROVERTED];
            const extrovert = [shared_1.PersonalityTrait.EXTROVERTED];
            const same = [shared_1.PersonalityTrait.EXTROVERTED];
        ...
        it('should set high matchLevel for score >= 75', () => {
            const result = extractor.evaluatePersonalityMatch([shared_1.PersonalityTrait.OPTIMISTIC, shared_1.PersonalityTrait.HUMOROUS, shared_1.PersonalityTrait.GENTLE], [shared_1.PersonalityTrait.OPTIMISTIC, shared_1.PersonalityTrait.HUMOROUS, shared_1.PersonalityTrait.GENTLE], [shared_1.InterestCategory.TRAVEL, shared_1.InterestCategory.MOVIES], [shared_1.InterestCategory.TRAVEL, shared_1.InterestCategory.MOVIES, shared_1.InterestCategory.MUSIC]);
        ...
        it('should set medium matchLevel for score 45-74', () => {
            ...
            const result = extractor.evaluatePersonalityMatch([shared_1.PersonalityTrait.INTROVERTED], [shared_1.PersonalityTrait.EXTROVERTED], [shared_1.InterestCategory.TRAVEL], [shared_1.InterestCategory.TRAVEL]);
        ...
        it('should set low matchLevel for score < 45', () => {
            const result = extractor.evaluatePersonalityMatch([shared_1.PersonalityTrait.RATIONAL], [shared_1.PersonalityTrait.EXTROVERTED], [], []);
            expect(result.matchLevel).toBe('low');
            expect(result.overallScore).toBeLessThan(45);
        });
        it('should apply marriage purpose weight', () => {
            const baseResult = extractor.evaluatePersonalityMatch([shared_1.PersonalityTrait.OPTIMISTIC], [shared_1.PersonalityTrait.OPTIMISTIC], [], []);
            const marriageResult = extractor.evaluatePersonalityMatch([shared_1.PersonalityTrait.OPTIMISTIC], [shared_1.PersonalityTrait.OPTIMISTIC], [], [], shared_1.DatingPurpose.MARRIAGE);
        ...
        it('should include match advice for all levels', () => {
            const high = extractor.evaluatePersonalityMatch([shared_1.PersonalityTrait.OPTIMISTIC, shared_1.PersonalityTrait.HUMOROUS], [shared_1.PersonalityTrait.OPTIMISTIC, shared_1.PersonalityTrait.HUMOROUS], [shared_1.InterestCategory.TRAVEL], [shared_1.InterestCategory.TRAVEL]);
            expect(high.advice).toContain('匹配度很高');
            const low = extractor.evaluatePersonalityMatch([shared_1.PersonalityTrait.RATIONAL], [shared_1.PersonalityTrait.EXTROVERTED], [], []);
            expect(low.advice).toBeDefined();
            expect(low.advice.length).toBeGreaterThan(0);
        });
        it('should cap overall score at 100', () => {
            const result = extractor.evaluatePersonalityMatch([shared_1.PersonalityTrait.OPTIMISTIC], [shared_1.PersonalityTrait.OPTIMISTIC], [shared_1.InterestCategory.TRAVEL, shared_1.InterestCategory.MOVIES, shared_1.InterestCategory.MUSIC], [shared_1.InterestCategory.TRAVEL, shared_1.InterestCategory.MOVIES, shared_1.InterestCategory.MUSIC], shared_1.DatingPurpose.MARRIAGE);
        ...
        it('should correctly identify shared traits and interests', () => {
            const ownerTraits = [
                shared_1.PersonalityTrait.OPTIMISTIC,
                shared_1.PersonalityTrait.HUMOROUS,
                shared_1.PersonalityTrait.INDEPENDENT,
            ];
            const partnerTraits = [
                shared_1.PersonalityTrait.OPTIMISTIC,
                shared_1.PersonalityTrait.GENTLE,
                shared_1.PersonalityTrait.INDEPENDENT,
            ];
            const ownerInterests = [
                shared_1.InterestCategory.TRAVEL,
                shared_1.InterestCategory.MOVIES,
                shared_1.InterestCategory.MUSIC,
            ];
            const partnerInterests = [
                shared_1.InterestCategory.TRAVEL,
                shared_1.InterestCategory.GAMING,
                shared_1.InterestCategory.MUSIC,
            ];
            const result = extractor.evaluatePersonalityMatch(ownerTraits, partnerTraits, ownerInterests, partnerInterests);
            expect(result.sharedTraits).toContain(shared_1.PersonalityTrait.OPTIMISTIC);
            expect(result.sharedTraits).toContain(shared_1.PersonalityTrait.INDEPENDENT);
            expect(result.sharedTraits).not.toContain(shared_1.PersonalityTrait.HUMOROUS);
            expect(result.sharedTraits).not.toContain(shared_1.PersonalityTrait.GENTLE);
            expect(result.sharedTraitsCount).toBe(2);
            expect(result.sharedInterests).toContain(shared_1.InterestCategory.TRAVEL);
            expect(result.sharedInterests).toContain(shared_1.InterestCategory.MUSIC);
            expect(result.sharedInterests).not.toContain(shared_1.InterestCategory.MOVIES);
            expect(result.sharedInterestsCount).toBe(2);
        });
    });
});
//# sourceMappingURL=agentDateExtractor.test.js.map