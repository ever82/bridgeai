...
export class SupplyQualityAssessor {
    ...
    assess(supply) {
        ...
        try {
            ...
            const overallScore = Math.round(completenessScore * SCORING_WEIGHTS.completeness +
    ...
    assessCompleteness(supply) {
        ...
        if (supply.pricing) {
            let priceScore = 0;
            if (supply.pricing.type)
                priceScore += 0.3;
            if (supply.pricing.currency)
                priceScore += 0.2;
        ...
        if (supply.experience) {
            ...
            if (supply.experience.certifications?.length)
                expScore += 0.3;
            if (supply.experience.totalProjects !== undefined)
                expScore += 0.3;
    ...
    assessCredibility(supply) {
        ...
        if (supply.capabilities?.length > 0 && supply.skills?.length > 0) {
            ...
            const skillOverlap = supply.skills.filter(s => capKeywords.some(k => k.toLowerCase().includes(s.toLowerCase()) || s.toLowerCase().includes(k.toLowerCase())));
        ...
        if (supply.quality) {
            const scoreRange = Math.max(supply.quality.overallScore, supply.quality.completenessScore, supply.quality.clarityScore, supply.quality.relevanceScore) -
                Math.min(supply.quality.overallScore, supply.quality.completenessScore, supply.quality.clarityScore, supply.quality.relevanceScore);
            if (scoreRange > 50) {
     ...
     */
    assessCompetitiveness(supply) {
        let score = 40; // Baseline
        // Detail level bonus
        if (supply.description?.length > 200)
            score += 10;
        if (supply.capabilities?.length >= 3)
            score += 10;
        if (supply.skills?.length >= 5)
            score += 5;
        // Experience bonus
        if (supply.experience?.years !== undefined) {
            if (supply.experience.years >= 5)
                score += 10;
            else if (supply.experience.years >= 3)
                score += 5;
        }
     ...
     */
    generateOptimizationSuggestions(supply, scores) {
        const suggestions = [];
...
//# sourceMappingURL=supplyQualityAssessor.js.map