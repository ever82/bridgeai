...
// 从集中配置构建敏感词列表
const SENSITIVE_WORDS = DEFAULT_SENSITIVE_WORDS.map((w) => w.word);
/**
 * 敏感词过滤检查
 * @param content 内容
 * @returns 检查结果
 ...
 * @returns 检查结果
 */
async function checkContentLength(content) {
    const minLength = 5;
 ...
 * @returns 检查结果
 */
async function checkRepeatedChars(content) {
 ...
 * @returns 检查结果
 */
async function checkSpamPatterns(content) {
    const spamPatterns = [
 ...
 * @returns 检查结果
 */
async function checkMeaninglessContent(content) {
    // 检查是否全是标点符号或空格
 ...
 * @returns 检查结果
 */
async function checkAISemanticAnalysis(content, title) {
    const lowerContent = content.toLowerCase();
    let score = 0;
    const details = [];
    // 1. 负面情感词密度分析
    ...
    if (title && title.length > 0) {
        ...
        const contentNegative = /(差|烂|垃圾|后悔|失望|骗人|不好|worst|terrible|bad|hate)/i.test(lowerContent);
        const titleNegative = /(差评|失望|垃圾|骗人|后悔|差|烂|worst|terrible|bad|hate)/i.test(lowerTitle);
...
const MODERATION_RULES = [
    {
        ...
        check: async (content) => {
            ...
            return {
                ...
                details: matches.length > 0 ? `发现敏感词: ${matches.map(m => m.word).join(', ')}` : undefined,
            };
        },
    },
    {
    ...
    },
    {
        ...
        enabled: true,
        weight: 1.0,
        check: async (content, title) => checkAISemanticAnalysis(content, title),
    },
];
/**
 * 执行内容审核
 ...
 * @returns 审核结果
 */
export async function moderateContent(content, title) {
    ...
    for (const rule of MODERATION_RULES) {
        ...
        try {
            ...
            logger.error(`Moderation rule ${rule.name} failed`, error);
        }
    }
    // 标准化分数到 0-100
    ...
    if (result.blocked) {
        ...
        if (review) {
            try {
                ...
                logger.error(`Failed to handle violation for review ${reviewId}:`, error);
            }
        }
    }
 ...
 * @returns 处理结果统计
 */
export async function batchModeratePendingReviews(limit = 100) {
    ...
    for (const review of pendingReviews) {
        try {
            ...
            logger.error(`Failed to moderate review ${review.id}`, error);
        }
    }
 ...
 * @returns 审核日志列表
 */
export async function getModerationLogs(reviewId) {
 ...
 */
export async function manualModerateReview(reviewId, action, reason, moderatorId) {
    const review = await prisma.review.findUnique({
        where: { id: reviewId },
    });
    if (!review) {
...
//# sourceMappingURL=reviewModerationService.js.map