 ...
 */
export function isCapabilityEnabled(sceneId, capabilityId) {
 ...
 */
export function getEnabledCapabilities(sceneId) {
    const sceneConfig = getSceneConfig(sceneId);
    if (!sceneConfig) {
        return [];
    }
 ...
 */
export function getAllCapabilities(sceneId) {
    const sceneConfig = getSceneConfig(sceneId);
    if (!sceneConfig) {
        return [];
    }
 ...
 */
export function getCapability(sceneId, capabilityId) {
    const sceneConfig = getSceneConfig(sceneId);
    if (!sceneConfig) {
        return undefined;
    }
 ...
 */
export function areDependenciesSatisfied(sceneId, capabilityId) {
    const sceneConfig = getSceneConfig(sceneId);
    if (!sceneConfig) {
        return false;
    }
 ...
 */
export function hasCircularDependencies(sceneId, capabilityId, visited = new Set()) {
 ...
 */
export function getCapabilityConfig(sceneId, capabilityId) {
 ...
 */
export function validateCapabilityVersion(sceneId, capabilityId, minVersion) {
    const capability = getCapability(sceneId, capabilityId);
    if (!capability) {
        return false;
    }
 ...
 */
export function getMissingDependencies(sceneId, capabilityId) {
    const sceneConfig = getSceneConfig(sceneId);
    if (!sceneConfig) {
        return [];
    }
 ...
 */
export function getCapabilityStatus(sceneId, capabilityId) {
    const sceneConfig = getSceneConfig(sceneId);
    const capability = sceneConfig?.capabilities.find(c => c.id === capabilityId);
    if (!capability) {
        return {
 ...
 */
export function areAllCapabilitiesEnabled(sceneId, capabilityIds) {
    return capabilityIds.every(id => isCapabilityEnabled(sceneId, id));
}
/**
 * Check if any of the capabilities is enabled
 */
export function isAnyCapabilityEnabled(sceneId, capabilityIds) {
    return capabilityIds.some(id => isCapabilityEnabled(sceneId, id));
}
/**
 * Get capabilities by category
 */
export function getCapabilitiesByTag(sceneId, tag) {
    const sceneConfig = getSceneConfig(sceneId);
    if (!sceneConfig) {
        return [];
    }
    ...
    return sceneConfig.capabilities.filter(capability => capability.name.includes(tag) || capability.description.includes(tag));
}
/**
 * Get scene capabilities summary
 */
export function getSceneCapabilitiesSummary(sceneId) {
    const sceneConfig = getSceneConfig(sceneId);
    if (!sceneConfig) {
    ...
    const available = capabilities.filter(c => c.enabled && areDependenciesSatisfied(sceneId, c.id)).length;
 ...
 */
export function getAllScenesCapabilitySummary() {
    const summary = {};
...
//# sourceMappingURL=sceneCapabilityService.js.map