...
export class RBACService {
    ...
    async initializeDefaults() {
        ...
        for (const [key, roleData] of Object.entries(DEFAULT_ROLES)) {
            ...
            const permissionNames = DEFAULT_ROLE_PERMISSIONS[roleData.name] || [];
     ...
     * Create a new role
     */
    async createRole(data, grantedBy) {
     ...
     * Update a role
     */
    async updateRole(id, data, grantedBy) {
        const { permissionIds, ...roleData } = data;
     ...
     * Assign role to user
     */
    async assignRole(userId, roleId, grantedBy, expiresAt) {
        const role = await prisma.role.findUnique({
            where: { id: roleId },
        });
        if (!role) {
    ...
    async hasPermission(userId, permissionName) {
        ...
        return (permissions.some((p) => p.name === permissionName || p.name === '*:admin' || p.resource === '*') || permissions.some((p) => {
