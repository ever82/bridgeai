...
export class PhotoUnlockService {
     ...
     */
    async checkPhotoAccess(userId, photoId) {
        // Check if user has a valid unlock token for this photo
        const existingUnlock = await this.prisma
            .$queryRawUnsafe(`SELECT * FROM points_freezes WHERE reference_id = $1 AND account_id = $2 AND status = 'USED' AND expires_at > NOW()`, photoId, `account-${userId}`)
            .catch(() => []);
        ...
        try {
            ...
            if (access.hasAccess) {
                return {
                    ...
                    expiresAt: new Date(Date.now() + UNLOCK_CONFIG.tokenExpiryHours * 3600000).toISOString(),
                };
            }
            // Process payment
            ...
            const unlockTokenResult = await visionSharePaymentService.generateUnlockToken(userId, photoId);
            // Generate photo URL
    ...
    async validateToken(token) {
        try {
            const result = await this.prisma.$queryRawUnsafe(`SELECT * FROM points_freezes WHERE id = $1 AND status = 'USED' AND expires_at > NOW()`, token);
            if (result.length === 0) {
                return { valid: false };
            }
            const record = result[0];
            // Extract userId and photoId from reference
            const photoId = record.reference_id;
            const userId = record.account_id?.replace('account-', '');
     ...
     */
    async generatePhotoUrl(photoId, userId) {
    ...
    async cleanupExpiredTokens() {
        const result = await this.prisma
            .$executeRawUnsafe(`DELETE FROM points_freezes WHERE expires_at < NOW() AND reason = 'photo_unlock'`)
            .catch(() => 0);
     ...
     */
    async getUnlockedPhotos(userId) {
        const unlocks = await this.prisma
            .$queryRawUnsafe(`SELECT reference_id as photo_id, created_at, expires_at FROM points_freezes WHERE account_id = $1 AND reason = 'photo_unlock' AND status = 'USED' AND expires_at > NOW() ORDER BY created_at DESC`, `account-${userId}`)
            .catch(() => []);
        return unlocks;
    }
}
...
//# sourceMappingURL=photoUnlockService.js.map