 ...
 * 信用分过滤服务
 */
import { isAndFilter, CREDIT_LEVEL_THRESHOLDS, } from '@bridgeai/shared';
// Re-export credit types for convenience
export { CREDIT_LEVEL_THRESHOLDS } from '@bridgeai/shared';
 ...
 */
export function getCreditLevelLabel(level) {
    const labels = {
        excellent: '优秀',
        good: '良好',
        general: '一般',
 ...
 */
export function getCreditLevelColor(level) {
    const colors = {
        excellent: '#4CAF50',
        good: '#8BC34A',
        general: '#FFC107',
 ...
 */
export function buildCreditFilterCondition(options) {
    const conditions = [];
 ...
 */
export async function filterAgentsByCredit(options, pagination) {
    try {
 ...
 */
export async function checkCreditThreshold(agentId, minCreditScore) {
    try {
 ...
 */
export function addCreditFilterToDSL(dsl, options) {
...
//# sourceMappingURL=creditFilterService.js.map