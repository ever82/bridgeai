 ...
 */
export function generateMigrationPlan(fromScene, toScene) {
 ...
 */
function generateAutoMappings(fromConfig, toConfig) {
    const mappings = [];
    ...
    for (const fromField of fromConfig.fields) {
        ...
        // Similar name match
        const similarField = toConfig.fields.find(toField => toField.name.includes(fromField.name) ||
 ...
 */
function generateAutoTransformations(fromConfig, toConfig) {
 ...
 */
function generateWarnings(fromConfig, toConfig) {
    const warnings = [];
    ...
    if (lostFields.length > 0) {
        warnings.push(`以下字段在目标场景中不存在，将被丢弃: ${lostFields.map(f => f.label).join(', ')}`);
    }
 ...
 */
export async function previewMigration(agentId, fromScene, toScene) {
    // Get migration plan
    ...
    const currentData = profile.l2Data || {};
    // Apply transformations to create preview
    const previewData = applyMigrationTransformations(currentData, migration.fieldMappings, migration.transformations);
    ...
    const needsManualInput = toConfig?.fields.filter(f => f.required && !previewData[f.name]).map(f => f.name) || [];
    return {
        migration,
        currentData,
        previewData,
 ...
 */
function applyMigrationTransformations(data, mappings, transformations) {
    const result = {};
    ...
    for (const transformation of transformations) {
        if (result[transformation.field] !== undefined) {
            result[transformation.field] = transformField(result[transformation.field], transformation, result);
        }
    }
    return result;
}
 ...
 */
function transformField(value, transformation, context) {
    switch (transformation.type) {
        ...
        case 'merge': {
            ...
            const fields = transformation.config.fields;
 ...
 * Execute migration
 */
export async function executeMigration(agentId, fromScene, toScene, manualData) {
    try {
        // Preview first
 ...
 */
export async function getMigrationHistory(agentId) {
 ...
 */
export function validateMigration(fromScene, toScene) {
 ...
 * Estimate data loss
 */
export function estimateDataLoss(fromScene, toScene) {
    const fromConfig = getSceneConfig(fromScene);
    const toConfig = getSceneConfig(toScene);
    if (!fromConfig || !toConfig) {
...
//# sourceMappingURL=sceneMigrationService.js.map