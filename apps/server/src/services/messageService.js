 ...
 */
export async function deliverOfflineMessage(offlineMessageId, attempt = 1) {
    ...
    try {
        ...
        if (attempt >= MAX_RETRY_ATTEMPTS) {
            console.error(`[MessageService] Max retries (${MAX_RETRY_ATTEMPTS}) exceeded for offline message ${offlineMessageId}`);
            return { success: false };
        }
        const retryIn = calculateRetryDelay(attempt);
        console.warn(`[MessageService] Delivery attempt ${attempt} failed for offline message ${offlineMessageId}, retrying in ${retryIn}ms`);
 ...
 */
export function scheduleMessageRetry(offlineMessageId, retryIn, attempt = 1) {
 ...
 */
async function createOfflineMessages(conversationId, messageId, senderId) {
    ...
    const offlinePromises = conversation.participantIds
        ...
        .map(userId => prisma.offlineMessage.create({
        data: {
            userId,
            messageId,
            conversationId,
 ...
 */
async function createChatRoomOfflineMessages(chatRoomId, messageId, senderId, tx) {
    ...
    const offlinePromises = offlineParticipants.map(p => client.offlineMessage.create({
        data: {
            userId: p.userId,
            messageId,
 ...
 */
async function sendOfflinePushNotifications(offlineParticipants, sender, messageContent, chatRoomId) {
    if (!sender || offlineParticipants.length === 0) {
        return;
    }
    // Preview content (first 100 chars for push notification body)
    const contentPreview = messageContent.length > 100 ? messageContent.substring(0, 100) + '...' : messageContent;
    const senderName = sender.name || 'Someone';
    // Send push notifications to all offline users in parallel
    await Promise.all(offlineParticipants.map(async (participant) => {
        try {
            ...
            await pushNotificationService.sendMessageNotification(participant.userId, chatRoomId, senderName, contentPreview);
        }
        catch (error) {
            // Log but don't fail the whole operation - push is best-effort
            console.error(`[MessageService] Failed to send push notification to user ${participant.userId}:`, error);
        }
    }));
}
/**
 ...
 */
export async function getMessagesByConversation(filters) {
 ...
 * Create read receipt
 */
export async function createReadReceipt(messageId, userId) {
...
export async function getOfflineMessages(userId) {
    ...
    return Promise.all(offlineMessages
        .map(async (om) => {
 ...
 */
export async function markOfflineMessagesDelivered(userId, messageIds) {
 ...
 */
export async function searchMessages(conversationId, query, limit = 20) {
    ...
    while (results.length < limit) {
        ...
        const filtered = decryptedMessages.filter(msg => msg.content.toLowerCase().includes(query.toLowerCase()));
 ...
 */
export async function createChatRoomMessage(input) {
    const encryptedContent = await encryptionService.encrypt(input.content);
    const result = await prisma.$transaction(async (tx) => {
        ...
        const offlineParticipants = await createChatRoomOfflineMessages(input.chatRoomId, chatMessage.id, input.senderId, tx);
        return {
            ...chatMessage,
            content: input.content, // Return decrypted content for sender
            sender,
...
export async function getChatRoomMessages(filters) {
    ...
    where.NOT = { metadata: { path: ['deleted'], equals: true } };
...
export async function syncChatRoomMessages(input) {
    ...
    const lastCreatedAt = messages.length > 0 ? messages[messages.length - 1].createdAt : lastMessageCreatedAt;
    // Check if there are more messages
    const hasMore = lastCreatedAt != null
 ...
 */
export async function editChatRoomMessage(messageId, userId, newContent) {
 ...
 */
export async function deleteChatRoomMessage(messageId, userId) {
    const message = await prisma.chatMessage.findUnique({
        where: { id: messageId },
 ...
 */
export async function searchChatRoomMessages(chatRoomId, query, limit = 20) {
    if (!encryptionService.isActive()) {
        const messages = await prisma.chatMessage.findMany({
            where: {
                chatRoomId,
