 ...
 * @returns 授权 URL
 */
export function getOAuthAuthorizationUrl(provider, state) {
 ...
 * @returns 创建的用户
 */
async function createOAuthUser(provider, data) {
 ...
 * @param data 更新数据
 */
async function updateOAuthConnection(userId, provider, data) {
 ...
 * @param code 授权码
 */
export async function bindOAuthAccount(userId, provider, code) {
    // 获取 OAuth 用户信息
    const tokenData = await exchangeCodeForToken(provider, code);
    const userInfo = await getOAuthUserInfo(provider, tokenData.accessToken);
    // 检查是否已绑定
 ...
 * @param provider 提供商
 */
export async function unbindOAuthAccount(userId, provider) {
 ...
 * @returns OAuth 绑定列表
 */
export async function getUserOAuthConnections(userId) {
