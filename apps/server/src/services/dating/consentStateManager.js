 ...
 * - 计算最终结果
 */
import { ConsentStatus, ReferralResult, createMutualConsent, updateUserDecision, expireConsent, isBothDecided, ConsentExpiredError, ConsentChangeLimitError, } from '../../models/MutualConsent';
import { sendReferralNotification, sendOtherUserDecidedNotification, sendTimeoutWarningNotification, sendTimeoutExpiredNotification, sendCancelNotification, } from './referralNotificationService';
// 模拟数据存储（实际项目中应使用数据库）
const consentStore = new Map();
/**
 * 创建新的双向同意记录
 */
export async function createConsent(referralId, userAId, userBId, timeoutHours) {
    ...
    // 发送决策邀请通知给双方用户
    try {
        await sendReferralNotification({ id: referralId, userAId, userBId }, consent);
    }
    catch (err) {
 ...
 * 获取引荐的同意记录
 */
export async function getConsentByReferral(referralId) {
 ...
 * 记录用户决策
 */
export async function recordDecision(consentId, userId, decision, reason) {
 ...
 * 获取用户决策状态
 */
export async function getUserConsentState(consentId, userId) {
    const consent = await getConsent(consentId);
    if (!consent)
        return null;
    const now = new Date();
    ...
    // 检查是否可以变更决策
    const canChange = !isExpired && userConsent.status !== ConsentStatus.PENDING && userConsent.changedCount < 3; // 最多变更3次
    return {
        consent,
        canChange,
        timeRemaining,
 ...
 * 获取对方用户的决策状态（不透露具体选择）
 */
export async function getOtherUserDecisionStatus(consentId, userId) {
    const consent = await getConsent(consentId);
    if (!consent)
        throw new ConsentNotFoundError('Consent not found');
 ...
 * 检查是否需要发送超时提醒
 */
export async function checkAndSendTimeoutReminders() {
    const now = new Date();
    const reminderThresholds = [24, 4]; // 到期前24小时和4小时
    for (const consent of consentStore.values()) {
        if (consent.result !== ReferralResult.PENDING)
            continue;
        ...
        for (const threshold of reminderThresholds) {
            ...
            if (hoursUntilExpiry <= threshold && hoursUntilExpiry > threshold - 0.08) {
                // 发送超时提醒
                try {
                    await sendTimeoutWarningNotification({ id: consent.referralId, userAId: consent.userAId, userBId: consent.userBId }, consent, threshold);
                }
                catch (err) {
 ...
 * 获取等待决策的同意记录
 */
export async function getPendingConsents(userId) {
    const pending = [];
    for (const consent of consentStore.values()) {
 ...
 * 取消同意记录
 */
export async function cancelConsent(consentId, _reason) {
    const consent = await getConsent(consentId);
    if (!consent) {
        throw new ConsentNotFoundError('Consent not found');
    }
    ...
    // 发送取消通知
    try {
        await sendCancelNotification({ id: consent.referralId, userAId: consent.userAId, userBId: consent.userBId }, consent.userAId);
    }
    catch (err) {
...
//# sourceMappingURL=consentStateManager.js.map