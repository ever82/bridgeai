...
export function calculateProfileQuality(profile) {
    ...
    const overallScore = Math.round(completenessMetrics.score * 0.4 +
 ...
 */
function calculateCompletenessMetrics(profile) {
 ...
 */
function isSectionFilled(field, value) {
 ...
 */
function calculateRichnessMetrics(profile) {
    let score = 0;
    const suggestions = [];
    ...
    if (profile.lifestyle) {
        ...
        const filledCount = lifestyleFields.filter(f => profile.lifestyle?.[f]).length;
 ...
 */
function calculateMatchPotentialMetrics(profile) {
 ...
 */
function generateRecommendations(profile, metrics) {
 ...
 */
function getFieldSuggestion(field) {
    const suggestions = {
 ...
 */
export function calculateCompletenessPercentage(profile) {
    const weights = {
        basicConditions: 30,
        personality: 10,
        interests: 20,
...
//# sourceMappingURL=profileQualityService.js.map