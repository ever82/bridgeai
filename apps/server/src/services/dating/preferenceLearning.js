...
// 默认值
// ============================================
const DEFAULT_LEARNED_PREFERENCES = {
    weights: {
        basicConditions: 0.2,
        personality: 0.2,
        interests: 0.15,
 ...
 * 从用户反馈中学习偏好
 */
export function learnFromFeedback(input) {
    ...
    // 分析喜欢的反馈
    const likedFeedbacks = feedbacks.filter(f => f.action === ACTION_KEYS.LIKE || f.action === ACTION_KEYS.SUPER_LIKE);
    ...
    logger.info(`Updated learned preferences for user ${userId}: version=${current.learningVersion}, feedbacks=${feedbacks.length}`);
    return current;
}
/**
 * 获取用户的已学习偏好
 */
export function getLearnedPreferences(userId) {
 ...
 * 应用学习到的权重到匹配算法配置
 */
export function getAdjustedWeights(userId) {
    ...
    for (const [dim, adj] of Object.entries(prefs.dimensionAdjustments)) {
        if (dim in base) {
            base[dim] = Math.max(0.05, base[dim] + adj);
        }
    }
 ...
 * 获取过滤条件（基于跳过原因偏好）
 */
export function getLearnedFilters(userId) {
    const prefs = getLearnedPreferences(userId);
 ...
 * 重置用户的学习偏好
 */
export function resetLearnedPreferences(userId) {
...
//# sourceMappingURL=preferenceLearning.js.map