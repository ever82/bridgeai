 ...
 */
export function getVisibleFieldsAtStage(profile, stage, isOwner, isMatched) {
    if (isOwner) {
    ...
    Object.entries(visibility.fieldVisibility || {}).forEach(([field, level]) => {
        if (shouldFieldBeVisible(level, stage, isMatched)) {
 ...
 */
function shouldFieldBeVisible(visibility, stage, isMatched) {
    switch (visibility) {
 ...
 */
export function filterProfileForViewer(profile, viewerUserId, relationshipStage = 'initial', isMatched = false) {
 ...
 */
export function canAccessField(profile, field, viewerUserId, relationshipStage, isMatched, isVerified = false) {
 ...
 */
export function getRecommendedPrivacySettings(level) {
    const base = {
        profileVisibility: VisibilityLevel.PUBLIC,
        fieldVisibility: {
            basicInfo: VisibilityLevel.PUBLIC,
 ...
 */
export function maskFieldValue(field, value) {
 ...
 */
export function getDisclosureRecommendations(profile) {
    ...
    if (profile.basicConditions?.location?.district &&
