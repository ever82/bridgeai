 ...
 * - 人机切换标记
 */
export var ChatRoomType;
(function (ChatRoomType) {
    ChatRoomType["AGENT"] = "agent";
    ChatRoomType["HUMAN"] = "human";
    ChatRoomType["HYBRID"] = "hybrid";
})(ChatRoomType || (ChatRoomType = {}));
export var ChatRoomStatus;
(function (ChatRoomStatus) {
    ChatRoomStatus["ACTIVE"] = "active";
    ChatRoomStatus["INACTIVE"] = "inactive";
    ChatRoomStatus["CLOSED"] = "closed";
    ChatRoomStatus["SUSPENDED"] = "suspended";
})(ChatRoomStatus || (ChatRoomStatus = {}));
// 模拟存储
const chatRoomStore = new Map();
/**
 * 创建真人聊天房间
 */
export async function createHumanChatRoom(referral) {
    const now = new Date();
    const room = {
        id: generateRoomId(),
 ...
 * 生成欢迎消息
 */
export function generateWelcomeMessages(room) {
    const messages = [];
    // 系统欢迎消息
    messages.push({
        type: 'system',
 ...
 * 发送欢迎消息
 */
async function sendWelcomeMessages(roomId, messages) {
    // 调用消息服务发送系统消息
 ...
 * 获取用户的所有真人聊天房间
 */
export async function getUserHumanChatRooms(userId) {
    const rooms = [];
    ...
    // 按最后消息时间排序
    return rooms.sort((a, b) => (b.metadata.lastMessageAt?.getTime() || 0) - (a.metadata.lastMessageAt?.getTime() || 0));
}
/**
 * 用户加入房间
 */
export async function joinRoom(roomId, userId) {
    ...
    const updatedRoom = {
        ...room,
        participants: room.participants.map(p => p.userId === userId ? { ...p, joinedAt: now, isOnline: true } : p),
    };
...
export async function markAsRead(roomId, userId) {
    ...
    const updatedRoom = {
        ...room,
        participants: room.participants.map(p => p.userId === userId ? { ...p, lastReadAt: new Date() } : p),
        metadata: {
            ...room.metadata,
            unreadCount: {
 ...
 * 更新未读消息数
 */
export async function incrementUnreadCount(roomId, userId) {
    const room = await getChatRoom(roomId);
    if (!room)
        return;
 ...
 * 更新最后消息信息
 */
export async function updateLastMessage(roomId, messagePreview) {
    const room = await getChatRoom(roomId);
    if (!room)
        return;
    const updatedRoom = {
...
// 通知函数（待实现）
async function notifyParticipantOnline(roomId, userId) {
    console.log(`User ${userId} is now online in room ${roomId}`);
}
async function notifyParticipantOffline(roomId, userId) {
    console.log(`User ${userId} is now offline in room ${roomId}`);
}
async function notifyRoomClosed(roomId, reason) {
...
//# sourceMappingURL=humanChatRoomService.js.map