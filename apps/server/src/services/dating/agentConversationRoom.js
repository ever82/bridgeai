...
export class RoomStatusError extends Error {
    constructor(roomId, expected, actual) {
 ...
 * 设置房间超时定时器
 */
function scheduleRoomTimeout(roomId, timeoutMs) {
    // 清除已有定时器
 ...
 * 添加系统消息
 */
async function addSystemMessage(roomId, content, _round) {
 ...
 */
export async function createRoom(agentAId, agentBId, userIdA, userIdB, config) {
    const now = new Date();
    ...
    logger.info(`[AgentConversationRoom] Created room ${room.id} for agents ${agentAId} and ${agentBId}`);
    // 尝试持久化到数据库（演示阶段可选）
    try {
 ...
 * 添加消息到房间
 */
export async function addMessage(roomId, senderId, senderType, content) {
    const room = roomStore.get(roomId);
    if (!room) {
        throw new RoomNotFoundError(roomId);
    }
    ...
    logger.debug(`[AgentConversationRoom] Message added to room ${roomId} by ${senderType} (${senderId})`);
    return message;
}
/**
 * 检查房间是否超时
 * 返回 true 表示已超时并处理
 */
export async function checkRoomTimeout(roomId) {
    const room = roomStore.get(roomId);
    if (!room) {
        throw new RoomNotFoundError(roomId);
    }
    ...
    const now = Date.now();
    const lastActivity = room.lastMessageAt?.getTime() || room.startedAt?.getTime() || room.createdAt.getTime();
...
export async function incrementRound(roomId) {
    ...
    if (nextRound > room.maxRounds) {
        ...
        await addSystemMessage(roomId, `已达到最大轮次限制 (${room.maxRounds})，对话自动完成。`, room.currentRound);
 ...
 * 标记对话为完成状态，生成摘要和质量评分
 */
export async function completeRoom(roomId, summary, qualityScore) {
    const room = roomStore.get(roomId);
    if (!room) {
        throw new RoomNotFoundError(roomId);
    }
    ...
    // 添加系统消息
    await addSystemMessage(roomId, `对话已完成。质量评分: ${qualityScore ?? '未评分'}。`, room.currentRound);
    logger.info(`[AgentConversationRoom] Room ${roomId} completed. Quality: ${qualityScore ?? 'N/A'}`);
    ...
    if (qualityScore !== null && qualityScore >= 0.6) {
        try {
            ...
            await createReferralFromConversation(roomId, updatedRoom.agentAId, updatedRoom.agentBId, updatedRoom.userIdA, updatedRoom.userIdB, {
                summary: summary || '',
                qualityScore,
                sharedInterests,
 ...
 * 强制终止对话，记录原因
 */
export async function terminateRoom(roomId, reason) {
    const room = roomStore.get(roomId);
    if (!room) {
        throw new RoomNotFoundError(roomId);
    }
 ...
 * 获取用户相关的活跃房间
 */
export async function getActiveRoomsByUser(userId) {
    const rooms = [];
 ...
 * 从消息中提取共同兴趣关键词（简单实现）
 */
function extractSharedInterests(messages) {
...
//# sourceMappingURL=agentConversationRoom.js.map