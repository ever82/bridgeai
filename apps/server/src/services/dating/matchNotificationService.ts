...
export enum MatchNotificationType {
  ...
  FEEDBACK_REQUEST = 'feedback_request', // 反馈请求
}
...
export interface NotificationPreferences {
  enabled: boolean;
  quietHoursStart?: string; // HH:mm 格式，免打扰开始时间
  quietHoursEnd?: string; // HH:mm 格式，免打扰结束时间
  preferredTime?: string; // HH:mm 格式，偏好推送时间
...
const notificationStore = new Map<string, MatchNotification[]>();
const preferenceStore = new Map<string, NotificationPreferences>();
const sentCountToday = new Map<string, { count: number; resetAt: number }>();
 ...
 * 记录已发送通知
 */
function recordSent(userId: string): void {
  const counter = sentCountToday.get(userId);
  if (counter) {
    counter.count++;
  }
 ...
 * 生成个性化每日推荐推送内容
 */
function generateDailyRecommendationContent(
  matchCount: number,
  topMatch?: MatchScore
 ...
 * 生成新匹配推送内容
 */
function generateNewMatchContent(match: MatchScore): { title: string; body: string } {
 ...
 * 生成反馈请求推送内容
 */
function generateFeedbackContent(): { title: string; body: string } {
  const messages = [
 ...
 * 发送每日推荐推送通知
 */
export async function sendDailyRecommendationNotification(
  userId: string,
  matchScores: MatchScore[]
): Promise<MatchNotification | null> {
  if (!canSendNotification(userId)) {
    logger.info(
      `Skipping daily recommendation notification for user ${userId}: frequency limit or quiet hours`
    );
    return null;
  }
  ...
  recordSent(userId);
  logger.info(
    `Sent daily recommendation notification to user ${userId}: ${matchScores.length} matches`
  );
...
export async function sendNewMatchNotification(
  ...
  if (!canSendNotification(userId)) return null;
...
export function getUserNotifications(
  userId: string,
  options?: { type?: MatchNotificationType; limit?: number }
): MatchNotification[] {
