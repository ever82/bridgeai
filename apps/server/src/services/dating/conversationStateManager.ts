 ...
 * 验证阶段转换是否合法
 */
export function validateTransition(from: ConversationPhase, to: ConversationPhase): boolean {
 ...
 * 推进到下一轮
 */
export function advanceRound(state: ConversationState): ConversationState {
  const now = new Date();
  const updatedState: ConversationState = {
    ...state,
...
export function checkPhaseTransition(
  ...
  if (phase === 'deepening') {
    const isApproachingEnd =
      currentRound >= Math.floor(maxRounds * DEEPENING_TO_CLOSING_APPROACH_THRESHOLD);
    const isHighQuality =
      qualityScore !== undefined && qualityScore >= DEEPENING_TO_CLOSING_QUALITY_THRESHOLD;
 ...
 * 获取状态摘要
 */
export function getStateSummary(state: ConversationState): Record<string, any> {
  const now = new Date();
 ...
 * 持久化状态到数据库
 */
export async function persistState(state: ConversationState): Promise<boolean> {
  try {
    const { prisma } = await import('../../db/client').catch(() => ({ prisma: null }));
    if (!prisma) {
 ...
 * 从缓存或数据库加载状态
 */
export async function loadState(roomId: string): Promise<ConversationState | null> {
  // 先查缓存
  ...
  try {
    ...
    const state: ConversationState = {
      roomId,
      status: (room.status as ConversationStatus) || 'pending',
      ...
      phaseHistory: (meta.phaseHistory as PhaseTransition[]) || [],
      metadata: meta,
    };
    ...
    logger.error('Failed to load state', { roomId, error: (err as Error).message });
    return null;
  }
}
 ...
 * 终止对话
 */
export function terminateConversation(
  state: ConversationState,
  reason?: string
): StateTransitionResult {
  const previousState = { ...state };
 ...
 * 暂停对话
 */
export function pauseConversation(state: ConversationState): ConversationState {
  const updatedState: ConversationState = {
    ...state,
    status: 'paused',
 ...
 * 恢复对话
 */
export function resumeConversation(state: ConversationState): ConversationState {
  const updatedState: ConversationState = {
    ...state,
    status: 'active',
    lastActivityTime: new Date(),
