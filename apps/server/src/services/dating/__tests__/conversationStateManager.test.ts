...
describe('conversationStateManager', () => {
  describe('createInitialState', () => {
    it('should create with correct defaults', () => {
      const state = createInitialState('room-123');
  ...
  describe('validateTransition', () => {
    it('should allow same-phase transition', () => {
      expect(validateTransition('intro', 'intro')).toBe(true);
      expect(validateTransition('exploring', 'exploring')).toBe(true);
    });

    it('should allow valid transitions', () => {
      // intro -> exploring
      expect(validateTransition('intro', 'exploring')).toBe(true);
      // intro -> completed (early completion)
      expect(validateTransition('intro', 'completed')).toBe(true);
      // exploring -> deepening
      expect(validateTransition('exploring', 'deepening')).toBe(true);
      // exploring -> closing
      expect(validateTransition('exploring', 'closing')).toBe(true);
      // exploring -> completed
      expect(validateTransition('exploring', 'completed')).toBe(true);
      // deepening -> closing
      expect(validateTransition('deepening', 'closing')).toBe(true);
      // deepening -> completed
      expect(validateTransition('deepening', 'completed')).toBe(true);
      // closing -> completed
      expect(validateTransition('closing', 'completed')).toBe(true);
    });

    it('should reject invalid transitions', () => {
      // Cannot go backwards
      expect(validateTransition('exploring', 'intro')).toBe(false);
      expect(validateTransition('deepening', 'intro')).toBe(false);
      expect(validateTransition('deepening', 'exploring')).toBe(false);
      expect(validateTransition('closing', 'intro')).toBe(false);
      expect(validateTransition('closing', 'exploring')).toBe(false);
      expect(validateTransition('closing', 'deepening')).toBe(false);
      // Cannot transition from completed
      expect(validateTransition('completed', 'intro')).toBe(false);
      expect(validateTransition('completed', 'exploring')).toBe(false);
      // Cannot skip phases
      expect(validateTransition('intro', 'deepening')).toBe(false);
      expect(validateTransition('intro', 'closing')).toBe(false);
      expect(validateTransition('exploring', 'completed')).toBe(true); // Valid
    });
  });
  ...
  describe('transitionPhase', () => {
    it('should allow valid phase transitions', () => {
      ...
      const result = transitionPhase(state, 'exploring', 'auto');
    ...
    it('should reject invalid phase transitions', () => {
      ...
      const result = transitionPhase(state, 'deepening', 'auto');
    ...
    it('should set status to completed when transitioning to completed phase', () => {
      ...
      const result = transitionPhase(state, 'completed', 'manual');
    ...
    it('should record phase transition history with correct timestamps', () => {
      ...
      const result = transitionPhase(state, 'exploring', 'manual');

      expect(result.newState.phaseHistory[0].timestamp.getTime()).toBeGreaterThanOrEqual(
  ...
  describe('advanceRound', () => {
    ...
    it('should not reset startTime on subsequent advances', () => {
      let state = createInitialState('room-123');
  ...
  describe('checkPhaseTransition', () => {
    it('should suggest exploring after minimum rounds in intro', () => {
      const state = createInitialState('room-123', { maxRounds: 10 });
      state.phase = 'intro';
      state.currentRound = 2;

      const suggested = checkPhaseTransition(state);
    ...
    it('should suggest closing when quality is high', () => {
      ...
      const suggested = checkPhaseTransition(state, undefined, 0.9);
  ...
  describe('terminateConversation', () => {
    it('should mark conversation as terminated', () => {
      ...
      const result = terminateConversation(state, 'User requested');
    ...
    it('should use default reason when not provided', () => {
      ...
      const result = terminateConversation(state);
  ...
  describe('pauseConversation', () => {
    it('should set status to paused', () => {
      ...
      const result = pauseConversation(state);
  ...
  describe('resumeConversation', () => {
    it('should set status back to active', () => {
      ...
      const result = resumeConversation(state);
