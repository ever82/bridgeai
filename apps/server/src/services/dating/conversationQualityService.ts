...
export interface QualityMetrics {
  fluency: number; // 0-1 对话流畅度
  topicDepth: number; // 0-1 话题深度
  engagement: number; // 0-1 参与度
  coherence: number; // 0-1 连贯性
  personaConsistency: number; // 0-1 人设一致性
  overall: number; // 0-1 综合质量分
}
...
// 内存缓存：房间质量评估记录
const assessmentCache = new Map<string, QualityAssessment[]>();
...
export async function assessConversation(
  ...
  const [fluency, topicDepth, engagement, coherence, personaConsistency, issues] =
    await Promise.all([
 ...
 * 基于消息长度变化、响应速度、语言连贯性
 */
export async function calculateFluency(
 ...
 * 基于关键词密度、话题延展性、观点表达
 */
export async function evaluateTopicDepth(
  messages: Array<{ role: string; content: string }>,
  topics: string[]
): Promise<number> {
 ...
 * 基于角色特征、语言风格的一致性
 */
async function evaluatePersonaConsistency(
  messages: Array<{ role: string; content: string; agentId?: string }>
): Promise<number> {
  if (messages.length < 2) return 0.5;
...
export async function detectIssues(
  ...
  if (messages.length < 2) return issues;
 ...
 * 获取质量趋势
 */
export async function getQualityTrend(roomId: string): Promise<QualityTrend> {
  ...
  const averageScore =
    Math.round(
      (assessments.reduce((sum, a) => sum + a.metrics.overall, 0) / assessments.length) * 100
    ) / 100;
 ...
 * 生成质量报告
 */
export async function generateQualityReport(roomId: string): Promise<QualityAssessment[]> {
  const assessments = assessmentCache.get(roomId) || [];
...
// 辅助函数：根据指标生成建议
function generateSuggestions(metrics: QualityMetrics, issues: QualityIssue[]): string[] {
