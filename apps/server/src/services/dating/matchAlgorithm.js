 ...
 */
import { calculateDatingSimilarity, } from '@bridgeai/shared';
 ...
 * 计算单个目标用户的匹配分数
 */
export function calculateMatchScore(sourceProfile, targetProfile, weights) {
 ...
 * 批量计算匹配分数并排序
 */
export function rankMatches(sourceProfile, candidateProfiles, config = {}) {
    ...
    logger.info(`Ranked ${scores.length} matches from ${candidateProfiles.length} candidates (minScore=${cfg.minMatchScore})`);
    return scores;
}
/**
 * 生成每日推荐候选（含多样性保证）
 */
export function generateDailyRecommendations(sourceProfile, candidateProfiles, config = {}) {
    const cfg = { ...DEFAULT_CONFIG, ...config };
    // 先按匹配分数排序
 ...
 * 计算候选人与已选列表的多样性
 */
function calculateDiversityScore(selected, candidate) {
    if (selected.length === 0)
        return 1;
    // 基于维度分数计算平均差异
    const avgDiff = selected.reduce((sum, s) => {
 ...
 * 获取匹配维度的详细分析（用于展示）
 */
export function getMatchAnalysis(match) {
    ...
    else if (match.totalScore >= 60)
        overallRating = 'good';
    else if (match.totalScore >= 40)
        overallRating = 'fair';
    else
        overallRating = 'poor';
