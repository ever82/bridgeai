...
describe('Points Account Data Models', () => {
    ...
    describe('PointsAccount Model', () => {
        ...
        it('should enforce unique constraint on userId', async () => {
            const userId = 'user-123';
            mockPrisma.pointsAccount.create.mockRejectedValue(new Error('Unique constraint failed on the fields: (`user_id`)'));
            await expect(mockPrisma.pointsAccount.create({
                data: { userId },
