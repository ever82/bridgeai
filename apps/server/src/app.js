...
app.use('/api', (req, res, next) => {
    if (['POST', 'PUT', 'PATCH'].includes(req.method)) {
        ...
        if (contentType && !expectsJson) {
            res
                .status(415)
                .json(ApiResponse.error(`Unsupported Media Type: ${contentType.replace(/</g, '&lt;').replace(/>/g, '&gt;')}`, 'UNSUPPORTED_MEDIA_TYPE', 415));
            return;
        }
    }
    next();
    ...
    try {
        ...
        if (router) {
            ...
            if (methods.length > 0 && !methods.includes(req.method.toUpperCase())) {
                ...
                res
                    .status(405)
                    .json(ApiResponse.error(`Method ${req.method} not allowed for ${req.path}`, 'METHOD_NOT_ALLOWED', 405));
                return;
            }
        }
    }
