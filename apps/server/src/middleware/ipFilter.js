import { defaultIPListConfig, parseEnvironmentWhitelist, parseEnvironmentBlacklist, parseEnvironmentCIDRWhitelist, parseEnvironmentCIDRBlacklist, isIPInCIDR, isValidIP, isValidCIDR, } from '../config/ipList';
// In-memory storage for dynamic IP lists
let config = { ...defaultIPListConfig };
const reputationDB = new Map();
...
function matchesCIDR(ip, ranges) {
    return ranges.some(range => isIPInCIDR(ip, range.range));
}
// Check if IP is whitelisted
function isWhitelisted(ip, _req) {
...
function isBlacklisted(ip, _req) {
...
function checkGeoRestrictions(req) {
...
export function ipFilter(req, res, next) {
    ...
    if (config.reputationCheck) {
        ...
        if (repCheck.shouldBlock) {
            handleBlocked(req, res, next, 'IP_REPUTATION', `IP reputation score (${repCheck.reputation.score}) exceeds threshold`);
            return;
        }
    }
    // IP is allowed
...
function handleBlocked(req, res, _next, code, message) {
    const ip = getClientIP(req);
 ...
 */
export function addWhitelistRange(range) {
 ...
 */
export function removeWhitelistRange(range) {
 ...
 */
export function addBlacklistRange(range) {
    if (!isValidCIDR(range.range)) {
        return false;
    }
 ...
 */
export function removeBlacklistRange(range) {
 ...
 */
export function updateIPReputation(reputation) {
