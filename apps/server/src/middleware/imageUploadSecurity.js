 ...
 */
export { ImageSecurityService, } from '../services/security/imageSecurity';
 ...
 */
export function secureImageUpload(options = {}) {
 ...
 */
export function secureImagesUpload(options = {}) {
    const opts = { ...DEFAULT_OPTIONS, ...options };
    const upload = multer({
        storage,
        fileFilter: imageFileFilter,
...
// ---------------------------------------------------------------------------
function createSecurityHandler(opts) {
    return async (req, res, next) => {
        try {
            ...
            const results = await Promise.all(files.map(file => service.secureImage(file.buffer, file.mimetype, file.originalname, secConfig)));
            // Check for failures
            ...
            if (files.length === 1 && req.file) {
                req.imageSecurityResult =
                    results[0];
            }
            req.imageSecurityResults = results;
            next();
        }
        catch (error) {
            next(new AppError(error instanceof Error ? error.message : 'Image security check error', 'IMAGE_SECURITY_ERROR', 500));
        }
    };
}
...
// ---------------------------------------------------------------------------
export function uploadSecurityErrorHandler(err, _req, res, next) {
