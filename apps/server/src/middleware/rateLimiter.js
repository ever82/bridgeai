...
import { rateLimitConfigs, userTierLimits, getRateLimitConfig, } from '../config/rateLimit';
 ...
 */
export function getRateLimitHeaders(identifier, config) {
 ...
 */
export function userRateLimiter(req, res, next) {
    ...
    if (record.tokens >= 1) {
        ...
        res
            .status(429)
            .json(ApiResponse.error(`Rate limit exceeded for ${tier} users. Please try again later.`, 'USER_RATE_LIMIT_EXCEEDED', 429, { retryAfter }));
    }
}
/**
...
export function endpointRateLimiter() {
    return rateLimit({
        ...
        handler: (req, res) => {
            ...
            res
                .status(429)
                .json(ApiResponse.error(config.message || 'Too many requests, please try again later.', 'RATE_LIMIT_EXCEEDED', 429));
        },
        skip: (req) => {
...
export const enhancedIpLimiter = rateLimit({
    ...
    handler: (req, res) => {
        res
            .status(429)
            .json(ApiResponse.error('Too many requests from this IP, please try again later.', 'IP_RATE_LIMIT_EXCEEDED', 429));
    },
    skip: (req) => {
        // Skip health checks
        return req.path === '/health' || req.path === '/ready';
...
export const strictAuthLimiter = rateLimit({
    ...
    handler: (req, res) => {
        res.status(429).json(ApiResponse.error(rateLimitConfigs.auth.message, 'AUTH_RATE_LIMIT_EXCEEDED', 429, {
...
export const reportLimiter = rateLimit({
    ...
    handler: (req, res) => {
        res.status(429).json(ApiResponse.error(rateLimitConfigs.reports.message, 'REPORT_RATE_LIMIT_EXCEEDED', 429, {
 ...
 */
export function falseReportPenaltyCheck(req, res, next) {
    ...
    import('../db/client')
        ...
        if (dismissedCount >= 3) {
            res
                .status(429)
                .json(ApiResponse.error('Your report privileges are temporarily restricted due to previous false reports.', 'FALSE_REPORT_PENALTY', 429, { dismissedRecent: dismissedCount }));
        }
        else {
            next();
        }
...
export function combinedRateLimiter() {
    return rateLimit({
        ...
        handler: (req, res) => {
            res
                .status(429)
                .json(ApiResponse.error('Too many requests, please try again later.', 'RATE_LIMIT_EXCEEDED', 429, { retryAfter: 900 }));
        },
        skip: (req) => {
 ...
 */
export function resetUserRequestStore() {
