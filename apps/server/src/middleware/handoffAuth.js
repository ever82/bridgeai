import { DEFAULT_HANDOFF_CONFIG, HandoffErrorCode, } from '@bridgeai/shared';
import { rbacService } from '../services/rbacService';
import { getRequestContext } from './requestContext';
// In-memory rate limit store (in production, use Redis)
const handoffRateLimitStore = new Map();
const handoffAuditLogs = [];
/**
 * Middleware to check handoff permissions
 * Loads user permissions and determines handoff capabilities
 */
export async function handoffPermissionMiddleware(req, res, next) {
    ...
    try {
        ...
        const hasAllowedRole = DEFAULT_HANDOFF_CONFIG.allowedRoles.some(role => roleNames.includes(role));
        // Check rate limit
 ...
 */
export function requireTakeoverPermission(req, res, next) {
    if (!req.handoff) {
        res.status(401).json({
            success: false,
 ...
 */
export function requireHandoffPermission(req, res, next) {
    if (!req.handoff) {
        res.status(401).json({
            success: false,
            error: { code: HandoffErrorCode.UNAUTHORIZED, message: 'Handoff permissions not loaded' },
 ...
 */
export function checkFrequentSwitching(config = {}) {
 ...
 */
export function validateForcedTakeover(req, res, next) {
 ...
 */
export function auditHandoffAction(action) {
    return (req, res, next) => {
 ...
 */
function checkHandoffRateLimit(userId) {
    const now = Date.now();
 ...
 */
function logHandoffAudit(entry) {
 ...
 */
export function getAllHandoffAuditLogs() {
    return [...handoffAuditLogs];
}
/**
 * Clear rate limit for a user (for testing or admin override)
 */
export function clearHandoffRateLimit(userId) {
    handoffRateLimitStore.delete(userId);
}
/**
 * Get rate limit status for a user
 */
export function getHandoffRateLimitStatus(userId) {
    const now = Date.now();
    const oneHourAgo = now - 60 * 60 * 1000;
    const timestamps = handoffRateLimitStore.get(userId) || [];
    const recentTimestamps = timestamps.filter(ts => ts > oneHourAgo);
    // Calculate reset time (1 hour after oldest timestamp)
    const resetAt = recentTimestamps.length > 0 ? Math.min(...recentTimestamps) + 60 * 60 * 1000 : now;
    return {
...
//# sourceMappingURL=handoffAuth.js.map