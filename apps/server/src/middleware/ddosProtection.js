...
// Block an IP
function blockIP(ip, reason, attackType) {
    if (isWhitelisted(ip))
        return;
    const now = Date.now();
    blockedIPs.set(ip, {
...
function updateTrafficRecord(ip) {
    const now = Date.now();
...
export function startDDoSCleanup() {
 ...
 */
export function slowAttackProtection(timeoutMs = 30000) {
    return (req, res, next) => {
        if (isTest) {
            next();
            return;
 ...
 */
export function manuallyBlockIP(ip, durationMinutes, reason) {
    if (isWhitelisted(ip)) {
        return false;
    }
    const now = Date.now();
 ...
 */
export function isMaliciousRequest(req) {
    const reasons = [];
    const ip = getClientIP(req);
