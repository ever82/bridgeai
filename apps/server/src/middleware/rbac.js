...
function permissionMatches(userPermission, requiredPermission) {
    ...
    return (userPermission === `${requiredResource}:admin` || userPermission === `${requiredResource}:*`);
}
/**
...
export function requirePermission(...requiredPermissions) {
    return async (req, res, next) => {
        ...
        try {
            ...
            // Check if user has all required permissions (with wildcard expansion)
            const { hasAll, missing: missingPermissions } = hasAllPermissions(permissionNames, requiredPermissions);
            if (!hasAll) {
 ...
 */
export async function loadUserPermissions(req, _res, next) {
    if (!req.user?.id) {
        next();
        return;
    }
