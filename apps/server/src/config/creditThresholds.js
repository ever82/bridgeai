 ...
 */
export const DEFAULT_SCENE_THRESHOLDS = {
    visionshare: {
        sceneId: 'visionshare',
        sceneName: '视觉分享',
        minCreditScore: 500,
        minCreditLevel: 'general',
...
let sceneThresholds = new Map(Object.entries(DEFAULT_SCENE_THRESHOLDS));
/**
 * Reset scene thresholds to defaults (for testing)
 */
export function resetSceneThresholds() {
 ...
 */
export function getSceneThreshold(sceneId) {
    return sceneThresholds.get(sceneId);
}
/**
 * Get all scene thresholds
 */
export function getAllSceneThresholds() {
    return Array.from(sceneThresholds.values());
}
/**
 * Update scene threshold
 */
export function updateSceneThreshold(sceneId, updates) {
 ...
 * Add exemption rule
 */
export function addExemptionRule(sceneId, rule) {
 ...
 */
export function removeExemptionRule(sceneId, ruleId) {
    const threshold = sceneThresholds.get(sceneId);
    if (!threshold)
        return false;
 ...
 */
export function isUserExempted(sceneId, userId, agentId) {
    const threshold = sceneThresholds.get(sceneId);
    if (!threshold)
 ...
 */
export function checkSceneCreditThreshold(sceneId, creditScore, userId, agentId) {
    const threshold = sceneThresholds.get(sceneId);
    ...
    // Check credit score
    const meetsThreshold = creditScore !== null && creditScore !== undefined && creditScore >= threshold.minCreditScore;
    return {
        meetsThreshold,
        requiredScore: threshold.minCreditScore,
        requiredLevel: threshold.minCreditLevel,
 ...
 */
export function getThresholdChangeNotification(sceneId, oldThreshold, newThreshold) {
    const threshold = sceneThresholds.get(sceneId);
 ...
 */
export function getInsufficientCreditNotification(sceneId, currentScore, requiredScore) {
    const threshold = sceneThresholds.get(sceneId);
    const sceneName = threshold?.sceneName || sceneId;
...
//# sourceMappingURL=creditThresholds.js.map