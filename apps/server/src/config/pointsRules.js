...
// 合并所有规则
export const ALL_POINTS_RULES = {
    ...EARN_RULES,
    ...SPEND_RULES,
};
// 运行时规则配置（可动态修改）
let runtimeLimits = { ...DEFAULT_LIMITS };
let runtimeValueConfig = { ...DEFAULT_VALUE_CONFIG };
let runtimeTransferConfig = { ...DEFAULT_TRANSFER_CONFIG };
let runtimeFreezeConfig = { ...DEFAULT_FREEZE_CONFIG };
/**
 * 获取积分限制配置
 */
export function getPointsLimitConfig() {
    return { ...runtimeLimits };
}
/**
 * 更新积分限制配置
 */
export function updatePointsLimitConfig(config) {
    runtimeLimits = { ...runtimeLimits, ...config };
}
/**
 * 获取积分价值配置
 */
export function getPointsValueConfig() {
    return { ...runtimeValueConfig };
}
/**
 * 更新积分价值配置
 */
export function updatePointsValueConfig(config) {
 ...
 * 更新转账配置
 */
export function updateTransferConfig(config) {
    runtimeTransferConfig = { ...runtimeTransferConfig, ...config };
}
/**
 * 获取冻结配置
 */
export function getFreezeConfig() {
    return { ...runtimeFreezeConfig };
}
/**
 * 更新冻结配置
 */
export function updateFreezeConfig(config) {
 ...
 * 获取场景相关的消耗规则
 */
export function getSpendRulesByScene(scene) {
    return Object.values(SPEND_RULES).filter(rule => rule.enabled && rule.scene === scene);
}
/**
 * 计算充值获得的积分
 */
export function calculateRechargePoints(rmbAmount) {
 ...
 * 计算积分对应的人民币价值
 */
export function calculatePointsRmbValue(points) {
    const config = getPointsValueConfig();
 ...
 * 重置配置为默认值（用于测试）
 */
export function resetPointsConfig() {
...
//# sourceMappingURL=pointsRules.js.map