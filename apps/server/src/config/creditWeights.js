...
export const FACTOR_WEIGHTS = [
    ...
    },
    {
        ...
        weight: 0.15, // 社交信用占15%
        subFactors: [
            { name: 'rating_score', weight: 0.35, maxScore: 100 }, // 评价分数
            { name: 'rating_count', weight: 0.25, maxScore: 100 }, // 评价数量
            { name: 'complaint_count', weight: 0.2, maxScore: 100 }, // 被举报次数(负向)
            { name: 'connection_count', weight: 0.1, maxScore: 100 }, // 连接数
            { name: 'reply_rate', weight: 0.1, maxScore: 100 }, // 评价回复率
        ],
    },
];
// 评分计算辅助函数
export function getFactorWeight(type) {
    const factor = FACTOR_WEIGHTS.find(f => f.type === type);
    return factor?.weight ?? 0;
}
export function getSubFactorWeight(type, subFactorName) {
    const factor = FACTOR_WEIGHTS.find(f => f.type === type);
    if (!factor)
        return null;
...
// 计算维度加权得分
export function calculateWeightedScore(type, subFactorScores) {
    const factor = FACTOR_WEIGHTS.find(f => f.type === type);
    if (!factor)
        return 0;
...
//# sourceMappingURL=creditWeights.js.map