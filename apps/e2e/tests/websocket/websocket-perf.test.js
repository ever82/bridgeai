...
function createClient(namespace = '/') {
    return new Promise((resolve, reject) => {
        ...
        const socket = (0, socket_io_client_1.io)(`${SERVER_URL}${namespace === '/' ? '' : namespace}`, {
 ...
 */
async function createClients(count, namespace = '/') {
...
describe('WebSocket E2E Performance', () => {
    ...
    describe('Concurrent connections', () => {
        it('should handle multiple simultaneous connections to main namespace', async () => {
            ...
            console.log(`[Perf] ${CONCURRENT_CONNECTIONS} connections established in ${elapsed.toFixed(1)}ms ` +
        ...
        it('should handle connections across multiple namespaces concurrently', async () => {
            ...
            console.log(`[Perf] ${perNamespace * 4} cross-namespace connections in ${elapsed.toFixed(1)}ms`);
    ...
    describe('Message broadcast latency', () => {
        it('should broadcast messages to room members within latency threshold', async () => {
            ...
            console.log(`[Perf] Broadcast latency: avg=${avgLatency.toFixed(2)}ms, max=${maxLatency.toFixed(2)}ms ` +
        ...
        it('should sustain low latency under message burst', async () => {
            ...
            console.log(`[Perf] Burst: ${MESSAGE_COUNT} messages to ${receivers.length} receivers ` +
    ...
    describe('Room system concurrency', () => {
        it('should handle concurrent room join/leave operations', async () => {
            ...
            const joinResults = await Promise.all(clients.map(c => new Promise(resolve => {
                c.emit('chat:join', { roomId }, (response) => {
                    resolve({
            ...
            const leaveResults = await Promise.all(clients.map(c => new Promise(resolve => {
                c.emit('chat:leave', { roomId }, (response) => {
        ...
        it('should handle multiple rooms with overlapping members', async () => {
            ...
            const joinStart = performance.now();
            const allJoinPromises = clients.map(client => Promise.all(rooms.map(roomId => new Promise(resolve => {
                client.emit('chat:join', { roomId }, (response) => resolve(response.success));
                setTimeout(() => resolve(false), 10000);
            }))));
            ...
            console.log(`[Perf] ${totalOps} room joins (${clientCount} clients x ${ROOM_COUNT} rooms) ` +
    ...
    describe('Online presence synchronization', () => {
        it('should track online status for multiple users', async () => {
            ...
            const subscribePromises = clients.map(client => Promise.all(clients.slice(0, 5).map(target => new Promise(resolve => {
                client.emit('presence:subscribe', { targetUserId: `user-${clients.indexOf(target)}` }, (response) => resolve(response));
                setTimeout(() => resolve({ success: false }), 10000);
            }))));
            await Promise.all(subscribePromises);
            const subscribeElapsed = performance.now() - subscribeStart;
            console.log(`[Perf] Presence subscriptions: ${clientCount * 5} ops in ${subscribeElapsed.toFixed(1)}ms`);
            disconnectAll(clients);
        }, 30000);
        it('should broadcast presence updates within latency threshold', async () => {
            ...
            const updatePromise = new Promise(resolve => {
                subscriber.on('presence:status_changed', (_data) => {
        ...
        it('should handle rapid presence status changes', async () => {
            ...
            const updatePromises = clients.map(client => Promise.all(Array.from({ length: Math.ceil(updateCount / clientCount) }, (_, i) => new Promise(resolve => {
                const status = statuses[i % statuses.length];
                client.emit('presence:update', { status }, (response) => resolve(response.success));
            ...
            console.log(`[Perf] Presence updates: ${totalUpdates} in ${updateElapsed.toFixed(1)}ms ` +
                `(${updatesPerSecond.toFixed(0)} updates/s)`);
            disconnectAll(clients);
        }, 30000);
    });
});
//# sourceMappingURL=websocket-perf.test.js.map