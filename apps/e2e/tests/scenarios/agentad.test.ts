  ...
  test.describe('Agent优惠协商谈判', () => {
    ...
    test('Agent协商应该有回合限制', async ({ apiContext, testUser }) => {
      ...
      const configResponse = await apiContext.get(
        `/api/agentad/negotiations/${negotiation.id}/config`,
        {
          headers: {
            Authorization: `Bearer ${testUser.token}`,
          },
    ...
    test('协商成功应该生成协议', async ({ apiContext, testUser }) => {
      ...
      // 确认协议
      const confirmResponse = await apiContext.post(
        `/api/agentad/negotiations/${negotiation.id}/confirm`,
        {
          headers: {
            Authorization: `Bearer ${testUser.token}`,
          },
  ...
  test.describe('多商家比价', () => {
    ...
    test('比价结果应该按指定方式排序', async ({ apiContext, testUser }) => {
      ...
      for (let i = 1; i < comparison.results.length; i++) {
        expect(comparison.results[i - 1].price).toBeLessThanOrEqual(comparison.results[i].price);
      }
    });
