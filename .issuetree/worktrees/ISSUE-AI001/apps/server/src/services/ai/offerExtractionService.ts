...
import { OfferType, ApplicableScope, UserGroup, AIExtractionData } from '../../types/offer.types';
...
function parseNaturalLanguageOffer(text: string): Partial<ExtractedOfferData> {
  ...
  if (
    lowerText.includes('买一送一') ||
    lowerText.includes('买1送1') ||
  ...
  if (
    lowerText.includes('新用户') ||
    lowerText.includes('新客') ||
...
export function validateExtractedData(data: Partial<ExtractedOfferData>): {
  ...
  if (data.type === OfferType.DISCOUNT || data.type === OfferType.PERCENTAGE) {
    ...
    if (
      data.type === OfferType.PERCENTAGE &&
