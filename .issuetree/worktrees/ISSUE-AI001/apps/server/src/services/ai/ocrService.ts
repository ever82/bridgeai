...
import { ImageInput, OCRResult, TextBlock, BoundingBox, IVisionModelAdapter } from './vision/types';
  ...
  async extractText(
    ...
    try {
      ...
      const detectHandwriting =
        options?.detectHandwriting ?? this.config.supportHandwriting ?? true;

      // 构建OCR提示
      const ocrPrompt = this.buildOCRPrompt(
        language,
        detectHandwriting,
...
If no text is present, return an empty array [].`;
   ...
   * 提取特定区域的文字
   */
  async extractTextInRegion(image: ImageInput, boundingBox: BoundingBox): Promise<OCRResult> {
...
If a field is not found, use null or empty string.`;
  ...
  private buildOCRPrompt(
    ...
    const languageHint =
      language === 'auto' ? 'Detect the language automatically' : `Primary language: ${language}`;
