...
export class AgentJobExtractor extends BaseSceneExtractor<AgentJobData> {
  ...
  protected readonly requiredFields = ['skills', 'salaryExpectation'];
  ...
  private extractAgentJobEntities(text: string): SceneExtractedEntity[] {
    const patterns: Record<string, RegExp[]> = {
      ...
      experience: [/(\d+)\s*年\s*经验/g, /经验\s*(\d+)\s*年/g, /工作\s*(\d+)\s*年/g],
      salary: [
        /薪资\s*[约大概]?\s*(\d+)/g,
        /(\d+)\s*[-~到至]\s*(\d+)\s*[Kk千]/g,
        /(\d+)\s*[Kk千万]/g,
      ...
      jobType: [/(全职|兼职|实习|自由职业|远程|外包)/g],
      location: [/([\u4e00-\u9fa5]{2,5}(?:市|区|县))/g, /(远程|在家|线上|居家)/g],
      benefit: [/(五险一金|社保|公积金|医疗保险|年假|带薪)/g, /(双休|单休|弹性|加班)/g],
    };
  ...
  private extractExperience(
    text: string,
    entities: SceneExtractedEntity[]
  ): AgentJobData['structured']['experience'] {
  ...
  private extractSalaryExpectation(
    text: string,
    entities: SceneExtractedEntity[]
  ): AgentJobData['structured']['salaryExpectation'] {
  ...
  private extractLocation(
    text: string,
    entities: SceneExtractedEntity[]
  ): AgentJobData['structured']['location'] {
  ...
  private calculateConfidence(
    entities: SceneExtractedEntity[],
    structured: AgentJobData['structured']
  ): number {
