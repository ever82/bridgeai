...
export class ExtractionValidator {
  ...
  private validateFields(context: ValidationContext, report: ValidationReport): void {
    ...
    for (const field of schema.fields) {
      ...
      // Enum validation
      if (
        (field.type === L2FieldType.ENUM || field.type === L2FieldType.MULTI_SELECT) &&
        field.options
      ) {
      ...
      if (
        (field.type === L2FieldType.TEXT || field.type === L2FieldType.LONG_TEXT) &&
        typeof value === 'string'
      ) {
  ...
  private determineConfirmationNeeded(report: ValidationReport, options: ValidationOptions): void {
    ...
    if (report.summary.completenessScore < 50) {
      ...
      report.confirmationPrompt =
        'Information is incomplete. Please review and confirm or provide more details.';
      return;
    }
    ...
    if (report.warnings.length > 2) {
      report.confirmationNeeded = true;
      report.confirmationPrompt =
        'There are some items that need attention. Please review before proceeding.';
      return;
    }
