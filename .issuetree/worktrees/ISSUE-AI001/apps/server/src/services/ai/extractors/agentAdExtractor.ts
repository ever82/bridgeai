...
export class AgentAdExtractor extends BaseSceneExtractor<AgentAdData> {
  ...
  protected readonly requiredFields = ['product'];
  ...
  private extractAgentAdEntities(text: string): SceneExtractedEntity[] {
    const patterns: Record<string, RegExp[]> = {
      ...
      platform: [/(淘宝|天猫|京东|拼多多|亚马逊|苏宁)/g],
      urgency: [/(急|紧急|马上|立刻|尽快)/g, /(不急|可以等|慢慢)/g],
    };
  ...
  private extractProductInfo(
    text: string,
    entities: SceneExtractedEntity[]
  ): AgentAdData['structured']['product'] {
    ...
    const productPatterns = [
      ...
      {
        pattern: /MacBook\s*(Pro|Air)?/i,
        name: (m: string[]) => `MacBook${m[1] ? ' ' + m[1] : ''}`,
      },
    ];
  ...
  private calculateConfidence(
    entities: SceneExtractedEntity[],
    structured: AgentAdData['structured']
  ): number {
