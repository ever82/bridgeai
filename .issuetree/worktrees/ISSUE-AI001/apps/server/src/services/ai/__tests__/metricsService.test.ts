...
describe('LLMMetricsService', () => {
  ...
  describe('Prometheus Metrics Format', () => {
    ...
    it('should include latency histogram buckets', () => {
      ...
      expect(prometheusMetrics).toContain(
        'llm_latency_seconds_bucket{provider="openai",model="gpt-4",le="'
      );
      expect(prometheusMetrics).toContain(
        'llm_latency_seconds_count{provider="openai",model="gpt-4"}'
      );
      expect(prometheusMetrics).toContain(
        'llm_latency_seconds_sum{provider="openai",model="gpt-4"}'
      );
    });
    ...
    it('should include error metrics', () => {
      ...
      expect(prometheusMetrics).toContain(
        'llm_errors_total{provider="claude",model="claude-3-opus",type="RateLimitError"}'
      );
    });
  });
  ...
  describe('Circuit Breaker Events', () => {
    ...
    it('should filter events by date', () => {
      ...
      const recentEvents = metricsService.getCircuitBreakerEvents(
        undefined,
        new Date('2024-05-01')
      );
