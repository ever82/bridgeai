...
export class SupplyCompletionService {
  ...
  detectIncomplete(supply: Supply): { missing: string[]; incomplete: string[] } {
    ...
    if (!supply.pricing) {
      ...
      if (
        supply.pricing.type !== 'negotiable' &&
        supply.pricing.minRate === undefined &&
        supply.pricing.maxRate === undefined
      ) {
    ...
    if (incomplete.includes('location.mode')) {
      ...
      if (defaults.locationRemote !== undefined) {
        suggestions.push({
          ...
          suggestedValue: {
            remote: defaults.locationRemote,
            onsite: defaults.locationOnsite ?? false,
          },
          confidence: 60,
          source: 'default',
  ...
  private inferDescription(supply: Supply): string | undefined {
    ...
    if (supply.capabilities?.length > 0) {
      const capNames = supply.capabilities
        .slice(0, 3)
        .map(c => c.name)
        .join('、');
