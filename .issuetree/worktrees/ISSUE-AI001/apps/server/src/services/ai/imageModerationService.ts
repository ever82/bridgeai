...
export class ImageModerationService {
   ...
   * 获取特定违规类型的风险评分
   */
  async getViolationScore(image: ImageInput, violationType: ViolationType): Promise<number> {
    const result = await this.moderate(image);
  ...
  private parseModerationResponse(response: string): ImageModerationResult {
    ...
    try {
      ...
      return {
        isSafe,
        violationType,
        violationDetails: isSafe
          ? undefined
          : data.violationDetails || this.getDefaultViolationDescription(violationType),
