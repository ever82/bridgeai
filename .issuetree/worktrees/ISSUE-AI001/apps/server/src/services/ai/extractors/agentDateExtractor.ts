...
export class AgentDateExtractor extends BaseSceneExtractor<AgentDateData> {
  ...
  protected readonly requiredFields = ['partnerPreferences'];
  ...
  private extractAgentDateEntities(text: string): SceneExtractedEntity[] {
    const patterns: Record<string, RegExp[]> = {
      age: [/(\d{2})\s*[岁岁]?/g, /年龄\s*[在约大概]?\s*(\d{2})/g],
      height: [/(\d{3})\s*厘米/g, /身高\s*[在约大概]?\s*(\d{3})/g],
      education: [/(本科|硕士|博士|大专|高中|研究生|海归)/g],
      occupation: [/(医生|教师|工程师|程序员|设计师|销售|管理|公务员|创业)/g],
      interest: [/喜欢\s*([^，。]+)/g, /爱好\s*是?\s*([^，。]+)/g, /兴趣\s*是?\s*([^，。]+)/g],
      location: [/([\u4e00-\u9fa5]{2,5}(?:市|区|县))/g],
      budget: [/预算\s*[约大概]?\s*(\d+)/g, /(\d+)\s*[元块￥$]/g],
    };
  ...
  private extractPartnerPreferences(
    text: string,
    entities: SceneExtractedEntity[]
  ): AgentDateData['structured']['partnerPreferences'] {
  ...
  private calculateConfidence(
    entities: SceneExtractedEntity[],
    structured: AgentDateData['structured']
  ): number {
    let score = 0.4; // Base score

    // +0.3 for having partner preferences
    if (
      structured.partnerPreferences &&
