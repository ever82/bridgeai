...
export class ImageIndexingService {
   ...
   */
  private async generateTags(image: ImageForIndexing, description: string): Promise<Tag[]> {
    const prompt = `
    ...
    try {
      const response = await this.llmService.chatCompletion({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an AI that generates image tags. Return only valid JSON.',
          },
          { role: 'user', content: prompt },
        ],
        temperature: 0.2,
   ...
   */
  private categorizeImage(tags: Tag[], scenes: string[], objects: DetectedObject[]): string[] {
  ...
  async getIndex(imageId: string): Promise<ImageIndex | null> {
    return Array.from(this.indexStore.values()).find(idx => idx.imageId === imageId) || null;
  }
