...
async function analyzeScene(imageBuffer: Buffer): Promise<SceneAnalysis | null> {
  ...
  const scenes = [
    ...
    { scene: 'home', confidence: 0.9, attributes: ['indoor', 'private', 'residential'] },
    { scene: 'restaurant', confidence: 0.7, attributes: ['indoor', 'public', 'social'] },
  ];
...
async function detectText(imageBuffer: Buffer, minConfidence: number): Promise<TextAnalysis[]> {
  ...
  const mockTexts: TextAnalysis[] = [
    ...
    {
      text: '123 Main St',
      boundingBox: { x: 100, y: 600, width: 250, height: 40 },
      confidence: 0.88,
    },
  ];
