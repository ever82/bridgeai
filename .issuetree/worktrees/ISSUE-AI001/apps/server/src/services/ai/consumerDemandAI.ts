...
import { ExtractedDemandData, AgentAdRole, DemandUrgency } from '@bridgeai/shared';
 ...
 */
function mapToConsumerDemand(extractionResult: any, originalText: string): ExtractedDemandData {
 ...
 * @returns 问题列表
 */
export function generateClarificationQuestions(extractedData: ExtractedDemandData): string[] {
 ...
 * 生成提取需求的人类可读摘要
 */
function generateDemandSummary(extractedData: ExtractedDemandData, originalText: string): string {
  const parts: string[] = [];
  ...
  if (extractedData.budget) {
    const budgetStr =
      extractedData.budget.max !== undefined
