...
export class LLMMetricsService extends EventEmitter {
  ...
  getPrometheusMetrics(): string {
    ...
    this.latencyHistogram.forEach((buckets, key) => {
      ...
      for (const bucket of LATENCY_BUCKETS) {
        ...
        lines.push(
          `llm_latency_seconds_bucket{provider="${provider}",model="${model}",le="${bucket / 1000}"} ${count}`
        );
      }

      lines.push(
        `llm_latency_seconds_bucket{provider="${provider}",model="${model}",le="+Inf"} ${buckets.length}`
      );
      lines.push(
        `llm_latency_seconds_count{provider="${provider}",model="${model}"} ${buckets.length}`
      );
    ...
    this.tokenCounter.forEach((value, key) => {
      const [provider, model] = key.split(':');
      lines.push(
        `llm_tokens_used_total{provider="${provider}",model="${model}",type="input"} ${value.input}`
      );
      lines.push(
        `llm_tokens_used_total{provider="${provider}",model="${model}",type="output"} ${value.output}`
      );
    });
    ...
    this.errorCounter.forEach((value, key) => {
      ...
      lines.push(
        `llm_errors_total{provider="${provider}",model="${model}",type="${errorType}"} ${value}`
      );
    });
  ...
  getStats(): {
    ...
    const totalTokens = Array.from(this.tokenCounter.values()).reduce(
      (sum, val) => ({
    ...
    const totalCost = Array.from(this.costCounter.values()).reduce((sum, val) => sum + val, 0);

    const allLatencies = Array.from(this.latencyHistogram.values()).flat();
    const averageLatency =
      allLatencies.length > 0 ? allLatencies.reduce((a, b) => a + b, 0) / allLatencies.length : 0;

    const totalErrors = Array.from(this.errorCounter.values()).reduce((sum, val) => sum + val, 0);
  ...
  getProviderStats(provider: LLMProvider): {
    ...
    const providerKeys = Array.from(this.requestCounter.keys()).filter(
      k => k.startsWith(`${provider}:`) && k.endsWith(':requests_total')
    );
    ...
    const cost = baseKeys.map(k => this.costCounter.get(k) || 0).reduce((a, b) => a + b, 0);

    const latencies = baseKeys.map(k => this.latencyHistogram.get(k) || []).flat();
    const averageLatency =
  ...
  private incrementErrorCounter(provider: LLMProvider, model: string, errorType: string): void {
  ...
  private recordTokens(provider: LLMProvider, model: string, input: number, output: number): void {
    const key = `${provider}:${model}`;
