...
export class ImageAnalysisService {
   ...
   * 提取视觉特征
   */
  async extractVisualFeatures(image: ImageInput): Promise<VisualFeatures> {
...
Focus on: scene setting, main subjects, activities, mood/atmosphere.`;
   ...
   * 提取活动标签
   */
  async extractActivityTags(image: ImageInput): Promise<string[]> {
...
- Be specific and accurate in your analysis`;
  ...
  private parseAnalysisResponse(
    response: string
  ): Omit<ImageAnalysisResult, 'confidence' | 'processingTimeMs'> {
    try {
      // 尝试提取JSON
